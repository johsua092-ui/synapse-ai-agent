"""Tests for synapse_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Synapse entry point
(synapse, synapse-agent, synapse-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Synapse entry point imports synapse_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported synapse_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("synapse_bootstrap", None)
    import synapse_bootstrap  # noqa: WPS433
    return synapse_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Synapse CLI
...
class TestEntryPointsImportBootstrap:
    """Every Synapse entry point must import synapse_bootstrap as its
    ...
    # Entry points that invoke Synapse as a process.  Each one must
    # import synapse_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "synapse_cli/main.py",   # synapse CLI (console_script)
        "run_agent.py",          # synapse-agent (console_script)
        "acp_adapter/entry.py",  # synapse-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import synapse_bootstrap' and that
        ...
        half-finished ``synapse update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``synapse_bootstrap`` as a top-level module) leaves synapse
        ...
        """
        # Resolve relative to the synapse-agent repo root.  Tests live
        # at tests/test_synapse_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # synapse start even when synapse_bootstrap hasn't been
        ...
        assert first_import_name == "synapse_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'synapse_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import synapse_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Synapse's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SYNAPSE_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SYNAPSE_PYTHON_SRC_ROOT"] = env
            elif "SYNAPSE_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SYNAPSE_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/synapse")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SYNAPSE_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SYNAPSE_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/synapse", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Synapse root.  The guard must relocate Synapse to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/synapse"])
        assert result[0] == "/opt/synapse"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Synapse root.
        assert result.index("/opt/synapse") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SYNAPSE_PYTHON_SRC_ROOT"] = "/env/synapse"
            hb.harden_import_path()
            assert sys.path[0] == "/env/synapse"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SYNAPSE_PYTHON_SRC_ROOT", None)
