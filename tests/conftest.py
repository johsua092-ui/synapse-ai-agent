"""Shared fixtures for the synapse-agent test suite.
...
2. **Isolated SYNAPSE_HOME.** SYNAPSE_HOME points to a per-test tempdir so
   code reading ``~/.synapse/*`` via ``get_synapse_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".synapse"`` instead of the canonical
   ``get_synapse_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SYNAPSE_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox SYNAPSE_HOME before ANY test module is imported ──────────────────
# `synapse_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_synapse_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.synapse/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes SYNAPSE_HOME, but fixtures run
...
# the REAL Synapse root — capture it BEFORE the sandbox rewires SYNAPSE_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.synapse (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("SYNAPSE_KANBAN_HOME", "").strip()
_PRE_SANDBOX_SYNAPSE_HOME = os.environ.get("SYNAPSE_HOME", "")


def _synapse_home_points_at_production(value: str) -> bool:
    """True when a pre-set SYNAPSE_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    SYNAPSE_HOME=~/.synapse``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``synapse_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.synapse`` — the escape vector that landed pytest
    ...
    SYNAPSE_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".synapse").resolve()
    except Exception:
        return True
...
if _synapse_home_points_at_production(os.environ.get("SYNAPSE_HOME", "")):
    _SESSION_SYNAPSE_HOME = tempfile.mkdtemp(prefix="synapse-test-home-")
    os.environ["SYNAPSE_HOME"] = _SESSION_SYNAPSE_HOME
    atexit.register(shutil.rmtree, _SESSION_SYNAPSE_HOME, True)
...
# like a real CLI") — which used to disarm synapse_state's live-DB guard in
# the child at the same moment the child lost the SYNAPSE_HOME redirect.
# SYNAPSE_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# synapse_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export SYNAPSE_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["SYNAPSE_TEST_ISOLATION"] = os.environ.get("SYNAPSE_HOME", "") or "1"

#: SYNAPSE_HOME as it stood when conftest was imported - i.e. before any test
...
SYNAPSE_HOME_AT_CONFTEST_IMPORT = os.environ.get("SYNAPSE_HOME", "")
...
# SYNAPSE_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SYNAPSE_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``synapse_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Synapse agent session inherits them and
    # synapse_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "SYNAPSE_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "SYNAPSE_INTERACTIVE",
    "SYNAPSE_QUIET",
    "SYNAPSE_TOOL_PROGRESS",
    "SYNAPSE_TOOL_PROGRESS_MODE",
    "SYNAPSE_MAX_ITERATIONS",
    "SYNAPSE_SESSION_PLATFORM",
    "SYNAPSE_SESSION_CHAT_ID",
    "SYNAPSE_SESSION_CHAT_NAME",
    "SYNAPSE_SESSION_CHAT_TYPE",
    "SYNAPSE_SESSION_THREAD_ID",
    "SYNAPSE_SESSION_SOURCE",
    "SYNAPSE_SESSION_KEY",
    "SYNAPSE_GATEWAY_SESSION",
    "SYNAPSE_CRON_SESSION",
    "_SYNAPSE_GATEWAY",
    "SYNAPSE_PLATFORM",
    "SYNAPSE_MODEL",
    "SYNAPSE_INFERENCE_MODEL",
    "SYNAPSE_INFERENCE_PROVIDER",
    "SYNAPSE_TUI_PROVIDER",
    "SYNAPSE_MANAGED",
    "SYNAPSE_MANAGED_DIR",
    "SYNAPSE_DEV",
    "SYNAPSE_CONTAINER",
    ...
    "SYNAPSE_TIMEZONE",
    "SYNAPSE_REDACT_SECRETS",
    "SYNAPSE_BACKGROUND_NOTIFICATIONS",
    "SYNAPSE_EXEC_ASK",
    "SYNAPSE_HOME_MODE",
    "SYNAPSE_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.synapse/kanban.db instead of the per-test SYNAPSE_HOME.
    "SYNAPSE_KANBAN_DB",
    "SYNAPSE_KANBAN_BOARD",
    "SYNAPSE_KANBAN_HOME",
    "SYNAPSE_KANBAN_WORKSPACES_ROOT",
    "SYNAPSE_KANBAN_LOGS_ROOT",
    "SYNAPSE_KANBAN_TASK",
    "SYNAPSE_KANBAN_WORKSPACE",
    "SYNAPSE_KANBAN_RUN_ID",
    "SYNAPSE_KANBAN_CLAIM_LOCK",
    "SYNAPSE_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "SYNAPSE_DELEGATED_CHILD_CONTEXT",
    "SYNAPSE_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "synapse" host.
    "SYNAPSE_HONCHO_HOST",
    ...
    "SYNAPSE_DASHBOARD_OAUTH_CLIENT_ID",
    "SYNAPSE_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SYNAPSE_HOME to per-test tempdirs so code that
    reads ``~/.synapse/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SYNAPSE_* vars that could change test semantics.
    for name in _SYNAPSE_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("SYNAPSE_HONCHO_HOST", "synapse")

    # 3. Redirect SYNAPSE_HOME to a per-test tempdir. Code that reads
    #    ``~/.synapse/*`` via ``get_synapse_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.synapse/*`` via
    #    ``Path.home() / ".synapse"`` instead of ``get_synapse_home()``
    #    is a bug to fix at the callsite.
    fake_synapse_home = tmp_path / "synapse_test"
    fake_synapse_home.mkdir()
    (fake_synapse_home / "sessions").mkdir()
    (fake_synapse_home / "cron").mkdir()
    (fake_synapse_home / "memories").mkdir()
    (fake_synapse_home / "skills").mkdir()
    monkeypatch.setenv("SYNAPSE_HOME", str(fake_synapse_home))
    ...
    # synapse_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("SYNAPSE_TEST_ISOLATION", str(fake_synapse_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("SYNAPSE_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. synapse_state computes ``DEFAULT_DB_PATH = get_synapse_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from synapse_state import ...``) that
    ...
    synapse_state_mod = sys.modules.get("synapse_state")
    if synapse_state_mod is not None and hasattr(synapse_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            synapse_state_mod, "DEFAULT_DB_PATH", fake_synapse_home / "state.db"
        )
    ...
    monkeypatch.setenv("SYNAPSE_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.synapse/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import synapse_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path SYNAPSE_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_synapse_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from synapse_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.synapse. This autouse
...
# allow-list because test-level fixtures legitimately move SYNAPSE_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session SYNAPSE_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``SYNAPSE_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Synapse root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_SYNAPSE_HOME and not _synapse_home_points_at_production(
        _PRE_SANDBOX_SYNAPSE_HOME
    ):
        # SYNAPSE_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_synapse_root().resolve()
    # No pre-existing SYNAPSE_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".synapse").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.synapse`` captured at import time. Hermetic tests that legitimately
    move SYNAPSE_HOME to sibling tempdirs are unaffected.

    Only patches when ``synapse_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("synapse_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import synapse_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.synapse. See #69283."
        )
...
# ``synapse_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Synapse root. This fixture wires the
# test-side knobs:
...
#     installs where SYNAPSE_HOME is not ~/.synapse) into the guard's
...
# so subprocess children that import synapse_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("synapse_state")
    ...
    if _PRE_SANDBOX_SYNAPSE_HOME and not _synapse_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_SYNAPSE_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Synapse home override redirects every later
    # ``get_synapse_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from synapse_constants import get_synapse_home_override, set_synapse_home_override

        if get_synapse_home_override() is not None:
            set_synapse_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal synapse config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``synapse-gateway`` process
...
# correlated with a ``tests/synapse_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> synapse-gateway``
...
def _wal_is_usable() -> bool:
    """True when Synapse will actually put a database into WAL mode here.

    Synapse refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Synapse can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Synapse managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``synapse_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_synapse_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_synapse_home`` fixture
    redirects ``SYNAPSE_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.synapse/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``synapse_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["SYNAPSE_VOICE_TTS"] = "1"``. The
...
#      ``synapse_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``synapse_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``synapse_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``synapse_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Synapse runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.synapse``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Synapse falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Synapse uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart synapse-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting synapse/python patterns are also blocked.
    """
    ...
    def _matches_synapse_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SYNAPSE_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_synapse_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch synapse-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m synapse_cli.main".
                if (
                    "synapse" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live synapse-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting synapse/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `synapse update` (or the
        # equivalent `python -m synapse_cli.main update`).  These commands
        ...
        if "update" in low and (
            # synapse update / synapse update --gateway / setsid bash -c ... synapse update
            ("synapse" in low and "update" in low.split())
            or
            # python -m synapse_cli.main update --gateway
            ("synapse_cli" in low and "update" in low.split())
            or
            # venv/bin/synapse update  (absolute path variant used in tests)
            (".venv/bin/synapse" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`synapse update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_SYNAPSE_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import synapse_cli.voice as _voice
    except Exception:
