...
#
# Installs a real, earlier Synapse the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `synapse`.
#
...
# Keep sandbox state out of the default .synapse-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".synapse-sandbox-e2e-$ROUTE"
export SYNAPSE_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/synapse/.synapse/synapse-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/synapse-agent.git"
...
LOG_DIR="${SYNAPSE_E2E_LOG_DIR:-$(mktemp -d -t synapse-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED synapse accept FLAG on `synapse update`?
#
...
# that silently rots. `synapse update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "synapse update --help 2>&1" | grep -qF -- "$flag"
}
...
require_synapse_works() {
  local when="$1" out
  out="$(in_sandbox "synapse --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "synapse --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "synapse runs $when"
}

# ── install the earlier Synapse ─────────────────────────────────────────────
...
require_synapse_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: synapse update'
    ...
    # synapse which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="synapse update --yes"
    else
      update_cmd="synapse update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "synapse update failed ($update_cmd)"
    fi
    require_landed_on_target 'synapse update'
    require_synapse_works 'after synapse update'
    ;;
  installer)
    ...
    require_synapse_works 'after installer re-run'
    ;;
esac
