...
def _make_agent():
    synapse_home = Path(tempfile.mkdtemp(prefix="synapse-test-home-"))
    (synapse_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._synapse_home", synapse_home),
