...
def synapse_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves SYNAPSE_HOME.
...
@pytest.fixture()
def server(synapse_home, monkeypatch):
    ...
    # Pin config resolution to the isolated SYNAPSE_HOME. Sibling test
    ...
    # module-level ``_synapse_home = get_synapse_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_synapse_home", synapse_home)
...
def test_moa_bare_returns_usage(server, session, synapse_home):
    _write_moa_config(synapse_home, """
moa:
