    ...
    with patch.dict(
        "sys.modules",
        {
            "synapse_constants": MagicMock(
                get_synapse_home=MagicMock(return_value="/tmp/synapse_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'synapse-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'synapse-release' patched"
    }
