...
def progress(tmp_path):
    ...
    status = tmp_path / "synapse-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Synapse to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `synapse update`, and reports the stage that was on screen
...
n="$(cat "$SYNAPSE_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$SYNAPSE_TEST_CALLS"
for f in "$TMPDIR"/synapse-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$SYNAPSE_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$SYNAPSE_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "synapse-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    synapse = install_root / "venv" / "bin" / "synapse"
    synapse.write_text(FAKE_SYNAPSE)
    synapse.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "SYNAPSE_TEST_CAPTURE": str(capture),
        "SYNAPSE_TEST_CALLS": str(calls),
        "SYNAPSE_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".synapse-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`synapse update` is the longest wait in the hand-off and the one the
