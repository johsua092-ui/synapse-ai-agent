"""Guard: Synapse-owned subprocesses must not resolve managed runtimes by bare PATH.

Synapse installs runtimes for itself — ``uv`` at ``$SYNAPSE_HOME/bin/uv``, Node at
``$SYNAPSE_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Synapse's own
...
* when a system copy also exists, the one Synapse does not own wins — which is
...
``iter_synapse_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Synapse provisions into SYNAPSE_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Synapse-owned subprocess code: plugins ship their own
...
def _source_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.py"):
        ...
        # synapse_agent-0.20.5/, build/ and *.egg-info dirs). CI jobs that
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Synapse-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$SYNAPSE_HOME/bin (uv) and $SYNAPSE_HOME/node are not on an "
        ...
        "  node/npm -> synapse_constants.find_node_executable()\n"
        "  PATH env -> synapse_constants.iter_synapse_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(synapse_constants, helper))


def test_managed_uv_helpers_exist():
    from synapse_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
