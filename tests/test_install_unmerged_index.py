...
The ``synapse update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m synapse_cli.main gateway run`` — image name
    ``pythonw``, not ``synapse.exe`` — so the ``taskkill /IM synapse.exe`` guard
    ...
    # The synapse.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM synapse.exe' in text
