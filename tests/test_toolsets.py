...
class TestToolsetConsistency:
    ...
    def test_synapse_platforms_share_core_tools(self):
        """All synapse-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on synapse-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["synapse-cli", "synapse-telegram", "synapse-discord", "synapse-whatsapp", "synapse-slack", "synapse-signal", "synapse-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_synapse_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("synapse-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("synapse-cli")
        second = resolve_toolset("synapse-cli")
        ...
        assert (
            "synapse-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("synapse-cli", include_registry=False)
        second = resolve_toolset("synapse-cli", include_registry=False)
        assert first == second
