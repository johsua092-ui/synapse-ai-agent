...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("synapse_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("SYNAPSE_SESSION_KEY", session_key)
        monkeypatch.setenv("SYNAPSE_EXEC_ASK", "1")
        monkeypatch.delenv("SYNAPSE_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("synapse-verify-", "synapse-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "synapse-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/synapse-verify-example.py",
            "rm -f /tmp/synapse-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../synapse-verify-example.py",
            "rm -f /tmp/a/../../tmp/synapse-verify-example.py",
            "rm -f /var/tmp/synapse-verify-example.py",
            "rm -f /tmp/synapse-verify-*",
            "rm -f /tmp/synapse-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/synapse-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/synapse-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\synapse-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\synapse-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\synapse-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"SYNAPSE_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.synapse/.env",
            "echo x | tee $SYNAPSE_HOME/.env",
            'echo x | tee "$SYNAPSE_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestSynapseConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.synapse/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.synapse/config.yaml",
            "echo '  mode: off' >> ~/.synapse/config.yaml",
            "echo x | tee ~/.synapse/config.yaml",
            "echo x | tee $SYNAPSE_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.synapse/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Synapse absolute config.yaml is
        # handled by the project patterns, not the Synapse-home rule.
        for cmd in (
            "cat ~/.synapse/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.synapse/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Synapse-home prefixes must fold to ~/ and
    ~/.synapse/ in dangerous-command detection.
    ...
    SSH, and Synapse config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_synapse_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/SYNAPSE_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("SYNAPSE_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.synapse/synapse-agent && source venv/bin/activate && python -m synapse_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m synapse_cli.main gateway run --replace &",
            "nohup python -m synapse_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart synapse-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Synapse security config via IFS.
            "sed${IFS}-i ~/.synapse/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep synapse) bypasses the pkill/killall name-matching
    ...
    def test_kill_pgrep_expansion_detected(self):
        for cmd in (
            'kill -9 $(pgrep -f "synapse.*gateway")',
            "kill -9 `pgrep synapse`",
            "kill $(pgrep gateway)",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof synapse)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof synapse_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof synapse`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Synapse service
    label achieves the same effect as `synapse gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_synapse_label_detected(self):
        for cmd in (
            "launchctl stop ai.synapse.gateway",
            "launchctl kickstart -k system/ai.synapse.gateway",
            "launchctl bootout system/ai.synapse.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.synapse.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Synapse labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a synapse-gateway identifier, so quoted prose and non-gateway synapse
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "synapse"/"ai.synapse" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("SYNAPSE_GATEWAY_SESSION", "SYNAPSE_CRON_SESSION",
                      "SYNAPSE_YOLO_MODE",
                      "SYNAPSE_SESSION_KEY", "SYNAPSE_INTERACTIVE")
        }
        os.environ.pop("SYNAPSE_YOLO_MODE", None)
        os.environ.pop("SYNAPSE_INTERACTIVE", None)
        # SYNAPSE_CRON_SESSION takes priority over SYNAPSE_GATEWAY_SESSION in
        ...
        os.environ.pop("SYNAPSE_CRON_SESSION", None)
        os.environ["SYNAPSE_GATEWAY_SESSION"] = "1"
        os.environ["SYNAPSE_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("synapse_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"SYNAPSE_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.synapse.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.synapse.gateway.plist",
    "launchctl stop ai.synapse.gateway",
    "launchctl restart ai.synapse.gateway",
    "launchctl bootout gui/501/ai.synapse.gateway",
    "launchctl remove ai.synapse.gateway",
    "launchctl disable gui/501/ai.synapse.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_SYNAPSE_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Synapse ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.synapse.update-checker",
