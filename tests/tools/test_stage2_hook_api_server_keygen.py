...
was gated on ``[ -f "$SYNAPSE_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'SYNAPSE_HOME="{home}"\n'
        # In tests we run unprivileged; as_synapse is a passthrough then.
        'as_synapse() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Synapse loads $SYNAPSE_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/synapse/.env.example -> $SYNAPSE_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $SYNAPSE_HOME/.env from it on first boot"
    )
