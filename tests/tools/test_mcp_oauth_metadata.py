...
Covers:
- :class:`SynapseTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.SynapseMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _SYNAPSE_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# SynapseTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = SynapseTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path))
        storage = SynapseTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (SynapseMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: SynapseTokenStorage, **context_attrs):
    ...
    provider = _SYNAPSE_PROVIDER_CLS.__new__(_SYNAPSE_PROVIDER_CLS)
    provider._synapse_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path))
        storage = SynapseTokenStorage("mgr-srv")
        ...
        with patch.object(
            _SYNAPSE_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path))
        storage = SynapseTokenStorage("flow-srv")
        ...
        with patch.object(
            _SYNAPSE_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
