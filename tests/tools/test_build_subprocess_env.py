...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    assert env["SYNAPSE_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_synapse_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see SYNAPSE_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('SYNAPSE_HOME'), "
    ...
    assert result["home"] == str(synapse_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `synapse` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_synapse_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no synapse console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``synapse``.
    ...
    bin_dir = local_mod._resolve_synapse_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "synapse.exe" if os.name == "nt" else "synapse")
    ):
        pytest.skip("no real synapse console-script install available")

    # Simulate the service-manager minimal PATH: synapse dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("synapse", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("synapse", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'synapse' must resolve from the child PATH {env.get('PATH')!r}"
    )
