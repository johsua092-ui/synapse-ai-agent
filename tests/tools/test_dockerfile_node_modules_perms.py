...
Older images made /opt/synapse/ui-tui and /opt/synapse/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SYNAPSE_TUI_DIR=/opt/synapse/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R synapse:synapse /opt/synapse/ui-tui" not in text
    assert "chown -R synapse:synapse /opt/synapse/node_modules" not in text
