...
class TestDelegateRequirements(unittest.TestCase):
    ...
    def test_schema_valid(self):
        ...
        # Per-call subagent customization: skills (force-load skills), toolsets
        # (narrow the child to selected toolsets, intersected with the parent),
        # and model (per-call model override) are all exposed to the model at
        # both the top level and per-task.
        for key in ("skills", "toolsets", "model"):
...
class TestStripBlockedTools(unittest.TestCase):
    ...
    def test_mixed_composite_is_subtracted_at_child_assembly(self):
        ...
        ``synapse-cli`` contains both allowed tools and every sensitive
        ...
        parent.enabled_toolsets = ["synapse-cli"]
...
class TestDelegateTask(unittest.TestCase):
    ...
    def test_nous_child_rederives_api_mode_from_model(self):
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            self.assertEqual(kwargs["model"], "synapse-4-405b")
...
class TestConcurrencyDefaults(unittest.TestCase):
    ...
    def test_load_config_prefers_active_persistent_config_over_cli_defaults(self):
        ...
        with patch.dict("sys.modules", {"cli": stale_cli}):
            with patch(
                "synapse_cli.config.load_config_readonly", return_value=active_config
            ):
