...
class TestWriteDenyExactPaths:
    ...
    def test_synapse_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.synapse/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "synapse_root"
        ...
        # Sanity check: SYNAPSE_HOME does point to the profile dir, not the root.
        from synapse_constants import get_synapse_home, get_default_synapse_root
...
class TestWriteAllowed:
    ...
    def test_synapse_control_files_requested_writable(self):
