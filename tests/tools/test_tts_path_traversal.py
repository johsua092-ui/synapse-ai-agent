...
def test_output_path_rejects_synapse_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    synapse_home = tmp_path / "synapse-home"
    synapse_home.mkdir()
    monkeypatch.setattr(file_safety, "_synapse_home_path", lambda: synapse_home)
    monkeypatch.setattr(file_safety, "_synapse_root_path", lambda: synapse_home)

    target = synapse_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    synapse_home = tmp_path / "synapse-home"
    token_dir = synapse_home / "mcp-tokens"
