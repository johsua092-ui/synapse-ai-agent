...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        synapse_home.mkdir()
        (synapse_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"SYNAPSE_HOME": str(synapse_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(synapse_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.synapse/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        synapse_home = tmp_path / ".synapse"
        synapse_home.mkdir()
        (synapse_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        synapse_home = tmp_path / ".synapse"
        skills_dir = synapse_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.synapse/skills"


    def test_custom_container_base(self, tmp_path):
        synapse_home = tmp_path / ".synapse"
        (synapse_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SYNAPSE_HOME": str(synapse_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.synapse")

        assert mounts[0]["container_path"] == "/home/user/.synapse/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.synapse/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.synapse/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SYNAPSE_HOME."""
        monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path / ".synapse"))
        (tmp_path / ".synapse").mkdir()

        # Create a sensitive file one level above synapse_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside synapse_home
    ...
    def test_nested_subdir_inside_synapse_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SYNAPSE_HOME must be allowed."""
        synapse_home = tmp_path / ".synapse"
        synapse_home.mkdir()
        subdir = synapse_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SYNAPSE_HOME pointing outside must be rejected."""
        synapse_home = tmp_path / ".synapse"
        synapse_home.mkdir()
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))

        # Create a sensitive file outside synapse_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside synapse_home pointing outside
        symlink = synapse_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SYNAPSE_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, synapse_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SYNAPSE_HOME."""
        synapse_home = tmp_path / ".synapse"
        synapse_home.mkdir()
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))
        ...
        sensitive.write_text("{}")
        self._write_config(synapse_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(synapse_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SYNAPSE_HOME via config must still mount."""
        synapse_home = tmp_path / ".synapse"
        synapse_home.mkdir()
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))

        (synapse_home / "oauth.json").write_text("{}")
        self._write_config(synapse_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        synapse_home = tmp_path / ".synapse"
        synapse_home.mkdir()
        (synapse_home / "cache" / "documents").mkdir(parents=True)
        (synapse_home / "cache" / "audio").mkdir(parents=True)
        (synapse_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.synapse/cache/documents" in paths
        assert "/root/.synapse/cache/audio" in paths
        assert "/root/.synapse/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_synapse_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        synapse_home = tmp_path / ".synapse"
        synapse_home.mkdir()
        # Use legacy dir name with content — get_synapse_dir prefers
        # populated old over new.
        legacy_doc = synapse_home / "document_cache"
        legacy_img = synapse_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(synapse_home / "document_cache") in host_paths
        assert str(synapse_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.synapse/cache/documents" in container_paths
        assert "/root/.synapse/cache/images" in container_paths
    ...
    def test_empty_synapse_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.synapse/attachments" in container_paths
        assert "/root/.synapse/images" in container_paths
        assert "/root/.synapse/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``SYNAPSE_HOME/images``, not
        ...
        """
        synapse_home = tmp_path / ".synapse"
        (synapse_home / "images").mkdir(parents=True)
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.synapse/images" in by_container
        assert by_container["/root/.synapse/images"] == str(synapse_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        synapse_home = tmp_path / ".synapse"
        (synapse_home / "images").mkdir(parents=True)
        upload = synapse_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.synapse/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        synapse_home = tmp_path / ".synapse"
        img_dir = synapse_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.synapse/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(synapse_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.synapse/cache/images/x.png"
        assert (synapse_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Synapse cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        synapse_home = tmp_path / ".synapse"
        (synapse_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))
        return str(synapse_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_synapse(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.synapse/attachments/drop.zip"

    def test_ssh_maps_to_tilde_synapse(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.synapse/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        synapse_home = tmp_path / ".synapse"
        doc_dir = synapse_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — SYNAPSE_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping SYNAPSE_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"SYNAPSE_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.synapse/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.synapse/google_token.json" in paths
        assert "/root/.synapse/.env" not in paths
