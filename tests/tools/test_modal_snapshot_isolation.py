...
def _restore_tool_modules():
    original_synapse_home = os.environ.get("SYNAPSE_HOME")
    original_modules = {
        name: module
        ...
        or name == "synapse_cli"
        or name.startswith("synapse_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_synapse_home is None:
            os.environ.pop("SYNAPSE_HOME", None)
        else:
            os.environ["SYNAPSE_HOME"] = original_synapse_home
        _reset_modules(("tools", "synapse_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    synapse_cli = types.ModuleType("synapse_cli")
    synapse_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["synapse_cli"] = synapse_cli
    synapse_home = tmp_path / "synapse-home"
    os.environ["SYNAPSE_HOME"] = str(synapse_home)
    sys.modules["synapse_cli.config"] = types.SimpleNamespace(
        get_synapse_home=lambda: synapse_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="synapse-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": synapse_home / "modal_snapshots.json",
