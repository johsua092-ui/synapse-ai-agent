...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SYNAPSE_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SYNAPSE_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_synapse_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SYNAPSE_HOME": "/h", "SYNAPSE_PROFILE": "p",
        "SYNAPSE_CONFIG": "/c.yaml", "SYNAPSE_ENV": "/e",
        "SYNAPSE_DELEGATED_CHILD_CONTEXT": "1",
        # other SYNAPSE_* → dropped (broad prefix removed)
        "SYNAPSE_BASE_URL": "https://x", "SYNAPSE_INTERACTIVE": "1",
        "SYNAPSE_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "SYNAPSE_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SYNAPSE_BASE_URL", "SYNAPSE_INTERACTIVE", "SYNAPSE_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SYNAPSE_* vars."""
    import logging
