...
Run from the PR worktree:
    ...
    cd ~/.synapse/synapse-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.synapse/synapse-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.synapse/synapse-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a synapse-agent checkout"
)
...
# Isolated SYNAPSE_HOME for the config write.
