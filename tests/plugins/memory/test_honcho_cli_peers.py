"""Regression tests for #76414: `synapse honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("synapse.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("synapse_work"). The lookup
...
a real honcho.json (temp SYNAPSE_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "synapse": {"peerName": "alice", "aiPeer": "synapse"},
            "synapse_work": {"peerName": "alice", "aiPeer": "synapse"},
            "synapse_my_profile": {"peerName": "bob", "aiPeer": "synapse"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "synapse_cli.profiles.list_profiles",
        ...
        assert host == "synapse_work"  # not "synapse.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'synapse_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("synapse.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["synapse_work"]
        cfg["hosts"]["synapse.work"] = {"peerName": "carol", "aiPeer": "synapse"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "synapse_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'synapse.work' (or '(not set)')."""
        monkeypatch.setattr(
            "synapse_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "synapse.work" not in out
        ...
        assert "alice" in work_line and "synapse" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "synapse.new" not in out  # well-formed key, no dot-form leak
