...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure SYNAPSE_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", synapse_home=str(tmp_path / ".synapse"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", synapse_home=str(synapse_home))
        return p
