...
1. `synapse secrets bitwarden setup --help` works (dispatch path)
2. `synapse update --check` works (update path)
3. `synapse secrets bitwarden disable` works (handler execution)
4. `synapse secrets onepassword status` works (lazy backend loads on demand)
...
def _run_synapse(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run synapse CLI as a subprocess from repo root.
    ...
    connectivity — but ``synapse update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-synapse-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-synapse-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`synapse secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_synapse(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`synapse secrets bitwarden disable` must exit 0."""
        result = _run_synapse(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`synapse secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_synapse(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`synapse secrets onepassword setup --help` must exit 0."""
        result = _run_synapse(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `synapse update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`synapse update --check` must not load cryptography._rust."""
        result = _run_synapse(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['synapse','update','--check']
...
sys.argv = ['synapse', 'update', '--check']

import synapse_cli.main as m
...
# resolves the synapse_cli.main module's lazily re-exported attribute —
# so the patch must land on synapse_cli.main._cmd_update_check.
with patch('synapse_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
