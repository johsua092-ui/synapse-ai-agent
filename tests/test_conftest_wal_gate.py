"""The conftest WAL gate must agree with synapse_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``synapse_state``. That is deliberate: importing
``synapse_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.synapse`` before the per-test ``SYNAPSE_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_synapse_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and synapse_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_synapse_state_at_collection():
    """The gate must stay import-free of synapse_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.synapse, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``synapse_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "synapse_state" or name.startswith("synapse_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported synapse_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.synapse during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("synapse_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["synapse_state"] = saved
    assert not blocked
