...
Override the image with ``SYNAPSE_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SYNAPSE_TEST_IMAGE", "synapse-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SYNAPSE_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SYNAPSE_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged synapse user
# ---------------------------------------------------------------------------
#
# Background: every Synapse runtime path inside the container drops to UID
# 10000 (the ``synapse`` user) via ``s6-setuidgid synapse``. ``docker exec``
...
# fails (PermissionError swallowed) for synapse, so a test that ran as root
...
# helpers below run every probe under ``-u synapse`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: synapse).
