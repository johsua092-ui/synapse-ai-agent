...
Build the real image and verify the actual runtime behavior:
  ...
  3. --user <synapse-uid> (10000) is allowed (supported non-root start)
"""
...
def test_arbitrary_user_uid_rejected(
    ...
    assert "SYNAPSE_UID" in combined or "PUID" in combined, (
...
def test_user_pinned_to_synapse_uid_works(
    built_image: str,
) -> None:
    """docker run --user 10000:10000 (the synapse UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (synapse UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout