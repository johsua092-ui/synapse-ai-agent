"""Runtime smoke test for Docker $SYNAPSE_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the synapse user after container boot.
...
registered later runs its log service as the dropped synapse user and
...
def test_logs_gateways_seeded_and_synapse_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by synapse after boot."""
    ...
    assert "logs=synapse" in r.stdout, (
        f"logs/ not owned by synapse: {r.stdout}"
    )
    assert "gateways=synapse" in r.stdout, (
        f"logs/gateways/ not owned by synapse: {r.stdout}"
    )
