...
def test_session_store_default_db_uses_runtime_synapse_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime SYNAPSE_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    synapse_state before a fixture redirected SYNAPSE_HOME used to pin every
    default SessionDB() at the developer's real ~/.synapse/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_synapse_home"
    fake_home.mkdir()
    monkeypatch.setenv("SYNAPSE_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "synapse sessions list" in raw["_README"]
