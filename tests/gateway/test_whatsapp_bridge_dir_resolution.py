...
(/opt/synapse/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable SYNAPSE_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_synapse_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable SYNAPSE_HOME."""
    ...
    monkeypatch.setattr(
        "synapse_constants.get_synapse_home", lambda: synapse_home
    )
    ...
    expected = synapse_home / "scripts" / "whatsapp-bridge"
