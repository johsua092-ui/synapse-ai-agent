...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``SYNAPSE_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "SYNAPSE_SESSION_CHAT_ID": env.get("SYNAPSE_SESSION_CHAT_ID"),
        "SYNAPSE_SESSION_THREAD_ID": env.get("SYNAPSE_SESSION_THREAD_ID"),
        "SYNAPSE_SESSION_KEY": env.get("SYNAPSE_SESSION_KEY"),
    }
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("SYNAPSE_SESSION_CHAT_ID", "SYNAPSE_SESSION_THREAD_ID", "SYNAPSE_SESSION_KEY"):
    ...
    assert captured["bound"]["SYNAPSE_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["SYNAPSE_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``SYNAPSE_SESSION_*``
