...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/johsua092-ui/synapse-ai-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Synapse-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/johsua092-ui/synapse-ai-agent/issues/87051:
    ...
    response in a different chat lane than the Synapse topic the session runs
    in. The no-anchor branch must prefer the Synapse topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_synapse_topic_thread_id(self):
