...
SYNAPSE_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SYNAPSE_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``synapse config get terminal.backend`` still says local.
    """
    synapse_home = tmp_path / ".synapse"
    synapse_home.mkdir()
    (synapse_home / "config.yaml").write_text(
    ...
    (synapse_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
