...
closure the PR changed, against a real temp ``SYNAPSE_HOME``.
"""
...
def _fake_switch_result():
    ...
    from synapse_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "synapse_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "synapse_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_synapse_home", synapse_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_synapse_home() / config.yaml`` — point it here.
    monkeypatch.setattr("synapse_constants.get_synapse_home", lambda: synapse_home)
    monkeypatch.setattr("synapse_cli.config.get_synapse_home", lambda: synapse_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_synapse_home", default_home)
    _stub_picker_dependencies(monkeypatch)
