...
from synapse_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(synapse_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <synapse_home>/plugins/<name> and opt it in."""
    plugin_dir = synapse_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path / "synapse_empty"))
