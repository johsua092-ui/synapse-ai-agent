...
1. ``load_synapse_dotenv()`` auto-loads ``~/.synapse/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_synapse_dotenv(synapse_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "synapse_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
