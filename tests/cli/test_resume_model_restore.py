...
SessionDB.session_gateway_runtime (synapse_state.py) — the round trip that
makes `synapse --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare SynapseCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.SynapseCLI)
