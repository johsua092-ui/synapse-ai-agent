"""Tests for cli.SynapseCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full SynapseCLI (which requires extensive setup).
"""
...
def _make_self(prompt_response):
    ...
    self_._normalize_slash_confirm_choice = _bound(
        SynapseCLI._normalize_slash_confirm_choice, self_,
    )
    return self_
...
def test_gate_on_choice_once_returns_once():
    ...
    with patch(
        ...
        result = _bound(SynapseCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_split_destructive_skip_recognized_tokens():
    ...
    assert SynapseCLI._split_destructive_skip("/reset now") == ("", True)
    assert SynapseCLI._split_destructive_skip("/clear --yes") == ("", True)
    assert SynapseCLI._split_destructive_skip("/undo -y") == ("", True)
...
def test_split_destructive_skip_handles_empty_and_none():
    ...
    assert SynapseCLI._split_destructive_skip(None) == ("", False)
    assert SynapseCLI._split_destructive_skip("") == ("", False)
    assert SynapseCLI._split_destructive_skip("   ") == ("", False)
...
def test_confirm_destructive_slash_now_skips_modal():
    ...
    self_._split_destructive_skip = SynapseCLI._split_destructive_skip  # classmethod
...
def test_confirm_destructive_slash_yes_flag_skips_modal():
    ...
    self_._split_destructive_skip = SynapseCLI._split_destructive_skip
