...
def _make_cli(**kwargs):
    """Create a SynapseCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports synapse_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``SynapseCLI.run()``'s local ``KeyBindings`` instance, so they are not
