...
from synapse_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("synapse_cli.config.load_config", default_load_config)
        monkeypatch.setattr("synapse_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("synapse_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("synapse_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("synapse_cli.config.load_config", lambda: {})
        monkeypatch.setattr("synapse_cli.config.load_config_readonly", lambda: {})
...
class TestBuildNousSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "synapse_cli.nous_subscription.get_nous_subscription_features",
...
class TestBuildContextFilesPrompt:
    def test_empty_dir_loads_seeded_global_soul(self, tmp_path):
        ...
        assert "Synapse Agent" in result
    ...
    def test_synapse_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".synapse.md").write_text("Synapse-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Synapse-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path / "synapse_home"))
        synapse_home = tmp_path / "synapse_home"
        synapse_home.mkdir()
        (synapse_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
    ...
    # --- .synapse.md / SYNAPSE.md discovery ---
...
# =========================================================================
# .synapse.md helper functions
# =========================================================================


class TestFindSynapseMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".synapse.md").write_text("rules")
        assert _find_synapse_md(tmp_path) == tmp_path / ".synapse.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".synapse.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_synapse_md(sub) == tmp_path / ".synapse.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .synapse.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """SYNAPSE_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("SYNAPSE_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
