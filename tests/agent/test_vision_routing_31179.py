...
def isolated_home(monkeypatch):
    """Temp SYNAPSE_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="synapse_test_31179_")
    synapse_home = os.path.join(test_home, ".synapse")
    os.makedirs(synapse_home)
...
def _fresh_modules():
    """Drop cached synapse modules so each test reloads against current env."""
    _drop_reload_targets()
