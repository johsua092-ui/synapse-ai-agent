...
path via ``synapse chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SYNAPSE_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SYNAPSE_HOME at a temp dir and turn moa.save_traces on."""
    synapse_home = tmp_path / ".synapse"
    synapse_home.mkdir()
    monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))

    # save_moa_turn reads config via synapse_cli.config.load_config; stub it to
    ...
    return synapse_home / "moa-traces"
