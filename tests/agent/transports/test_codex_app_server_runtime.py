...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Synapse passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("SYNAPSE_HOME", "/users/alice/.synapse/profiles/backend-worker")
        monkeypatch.setenv("SYNAPSE_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "SYNAPSE_KANBAN_DB",
            "/users/alice/.synapse/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.synapse/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through synapse_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Synapse secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "SYNAPSE_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
