...
thread that did not bind the SYNAPSE_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_synapse_home()``,
so an unbound build thread fell back to ``~/.synapse`` (default) and leaked
default's full skills index + "Active Synapse profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient SYNAPSE_HOME, on a
    ...
    def build():
        # No set_synapse_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_synapse_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    synapse ROOT (get_default_synapse_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: SYNAPSE_HOME env points at the profile dir itself.
    monkeypatch.setenv("SYNAPSE_HOME", str(bot_home))
...
def test_profile_name_correct_when_ambient_is_another_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SYNAPSE_HOME", str(other_home))
