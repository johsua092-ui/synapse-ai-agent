...
manipulation. Synapse keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point SYNAPSE_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_synapse_home /
    get_default_synapse_root) instead of monkeypatching private helpers —
    ...
    SYNAPSE_HOME=<root>/profiles/<name> makes get_default_synapse_root()
