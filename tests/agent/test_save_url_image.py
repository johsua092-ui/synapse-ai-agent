...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SYNAPSE_HOME under tmp_path."""
    monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path / ".synapse"))
    (tmp_path / ".synapse").mkdir()

    # Force the constants/image cache helpers to re-read SYNAPSE_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("synapse_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_synapse_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SYNAPSE_HOME — gateway cleanup
