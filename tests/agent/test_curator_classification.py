...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"synapse-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "synapse-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "synapse-agent-dev"
...
# `synapse curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `synapse curator pin <umbrella>` in the rename block so
