"""Regression tests for johsua092-ui/synapse-ai-agent#2765.
...
service that didn't inherit ``~/.synapse/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "synapse memory status" in msg
