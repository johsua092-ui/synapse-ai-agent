...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SYNAPSE_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="synapse_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".synapse"))
    prev_home = os.environ.get("SYNAPSE_HOME")
    os.environ["SYNAPSE_HOME"] = os.path.join(test_home, ".synapse")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("synapse_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SYNAPSE_HOME"] = prev_home
