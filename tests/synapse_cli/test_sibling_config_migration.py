...
`synapse update` migrated only the active profile's config.yaml; sibling
...
context-local SYNAPSE_HOME override.
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        synapse_constants, "get_process_synapse_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_synapse_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_synapse_home_override() == before
