...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_synapse_home", lambda: synapse_home)
    monkeypatch.setattr(uninstall, "_is_default_synapse_home", lambda home: False)
    ...
    assert str(synapse_home) in output
    assert project_root.exists()
    assert synapse_home.exists()
