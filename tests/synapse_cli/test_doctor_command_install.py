"""Tests for the Command Installation check in synapse doctor."""
...
import synapse_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal SYNAPSE_HOME + PROJECT_ROOT for doctor tests."""
    ...
    synapse_bin = venv_bin_dir / "synapse"
    synapse_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    ...
    return home, project, synapse_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, synapse_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "synapse"
        wrong_target = tmp_path / "wrong_synapse"
        ...
        assert cmd_link.resolve() == synapse_bin.resolve()
