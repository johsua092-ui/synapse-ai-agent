"""Regression tests for the OAuth dispatcher in synapse_cli.web_server.
...
from synapse_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Synapse-Session-Token": _SESSION_TOKEN}
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https:///oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "synapse_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "synapse_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "synapse_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from synapse_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_synapse_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "synapse auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "synapse auth" not in detail
    finally:
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `synapse model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `synapse model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Synapse."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``synapse auth remove
    xai-oauth``.
    """
    from synapse_cli import auth as auth_mod
    from synapse_cli import web_server as ws
    ...
    # Prior `synapse auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to synapse_cli.auth.get_auth_status (the
    ...
    with patch("synapse_cli.auth.get_auth_status", return_value=fake_status):
