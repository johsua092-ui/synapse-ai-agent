...
Covers:
- Defense 1: gateway stop/restart refuse when _SYNAPSE_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "synapse gateway uninstall",
        "synapse  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "SYNAPSE GATEWAY RESTART",           # uppercase
    ])
    def test_synapse_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.synapse.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.synapse/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l synapse-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.synapse.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.synapse.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.synapse.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.synapse/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.synapse.gateway-apollo:/a.plist' "
            "'ai.synapse.gateway-cronus:/c.plist' 'ai.synapse.gateway-plutus:/p.plist' "
            "'ai.synapse.gateway:/Users/botuser/Library/LaunchAgents/ai.synapse.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated synapse note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.synapse.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.synapse.gateway',
        "launchctl kick'start' -k gui/501/ai.synapse.gateway",
        "launchctl kick\\start -k gui/501/ai.synapse.gateway",
        'launchctl "kickstart" -k gui/501/ai.synapse.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.synapse.gateway',
        "launchctl dis\\able gui/501/ai.synapse.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.synapse."gateway"',
        # Same class on the systemctl and synapse-CLI branches.
        'systemctl re"start" synapse-gateway',
        'synapse gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.synapse.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.synapse.update-checker',
        'systemctl re"start" synapse-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "synapse cron list",
        "synapse update",
        "synapse config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `synapse gateway start` is benign — starting a gateway from inside a
        ...
        "synapse gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway synapse
        # services must not be falsely blocked (the old `.*synapse` matched any
        # synapse token).
        "launchctl unload ai.synapse.update-checker.plist",
        "launchctl restart ai.synapse.daemon",
        ...
        "systemctl restart synapse-meta.service",
        "systemctl restart synapse-cron-helper",
        ...
        "echo after the synapse gateway restarted cleanly",
        "the synapse gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "synapse skill view gateway-notes && echo synapse gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `synapse` is a path component
        # there, not a command.
        "cat '/docs/synapse gateway restart-notes.md'",
        "less /home/user/notes/synapse gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "synapse gateway restart",
        "synapse gateway restart; echo done",
        "synapse gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;synapse gateway restart",
        "true && synapse gateway stop",
        "echo $(synapse gateway restart)",
        "echo `synapse gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.synapse.gateway"])',
        'subprocess.run(["synapse", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.synapse.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["synapse", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: synapse gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `synapse -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry SYNAPSE_HOME/SYNAPSE_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("SYNAPSE_PROFILE", "zeus")
        monkeypatch.delenv("SYNAPSE_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "synapse -p zeus gateway stop",
        "synapse -p zeus gateway restart",
        "synapse --profile zeus gateway restart",
        "synapse --profile zeus gateway stop",
        "synapse --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "synapse -v -p zeus gateway restart",
        "synapse -p zeus -v gateway restart",
        "synapse --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "synapse -p venus gateway stop",
        "synapse -p venus gateway restart",
        "synapse --profile venus gateway restart",
        "synapse --profile=venus gateway stop",
        "synapse -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "synapse -p zeus gateway start",
        "synapse -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("synapse gateway restart")
        assert _contains_gateway_lifecycle_command("synapse gateway stop")

    def test_synapse_home_derived_profile(self, monkeypatch):
        # Without SYNAPSE_PROFILE the guard falls back to the SYNAPSE_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("SYNAPSE_PROFILE", raising=False)
        monkeypatch.delenv("SYNAPSE_PROFILE_NAME", raising=False)
        import synapse_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("synapse -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("synapse -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("synapse -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("synapse gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_synapse_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade synapse then run synapse gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_synapse_gateway_once.sh). The script must live under
        # SYNAPSE_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path / ".synapse"))
        scripts_dir = tmp_path / ".synapse" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SYNAPSE_GATEWAY", raising=False)
        import synapse_cli.gateway as gw
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SYNAPSE_GATEWAY=1.

    Issue #37453: systemctl --user restart synapse-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        ...
        "systemctl --user restart synapse-gateway",
        "systemctl stop synapse-gateway.service",
        "synapse gateway restart",
        "synapse gateway uninstall",
        "launchctl kickstart gui/501/ai.synapse.gateway",
        "launchctl bootout gui/501/ai.synapse.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.synapse.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.synapse/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.synapse.gateway.restart-once.plist",
        "pkill -f synapse.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart synapse-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.synapse.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-synapse label: label-independent detection is the point
        # (#62891 second reproduction used `ai.synapse.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.synapse.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _SYNAPSE_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _SYNAPSE_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_SYNAPSE_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="synapse gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["synapse gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.synapse.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.synapse.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-synapse systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run synapse gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("synapse gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfesynapse gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SYNAPSE_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("synapse gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/synapse\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 synapse gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_synapse_home()` falls back to Path.home(), which raises
        RuntimeError when neither SYNAPSE_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart synapse-gateway' .",
        "grep -rn 'synapse gateway restart' ./logs",
        "rg 'synapse gateway restart' ../archive",
        "grep -c 'systemctl stop synapse-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM synapse_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell synapse gateway restart"',
        'sqlite3 ./db ".system systemctl restart synapse-gateway"',
        'psql ./x -c "\\! systemctl restart synapse-gateway"',
        "grep -r 'synapse gateway restart' . | sh",
        "grep -r 'synapse gateway restart' ./logs | bash",
        "grep -r 'synapse gateway restart' . | sudo sh",
        "grep -r 'x' . ; synapse gateway restart",
        "grep -r 'x' . && systemctl restart synapse-gateway",
        'grep -r "$(synapse gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart synapse-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell synapse gateway restart"',
        'sqlite3 db ".system systemctl restart synapse-gateway"',
        'psql -c "\\! systemctl restart synapse-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    synapse_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run synapse gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path / ".synapse"))
        (tmp_path / ".synapse").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run synapse gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "grep -c 'systemctl restart synapse-gateway' /var/log/syslog",
        "rg 'synapse gateway restart' /home/user/.synapse/logs/",
        "journalctl -u synapse-gateway --grep 'systemctl restart synapse-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'synapse_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell synapse gateway restart"',
        'psql -c "\\! systemctl restart synapse-gateway"',
        "grep 'systemctl restart synapse-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart synapse-gateway",
        'grep "$(systemctl restart synapse-gateway)" f',
        "grep 'restart' log; systemctl restart synapse-gateway",
        'sqlite3 db "SELECT 1"; synapse gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "synapse gateway restart",
        "sudo systemctl stop synapse-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart synapse-gateway%'\""
        )
