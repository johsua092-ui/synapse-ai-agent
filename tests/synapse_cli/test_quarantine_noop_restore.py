...
``synapse*.exe`` shims aside (``synapse.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``synapse`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("synapse", "synapse-agent", "synapse-acp", "synapse-gateway"):
...
# ---------------------------------------------------------------------------
# synapse_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "synapse.exe" in names, "synapse.exe must be restored after a no-op install"
    assert "synapse-acp.exe" in names
    assert "synapse-gateway.exe" in names
    assert (scripts / "synapse.exe").read_bytes() == b"MZ-old-synapse"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "synapse.exe").read_bytes() == b"MZ-new-synapse"
...
# ---------------------------------------------------------------------------
# synapse_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "synapse.exe" in names, "synapse.exe must be restored after a no-op recovery install"
    assert (scripts / "synapse.exe").read_bytes() == b"MZ-old-synapse"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "synapse.exe").write_bytes(b"MZ-new-synapse")
