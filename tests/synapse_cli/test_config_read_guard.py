...
Canonical owners:
  ...
  * ``synapse_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._synapse_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/synapse/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "synapse_cli/managed_scope.py",
