"""Tests for the unified provider catalog (synapse_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``synapse model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from synapse_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
