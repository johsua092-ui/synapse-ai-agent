...
#88251: ``synapse update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "synapse_constants.with_synapse_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "synapse_constants.display_synapse_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_synapse_home", lambda: tmp_path)
