...
``synapse serve``, the gateway, anything) → ``synapse serve`` printed only
...
* conflict → single stdout sentinel ``BACKEND_PORT_IN_USE port=<port>`` +
  ...
  transient-condition convention) — and NO ``SYNAPSE_BACKEND_READY``.
* free explicit port → boots and announces ``SYNAPSE_BACKEND_READY`` exactly
...
def test_probe_detects_held_socket():
    from synapse_cli.web_server import _port_bind_conflict
...
def test_addr_in_use_error_classification():
    ...
    from synapse_cli.web_server import _is_addr_in_use_error
...
def test_exit_code_is_distinct_tempfail():
    from synapse_cli.web_server import PORT_IN_USE_EXIT_CODE
...
# ---------------------------------------------------------------------------
# E2E: real start_server in a subprocess (temp SYNAPSE_HOME)
# ---------------------------------------------------------------------------
...
def _spawn_serve(port: int, tmp_path: Path) -> subprocess.Popen:
    ...
    env.update(
        ...
        SYNAPSE_SERVE_HEADLESS="1",
        PYTHONUNBUFFERED="1",
    )
    # Ensure a stray desktop-parent env can't arm the watchdog/reaper paths.
    for k in ("SYNAPSE_DESKTOP", "SYNAPSE_PARENT_PID", "SYNAPSE_PARENT_START_MARKER",
              "SYNAPSE_PARENT_NONCE"):
        env.pop(k, None)
    code = (
...
def test_conflict_emits_sentinel_and_exit_75(tmp_path):
    ...
    assert "SYNAPSE_BACKEND_READY" not in out  # never claimed ready
...
def test_free_port_boots_and_announces_ready(tmp_path):
    ...
    proc = _spawn_serve(port, tmp_path)
    try:
        ready, lines = _read_until(proc, "SYNAPSE_BACKEND_READY")
        out = "".join(lines)
        assert ready, f"no READY sentinel; output:\n{out}"
        assert f"SYNAPSE_BACKEND_READY port={port}" in out
...
def test_ephemeral_port_zero_unaffected(tmp_path):
    ...
    try:
        ...
        ready_line = next(l for l in lines if "SYNAPSE_BACKEND_READY" in l)
