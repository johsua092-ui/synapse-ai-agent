"""Tests for the post-pull HEAD-movement gate in ``synapse update``.
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the synapse_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns synapse_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/synapse_cli/test_cmd_update.py).
    """
    monkeypatch.setattr(synapse_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(synapse_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()  # pass the "is a git repo" gate
    monkeypatch.setattr(
        synapse_main, "_resolve_update_branch", lambda args: "main"
    )
    ...
    monkeypatch.setattr(
        synapse_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(synapse_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        synapse_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        synapse_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
        synapse_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        synapse_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    # Short-circuit the long tail: dependency install + desktop build.
    monkeypatch.setattr(synapse_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(synapse_main, "_clear_update_incomplete_marker", lambda: None)
    # Gateway restart path (called after a successful update).
    monkeypatch.setattr(synapse_main, "_finish_dashboard_update_cleanup", lambda *a: None)
    ...
    import synapse_cli.gateway as synapse_gateway

    monkeypatch.setattr(
        synapse_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        synapse_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
        synapse_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    synapse_main.cmd_update(args)  # completes normally (no SystemExit)
