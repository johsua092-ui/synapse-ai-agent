"""Tests for the top-level `./synapse` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./synapse` should use `synapse_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "synapse"
    called = []

    fake_main_module = types.ModuleType("synapse_cli.main")

    def fake_main():
        called.append("synapse_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "synapse_cli.main", fake_main_module)
    ...
    assert called == ["synapse_cli.main"]
