...
``SYNAPSE_PORTAL_BASE_URL=https://`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("SYNAPSE_PORTAL_BASE_URL", raising=False)
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "SYNAPSE_PORTAL_BASE_URL", "https://"
        )
        assert "" not in _NOUS_PORTAL_ALLOWED_HOSTS
        assert (
            _nous_portal_env_override() == "https://"
        )
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("synapse_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior SYNAPSE_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://"
        monkeypatch.setenv("SYNAPSE_HOME", str(tmp_path))
        monkeypatch.setenv("SYNAPSE_PORTAL_BASE_URL", staging_portal)
