"""Tests for synapse_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SYNAPSE_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Synapse-managed node dir.
"""
...
def _make_synapse_node(synapse_home: Path) -> Path:
    """Create a fake $SYNAPSE_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = synapse_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    synapse_home = fake_home / ".synapse"
    _make_synapse_node(synapse_home)
    ...
    removed = uninstall.remove_node_symlinks(synapse_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    synapse_home = fake_home / ".synapse"
    node_bin = _make_synapse_node(synapse_home)
