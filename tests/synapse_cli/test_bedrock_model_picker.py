...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SYNAPSE_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SYNAPSE_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SYNAPSE_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from synapse_cli.providers import SYNAPSE_OVERLAYS
        assert "bedrock" in SYNAPSE_OVERLAYS
