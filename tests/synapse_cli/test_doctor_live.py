"""Tests for ``synapse doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from synapse_cli import doctor_live
from synapse_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `synapse doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("synapse_cli.doctor.SYNAPSE_HOME", tmp_path / "home")
        monkeypatch.setattr("synapse_cli.doctor.PROJECT_ROOT", tmp_path / "root")
