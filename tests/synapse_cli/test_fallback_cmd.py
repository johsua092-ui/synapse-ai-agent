"""Tests for `synapse fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate SYNAPSE_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from synapse_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "Synapse-4-Llama-3.1-405B"},
            ]
        }
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from synapse_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("synapse_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("synapse_cli.main._require_tty"):
            from synapse_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / Synapse-4")
        with patch("synapse_cli.setup._curses_prompt_choice", return_value=1):
            from synapse_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from synapse_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `synapse fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "synapse_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
