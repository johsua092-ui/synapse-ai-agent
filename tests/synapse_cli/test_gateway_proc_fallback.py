...
See: johsua092-ui/synapse-ai-agent#7622
"""
...
_GATEWAY_CMD = "python -m synapse_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m synapse_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("synapse_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("synapse_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("synapse_cli.gateway.is_macos", return_value=True),
            patch("synapse_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "synapse_cli.gateway.get_launchd_label",
                return_value="ai.synapse.gateway.myprofile",
            ),
            patch(
                "synapse_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.synapse.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.synapse.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.synapse.gateway": 123,
            "ai.synapse.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "synapse_cli.gateway.get_launchd_label",
                return_value="ai.synapse.gateway",
            ),
            patch(
                "synapse_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.synapse.gateway", "ai.synapse.gateway-profile-b"],
            ),
            patch(
                "synapse_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "999\t0\tai.synapse.gateway-unmapped\n"
    ...
    def test_all_profiles_empty_when_no_gateway_labels(self):
        """When no ai.synapse.gateway* labels exist, all_profiles returns empty."""
        with (
            patch("synapse_cli.gateway.is_macos", return_value=True),
            patch("synapse_cli.gateway.supports_systemd_services", return_value=False),
            patch("subprocess.run") as mock_run,
    ...
    def test_all_profiles_handles_broken_pid_column(self):
        ...
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "-\t0\tai.synapse.gateway.broken\n"
                    "  123  \t0\tai.synapse.gateway.ok\n"
                ),
                stderr="",
            )
            pids = gateway_mod._get_service_pids(all_profiles=True)
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every synapse-gateway* unit unconditionally."""
        with (
            patch("synapse_cli.gateway.is_macos", return_value=False),
            patch("synapse_cli.gateway.supports_systemd_services", return_value=True),
            patch("subprocess.run") as mock_run,
        ):
            def _run_side_effect(args, **kwargs):
                ...
                if "list-units" in cmd_str:
                    return MagicMock(
                        returncode=0,
                        stdout="synapse-gateway-jarvis.service loaded active running\n",
                        stderr="",
                    )
