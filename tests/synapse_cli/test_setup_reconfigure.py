...
- Bare `synapse setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `synapse setup --quick` runs the narrower "fill in missing items" flow.
- `synapse setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("synapse_cli.setup.ensure_synapse_home", {}),
        ("synapse_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("synapse_cli.config.is_managed", {"return_value": False}),
        ("synapse_cli.setup.load_config", {"return_value": {}}),
        ("synapse_cli.setup.save_config", {}),
        ("synapse_cli.setup.get_env_value", {"return_value": None}),
        ("synapse_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("synapse_cli.setup._print_setup_summary", {}),
        ("synapse_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("synapse_cli.setup.load_config", {"return_value": {}}),
        ("synapse_cli.setup.save_config", {}),
        ("synapse_cli.auth.get_active_provider", {"return_value": None}),
        ("synapse_cli.setup.get_env_value", {"return_value": None}),
        ("synapse_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `synapse setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="synapse_cli.setup.prompt_choice",
                quick="synapse_cli.setup._run_quick_setup",
                model="synapse_cli.setup.setup_model_provider",
                terminal="synapse_cli.setup.setup_terminal_backend",
                agent="synapse_cli.setup.setup_agent_settings",
                gateway="synapse_cli.setup.setup_gateway",
                tools="synapse_cli.setup.setup_tools",
            )
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("synapse_cli.setup.prompt_choice", {"return_value": 0}),
                first="synapse_cli.setup._run_first_time_quick_setup",
            )
            from synapse_cli.setup import run_setup_wizard
            from synapse_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("synapse_cli.setup.prompt_choice", {"return_value": 2}),
                blank="synapse_cli.setup._run_blank_slate_setup",
            )
            from synapse_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "synapse_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["synapse", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
