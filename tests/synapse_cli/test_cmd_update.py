...
from synapse_cli.main import cmd_update, PROJECT_ROOT
...
def _patch_managed_uv(request):
    ...
    with patch("synapse_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("synapse_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("synapse_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def _patch_gateway_discovery():
    ...
    """
    with patch("synapse_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("synapse_cli.gateway.supports_systemd_services", return_value=False), \
         patch("synapse_cli.gateway.find_profile_gateway_processes", return_value=[]):
        yield


class TestCmdUpdateNpmLockfileCache:
    @staticmethod
    def _cache_file(synapse_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return synapse_root / f".npm_lock_hash_{cache_key}"



    def test_record_npm_lockfile_hash(self, tmp_path, monkeypatch):
        from synapse_cli import main as hm
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `synapse update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".synapse"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(synapse_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            synapse_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("SYNAPSE_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("SYNAPSE_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $SYNAPSE_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("synapse_cli.managed_uv.resolve_uv", lambda: None)
...
class TestUpdateManagedPythonEnvIsolation:
    ...
    forces UV_PYTHON_INSTALL_DIR to .synapse-runtime/python), then re-points
    ...
    def test_update_uv_env_points_venv_and_runtime_store(self):
        ...
        assert ".synapse-runtime" in uv_env.get("UV_PYTHON_INSTALL_DIR", "")
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind johsua092-ui/synapse-ai-agent silently misses updates.
        """
        from synapse_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/synapse-agent.git",
    ...
    def test_fork_upstream_sync_that_moves_head_runs_post_update_steps(
        ...
        with patch.object(
            ...
            "synapse_cli.update_receipt.collect_fleet_versions",
            return_value=[],
        ), patch(
            ...
            "synapse_cli.gateway.find_profile_gateway_processes",
            return_value=[],
        ), patch(
            "synapse_cli.gateway._get_service_pids",
            return_value=set(),
        ), patch.object(
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "synapse_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "synapse_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "synapse_cli.update_cmd._reload_config_modules"
        ), patch(
            "synapse_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "synapse_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("synapse_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "synapse_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "synapse_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "synapse_cli.update_cmd._reload_config_modules"
        ), patch(
            "synapse_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "synapse_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "synapse_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "synapse_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "synapse_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "synapse_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "synapse_cli.update_cmd._reload_config_modules"
        ), patch(
            "synapse_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "synapse_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("synapse_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    Before the fix, ``synapse update`` ran in the PRE-pull Python process.
    After ``git pull`` updated the source on disk, function-level imports
    returned the OLD cached ``synapse_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD synapse_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import synapse_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.synapse"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.synapse/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.synapse/profiles/work"))
        ...
        with (
            patch("synapse_cli.profiles.list_profiles", return_value=all_profiles),
            patch("synapse_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("synapse_cli.profiles.list_profiles", return_value=[default_p]),
            patch("synapse_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``synapse update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``synapse update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``synapse update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from synapse_cli.main import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "synapse_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        def write_source_zip(_url, destination):
            with zipfile.ZipFile(destination, "w") as archive:
                archive.writestr("synapse-agent-main/apps/desktop/package.json", "{}")
        ...
        monkeypatch.setattr(update_cmd, "get_synapse_home", lambda: tmp_path / "synapse-home")

        with (
            patch("synapse_cli.config.load_config", return_value={}),
            patch("subprocess.run", side_effect=fail_git_fetch),
            patch("urllib.request.urlretrieve", side_effect=write_source_zip),
            patch("synapse_cli.managed_uv.ensure_uv", return_value="uv"),
            patch("synapse_cli.managed_uv.update_managed_uv"),
            patch(
            ...
            patch("synapse_cli.model_catalog.seed_cache_from_checkout", return_value=False),
        ):
