...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "synapse_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # SYNAPSE_HOME.  For a standard install (SYNAPSE_HOME unset) that root is
        # the platform-native default (~/.synapse), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from synapse_constants import get_default_synapse_root
        assert env.get("SYNAPSE_HOME") == str(get_default_synapse_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (SYNAPSE_DESKTOP=1) must NOT
