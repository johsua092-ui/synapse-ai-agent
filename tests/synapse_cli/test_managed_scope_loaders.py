...
synapse_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # synapse_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("SYNAPSE_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import synapse_time

    assert synapse_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (SYNAPSE_TIMEZONE, SYNAPSE_REDACT_SECRETS, SYNAPSE_MAX_ITERATIONS,
    ...
    # writing SYNAPSE_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
