"""Tests for IRC gateway configuration via `synapse setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Synapse install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Synapse install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#synapse")
            monkeypatch.setenv("IRC_NICKNAME", "synapse-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `synapse setup gateway` experience with IRC present."""
