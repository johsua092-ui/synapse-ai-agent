...
When ``SYNAPSE_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.synapse/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.synapse            (has its own auth.json fixture)
    * Profile     -> tmp_path/.synapse/profiles/coder   (active, SYNAPSE_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Synapse is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (SYNAPSE_HOME != ~/.synapse, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    synapse_home = tmp_path / "classic"
    synapse_home.mkdir()
