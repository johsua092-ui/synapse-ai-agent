"""ensure_synapse_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".synapse"
    second = tmp_path / "b" / ".synapse"

    monkeypatch.setenv("SYNAPSE_HOME", str(first))
    cfg.ensure_synapse_home()

    # Profile switch: SYNAPSE_HOME moves → the new path is ensured too.
    monkeypatch.setenv("SYNAPSE_HOME", str(second))
    cfg.ensure_synapse_home()
