...
profile switcher can target any profile's SYNAPSE_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via SYNAPSE_HOME.
"""
import json
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_synapse_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_synapse_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import synapse_cli.mcp_config as mcp_config
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same SYNAPSE_HOME the rest
        ...
        monkeypatch.setattr(
            "synapse_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_synapse_home()))
            return None
        ...
        assert resp.json()["synapse_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_synapse_home(self, isolated_profiles, monkeypatch):
        import synapse_cli.web_server as web_server

        monkeypatch.setattr(
            "synapse_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["SYNAPSE_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "SYNAPSE_TUI_GATEWAY_URL" not in env
...
class TestProfileScopedAudio:
    ...
    def test_transcribe_runs_inside_target_profile_home(
        ...
        def _fake_transcribe(path):
            ...
            seen["home"] = str(get_synapse_home())
