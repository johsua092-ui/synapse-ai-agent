"""Tests for the per-profile Projects store (synapse_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Synapse Agent", folders=["/tmp/synapse"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "synapse-agent"
    assert proj.name == "Synapse Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/synapse"
    assert [f.path for f in proj.folders] == ["/tmp/synapse"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "synapse-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SYNAPSE_HOME.
