...
synapse_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `synapse config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_synapse_home = get_synapse_home()`` at import time (module
    singleton), so monkeypatching SYNAPSE_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SYNAPSE_HOME.)
    """
    import cli

    cli._synapse_home = home
