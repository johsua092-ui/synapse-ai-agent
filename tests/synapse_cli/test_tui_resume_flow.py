...
def main_mod(monkeypatch):
    import synapse_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_synapse_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import synapse_cli.oneshot as oneshot
        from synapse_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """synapse -z bypasses SynapseCLI, but recall still needs SessionDB."""
    from synapse_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "synapse_state", mod("synapse_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "synapse_cli.config",
        mod("synapse_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "synapse_cli.models",
        mod("synapse_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "synapse_cli.tools_config",
        mod("synapse_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["SYNAPSE_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/synapse-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["SYNAPSE_MODEL"] == "nous/synapse-test"
    assert env["SYNAPSE_INFERENCE_MODEL"] == "nous/synapse-test"
    assert env["SYNAPSE_TUI_PROVIDER"] == "nous"
    assert env["SYNAPSE_INFERENCE_PROVIDER"] == "nous"
    assert env["SYNAPSE_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["SYNAPSE_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("synapse-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_synapse_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("SYNAPSE_TUI_DIR", raising=False)
