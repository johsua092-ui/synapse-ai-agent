"""Tests for synapse_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `synapse setup` and `synapse import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("synapse_constants.is_container", lambda: container)
