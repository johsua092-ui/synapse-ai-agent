"""Orphan Desktop-local ``synapse serve`` reap at backend start.
...
def test_desktop_local_serve_shape_matches_ephemeral_loopback():
    assert _is_desktop_local_serve_cmdline(
        "python -m synapse_cli.main serve --host 127.0.0.1 --port 0"
    )
    assert _is_desktop_local_serve_cmdline(
        "synapse serve --isolated --host 127.0.0.1 --port 0 --ssh-owner-nonce abc"
    )
    assert _is_desktop_local_serve_cmdline(
        "/venv/bin/synapse serve --host=127.0.0.1 --port=0"
    )


def test_desktop_local_serve_shape_spares_fixed_port_and_non_serve():
    assert not _is_desktop_local_serve_cmdline(
        "synapse serve --host 100.106.105.2 --port 9119 --skip-build"
    )
    assert not _is_desktop_local_serve_cmdline(
        "synapse serve --host 127.0.0.1 --port 9119"
    )
    assert not _is_desktop_local_serve_cmdline("synapse gateway run --replace")
    assert not _is_desktop_local_serve_cmdline(
        "vim notes about synapse serve --port 0"
    )


def test_reap_only_kills_ppid1_local_serves():
    scanned = [
        (111, "synapse serve --host 127.0.0.1 --port 0"),  # orphan local
        (222, "synapse serve --host 127.0.0.1 --port 0"),  # still has parent
        (333, "synapse serve --host 100.1.2.3 --port 9119"),  # fixed remote
        (444, "synapse serve --isolated --host 127.0.0.1 --port 0"),  # orphan isolated
    ]
    ...
    with (
        patch(
            "synapse_cli.dashboard_procs._scan_dashboard_processes",
            return_value=scanned,
        ),
        patch(
            "synapse_cli.dashboard_procs._process_ppid",
            side_effect=lambda pid: ppids.get(pid),
        ),
        patch("os.kill", side_effect=fake_kill),
        patch("sys.platform", "darwin"),
    ):
        os.environ.pop("SYNAPSE_DESKTOP_CHILD_PID", None)
...
def test_reap_passes_child_pid_exclude_to_scan():
    with (
        ...
        patch("sys.platform", "darwin"),
        patch.dict(os.environ, {"SYNAPSE_DESKTOP_CHILD_PID": "999,111"}, clear=False),
    ):
...
def _valid_lock_payload(pid: int, ownership_id: str, spawn_nonce: str) -> dict:
    ...
    return {
        ...
        "port": 0,
        "profile": "default",
        "synapsePath": "/opt/synapse/bin/synapse",
        "synapseHome": "~/.synapse",
        "logPath": f"~/.synapse/desktop-ssh/{ownership_id}/{spawn_nonce}.log",
...
def test_valid_lockfile_payload_rejects_wrong_owner_and_shape():
    ...
    bad_log["logPath"] = "~/.synapse/desktop-ssh/{oid}/other.log".format(oid=oid)
...
def test_reap_spare_lock_owned_ssh_remote_backend_of_foreign_client():
    ...
    scanned = [
        (555, "synapse serve --host 127.0.0.1 --port 0"),  # lock-owned remote
        (666, "synapse serve --host 127.0.0.1 --port 0"),  # genuine orphan
    ]
    ppids = {555: 1, 666: 1}
    terms: list[int] = []
    live = {555, 666}
...
def test_reap_spares_young_backend_until_desktop_can_write_lock():
    ...
    scanned = [(777, "synapse serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
    ...
    with (
        ...
        patch("synapse_cli.dashboard_procs._process_ppid", return_value=1),
        patch("os.kill", side_effect=fake_kill),
        patch("sys.platform", "darwin"),
    ):
        result = _reap_orphaned_desktop_local_serves(
...
def test_reap_spares_backend_when_process_age_is_unknown():
    scanned = [(778, "synapse serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_age_boundary_makes_180_second_orphan_eligible():
    scanned = [
        (779, "synapse serve --isolated --host 127.0.0.1 --port 0"),
        (780, "synapse serve --isolated --host 127.0.0.1 --port 0"),
    ]
    terms: list[int] = []
    live = {779, 780}
...
def test_reap_spare_lock_owned_backend_even_without_exclude_match(tmp_path):
    ...
    spares a matching orphaned serve even when SYNAPSE_DESKTOP_CHILD_PID is
    ...
    scanned = [(4242, "synapse serve --host 127.0.0.1 --port 0")]
    terms: list[int] = []
