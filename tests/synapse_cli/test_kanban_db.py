"""Tests for the Kanban DB layer (synapse_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated SYNAPSE_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import synapse_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("SYNAPSE_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("SYNAPSE_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Synapse' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Synapse' DB/metadata/logs on task completion.
    """
...
# spawned with `synapse -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's SYNAPSE_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's SYNAPSE_HOME."""

    def _set_home(self, monkeypatch, tmp_path, synapse_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))
        monkeypatch.delenv("SYNAPSE_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.synapse/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.synapse/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.synapse/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # SYNAPSE_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile SYNAPSE_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # SYNAPSE_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is SYNAPSE_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["SYNAPSE_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["SYNAPSE_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["SYNAPSE_KANBAN_TASK"] == "t_dispatch_env"
        assert env["SYNAPSE_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "SYNAPSE_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see synapse_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import synapse_state as _synapse_state
    monkeypatch.setattr(
        _synapse_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _synapse_state._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    synapse_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("synapse_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="synapse_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        synapse_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "synapse_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'synapse kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_synapse_argv()
#
# Workers spawned by the dispatcher must use a `synapse` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["synapse", ...]`
...
def test_resolve_synapse_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m synapse_cli.main`.

    Pins the correct module name (NOT `synapse` — there is no top-level
    `synapse` package). Regression for #23198: the original PR shipped
    `python -m synapse` which fails with `No module named synapse` on every
    invocation.
    """
    import shutil
    import sys
    import synapse_cli.kanban_db as kb

    monkeypatch.delenv("SYNAPSE_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_synapse_argv()
    assert argv == [sys.executable, "-m", "synapse_cli.main"]


def test_resolve_synapse_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `synapse_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m synapse_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("SYNAPSE_BIN", None)
    ...
    assert "Synapse Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="synapse_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from synapse_cli.kanban_db import connect
