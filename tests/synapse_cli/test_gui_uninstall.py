"""Tests for synapse_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import synapse_cli.gui_uninstall as gu


def _make_agent(synapse_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = synapse_home / "synapse-agent"
    (agent_root / "synapse_cli").mkdir(parents=True)
    (agent_root / "synapse_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(synapse_home: Path) -> None:
    """Create the source-built GUI artifacts a `synapse desktop` run produces."""
    desktop = synapse_home / "synapse-agent" / "apps" / "desktop"
    ...
    (synapse_home / "synapse-agent" / "node_modules").mkdir(parents=True)
    (synapse_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(synapse_home: Path) -> None:
    (synapse_home / "config.yaml").write_text("x: 1\n")
    (synapse_home / ".env").write_text("KEY=secret\n")
    (synapse_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(synapse_home)
    _make_gui_build(synapse_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(synapse_home)
    ...
    assert summary["synapse_home"] == str(synapse_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `synapse desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    synapse_home = tmp_path / ".synapse"
    _make_agent(synapse_home)
    icon = lde.icon_path(synapse_home / "synapse-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(synapse_home)
    ...
    assert lde.icon_path(synapse_home / "synapse-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (synapse_home / "synapse-agent" / "synapse_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".synapse")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import synapse_cli.uninstall as uninstall
