...
Before the fix, ``synapse whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``synapse gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SYNAPSE_HOME", str(synapse))
...
def _env_value(synapse_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from synapse_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("synapse_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``synapse whatsapp`` with an existing paired session and
