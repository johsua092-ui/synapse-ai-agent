...
def synapse_home(tmp_path, monkeypatch):
    ...
    # /etc/synapse on the dev/CI box can't influence the test.
    monkeypatch.setenv("SYNAPSE_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(synapse_home, monkeypatch):
...
def test_env_expansion_in_user_config(synapse_home, monkeypatch):
    from synapse_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_synapse_dotenv(synapse_home=str(home))
