...
Regression guard for the bug where `synapse model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`synapse_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in SYNAPSE_OVERLAYS
        overlay = SYNAPSE_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `synapse_cli/main.py` (cmd_model): read
