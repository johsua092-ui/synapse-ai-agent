...
lane). Spawns REAL processes with realistic Synapse argv shapes and drives
...
the empirical premise-check for each issue:
  ...
  #90778 — holder message mislabels `synapse dashboard` as the Desktop
...
def _detect() -> list[tuple[int, str, str]]:
    from synapse_cli.update_cmd import _detect_venv_python_processes
...
class TestDetection:
    def test_detects_synapse_argv_process(self):
        """Baseline: a live process running `-m synapse_cli.main serve` with
        cwd under the install root is detected as a venv holder."""
        proc = _spawn(["-m", "synapse_cli.main", "serve"])
        try:
            matches = _detect()
            ...
            assert "synapse_cli.main" in cmdline
        finally:
            _kill(proc)

    def test_foreign_python_not_detected(self):
        """A python process with no Synapse argv and cwd OUTSIDE the install
    ...
    def test_long_runtime_path_gateway_detected_with_full_argv(self):
        ...
        padding = os.path.join("C:\\", "Users", "x" * 90, ".synapse-runtime")
        proc = _spawn([padding, "-m", "synapse_cli.main", "gateway", "run"])
        try:
            matches = _detect()
...
class TestClassification:
    def test_pausable_exemption_sees_long_path_gateway(self):
        ...
        from synapse_cli.update_cmd import _leftover_pausable_gateway_pids

        padding = os.path.join("C:\\", "Users", "y" * 90, ".synapse-runtime")
        proc = _spawn([padding, "-m", "synapse_cli.main", "gateway", "run"])
        try:
...
class TestHolderMessage:
    ...
    def test_dashboard_not_labeled_desktop_backend(self):
        from synapse_cli.update_cmd import _format_venv_python_holders_message

        proc = _spawn(["-m", "synapse_cli.main", "dashboard"])
        try:
            matches = [m for m in _detect() if m[0] == proc.pid]
            ...
            assert "close the desktop app" not in message.lower(), (
                "standalone `synapse dashboard` mislabeled as the Desktop "
                f"backend (#90778):\n{message}"
            )
        finally:
            _kill(proc)
    ...
    def test_substring_subcommand_not_mislabeled(self):
        ...
        proc = _spawn(["-m", "synapse_cli.main", "kanban", "--preserve-cache"])
        try:
            matches = [m for m in _detect() if m[0] == proc.pid]
...
class TestAncestorExclusion:
    ...
    def test_gateway_parent_visible_to_child_scan(self, tmp_path):
        ...
        # `... -m synapse_cli.main gateway run`, not a multi-line -c blob).
