"""Regression: synapse doctor must not run a generic Bearer-auth health
...
with HTTP 404. The dedicated check at synapse_cli/doctor.py already covers
Anthropic with the right headers, so the pluggable profile must be
skipped by `_build_apikey_providers_list()`.

See: johsua092-ui/synapse-ai-agent#22346
"""
...
def test_build_apikey_providers_list_skips_dedicated_check_providers():
    from synapse_cli import doctor
