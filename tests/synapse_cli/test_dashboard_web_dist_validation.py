"""Regression tests: `synapse dashboard` validates SYNAPSE_WEB_DIST before serving.

A custom SYNAPSE_WEB_DIST without --skip-build previously skipped BOTH the
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "synapse_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "synapse_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )


def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """SYNAPSE_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("SYNAPSE_WEB_DIST", str(empty_dist))
    ...
    assert "SYNAPSE_WEB_DIST" in out and str(empty_dist) in out
