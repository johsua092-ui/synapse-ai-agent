"""Tests for the AI_AGENT / SYNAPSE_AGENT harness-attribution env vars.
...
Synapse-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Synapse' id in the public agent-harness
registry (``synapse-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Synapse process env, and survives the
cross-session leak guard that strips ``SYNAPSE_SESSION_*`` from subprocess
...
from synapse_cli.main import _advertise_agent_env

# Registry id — must stay in sync with huggingface.js agent-harnesses.ts.
HARNESS_ID = "synapse-agent"
...
class TestAdvertiseAgentEnv:
    def test_sets_both_vars_when_unset(self, monkeypatch):
        ...
        assert os.environ["SYNAPSE_AGENT"] == "true"
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__SYNAPSE_CWD_testsession0__"
        env._snapshot_path = "/tmp/synapse-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'SYNAPSE_AGENT="${SYNAPSE_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT SYNAPSE=$SYNAPSE_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "SYNAPSE_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} SYNAPSE=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", SYNAPSE_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi SYNAPSE=false" in out.stdout
