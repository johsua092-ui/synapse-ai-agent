...
the SAME provider universe as ``synapse model`` (the CLI/TUI picker).
...
def test_every_synapse_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `synapse model` universe."""
    ...
    assert not missing, (
        "providers shown in `synapse model` but not configurable in the desktop "
