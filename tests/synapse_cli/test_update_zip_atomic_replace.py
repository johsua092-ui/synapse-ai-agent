...
which broke ``synapse --tui`` because ``ui-tui/`` had vanished.
...
from synapse_cli.main import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.synapse-update-staging").exists()
    assert not (dst.parent / "ui-tui.synapse-update-old").exists()
