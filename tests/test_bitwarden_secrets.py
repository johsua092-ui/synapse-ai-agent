...
binary download are exercised manually by `synapse secrets bitwarden
...
def synapse_home(tmp_path, monkeypatch):
    """Point Synapse at an isolated home directory."""
    home = tmp_path / ".synapse"
    home.mkdir()
    monkeypatch.setenv("SYNAPSE_HOME", str(home))
    # Some modules cache get_synapse_home; clear if needed.
    import synapse_constants
    if hasattr(synapse_constants, "_SYNAPSE_HOME_CACHE"):
        synapse_constants._SYNAPSE_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(synapse_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from synapse_cli.env_loader import _apply_external_secret_sources
