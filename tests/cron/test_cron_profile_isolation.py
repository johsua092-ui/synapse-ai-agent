...
profile's SYNAPSE_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.synapse/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active SYNAPSE_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever SYNAPSE_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    SYNAPSE_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import synapse_constants

    monkeypatch.setattr(
        synapse_constants, "_get_platform_default_synapse_home", lambda: root
    )
    monkeypatch.setenv("SYNAPSE_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile SYNAPSE_HOME (<root>/profiles/<name>), the cron store
    ...
    assert synapse_constants.get_synapse_home().resolve() == profile_home.resolve()
    assert synapse_constants.get_default_synapse_root().resolve() == root.resolve()

    # cron/jobs.py computes SYNAPSE_DIR from get_synapse_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.SYNAPSE_DIR.resolve() == profile_home.resolve()
        assert (
