...
the shared /blueprint command handler (synapse_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated SYNAPSE_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("synapse://blueprint/morning-brief?")
