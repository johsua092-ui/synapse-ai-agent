...
def cron_store(tmp_path, monkeypatch):
    ...
    (synapse_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("SYNAPSE_HOME", str(synapse_home))
    monkeypatch.setattr(J, "SYNAPSE_DIR", synapse_home)
    monkeypatch.setattr(J, "CRON_DIR", synapse_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", synapse_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", synapse_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_synapse_now", lambda: now):
            due = J.get_due_jobs()
