...
which is already bounded by SYNAPSE_CRON_TIMEOUT — the worker thread submitted
...
timeout (SYNAPSE_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SYNAPSE_CRON_SESSION_DB_TIMEOUT", "0.2")
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed SYNAPSE_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors SYNAPSE_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("SYNAPSE_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "SYNAPSE_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("SYNAPSE_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._synapse_home", tmp_path), \
                 ...
                 patch("synapse_cli.env_loader.load_synapse_dotenv"), \
                 patch("synapse_cli.env_loader.reset_secret_source_cache"), \
                 patch("synapse_state.SessionDB", side_effect=_hanging_then_capture), \
                 patch(
                     "synapse_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
