...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.synapse/synapse-agent/utils.py)
...
2. ``synapse update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``synapse_cli/main.py`` (the ``synapse update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``synapse_constants`` after the
