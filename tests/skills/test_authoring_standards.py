...
def test_required_frontmatter_fields(p):
    ...
    synapse = (fm.get("metadata") or {}).get("synapse") or {}
    if not (synapse.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.synapse.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    synapse = (fm.get("metadata") or {}).get("synapse") or {}
    dangling = [
        rs for rs in (synapse.get("related_skills") or []) if rs not in _all_names()
    ]
