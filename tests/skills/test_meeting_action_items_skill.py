...
def test_frontmatter_required_fields():
    ...
    synapse = fm["metadata"]["synapse"]
    assert synapse["tags"]
    assert "related_skills" in synapse
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Synapse Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["synapse"]["related_skills"]:
        hits = (
