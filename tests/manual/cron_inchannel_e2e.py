...
Uses a throwaway SYNAPSE_HOME so it never touches ~/.synapse. No real names.
"""
...
def _fresh_home():
    """Point SYNAPSE_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_synapse_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["SYNAPSE_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER SYNAPSE_HOME is set.
...
# a different get_synapse_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SYNAPSE_HOME (throwaway): {HOME}")
