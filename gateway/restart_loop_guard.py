...
Defenses 1 and 2 (the ``_SYNAPSE_GATEWAY`` guard on ``synapse gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.synapse.gateway")``,
...
State lives in ``<SYNAPSE_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_synapse_home() / "gateway" / "restart_loop.json"
