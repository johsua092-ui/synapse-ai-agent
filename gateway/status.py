...
The PID file lives at ``{SYNAPSE_HOME}/gateway.pid``.  SYNAPSE_HOME defaults to
``~/.synapse`` but can be overridden via the environment variable.  This means
separate SYNAPSE_HOME directories naturally get separate PID files — a property
...
from synapse_constants import get_synapse_home, _get_platform_default_synapse_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_synapse_home() / "gateway-starts.log"
...
def _get_process_synapse_home() -> Path:
    """Return the process-level SYNAPSE_HOME, skipping context-local overrides.
    ...
    ``get_synapse_home()`` honors ``_SYNAPSE_HOME_OVERRIDE`` contextvar used for
...
def _canonical_synapse_home(path: Path | str) -> Path:
    """Return a stable absolute SYNAPSE_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_synapse_home(left: Path | str, right: Path | str) -> bool:
    """Compare SYNAPSE_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_synapse_home(left))) == os.path.normcase(
        str(_canonical_synapse_home(right))
    )


# Mirrors synapse_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (synapse_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a SYNAPSE_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.synapse/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ``"default"`` for the deployment's root home, and ``None`` when no label
    can be inferred.  Never raises — this feeds diagnostics only.
    """
    try:
        canonical = _canonical_synapse_home(home)
    except Exception:
        return None
    ...
    try:
        ...
        if _same_synapse_home(canonical, get_default_synapse_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_synapse_home(canonical, _get_platform_default_synapse_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    SYNAPSE_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``synapse_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SYNAPSE_HOME."""
    home = _get_process_synapse_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SYNAPSE_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "synapse" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Synapse' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Synapse gateway lifecycle subcommand from a command line.
    ...
    test also matched ``synapse_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\synapse-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Synapse's
    ...
    for token in tokens:
        ...
        if basename in ("synapse-gateway", "synapse-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "synapse_cli.main" in joined
        or "synapse_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("synapse", "synapse.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `synapse gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Synapse-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Synapse gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a SYNAPSE_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.synapse`` or ``$SYNAPSE_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``synapse_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``SYNAPSE_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a SYNAPSE_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"synapse_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit SYNAPSE_HOME= on the argv. SYNAPSE_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "synapse_home=" in command_lc and f"synapse_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # SYNAPSE_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "synapse_home": str(_canonical_synapse_home(_get_process_synapse_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``synapse_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `synapse update` (and the
    ...
    """
    try:
        from synapse_cli.build_info import get_code_identity
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `synapse status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``SYNAPSE_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``synapse_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    SYNAPSE_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SYNAPSE_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from synapse_home.
    profile = _profile_label_for_home(_get_process_synapse_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``synapse.service`` + ``synapse-
# gateway.service``).
#
...
def _get_takeover_marker_path(synapse_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``synapse_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = synapse_home or _get_process_synapse_home()
    return _canonical_synapse_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-SYNAPSE_HOME --replace while
    ...
    our_home = _get_process_synapse_home()
    target_home = record.get("target_synapse_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_synapse_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_synapse_home")
        if replacer_home is not None and not _same_synapse_home(
    ...
    # ``synapse gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's SYNAPSE_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_synapse_home(
            target_home or _get_process_synapse_home()
        )
        ...
        record = {
            ...
            "target_synapse_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_synapse_home": str(
                _canonical_synapse_home(_get_process_synapse_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target SYNAPSE_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("synapse_home")
    if not isinstance(pid_record_home, str) or not _same_synapse_home(
