...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<synapse_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active SYNAPSE_HOME."""
    from synapse_constants import get_synapse_home

    flush_dir = get_synapse_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<synapse_home>/pending_messages/``), so a runtime cap rotation no
