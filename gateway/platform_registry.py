...
class PlatformEntry:
    ...
    # status displays (``synapse setup``, ``synapse status``, the dashboard
    ...
    # Env vars this platform needs (for ``synapse setup`` display).
    ...
    # built-ins).  Used by ``synapse gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved SYNAPSE_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `synapse`
        # invocation -- including plain `synapse chat`, which never touches any
        ...
        # `synapse setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``synapse setup``/``gateway status``, channel
