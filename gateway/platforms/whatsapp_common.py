...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Synapse Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SYNAPSE_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/synapse), this function
    mirrors the bridge source to a writable SYNAPSE_HOME location and returns that
    ...
    # Try SYNAPSE_HOME location first
    synapse_home = get_synapse_home()
    synapse_home_bridge = synapse_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SYNAPSE_HOME if needed
    if synapse_home_bridge.exists():
        return synapse_home_bridge

    # Mirror the bridge source to SYNAPSE_HOME
    try:
        synapse_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            synapse_home_bridge,
            dirs_exist_ok=False,
