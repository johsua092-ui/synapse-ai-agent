...
There are two ways to add a platform to the Synapse gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.synapse/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Synapse code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `synapse gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `synapse_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Synapse core.
...
And add it to the `synapse-gateway` composite:

```python
"synapse-gateway": {
    "includes": [..., "synapse-your-platform"]
}
```
...
## 12. Status Display (`synapse_cli/status.py`)
...
## 13. Gateway Setup Wizard (`synapse_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ synapse_cli/ toolsets.py \
  --include="*.py" -l | sort -u
