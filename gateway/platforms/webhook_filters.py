...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.synapse to the active profile home."""
    ...
    if raw.startswith("~/.synapse/"):
        return synapse_home / raw.removeprefix("~/.synapse/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return synapse_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under SYNAPSE_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from synapse_constants import get_synapse_home

    scripts_root = (get_synapse_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.synapse" or raw_text.startswith("~/.synapse/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__synapse_ignore__") is True
        ):
            return False, None
