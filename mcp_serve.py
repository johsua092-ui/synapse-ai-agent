"""
Synapse MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Synapse-specific extra)

Usage:
    synapse mcp serve
    synapse mcp serve --verbose
...
logger = logging.getLogger("synapse.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SYNAPSE_HOME."""
    try:
        from synapse_constants import get_synapse_home
        return get_synapse_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SYNAPSE_HOME", Path.home() / ".synapse")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from synapse_constants import get_synapse_home
        directory_file = get_synapse_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SYNAPSE_HOME", Path.home() / ".synapse")
...
class EventBridge:
    ...
    This is the Synapse equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from synapse_constants import get_synapse_home
            db_file = get_synapse_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SYNAPSE_HOME", Path.home() / ".synapse")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Synapse MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "synapse",
        instructions=(
            "Synapse Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Synapse MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
