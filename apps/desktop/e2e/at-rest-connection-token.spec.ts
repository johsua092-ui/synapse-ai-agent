 ...
 * dir (macOS `~/Library/Application Support/Synapse/connection.json`, Windows
 * `AppData\Roaming\Synapse\connection.json`). Anything that can read the file
 ...
 *      or in SYNAPSE_HOME (logs included).
 ...
 * `X-Synapse-Session-Token` header it receives — a dropped or mangled token
 ...
 * gateway token: `51c68d4ab` ("Add Synapse desktop app (#20059)"), the commit
 ...
 */
const SENTINEL_TOKEN = 'synapse-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'SynapseE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-Synapse-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote Synapse gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-synapse-session-token']
 ...
 * backend, so we skip spawning `synapse serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startSynapse() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    SYNAPSE_DESKTOP_APP_NAME: STABLE_APP_NAME,
    SYNAPSE_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { synapseDesktop?: Record<string, unknown> }).synapseDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture Synapse should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { synapseDesktop: any }).synapseDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // SYNAPSE_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.synapseHome, needles),
      'the gateway session token leaked into a SYNAPSE_HOME file (logs included)',
    ).toEqual([])
