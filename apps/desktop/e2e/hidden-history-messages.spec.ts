...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.synapseHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.synapseHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.synapseHome)
  const builder = await RealSessionBuilder.start(sandbox.synapseHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.synapseHome, mock.url)
  fs.appendFileSync(path.join(sandbox.synapseHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.synapseHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
