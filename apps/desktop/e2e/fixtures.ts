/**
 * Shared E2E fixtures for the Synapse desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → synapse serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `synapse desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `synapse-e2e-${prefix}-${Math.random()}`))
  const synapseHome = path.join(root, 'synapse-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(synapseHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(synapseHome, 'config.yaml')
 ...
 */
export function writeEnvFile(synapseHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(synapseHome, '.env')
 ...
 */
function writeEmptyConfig(synapseHome: string): void {
  const configPath = path.join(synapseHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - SYNAPSE_HOME → sandbox synapse-home (isolated config/sessions)
 *  - SYNAPSE_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - SYNAPSE_DESKTOP_IGNORE_EXISTING=1 → don't pick up `synapse` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - SYNAPSE_DESKTOP_SYNAPSE_ROOT → REPO_ROOT (dev checkout resolution)
 *  - SYNAPSE_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    SYNAPSE_HOME: sandbox.synapseHome,
    SYNAPSE_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    SYNAPSE_DESKTOP_IGNORE_EXISTING: '1',
    SYNAPSE_DESKTOP_SYNAPSE_ROOT: REPO_ROOT,
    SYNAPSE_DESKTOP_APP_NAME: `SynapseE2E-${Date.now()}`,
    ...
    SYNAPSE_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated SYNAPSE_HOME + userData
 * @param env      - the process environment (already has SYNAPSE_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.synapseHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.synapseHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via SYNAPSE_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`synapse serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.synapseHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { SYNAPSE_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Synapse backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
 ...
 * progress without spawning a real Synapse backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * SYNAPSE_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    SYNAPSE_DESKTOP_BOOT_FAKE: '1',
    SYNAPSE_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + synapse-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).SYNAPSE_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).SYNAPSE_DESKTOP_SYNAPSE
  delete (env as Record<string, string | undefined>).SYNAPSE_DESKTOP_SYNAPSE_ROOT
