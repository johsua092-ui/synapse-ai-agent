...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Synapse E2E'], { cwd: repo })
...
function configureRepoCwd(synapseHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(synapseHome, mockUrl)
  fs.appendFileSync(path.join(synapseHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(synapseHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.synapseHome, mock.url, repo)
