 ...
 * AIAgent loop, using the E2E mock provider configured in `synapseHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(synapseHome: string) {
  ...
  static async start(synapseHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(synapseHome)
