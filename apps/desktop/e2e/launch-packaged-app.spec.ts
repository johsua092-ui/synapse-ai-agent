...
/**
 * E2E smoke tests for the packaged Synapse desktop app.
 *
 ...
 * (credential stripping, isolated SYNAPSE_HOME + userData, unique app name).
 *
...
test('window opens with the Synapse title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      synapseDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).synapseDesktop?.hud?.open({ sessionId: null })
  )
