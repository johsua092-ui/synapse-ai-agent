 ...
 * packaged binary) with a real `synapse serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → synapse serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Synapse title', async () => {
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { synapseDesktop?: Record<string, unknown> }).synapseDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → synapse serve is spawned → WS connects → config
