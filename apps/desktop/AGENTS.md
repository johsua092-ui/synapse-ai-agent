...
How to build Synapse Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Synapse surface can also
...
- A **runtime home change** (switching the underlying `SYNAPSE_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Synapse — do not assume
