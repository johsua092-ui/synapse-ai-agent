...
// no longer bundles the Synapse Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Synapse.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Synapse'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Synapse.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Synapse'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SYNAPSE_HOME for non-sandboxed runs -- matches main.ts's
// resolveSynapseHome(). On Windows it's %LOCALAPPDATA%\synapse; elsewhere
// it's ~/.synapse. The fresh-install sandbox launchFresh() sets its own
// SYNAPSE_HOME and never touches this.
const DEFAULT_SYNAPSE_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'synapse')
  }
  return path.join(os.homedir(), '.synapse')
})()
const VENV_ROOT = path.join(DEFAULT_SYNAPSE_HOME, 'synapse-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'synapse-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SYNAPSE_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Synapse-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Synapse-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Synapse-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Synapse-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SYNAPSE_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SYNAPSE_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const synapseHome = path.join(sandbox, 'synapse-home')
  ...
  env.SYNAPSE_DESKTOP_CWD = cwd
  env.SYNAPSE_DESKTOP_IGNORE_EXISTING = '1'
  env.SYNAPSE_DESKTOP_TEST_MODE = 'fresh-install'
  env.SYNAPSE_DESKTOP_USER_DATA_DIR = userDataDir
  env.SYNAPSE_HOME = synapseHome
  delete env.SYNAPSE_DESKTOP_SYNAPSE
  delete env.SYNAPSE_DESKTOP_SYNAPSE_ROOT
  ...
  console.log(`  SYNAPSE_HOME: ${synapseHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(synapseHome, 'synapse-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Synapse Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of synapse_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'synapse-agent', 'synapse_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Synapse
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SYNAPSE_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SYNAPSE_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
