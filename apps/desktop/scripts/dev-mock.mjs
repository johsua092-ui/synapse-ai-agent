...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `synapse-dev-mock-${Date.now()}`))
  ...
  return { root, synapseHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(synapseHome, mockUrl) {
  fs.writeFileSync(
    path.join(synapseHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(synapseHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.synapseHome, mock.url)
  console.log(`  SYNAPSE_HOME: ${sandbox.synapseHome}`)
  ...
  const env = {
    ...
    SYNAPSE_DESKTOP_APP_NAME: `SynapseDevMock-${Date.now()}`,
  }
