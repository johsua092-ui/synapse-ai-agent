...
const UPSTREAM_URL = 'https://github.com/johsua092-ui/synapse-ai-agent.git'
...
const LEGACY_STORAGE_KEY = 'synapse.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'synapse.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.synapseDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `synapse-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const synapseHome = join(sandbox, 'synapse-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(synapseHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(synapseHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        SYNAPSE_DESKTOP_APP_NAME: `SynapseShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        SYNAPSE_DESKTOP_SYNAPSE_ROOT: target.targetRoot,
        SYNAPSE_DESKTOP_IGNORE_EXISTING: '1',
        SYNAPSE_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'synapse-short-session-ab-'))
  const prepared = []
