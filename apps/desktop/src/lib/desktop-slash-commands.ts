...
/** Why a known Synapse command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Synapse profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Synapse Agent version', surface: exec() },
...
function isKnownSynapseSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Synapse' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSynapseSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Synapse and were never asked for. It is
