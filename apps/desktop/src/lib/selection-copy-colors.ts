...
// Synapse is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.synapseMode
 ...
 * the only carriers of Synapse' palette — are dropped so the paste target's
