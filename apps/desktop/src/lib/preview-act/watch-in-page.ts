...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __synapseWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__synapseWatch || (win.__synapseWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('synapse-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('synapse-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __synapseWatchTag?: number }).__synapseWatchTag
