 ...
 * `synapse://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `synapse://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `synapse://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type SynapseOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const SYNAPSE_PROTOCOL = 'synapse:'
...
export function normalizeSynapseOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('synapse://') || trimmed.startsWith(`${SYNAPSE_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // synapse://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // synapse://index-network/intent/1 → /index-network/intent/1
...
export function resolveSynapseOpenPath(target: SynapseOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeSynapseOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeSynapseOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeSynapseOpenString(target.path)
...
export function pathFromSynapseDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  ...
  if (kind === 'open') {
    return resolveSynapseOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveSynapseOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `synapse://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromSynapseDeepLink('open', name, params)
}
