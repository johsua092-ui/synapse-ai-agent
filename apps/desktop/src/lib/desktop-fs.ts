...
import type {
  SynapseConnection,
  SynapseReadDirResult,
  SynapseReadFileTextResult,
  SynapseSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: SynapseConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: SynapseConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<SynapseReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<SynapseReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<SynapseReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<SynapseReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.synapseDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: SynapseSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
