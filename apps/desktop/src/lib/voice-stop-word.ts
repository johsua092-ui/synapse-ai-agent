...
//
// When someone is in a hands-free "Hey Synapse" voice chat, the natural way to
...
// phrase (optionally addressed to Synapse), so a real turn that merely contains
...
// Optional address prefixes so "synapse stop" / "ok stop" / "hey synapse, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey synapse', 'hey synapse,', 'synapse', 'synapse,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("synapse") is not a stop command on its own.
      continue
    }
 ...
 * to Synapse). Returns false for anything that merely contains "stop" as part of
