import type { GatewayWsUrlResult } from '@synapse/shared'
import type { TranslucencyState } from '@synapse/shared/translucency'
...
declare global {
  interface Window {
    synapseDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SynapseConnection>
      ...
      // Resume this session in the user's own terminal emulator (`synapse --tui
      ...
      // Synapse Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new SYNAPSE_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SynapseApiRequest) => Promise<T>
      notify: (payload: SynapseNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<SynapseReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<SynapseReadFileTextResult>
      selectPaths: (options?: SynapseSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SynapsePreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SynapsePreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<SynapsePreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: SynapseActiveWork) => void
      setTitleBarTheme?: (payload: SynapseTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SynapseReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<SYNAPSE_HOME>/desktop-plugins),
      ...
      /** LOCAL `<SYNAPSE_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<SYNAPSE_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<SynapseGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<SynapseGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<SynapseGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<SynapseRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: SynapseReviewScope, baseRef?: null | string) => Promise<SynapseReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: SynapseReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<SynapseReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<SynapseRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<SynapsePrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: SynapseTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SynapseTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SynapseWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: SynapsePreviewFileChanged) => void) => () => void
...
export interface SynapseTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SynapseTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `synapse update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  synapseRoot?: string
...
export interface SynapseTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface SynapseActiveWork {
  count: number
  titles: string[]
}

export interface SynapseWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Synapse Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Synapse Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Synapse Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the synapse gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Synapse Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface SynapseApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface SynapsePreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SynapseReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SynapsePreviewWatch {
  id: string
  path: string
}
...
export interface SynapseGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface SynapseGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface SynapseGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface SynapseRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface SynapseRepoStatus {
  branch: null | string
...
// turn began.
export type SynapseReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface SynapseReviewFile {
  path: string
  added: number
  removed: number
...
export interface SynapseReviewList {
  files: SynapseReviewFile[]
...
export interface SynapseReviewPr {
  url: string
  state: string
  number: number
}
...
export interface SynapseBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface SynapseRepoPullRequests {
  ghReady: boolean
  prs: SynapseBranchPullRequest[]
}
...
export interface SynapsePrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface SynapseReviewShipInfo {
  ghReady: boolean
  pr: SynapseReviewPr | null
}

export interface SynapseReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SynapseReadDirResult {
  entries: SynapseReadDirEntry[]
  error?: string
}

export interface SynapsePreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SynapseSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
