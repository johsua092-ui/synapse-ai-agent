 ...
 *   (`@synapse/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default SynapsePlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<synapse home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<synapse home>/plugins/<name>/desktop/
...
import { createPluginContext, type SynapsePlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.synapse/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@synapse/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @synapse/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: SynapsePlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default SynapsePlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of synapse-bots predating its adoption in-tree) must
...
//  - `<synapse home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<synapse home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's synapse_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.synapse/plugins` is installed-but-inert until the
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update Synapse Desktop to load larger plugins"
    )
  }
