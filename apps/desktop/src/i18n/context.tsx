...
export interface I18nConfigClient {
  getConfig: () => Promise<SynapseConfigRecord>
  saveConfig: (config: SynapseConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.synapseDesktop?.api) {
      return Promise.resolve({})
    }

    return getSynapseConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.synapseDesktop?.api) {
    ...
    return saveSynapseConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: SynapseConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SynapseConfigRecord, locale: Locale): SynapseConfigRecord {
