...
function sdkNamedImports(source) {
  const match = source.match(/import\s+\{([\s\S]*?)\}\s+from '@synapse\/plugin-sdk'/)
...
function writeLegacySdk(root) {
  const packageRoot = join(root, 'node_modules', '@synapse', 'plugin-sdk')
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: '@synapse/plugin-sdk', type: 'module', exports: './index.js' }, null, 2)}\n`
  )
...
test('legacy SDK without optional capability exports still links Bot Mode', async t => {
  const root = mkdtempSync(join(tmpdir(), 'synapse-bot-mode-legacy-sdk-'))
  ...
  assert.equal(loaded.default.id, 'synapse-bots')
