...
// Bot Mode title or `synapse profile rename` display_name — must update what
...
test('mentionNameForms: slugged + collapsed, reserved tokens dropped', () => {
  ...
  // A bot renamed "Synapse" or "everyone" cannot hijack reserved tags.
  assert.equal(mentionNameForms('Synapse').length, 0)
...
test('botMentionTag: renamed slug wins, profile handle is the fallback', () => {
  ...
  assert.equal(botMentionTag({ name: 'default' }), 'synapse')
  // display_name (synapse profile rename) drives the tag too.
...
test('parseGroupChatMentions: display_name and ui_meta title forms address a member', () => {
  ...
  const members = [
    ...
    { name: 'ops', title: '', ui_meta: { 'synapse-bots': { title: 'Night Watch' } } }
  ]
