...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@synapse\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Synapse". Regression:
  ...
  assert.equal(name(active, undefined), 'Synapse')
})
...
test('botHandle: precomputed multi-source handle wins; default stays synapse', () => {
  ...
  assert.equal(botHandle('default'), 'synapse')
})
...
test('displayName: local default stays Synapse; remote default uses the device label', () => {
...
test('resolveRosterMentions: @synapse in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'synapse' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @synapse and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
