...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `SynapsePlugin` and
...
`$SYNAPSE_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`synapse-desktop-plugins` skill.
