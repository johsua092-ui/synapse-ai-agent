...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'synapse:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.synapseDesktop?.saveImageFromUrl) {
        if (await window.synapseDesktop.saveImageFromUrl(src)) {
