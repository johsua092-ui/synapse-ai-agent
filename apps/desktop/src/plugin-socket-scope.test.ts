...
// (#73044). Exercises the REAL synapse + store/gateway + store/profile chain:
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/synapse')
...
const conn = (over: Partial<SynapseConnection> = {}): SynapseConnection =>
  ({
    authMode: 'oauth',
