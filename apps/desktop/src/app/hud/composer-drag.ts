...
function setWorkspaceTransfer(transferring: boolean): void {
  window.synapseDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}
 ...
 * (see synapse:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      window.synapseDesktop?.hud?.moveBy?.({
        x: dx,
        y: dy,
        width: state.originW,
