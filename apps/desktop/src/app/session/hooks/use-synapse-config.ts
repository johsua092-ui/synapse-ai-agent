...
interface SynapseConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useSynapseConfig({ activeSessionIdRef }: SynapseConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshSynapseConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getSynapseConfig(), getSynapseConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Synapse' built-in medium over the user's config.
  ...
  return { refreshSynapseConfig, sttEnabled, voiceMaxRecordingSeconds }
}
