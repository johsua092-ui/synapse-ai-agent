import type { BillingBlock } from '@synapse/shared'
 ...
 * billing-specific toast — never the generic "Synapse error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Synapse reported an error',
