...
import { useSynapseConfig } from './use-synapse-config'

vi.mock('@/synapse', () => ({
  getSynapseConfig: vi.fn(),
  getSynapseConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'synapse.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getSynapseConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getSynapseConfig>>)

describe('useSynapseConfig refreshSynapseConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Synapse' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useSynapseConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshSynapseConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useSynapseConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getSynapseConfig>>>()
    vi.mocked(getSynapseConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useSynapseConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshSynapseConfig(true)
    })
    expect(getSynapseConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getSynapseConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getSynapseConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getSynapseConfig>>>()
    vi.mocked(getSynapseConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshSynapseConfig(true)
      refreshC = result.current.refreshSynapseConfig(true)
    })
