...
const preamble = () => `  var w = window;
  ...
  w.__synapseActHolder = w.__synapseActHolder || {};
  w.__synapseAct = ${actEngineSource()};
  w.__synapseWatch_fn = (${watchInPage.toString()});
  w.__synapseWatchTag = ${WATCH_TAG};
  var holder = w.__synapseActHolder;
  var act = function (a) { return w.__synapseAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__synapseWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__synapseHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'SYNAPSE-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
