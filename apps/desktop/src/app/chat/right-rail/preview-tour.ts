...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__synapseTourEngine) {
    ${driverIife}
    w.__synapseTourHolder = {};
    w.__synapseTourCollect = (${collectTourTargets.toString()});
    w.__synapseTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__synapse-tour-style')) {
    var style = document.createElement('style');
    style.id = '__synapse-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__synapseTourEngine(
    w.driver.js.driver,
    w.__synapseTourHolder,
    ${JSON.stringify(action)},
    w.__synapseTourCollect,
    document
  ));
})()`
}
