...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/synapse'
...
// highlight markers (sqlite snippet() delimiters — see synapse_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('synapse:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('synapse:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Synapse to
