...
import { disconnectOAuthProvider, listOAuthProviders } from '@/synapse'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/synapse'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.synapseDesktop?.terminal)
...
//      in synapse_cli/provider_catalog.py) — the SAME provider identity
//      `synapse model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Synapse can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Synapse can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Synapse never deletes creds another tool owns behind a silent API call.
