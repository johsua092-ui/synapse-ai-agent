 ...
 * agent sources (local runtime + any number of remote gateways / Synapse Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.synapseDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.synapseDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `synapse update` to every eligible source; per-connection results
