...
import type { ConfigFieldSchema, SynapseConfigRecord } from '@/types/synapse'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<SynapseConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useSynapseConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => synapseConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['synapse-config-schema'] : ['synapse-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getSynapseConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveSynapseConfig(config, scopeProfile)
  ...
  const applyConfig = (next: SynapseConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: SynapseConfigRecord) => {
