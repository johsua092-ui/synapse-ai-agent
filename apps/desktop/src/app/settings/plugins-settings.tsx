...
import { getProfiles } from '@/synapse'
...
// User-installed plugins first — mirrors `synapse plugins list --user`.
...
function reveal(file: string) {
  void window.synapseDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // synapse_home breaks against a remote backend (#66899).
    const dir = await window.synapseDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.synapseDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's synapse home (profile-aware), so the
// path comes from the gateway — not from the renderer's local SYNAPSE_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.synapseDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Synapse (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
