...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveSynapseConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Synapse wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.synapseDesktop` is
    ...
    const settings = window.synapseDesktop?.settings
