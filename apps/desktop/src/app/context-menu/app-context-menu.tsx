...
import { SYNAPSE_CONTEXT_MENU_TRIGGER_ATTR } from '@/components/ui/context-menu'
    ...
    [
      ...
      terminal.paste ? (
        <Item
          ...
          onSelect={() =>
            void window.synapseDesktop?.readClipboard().then(text => (text ? terminal.paste?.(text) : undefined))
          }
        />
      ) : null,
      <Item
...
function domSections(open: Extract<OpenContextMenu, { kind: 'dom' }>, t: Translations): ReactNode[][] {
  ...
  const editableCommand = (command: 'copy' | 'cut' | 'paste') => {
    withEditableFocus(() => void window.synapseDesktop?.contextMenuEdit?.(command))
  }
  ...
  const spellcheckAction = (action: { kind: 'add' | 'replace'; word: string }) => {
    withEditableFocus(() => void window.synapseDesktop?.contextMenuSpellcheck?.(action))
  }
    ...
    sections.push(
      [
        ...
        <Item
          ...
          onSelect={() => void window.synapseDesktop?.contextMenuCopyImage?.()}
        />,
        target.imageUrl ? (
          <Item
            icon="copy"
        ...
        target.imageUrl ? (
          <Item
            ...
            onSelect={() => void window.synapseDesktop?.saveImageFromUrl?.(target.imageUrl)}
          />
        ) : null
      ].filter(Boolean)
    )
      ...
      [
        ...
        imageUrl ? (
          <Item
            ...
            label={copy.image.saveImageAs}
            onSelect={() => void window.synapseDesktop?.saveImageFromUrl?.(imageUrl)}
          />
        ) : null
      ].filter(Boolean)
    )
  ...
  return [
    ...
    [
      <Item
        icon="cloud-download"
        key="shell-update"
        label={t.commandCenter.updateSynapse}
...
export function AppContextMenu() {
  ...
  useEffect(() => {
    ...
    const onContextMenu = (event: MouseEvent) => {
      ...
      if (element?.closest(`[${SYNAPSE_CONTEXT_MENU_TRIGGER_ATTR}], [data-slot="context-menu-trigger"]`)) {
        return
      }
  ...
  useEffect(() => window.synapseDesktop?.onContextMenuSpellcheck?.(augmentSpellcheck), [])
