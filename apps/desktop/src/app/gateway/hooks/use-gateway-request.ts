...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as SynapseGateway | null
  const gatewayRef = useRef<SynapseGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.synapseDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<SynapseGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as SynapseGateway | null
      }),
    []
  )
