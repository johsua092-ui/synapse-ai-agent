...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { synapseDesktop: { readDir: typeof readDir } }).synapseDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SynapseReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { synapseDesktop: unknown }).synapseDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SynapseReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SynapseReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: SynapseReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: SynapseReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: SynapseReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SynapseReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.synapseDesktop is missing', async () => {
    delete (window as unknown as { synapseDesktop?: unknown }).synapseDesktop
