...
import type { SynapseReadDirEntry, SynapseReadDirResult } from '@/global'
...
export type ProjectTreeEntry = SynapseReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SynapseReadDirEntry) {
...
async function filterIgnored(entries: SynapseReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SynapseReadDirResult> {
  if (!window.synapseDesktop) {
