...
const SYNAPSE_PATHS_MIME = 'application/x-synapse-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(SYNAPSE_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(SYNAPSE_PATHS_MIME)
  ...
  const getPath = window.synapseDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.synapseDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (synapse --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.synapseDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[synapse-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.synapseDesktop?.terminal?.write(sessionId, `${command}\r`)
