...
import { pathFromSynapseDeepLink, resolveSynapseOpenPath } from '@/lib/synapse-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.synapseDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.synapseDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.synapseDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.synapseDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // synapse://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.synapseDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future synapseDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveSynapseOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.synapseDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (synapse://index-network/intent/1, synapse://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromSynapseDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.synapseDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.synapseDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.synapseDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.synapseDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
