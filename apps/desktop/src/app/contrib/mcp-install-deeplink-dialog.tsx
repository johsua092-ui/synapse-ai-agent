...
/**
 * Explicit-confirm gate for `synapse://mcp/install` deep links. The payload is
 ...
 * confirming lets Synapse spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getSynapseConfigRecord())
      ...
      setSynapseConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
