...
import { getLatestSessionMessages } from '@/synapse'
...
import { useSynapseConfig } from '../session/hooks/use-synapse-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshSynapseConfig, sttEnabled, voiceMaxRecordingSeconds } = useSynapseConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('synapse:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('synapse:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshSynapseConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshSynapseConfig])
