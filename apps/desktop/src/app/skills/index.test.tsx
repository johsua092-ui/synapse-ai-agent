...
vi.mock('@/synapse', async importOriginal => ({
  ...(await importOriginal<typeof SynapseApi>()),
...
describe('SkillsView toolset management', { timeout: 60_000 }, () => {
    ...
    await act(async () => {
      window.dispatchEvent(
        new MessageEvent('message', {
          data: { type: 'synapse-skill-pick', name: 'web-research', identifier: 'web-research' },
          origin: 'https://'
        })
      )
    })
  ...
  it('offers (connection, profile) scope rows on multi-connection desktops', async () => {
    ...
    ;(window as { synapseDesktop?: unknown }).synapseDesktop = { connections, getAgentRoster }
