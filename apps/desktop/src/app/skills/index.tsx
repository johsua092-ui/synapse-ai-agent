...
import type { SkillInfo, ToolsetInfo } from '@/types/synapse'
...
export function SkillsView({
  ...
  const registryBridge = window.synapseDesktop?.connections
  const rosterBridge = window.synapseDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Synapse (default)' : p.name,
      value: p.name
    }))
