...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Synapse doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in synapse_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `synapse model` uses — instead of
...
export type SynapseConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `synapse model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Synapse simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (synapse.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors synapse_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Synapse, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `synapse tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Synapse); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** synapse-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** synapse-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/synapse/update/check` — the backend's own update state.
