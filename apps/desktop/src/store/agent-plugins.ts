...
/**
 * Feature store for backend (agent) plugins — the native Synapse plugins plus
 ...
 * primitives `synapse plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Synapse. */
  portable?: boolean
}
 ...
 *  SYNAPSE_HOME. Always refetches (it's a cheap local disk scan on the
