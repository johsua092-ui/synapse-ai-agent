 ...
 *  Working in `~/www/synapse-agent`, the draft says "synapse-agent" constantly
 *  — a "Use skill: synapse-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`synapse-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/synapse-agent-dev`, `plugin:skill`). */
