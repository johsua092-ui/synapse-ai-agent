...
import type { SynapseGitWorktree, SynapseRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: SynapseGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, SynapseRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, SynapseGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<SynapseRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<SynapseGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<SynapseRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<SynapseGitWorktree[]>>()
const $noRepoStatus = atom<SynapseRepoStatus | null>(null)
const $noWorktrees = atom<SynapseGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<SynapseRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<SynapseGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: SynapseRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: SynapseGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<SynapseRepoStatus | null>
  seq: number
  target: string
}
