 ...
 * `@synapse/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-synapse-glass]`
 * block in styles.css).
 */
...
} from '@synapse/shared/translucency'
 ...
 * sets `synapseDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.synapseDesktop?.glassSupported === 'boolean'
    ? window.synapseDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.synapseDesktop?.translucencySupported === 'boolean'
    ? window.synapseDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-synapse-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-synapse-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-synapse-glass-on', glassLive)
  root.toggleAttribute('data-synapse-glass', glassOn)
  root.toggleAttribute('data-synapse-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-synapse-glass-scope', scope)
  } else {
    root.removeAttribute('data-synapse-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.synapseDesktop?.setTranslucency?.(state)
  })
