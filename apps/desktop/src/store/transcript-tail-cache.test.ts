...
describe('transcript tail cache', () => {
  ...
  it('self-evicts a corrupt entry instead of returning garbage', () => {
    window.localStorage.setItem('synapse.transcript-tail.v1:sess-bad', '{not json')

    expect(loadTranscriptTail('sess-bad')).toBeNull()
    expect(window.localStorage.getItem('synapse.transcript-tail.v1:sess-bad')).toBeNull()
  })
  ...
  it('repairs a poisoned persisted tail carrying a duplicate toolCallId (#87857)', () => {
    ...
    window.localStorage.setItem('synapse.transcript-tail.v1:sess-poisoned', JSON.stringify(poisoned))
