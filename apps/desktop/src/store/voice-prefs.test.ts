...
vi.mock('@/synapse', () => ({
  getSynapseConfigRecord: vi.fn(async () => ({})),
  saveSynapseConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye synapse', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye synapse')
  })
