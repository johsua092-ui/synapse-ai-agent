...
import type { SynapseBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/synapse'
...
export const $pullRequestsByBranch = atom<Record<string, SynapseBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'synapse.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('synapse.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: SynapseBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
