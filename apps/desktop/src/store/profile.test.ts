...
const { getProfiles } = await import('@/synapse')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/synapse/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<SynapseConnection> = {}): SynapseConnection =>
  ({ baseUrl: 'https://synapse-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SynapseConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { synapseDesktop: { getConnection } })
