...
export function setZoomPercent(percent: number): void {
  window.synapseDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.synapseDesktop?.zoom) {
  void window.synapseDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.synapseDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
