...
import { getSynapseConfigRecord, saveSynapseConfig } from '@/synapse'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getSynapseConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveSynapseConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
