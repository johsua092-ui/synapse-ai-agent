...
import type { SynapseReviewFile, SynapseReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Synapse' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'synapse.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'synapse.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'synapse.desktop.reviewTreeMode'
const SELECTED_KEY = 'synapse.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SynapseReviewFile[]>([])
...
export const $reviewShipInfo = atom<SynapseReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['synapseDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SynapseReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly SynapseReviewFile[], path: string): SynapseReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.synapseDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.synapseDesktop?.openExternal?.(url)
    }
