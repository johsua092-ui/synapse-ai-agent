...
// /api/profiles/active `current`). "default" is the root ~/.synapse. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'synapse.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'synapse.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await synapseApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SYNAPSE_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.synapseDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<SynapseConnection | null> {
  const getConnection = window.synapseDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<SynapseConnection | null> {
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'synapse.desktop.showAllProfiles'
...
// Switch to the default (root ~/.synapse) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.synapseDesktop?.touchBackend?.(target).catch(() => undefined)
}
