 ...
 *
 * The archive is the CLI's own `synapse profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/synapse'
...
import type { ProfileDesktopOverlay } from '@/types/synapse'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Synapse profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.synapseDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.synapseDesktop?.selectPaths?.({
