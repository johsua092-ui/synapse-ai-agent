 ...
 * composer with the reply scrolling above it, so Synapse can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.synapseDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.synapseDesktop?.hud
...
export function resetHudLayout(): void {
  void window.synapseDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.synapseDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.synapseDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
