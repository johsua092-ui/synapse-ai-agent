 ...
 * app's bounds and stays visible while Synapse is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'synapse.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'synapse.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.synapseDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.synapseDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.synapseDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.synapseDesktop?.petOverlay?.close()
}
