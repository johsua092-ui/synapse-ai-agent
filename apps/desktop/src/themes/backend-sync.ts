/**
 * Live skin sync from the Synapse backend.
 *
 * The backend resolves the active skin (built-in or `$SYNAPSE_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Synapse
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { SynapseSkin } from '@synapse/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: SynapseSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as SynapseSkin)
