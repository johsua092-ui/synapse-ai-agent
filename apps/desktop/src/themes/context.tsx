...
const SKIN_KEY = 'synapse-desktop-theme-v2'
const MODE_KEY = 'synapse-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'synapse-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'synapse-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'synapse-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.synapseTheme = skinName
  root.dataset.synapseMode = rendered
  ...
  window.synapseDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('synapse-boot-background', chromeBg)
    window.localStorage.setItem('synapse-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.synapseThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.synapseDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Synapse authoring/activating a skin from a
