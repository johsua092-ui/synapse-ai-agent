import { JsonRpcGatewayClient } from '@synapse/shared'

import type { SynapseApiRequest } from '@/global'
...
// "Timed out connecting to Synapse backend" that hangs the UI (#48504).
//
...
export class SynapseGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Synapse gateway connection closed',
      connectErrorMessage: 'Could not connect to Synapse gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Synapse gateway is not connected',
...
// process to route them to that host (synapse:api's registry branch). Null
...
export function synapseApi<T>(request: SynapseApiRequest): Promise<T> {
  return window.synapseDesktop.api<T>({ ...connectionScoped(), ...request })
}
