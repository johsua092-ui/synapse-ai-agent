...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Synapse gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.synapseDesktop
  Object.defineProperty(window, 'synapseDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'synapseDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  ...
  it('shows the Nous Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    const restore = stubDesktop(remoteToken)
    $desktopBoot.set({
      error: 'Nous Cloud agent  is down (HTTP 503: server-side fault).',
      fakeMode: false,
    ...
    try {
      ...
      expect(screen.getByText(/ares-3009\.agents\.joshresearch\.com/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
