...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'synapse -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 Synapse (@synapse): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 synapse -p mr-tester chat --in "~" -Q -q "Message from 🤖 Synapse: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('synapse -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('synapse sessions list')).toBeNull()
  })
})

describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    const output = 'session_id: 20260813_220347_f69ac6\nHi Synapse! Good to hear from you.'

    expect(replyTextFromResult({ output })).toBe('Hi Synapse! Good to hear from you.')
  })
