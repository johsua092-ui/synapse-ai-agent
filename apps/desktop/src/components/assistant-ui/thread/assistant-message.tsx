...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's SYNAPSE_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.synapseDesktop?.logsRoot?.()
      ...
      const result = await window.synapseDesktop?.openDir?.(root)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {window.synapseDesktop?.logsRoot && (
