...
const SYNAPSE_REF_TYPES = WIRE_REFERENCE_KINDS
type SynapseRefType = (typeof SYNAPSE_REF_TYPES)[number]
...
const SYNAPSE_DIRECTIVE_RE = referenceRe()
...
export const synapseDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SYNAPSE_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...synapseDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Synapse directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.synapseDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.synapseDesktop?.readFileDataUrl(id)
