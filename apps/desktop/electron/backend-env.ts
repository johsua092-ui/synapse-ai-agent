...
/**
 * Synapse-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\synapse\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$SYNAPSE_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_synapse_node_dirs()` in synapse_constants.py, which the Electron main
 * process cannot import.
 */
function synapseManagedNodePathEntries(
  synapseHome,
  ...
  const root = pathModule.join(synapseHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const synapseNodeDirs = synapseManagedNodePathEntries(synapseHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([synapseNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeSynapseHomeRoot(synapseHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!synapseHome) {
    return synapseHome
  }

  const resolved = pathModule.resolve(String(synapseHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). synapse_bootstrap sets this inside the child too, but only
