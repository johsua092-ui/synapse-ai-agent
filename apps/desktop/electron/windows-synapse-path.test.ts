// Unit tests for the pure Windows `synapse` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvSynapseCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `synapse` shim shadows
//      the real synapse.cmd/synapse.exe.
...
//   3. resolveVenvSynapseCommand() — must probe the venv python via
//      canImportSynapseCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSynapse: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSynapse: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSynapse: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSynapse: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSynapse: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSynapse: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvSynapseCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportSynapseCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    synapseHome: '/fake/synapse-home',
...
test('resolveVenvSynapseCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvSynapseCommand('/root/venv/Scripts/synapse.exe', [], deps), null)
})

test('resolveVenvSynapseCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvSynapseCommand('/root/venv/Scripts/synapse.cmd', [], deps), null)
})

test('resolveVenvSynapseCommand: returns null when the basename is not synapse/synapse.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSynapseCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvSynapseCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSynapseCommand('/root/venv/bin/synapse.exe', [], deps), null)
})

test('resolveVenvSynapseCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvSynapseCommand('/root/venv/Scripts/synapse.exe', [], deps), null)
})

test('resolveVenvSynapseCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportSynapseCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvSynapseCommand('/root/venv/Scripts/synapse.exe', ['serve'], deps)
...
test('resolveVenvSynapseCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvSynapseCommand('/root/venv/Scripts/synapse.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'synapse_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvSynapseCommand: is case-insensitive on synapse.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvSynapseCommand('/root/venv/Scripts/SYNAPSE.EXE', [], deps))
  assert.ok(resolveVenvSynapseCommand('/root/venv/SCRIPTS/synapse.exe', [], deps))
})
