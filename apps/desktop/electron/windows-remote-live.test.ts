...
//   SYNAPSE_WIN_SSH_HOST   ssh alias/host of the Windows box
//   SYNAPSE_WIN_SSH_USER   remote user
//   SYNAPSE_WIN_SSH_SYNAPSE absolute path to the remote synapse.exe under test
const liveHost = process.env.SYNAPSE_WIN_SSH_HOST || ''
const liveUser = process.env.SYNAPSE_WIN_SSH_USER || ''
const configuredSynapse = process.env.SYNAPSE_WIN_SSH_SYNAPSE || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Synapse-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredSynapse)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredSynapse.replace('synapse.exe', 'python.exe')}' -m synapse_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredSynapse.replace('synapse.exe', 'python.exe')
        const terminate = `& '${python}' -m synapse_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.synapsePath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m synapse_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m synapse_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
