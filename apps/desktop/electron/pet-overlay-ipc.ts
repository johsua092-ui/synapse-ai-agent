...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('synapse:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('synapse:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('synapse:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('synapse:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('synapse:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('synapse:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('synapse:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('synapse:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('synapse:pet-overlay:control', payload)
  })
}
