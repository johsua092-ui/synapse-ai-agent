...
export function markerPath(synapseHome) {
  return path.join(synapseHome, '.synapse-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(synapseHome)
  let raw
 ...
 *
 * The Tauri-based synapse-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(synapseHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(synapseHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Synapse to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(synapseHome, opts)
