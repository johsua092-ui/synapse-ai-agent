 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Synapse Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.synapse-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(synapseHome) {
  return path.join(synapseHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.synapse/synapse-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(synapseHome) {
  if (!synapseHome) {
    return null
  }

  const candidate = path.join(synapseHome, 'synapse-agent', 'scripts', installScriptName())
...
function cachedScriptPath(synapseHome, commit) {
  return path.join(bootstrapCacheDir(synapseHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/johsua092-ui/synapse-ai-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(synapseHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(synapseHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, synapseHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass SYNAPSE_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          SYNAPSE_HOME: synapseHome || process.env.SYNAPSE_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, synapseHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, synapseHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--synapse-home', synapseHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, synapseHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, synapseHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, synapseHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(synapseHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, synapseHome, emit })
