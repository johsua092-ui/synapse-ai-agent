 ...
 *   1. SYNAPSE_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\synapse\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect SYNAPSE_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.SYNAPSE_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'synapse', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'synapse', 'git', 'usr', 'bin', 'bash.exe'))
  }
