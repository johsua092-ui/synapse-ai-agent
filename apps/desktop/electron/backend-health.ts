...
export interface SynapseReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForSynapseReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Synapse gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForSynapseReady().
 *
...
export function makeNousCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  ...
  const err = new Error(
    `Nous Cloud agent ${hostname} is down ` +
      ...
      'Check https:// for backend status, ' +
      'or switch to Local mode in Settings → Gateway. ' +
      'You can also reach out on Discord at discord.gg/Josh Research ' +
...
/**
 * True when the backend URL points at a Nous-managed Synapse Cloud instance
 * (e.g. ). These are Fly.io-hosted machines
 ...
 * `isReauthRequired` so startSynapse latches; `needsOauthLogin` alone only
...
export async function waitForSynapseReady(baseUrl: string, options: SynapseReadyOptions): Promise<void> {
  ...
  throw new Error(`Synapse backend did not become ready: ${detail}`)
}
