// Profile-delete routing logic for the `synapse:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_synapse_home() and
...
/** Parse a `synapse:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its SYNAPSE_HOME.
 *
...
/**
 * Route the next `synapse:api` request away from the primary/window backend
 ...
 * ensure_synapse_home() recreates the directory the delete just removed.
 */
