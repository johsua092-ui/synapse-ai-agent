export type ParentWatchdogEnv = {
  SYNAPSE_PARENT_PID: string
  SYNAPSE_PARENT_START_MARKER?: string
  SYNAPSE_PARENT_NONCE?: string
  /** Spawn tag consumed by synapse_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { SYNAPSE_PARENT_PID: String(pid) }
  env.SYNAPSE_SPAWN = spawnTag(pid, startMarker)
  ...
  env.SYNAPSE_PARENT_START_MARKER = startMarker
  env.SYNAPSE_PARENT_NONCE = nonce

  return env
}

/** Build the `SYNAPSE_SPAWN` tag mirrored by synapse_cli/process_identity.py.
