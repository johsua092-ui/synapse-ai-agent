...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/synapse-runner-test',
    sourceRepoRoot: null,
    synapseHome: '/tmp/synapse-runner-test',
    logRoot: '/tmp/synapse-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'synapse-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'synapse-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/synapse-agent', '--synapse-home', '/tmp/synapse', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/synapse',
      synapseHome: '/tmp/home'
    }),
    ['--dir', '/tmp/synapse', '--synapse-home', '/tmp/home', '--branch', 'main']
  )
})
