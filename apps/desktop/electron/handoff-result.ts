 ...
 * console. It writes SYNAPSE_HOME/.synapse-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Synapse within 30 minutes.
...
export function handoffResultPath(synapseHome: string): string {
  return path.join(synapseHome, '.synapse-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(synapseHome)
  let raw: string
