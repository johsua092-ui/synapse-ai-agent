...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'synapse serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/synapse serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m synapse_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/synapse --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Synapse Runtime\\synapse.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('synapse chat --query serve'), false)
