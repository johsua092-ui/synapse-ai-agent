// Launching the Synapse TUI in the user's OWN terminal emulator.
//
...
// `synapse --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Synapse runtime is often a venv Python invoked
//     as `python -m synapse_cli.main`, not a `synapse` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Synapse
