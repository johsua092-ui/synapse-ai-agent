...
const USER_DATA_OVERRIDE = process.env.SYNAPSE_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SYNAPSE_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.SYNAPSE_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with SYNAPSE_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[synapse] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[synapse] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. SYNAPSE_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[synapse] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[synapse] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via SYNAPSE_DESKTOP_PASSWORD_STORE, bridged by the
// `synapse desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[synapse] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[synapse] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `synapse desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Synapse sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[synapse] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[synapse] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[synapse] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[synapse] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[synapse] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('synapse:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Synapse burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[synapse] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[synapse] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[synapse] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[synapse] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SYNAPSE_HOME — the user-facing root for everything Synapse-related. Mirrors
// scripts/install.ps1's $SynapseHome and scripts/install.sh's $SYNAPSE_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\synapse (matches install.ps1)
//   macOS / Linux: ~/.synapse (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.synapse directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\synapse yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SYNAPSE_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SYNAPSE_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.synapse / %LOCALAPPDATA%\synapse.
function resolveSynapseHome() {
  if (process.env.SYNAPSE_HOME) {
    return normalizeSynapseHomeRoot(process.env.SYNAPSE_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'synapse-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SYNAPSE_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\synapse and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SYNAPSE_HOME')

    if (fromRegistry) {
      return normalizeSynapseHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'synapse')
    const legacy = path.join(app.getPath('home'), '.synapse')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.synapse setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.synapse')
}

const SYNAPSE_HOME = resolveSynapseHome()

function pathWithSynapseManagedNode(...entries) {
  const managed = synapseManagedNodePathEntries(SYNAPSE_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SYNAPSE_ROOT — the canonical mutable Synapse install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SYNAPSE_ROOT = path.join(SYNAPSE_HOME, 'synapse-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SYNAPSE_ROOT, 'venv')
...
// boots, going straight to `resolveSynapseBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SYNAPSE_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_SYNAPSE_ROOT, '.synapse-bootstrap-complete')
...
// active-profile.json records which Synapse profile the desktop launches its
// local backend as. When set, startSynapse() passes `synapse --profile <name>
// dashboard …`, which deterministically pins SYNAPSE_HOME (see
// _apply_profile_override in synapse_cli/main.py) and bypasses the sticky
// ~/.synapse/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors synapse_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// synapseDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SYNAPSE_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by synapse_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SYNAPSE_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (synapse_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SYNAPSE_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.SYNAPSE_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.SYNAPSE_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SYNAPSE_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.SYNAPSE_DESKTOP_APP_NAME || 'Synapse'
...
// 'synapse:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.joshresearch.synapse) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.joshresearch.synapse')
}

// Seed the native About panel with the live Synapse version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `synapse update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSynapseVersion(),
  copyright: 'Copyright © 2026 Josh Research'
})
...
// startSynapse(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SYNAPSE_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SYNAPSE_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startSynapse() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning synapse serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Synapse backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  synapseLog.push(...lines)

  if (synapseLog.length > 300) {
    synapseLog.splice(0, synapseLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-synapse-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('synapse:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the synapse:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('synapse:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_SYNAPSE_ROOT
  })
}
...
// The Tauri updater writes SYNAPSE_HOME/.synapse-update-in-progress for the whole
...
// (`force_kill_other_synapse`, taskkill /IM synapse.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(SYNAPSE_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Synapse will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(SYNAPSE_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Synapse update did not finish',
        `${result.message}\n\nDetails: ${path.join(SYNAPSE_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `synapse` — must not shadow `synapse.cmd`/`synapse.exe`.
...
function unwrapWindowsVenvSynapseCommand(command, backendArgs) {
  return resolveVenvSynapseCommand(command, backendArgs, {
...
// spawns `synapse serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `synapse` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'synapse_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `synapse --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SYNAPSE_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveSynapseBackend step 4); execFileSync of a .cmd without
...
function isSynapseSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'synapse_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SYNAPSE_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Synapse deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the synapse-import
...
// findGitBash — locate bash.exe on Windows. Resolves SYNAPSE_GIT_BASH_PATH
...
// normal SYNAPSE_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\synapse\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'synapse', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'synapse', 'git', 'bin', 'git.exe'))
  }
...
function recentSynapseLog() {
  return synapseLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's synapse root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SYNAPSE_ROOT.
// SYNAPSE_DESKTOP_SYNAPSE_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SYNAPSE_DESKTOP_SYNAPSE_ROOT && path.resolve(process.env.SYNAPSE_DESKTOP_SYNAPSE_ROOT),
    !IS_PACKAGED && isSynapseSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSynapseSourceRoot(ACTIVE_SYNAPSE_ROOT) ? ACTIVE_SYNAPSE_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SYNAPSE_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('synapse:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'synapse-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `synapse update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(SYNAPSE_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `synapse update` can write
// fresh entry points. On Windows this is the file the running backend
// `synapse.exe` holds open; on POSIX it's never mandatory-locked.
function venvSynapseShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'synapse.exe')
    : path.join(updateRoot, 'venv', 'bin', 'synapse')
}
...
// only signals the direct child, so on Windows a backend `synapse.exe` that
// spawned its own grandchildren (a `synapse` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Synapse backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Synapse backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.synapseBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Synapse backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.synapseBackendIdentity
...
// `synapseProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `synapse.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a synapse
// REPL, a pty terminal, the gateway) keep `synapse.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const synapseProcess = backendConnectionState.getProcess()

  stopBackendTreesForUpdate(synapseProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvSynapseShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentSynapseProcess = backendConnectionState.getProcess()

    if (currentSynapseProcess && Number.isInteger(currentSynapseProcess.pid)) {
      stragglers.push(currentSynapseProcess.pid)
    }
...
// `synapse update`). Instead we spawn the staged Synapse-Setup binary with
// --update and quit, so it can run `synapse update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `synapse update` → relaunch),
      ...
      // in: the SYNAPSE_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `synapse update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `synapse desktop`, never the Tauri installer that self-copies
      // synapse-setup.exe into SYNAPSE_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `synapse` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `synapse update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `synapse update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, synapseRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(SYNAPSE_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Synapse — this window will close and the updater will open. Don’t reopen Synapse yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(SYNAPSE_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // synapse.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running synapse, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Synapse install open ' +
        '(a second Synapse window or a terminal running synapse?). Close it and retry.'
    ...
    // Synapse processes running from this venv.  The updater normally refuses
    ...
    // `synapse update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `synapse update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          SYNAPSE_HOME,
          SYNAPSE_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithSynapseManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `synapse update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(SYNAPSE_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Synapse update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(SYNAPSE_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Synapse will keep running — try again, or run \`synapse update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSynapse = path.join(venvBin, IS_WINDOWS ? 'synapse.exe' : 'synapse')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Synapse launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the synapse
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.synapse-bootstrap-complete')),
      hasVenvSynapse: fileExists(venvSynapse),
...
// Python-level pre-update snapshot inside `synapse update`.
function preflightStateDb(synapseHome, rememberLog) {
  const stateDbPath = path.join(synapseHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(synapseHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(synapseHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(synapseHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `synapse update`, swaps/relaunches the app bundle, and writes
// .synapse-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'synapse update', percent: null })

    return { ok: true, manual: true, command: 'synapse update', synapseRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      SYNAPSE_HOME,
      SYNAPSE_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithSynapseManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Synapse — this window will close. Don’t reopen Synapse yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_SYNAPSE_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isSynapseSourceRoot(ACTIVE_SYNAPSE_ROOT) &&
    fileExists(venvPython) &&
    canImportSynapseCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SYNAPSE_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `synapse
...
function resolveWebDist() {
  const override = process.env.SYNAPSE_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SYNAPSE_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: synapse desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: synapse desktop --force-build`
  )
...
function resolveSynapseCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Synapse.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.SYNAPSE_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSynapseCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'synapse_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_SYNAPSE_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Synapse at ${ACTIVE_SYNAPSE_ROOT}`,
    command,
    args: ['-m', 'synapse_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      synapseHome: SYNAPSE_HOME,
      pythonPathEntries: [ACTIVE_SYNAPSE_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SYNAPSE_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
function resolveSynapseBackend(backendArgs) {
  ...
  //    installed `synapse` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isSynapseSourceRoot.)
  if (!IS_PACKAGED && isSynapseSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Synapse source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_SYNAPSE_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\synapse\\synapse-agent (Windows) or ~/.synapse/synapse-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Synapse runtime at ${ACTIVE_SYNAPSE_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `synapse` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SYNAPSE_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SYNAPSE_DESKTOP_IGNORE_EXISTING !== '1') {
    let synapseCommand = null
    const synapseOverride = process.env.SYNAPSE_DESKTOP_SYNAPSE

    if (synapseOverride) {
      const resolvedOverride = findOnPath(synapseOverride)

      if (resolvedOverride) {
        synapseCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(synapseOverride, { isWsl: IS_WSL })) {
        synapseCommand = synapseOverride
      } else {
        rememberLog(`Ignoring Windows Synapse override under WSL: ${synapseOverride}`)
      }
    } else {
      synapseCommand = findOnPath('synapse')
    }

    if (synapseCommand) {
      if (looksLikeDesktopAppBinary(synapseCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Synapse CLI: ${synapseCommand}`)
        synapseCommand = null
      }
    }

    if (synapseCommand) {
      const unwrapped = unwrapWindowsVenvSynapseCommand(synapseCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `synapse` shim
      ...
      const shellForProbe = isCommandScript(synapseCommand)

      // SYNAPSE_DESKTOP_SYNAPSE is an explicit deployment override (used by
      ...
      if (shouldTrustSynapseOverride(synapseOverride) || verifySynapseCli(synapseCommand, { shell: shellForProbe })) {
      ...
      rememberLog(
        `Ignoring existing Synapse CLI at ${synapseCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed synapse_cli module via system Python.
  ...
  if (python) {
    ...
    // having synapse_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\synapse\synapse-agent\venv.
    if (canImportSynapseCli(python)) {
      return {
        kind: 'python',
        label: `installed synapse_cli module via ${python}`,
        command: python,
        args: ['-m', 'synapse_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: synapse_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSynapseBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Synapse Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_SYNAPSE_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSynapseBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Synapse install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Synapse recovery was handed off to Synapse Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      synapseHome: SYNAPSE_HOME,
      logRoot: path.join(SYNAPSE_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Synapse install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Synapse bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SYNAPSE_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSynapse() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // synapse:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSynapseBackend(backend.args))
  }
  ...
  if (!isSynapseSourceRoot(ACTIVE_SYNAPSE_ROOT)) {
    throw new Error(
      `Synapse install at ${ACTIVE_SYNAPSE_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Synapse' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\synapse\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `synapse` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Synapse on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isSynapseSourceRoot()
    // plus an importable synapse_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Synapse venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Synapse at ${ACTIVE_SYNAPSE_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Synapse runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----synapse-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Synapse backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Synapse backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Synapse backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Synapse-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Synapse-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('synapse:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSynapseCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('synapse:preview-file-changed', payload)
}
...
// waitForSynapseReady how to read a 401 — rejected session vs gated route.
...
async function waitForSynapse(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForSynapseReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('synapse:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('synapse:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('synapse:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('synapse:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Synapse...".
function sendPowerResume() {
  ...
  webContents.send('synapse:power-resume')
}
...
// 'synapse:power-battery' push below.
ipcMain.handle('synapse:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('synapse:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('synapse:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('synapse:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('synapse:context-menu-spellcheck', {
...
//
// Hosted Synapse gateways gate the dashboard behind an OAuth provider (e.g.
// Josh Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``synapse_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / synapse #37247).
//     Both are set as HttpOnly cookies (``synapse_session_at`` ~15 min,
//     ``synapse_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:synapse-remote-oauth'
...
// "not signed in", which on the initial boot path (startSynapse → the renderer's
// single-shot boot() with no retry) surfaced as the "Synapse couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Synapse
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Synapse Cloud agent…' : 'Sign in to Synapse gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Synapse backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in synapse_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Synapse Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:synapse-remote-oauth`:
...
// convention (synapse_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Synapse surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://'

function resolvePortalBaseUrl() {
  const raw = process.env.SYNAPSE_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Synapse gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for synapse_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Synapse Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Synapse Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Synapse Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Synapse Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Synapse Cloud. Open Settings → Gateway, choose Synapse Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Synapse Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Synapse Cloud session has expired. Sign in to Synapse Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Synapse Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every synapse:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteSynapsePath`/`remoteProfile` —
...
// a valid stored value (pins the root SYNAPSE_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SYNAPSE_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.SYNAPSE_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Synapse Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteSynapsePath: (ssh || savedSsh)?.remoteSynapsePath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by SYNAPSE_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Synapse Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Synapse Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteSynapsePath: input.sshRemoteSynapsePath ?? existingBlock.remoteSynapsePath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (synapse #37247), and the gateway middleware transparently
  ...
  if (!token) {
    throw new Error(
      'Remote Synapse gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.SYNAPSE_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteSynapsePath || '')
    ...
    result = await lifecycle({
      ...
      remoteSynapsePath: sshConfig.remoteSynapsePath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForSynapse: (baseUrl, token) => waitForSynapse(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    synapseVersion: result.synapseVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.synapseVersion || 'synapse (version unknown)'} at ${result.synapsePath || '?'}`
  )
  ...
  return { ...connection, remoteSynapseVersion: result.synapseVersion || '' }
}
...
//   2. env override (SYNAPSE_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.SYNAPSE_DESKTOP_REMOTE_TOKEN,
      url: process.env.SYNAPSE_DESKTOP_REMOTE_URL
    },
    profile,
...
// env → global) so a startSynapse() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Synapse
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Josh Research" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteSynapsePath: input.sshRemoteSynapsePath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteSynapsePath || '')
          let synapsePath
          let synapseVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            synapseVersion = inspection.version
            supported = inspection.supported
          } else {
            synapsePath = await remoteLifecycle.locateSynapse(ssh, sshConfig.remoteSynapsePath || '')
            synapseVersion = await remoteLifecycle.probeSynapseVersion(ssh, synapsePath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, synapsePath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Synapse on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteSynapsePath: synapsePath,
            remoteSynapseVersion: synapseVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSynapse())
  ...
  // connect to Synapse gateway". Mirror the renderer's connect here so the test
...
function resetSynapseConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const synapseProcess = backendConnectionState.invalidate()
  stopBackendChild(synapseProcess)
...
// startSynapse() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetSynapseConnection() invalidates it.
  const synapseProcess = backendConnectionState.getProcess()
  const dying = synapseProcess && !synapseProcess.killed ? synapseProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('synapse:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('synapse:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startSynapse()
  ...
  // spawning, so two children never share one profile's SYNAPSE_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Synapse backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile) {
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Synapse backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(source, profile, key, poolEntry) {
  ...
  if (source.kind === 'ssh') {
    ...
    const sshConfig = normalizeSshConfig({
      ...
      remoteSynapsePath: source.remoteSynapsePath,
  ...
  await waitForSynapse(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
// local-spawn portion of startSynapse() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForSynapse(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited SYNAPSE_HOME env (see _apply_profile_override
  // step 3 in synapse_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSynapseBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const synapseCwd = resolveSynapseCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(SYNAPSE_HOME, 'profiles', key))
  )

  rememberLog(`Starting Synapse backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: synapseCwd,
        SYNAPSE_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        SYNAPSE_WEB_DIST: webDist,
        ...(readyFile ? { SYNAPSE_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Synapse backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Synapse backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Synapse backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForSynapse(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Synapse backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Synapse backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_synapse_home() recreates the deleted profile directory.
//
...
async function startSynapse() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Synapse Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startSynapse() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Synapse backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Synapse backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Synapse backend at ${remote.baseUrl}`, 24)
      await waitForSynapse(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Synapse backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, synapseLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.synapse/active_profile file) and
    // resolves SYNAPSE_HOME the same way `synapse -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Synapse runtime', 28)

        return resolveSynapseBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Synapse backend via ${backend.label}`, 84)
    rememberLog(`Starting Synapse backend via ${backend.label}`)
    ...
    const synapseProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin SYNAPSE_HOME for the child so Python's get_synapse_home()
          // resolves to the SAME location our resolveSynapseHome() picked. Without
          // this pin, Python falls back to ~/.synapse on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\synapse, which differs from C:\Users\<u>\.synapse.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SYNAPSE_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(synapseProcess)
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, synapseProcess)

    if (!processOwner) {
      stopBackendChild(synapseProcess)
      await waitForBackendExit(synapseProcess)
      releaseBackendChild(synapseProcess)
      throw new Error('Synapse backend start was superseded by a newer connection attempt.')
    }

    synapseProcess.stdout.on('data', rememberLog)
    synapseProcess.stderr.on('data', rememberLog)
    ...
    synapseProcess.once('error', error => {
      releaseBackendChild(synapseProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Synapse backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Synapse backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Synapse backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Synapse backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    synapseProcess.once('exit', (code, signal) => {
      releaseBackendChild(synapseProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Synapse backend exit (${signal || code})`)
      ...
      rememberLog(`Synapse backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Synapse backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Synapse backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSynapseLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Synapse backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(synapseProcess, {
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Synapse backend to become ready', 90)
    await Promise.race([waitForSynapse(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => synapseProcess.exitCode === null && !synapseProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Synapse backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Synapse backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Synapse is
...
// pushes pet state over IPC (synapse:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via synapse:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Synapse app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Synapse icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see synapse:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Synapse in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('synapse:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Synapse can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  // is in the synapse:hud:ignore-mouse handler). Native Wayland keeps the
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('synapse:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('synapse:hud:game-overlay', state)
    }
  }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('synapse:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `synapse:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, synapse:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('synapse:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('synapse:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('synapse:quick-entry:shown')
}
...
function createWindow() {
  ...
  startSynapse().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('synapse:connection', async (_event, profile) => {
  const connection = await ensureBackend(profile)
...
ipcMain.handle('synapse:connection:for', async (_event, payload) => {
...
// "Starting Synapse…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('synapse:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetSynapseConnection({ soft: true }),
...
ipcMain.handle('synapse:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('synapse:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('synapse:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('synapse:window:openInstance', async () => {
...
ipcMain.handle('synapse:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveSynapseBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Synapse is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `synapse-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, SYNAPSE_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('synapse:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('synapse:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('synapse:zoom:get', event => {
...
ipcMain.on('synapse:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('synapse:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSynapse() call restarts the
...
ipcMain.handle('synapse:bootstrap:repair', async () => {
  // Forceful repair: force the next startSynapse() through the full installer
  ...
  // "bypass active runtime" path inside startSynapse() and fall through
  ...
  resetSynapseConnection()

  return { ok: true }
})
ipcMain.handle('synapse:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('synapse:bootstrap:cancel', async () => {
...
ipcMain.handle('synapse:boot-progress:get', async () => bootProgressState)
ipcMain.handle('synapse:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('synapse:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('synapse:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('synapse:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('synapse:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('synapse:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('synapse:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('synapse:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('synapse:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('synapse:connections:set-primary', async (_event, id) => {
  const registry = setPrimaryConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('synapse:connections:set-launch-mode', async (_event, mode) => {
  const registry = setConnectionLaunchMode(readDesktopConnectionsRegistry(), String(mode || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('synapse:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('synapse:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteSynapsePath: entry.remoteSynapsePath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startSynapse()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteSynapsePath: connection.remoteSynapsePath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteSynapseProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root SYNAPSE_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('synapse:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// synapse:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
ipcMain.handle('synapse:gateway:ws-url-for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
...
// Fan out `synapse update` to every eligible registered connection at once.
...
// /api/synapse/update endpoint (the dashboard updater), which runs
// `synapse update` on THAT machine.
ipcMain.handle('synapse:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/synapse/update', {}, { timeoutMs: 15_000 })
...
// analogue of the synapse:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('synapse:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('synapse:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startSynapse() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('synapse:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Synapse Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('synapse:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('synapse:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('synapse:connection-config:apply', async (_event, payload) => {
...
ipcMain.handle('synapse:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('synapse:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SYNAPSE_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('synapse:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('synapse:requestMicrophoneAccess', async () => {
...
ipcMain.handle('synapse:window:readBelow', async event => {
...
async function handleSynapseApiRequest(request) {
  ...
  // backend calls ensure_synapse_home() which recreates the profile directory,
...
ipcMain.handle('synapse:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its SYNAPSE_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleSynapseApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleSynapseApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('synapse:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('synapse:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('synapse:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as synapse://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('synapse:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('synapse:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('synapse:data-url-read-max:get', () => ({
...
ipcMain.handle('synapse:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('synapse:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('synapse:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('synapse:readFileText', async (_event, filePath) => {
...
// `synapse:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('synapse:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('synapse:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('synapse:writeClipboard', (_event, text) => {
...
ipcMain.handle('synapse:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('synapse:readClipboard', () => clipboard.readText())

ipcMain.handle('synapse:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('synapse:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('synapse:context-menu:edit', (event, command) => {
...
ipcMain.handle('synapse:context-menu:copy-image', event => {
...
ipcMain.handle('synapse:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('synapse:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('synapse:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('synapse:saveClipboardImage', async () => {
...
ipcMain.handle('synapse:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('synapse:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('synapse:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('synapse:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('synapse:active-work', (event, payload) => {
...
ipcMain.on('synapse:titlebar-theme', (_event, payload) => {
...
ipcMain.on('synapse:native-theme', (_event, mode) => {
...
ipcMain.on('synapse:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('synapse:translucency', (_event, payload) => {
...
ipcMain.on('synapse:keep-awake', (_event, on) => {
...
ipcMain.handle('synapse:quick-entry:settings:get', async () => {
...
ipcMain.handle('synapse:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('synapse:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('synapse:quick-entry:submit', {
...
ipcMain.on('synapse:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('synapse:quick-entry:state', payload)
  }
})

ipcMain.on('synapse:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('synapse:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('synapse:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `synapse:window:openSession`).
...
ipcMain.handle('synapse:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('synapse:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('synapse:preview:reach', async (_event, url) => reachablePreviewUrl(String(url || '')))

ipcMain.handle('synapse:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSynapseCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('synapse:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSynapseCwd()
}))

ipcMain.handle('synapse:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('synapse:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('synapse:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('synapse:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('synapse:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('synapse:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('synapse:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: synapseLog.slice(-200) }))
...
ipcMain.on('synapse:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (synapse:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('synapse:updates:check', async () =>
...
ipcMain.handle('synapse:updates:apply', async (_event, payload) =>
...
ipcMain.handle('synapse:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('synapse:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Synapse version (the one `release.py` bumps in
// synapse_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Synapse version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSynapseVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'synapse_cli', '__init__.py')
...
// Renderer-bundle skew: `synapse update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Synapse version and push it into the native About panel
// just before showing it, so an in-place `synapse update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveSynapseVersion()} — app build out of date, update the desktop app`
        : resolveSynapseVersion(),
      copyright: 'Copyright © 2026 Josh Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('synapse:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveSynapseVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    synapseRoot: resolveUpdateRoot(),
...
// the actual removal via `synapse uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (synapse_cli/uninstall.py + synapse_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SYNAPSE_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isSynapseSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'synapse_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SYNAPSE_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Synapse agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import synapse_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_SYNAPSE_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_SYNAPSE_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `synapse-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `synapse-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('synapse:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('synapse:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('synapse:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('synapse:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// synapse:// deep links (e.g. synapse://blueprint/morning-brief?time=08:00,
// synapse://mcp/install?name=NAME&config=B64 — the vendor "Add to Synapse"
// button, or synapse://plugin/install?repo=owner/repo). Dev
// (`SYNAPSE_DESKTOP_DEV_SERVER`) registers synapse-dev:// instead — bare
// Electron or a stale OS handler often owns synapse:// on dev machines.
...
// ---------------------------------------------------------------------------
const SYNAPSE_PROTOCOL = DEV_SERVER ? 'synapse-dev' : 'synapse'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['synapse-dev', 'synapse'] : ['synapse']
...
function handleDeepLink(url) {
  ...
  // synapse://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('synapse:deep-link', payload)
...
ipcMain.handle('synapse:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${SYNAPSE_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SYNAPSE_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(SYNAPSE_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${SYNAPSE_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `synapse://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startSynapse), whose
...
app.whenReady().then(() => {
  ...
  // startSynapse() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching synapse:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Synapse never keeps another app's chord hostage.
