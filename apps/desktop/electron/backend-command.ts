// Backend subcommand routing for the desktop-managed Synapse process.
//
// The desktop app launches its own headless backend via `synapse serve` — it
...
// app hasn't updated yet, or an older `synapse` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Synapse profile to pin via `--profile`.
 */
 ...
 * `-m synapse_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `synapse_cli/subcommands/dashboard.py` source registers
