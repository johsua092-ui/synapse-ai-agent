 ...
 *
 * The staged Tauri `synapse-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `synapse update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `synapse update`, swaps/relaunches
 * the app, and writes .synapse-update-result.json. With the app gone before
 * the update starts, the SYNAPSE_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `synapse update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into SYNAPSE_HOME on *every* platform
 * (`synapse-setup.exe` on Windows, `synapse-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_synapse_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `synapse update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(synapseHome, 'synapse-setup.exe')
 ...
 *
 * `copy_self_to_synapse_home` deliberately no-ops during `--update`
 ...
 * "Another Synapse update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `synapse update`, which
 ...
 * the installer is written to SYNAPSE_HOME at install/repair time, making mtime
