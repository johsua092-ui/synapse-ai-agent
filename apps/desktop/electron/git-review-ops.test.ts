...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'synapse-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Synapse Test'], { cwd: dir })
