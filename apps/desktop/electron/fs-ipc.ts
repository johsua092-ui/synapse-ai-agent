...
// hardening, SYNAPSE_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('synapse:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('synapse:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('synapse:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('synapse:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<SYNAPSE_HOME>/desktop-plugins`,
  // resolved from the main-process SYNAPSE_HOME (see resolveSynapseHome) — NOT from
  // the connected backend. A remote backend reports its own `synapse_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped synapse_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(synapseHome, 'profiles', profile) : synapseHome
  ...
  ipcMain.handle('synapse:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<SYNAPSE_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where SYNAPSE_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('synapse:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<SYNAPSE_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('synapse:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('synapse:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('synapse:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('synapse:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('synapse:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('synapse:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
