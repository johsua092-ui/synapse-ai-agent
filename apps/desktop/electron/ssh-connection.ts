...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(SYNAPSE_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Synapse-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.synapse/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'synapse-desktop-ssh')
  }

  return path.join(os.homedir(), '.synapse', 'desktop-ssh')
}
