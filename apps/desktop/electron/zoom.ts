...
export const ZOOM_STORAGE_KEY = 'synapse:desktop:zoomLevel'
 ...
 * but forgot to emit 'synapse:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('synapse:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
