...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/synapse', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/synapse/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/synapse'), 'https://gw.example.com/synapse/auth/native/refresh')
})
