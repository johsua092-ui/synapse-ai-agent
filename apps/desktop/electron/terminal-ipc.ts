...
// registry, and the synapse:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // SYNAPSE_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.SYNAPSE_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Synapse' TUI into
    ...
    // Let a synapse/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from SYNAPSE_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.SYNAPSE_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `synapse:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('synapse:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('synapse:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('synapse:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('synapse:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('synapse:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
