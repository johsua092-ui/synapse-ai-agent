 ...
 * the three user-facing uninstall modes to the `synapse uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m synapse_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `synapse_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'synapse_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Synapse.app/Contents/MacOS/Synapse  → …/Synapse.app
 *   Windows: …\Synapse\Synapse.exe                 → …\Synapse  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Synapse.app/Contents/MacOS/Synapse → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Synapse.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Synapse.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Synapse$/i.test(dir) || /[\\/]synapse-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/synapse
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import synapse_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, synapseHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export SYNAPSE_HOME=${q(synapseHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import synapse_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Synapse install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SYNAPSE_HOME=${String(synapseHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
