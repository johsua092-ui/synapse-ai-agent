/**
 * windows-synapse-path.ts
 *
 * Pure, dependency-injected pieces of Windows `synapse` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvSynapseCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `synapse` shim shadowed the real
 *      synapse.cmd/synapse.exe; the shim then failed the --version probe and
 ...
 *      and Synapse launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvSynapseCommand() — unwrapWindowsVenvSynapseCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `synapse`) must
 * not shadow `synapse.cmd`/`synapse.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Synapse launcher that drives `synapse update`.
...
export interface ResolveVenvSynapseCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportSynapseCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `synapse`/`synapse.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/synapse(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m synapse_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportSynapseCli(). A
 * venv whose update died mid-`pip install` still has python.exe + synapse.exe
 ...
 *
 * Returns null when `command` is not a venv synapse shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvSynapseCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvSynapseCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^synapse(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Synapse at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Synapse Python at ${python}`,
    command: python,
