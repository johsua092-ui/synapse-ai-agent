...
//!
//! Driven when the installer is launched as `Synapse-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Synapse desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `synapse update`
//!      or repair bootstrap can race locked files),
//!   2. run `synapse update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in synapse_cli/main.py),
//!   3. run `synapse desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `synapse update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_synapse) and
...
/// `synapse update` exit code meaning "another synapse process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// synapse_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `synapse update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `synapse update` claims
/// the same file (see `synapse_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `synapse update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in synapse_cli/update_lock.py — all three read
...
/// a dashboard-spawned `synapse update`) still blocks.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let synapse_home = crate::paths::synapse_home();
    let install_root = synapse_home.join("synapse-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_synapse`
    ...
    //
    // The same marker is the cross-process update lock (synapse_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `synapse update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Synapse update is already running (PID {}, started {} ago). \
    ...
    let synapse = resolve_synapse(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the synapse CLI under {}. Is Synapse installed? \
    ...
    // async on Windows. If it still holds the venv shim, `synapse update`
    ...
    // ---- stage 2: synapse update -----------------------------------------
    // Pass --branch so `synapse update` targets the branch this installer was
    ...
    // without --branch, `synapse update` switches the checkout to `main` (a
    ...
    // --force skips `synapse update`'s Windows running-exe guard (which would
    ...
    // time `synapse update` runs there is no legitimate synapse.exe to protect,
    // and the guard would only produce a false "Synapse is still running" stop.
    //
    ...
    // probe only checks the synapse.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Synapse windows" message.
    ...
    // Retry-once for the update-boundary crash. `synapse update` lazily imports
    ...
    // second `synapse update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Synapse" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Synapse is still running. Close all Synapse windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "synapse update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::synapse_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: synapse desktop --build-only ----------------------------
    // `synapse update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `synapse update` already does above, and `synapse update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `synapse desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Synapse manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_synapse_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Synapse to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Synapse still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_synapse();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_synapse(install_root)];
...
/// Force-kill any `synapse.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `synapse.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\synapse.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startSynapse()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// synapse.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named synapse.exe.)
fn force_kill_other_synapse() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `synapse <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `synapse update` is precisely the shape that
...
/// Path to the venv synapse shim under an install root, regardless of existence.
fn venv_synapse(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("synapse.exe")
    } else {
        install_root.join("venv").join("bin").join("synapse")
    }
}

/// Resolve the synapse CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `synapse` on PATH.
fn resolve_synapse(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_synapse(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "synapse.exe" } else { "synapse" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let synapse_home = crate::paths::synapse_home();
    let mut envs = vec![(
        "SYNAPSE_HOME".to_string(),
        synapse_home.as_os_str().to_os_string(),
    )];
    // `synapse update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `synapse update` child claims that SAME lock (synapse_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Synapse is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in synapse_cli/update_lock.py.
    envs.push((
        "SYNAPSE_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        synapse_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let tmp = PathBuf::from(format!("{}.synapse-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.synapse-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::synapse_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_synapse_is_under_install_root() {
        let root = Path::new("/x/synapse-agent");
        let shim = venv_synapse(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/synapse-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/synapse-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "SYNAPSE_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the synapse update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_synapse(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/synapse-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".synapse-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `synapse update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(synapseHome, child.pid) races ahead
