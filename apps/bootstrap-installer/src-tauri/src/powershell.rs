...
/// read side does not see EOF until the last of them closes it. `synapse update`
...
/// stranded with it.
///
/// Same bound `Invoke-SynapseStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `synapse_home_override` propagates to the child as $SYNAPSE_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, synapse_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = synapse_home_override {
        cmd.env("SYNAPSE_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, synapse_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = synapse_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_synapse_home() {
    ...
    /// child. `synapse update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
