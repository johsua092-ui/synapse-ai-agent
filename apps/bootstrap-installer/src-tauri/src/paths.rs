...
//!
//! Mirrors `synapse_constants.get_synapse_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\synapse
//!   macOS:   ~/.synapse
//!   Linux:   ~/.synapse  (override via $SYNAPSE_HOME)
//!
//! NOTE (macOS): Python's get_synapse_home(), scripts/install.sh, and the
//! Electron desktop's resolveSynapseHome() ALL use ~/.synapse on macOS — there
...
/// Returns the canonical Synapse home directory, respecting $SYNAPSE_HOME if set.
pub fn synapse_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SYNAPSE_HOME") {
    ...
    {
        // %LOCALAPPDATA%\synapse — matches scripts/install.ps1's $SynapseHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("synapse");
        }
    }

    // macOS + Linux + fallback: ~/.synapse (matches Python get_synapse_home(),
    // install.sh, and the Electron desktop's resolveSynapseHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".synapse");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".synapse")
}

pub fn log_dir() -> PathBuf {
    synapse_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    synapse_home().join("bootstrap-cache")
}
...
/// SYNAPSE_HOME so it survives repo checkout deletion (unlike anything under
/// synapse-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\synapse\synapse-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_synapse`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under SYNAPSE_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves SYNAPSE_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    synapse_home().join(".synapse-update-in-progress")
}
...
pub fn copy_self_to_synapse_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SYNAPSE_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SYNAPSE_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".synapse-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SYNAPSE_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[synapse-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SYNAPSE_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_synapse_home() -> String {
    synapse_home().to_string_lossy().into_owned()
}
