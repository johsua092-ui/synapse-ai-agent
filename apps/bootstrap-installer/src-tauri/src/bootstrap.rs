...
//! SYNAPSE_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for SYNAPSE_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub synapse_home: Option<String>,
}
...
/// Spawn the locally-built Synapse desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_synapse_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_synapse_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Synapse desktop at {}. The desktop build step \
             may have been skipped or failed. Run `synapse desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Synapse desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Synapse
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_synapse_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
...
pub(crate) fn resolve_synapse_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
...
/// path so a bare re-open just opens Synapse instead of re-running setup.
pub(crate) fn synapse_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".synapse-bootstrap-complete").exists()
        && resolve_synapse_desktop_exe(install_root).is_some()
}
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    // writeFileAtomic(). synapse_is_installed() only checks existence, so a
    // partial direct write would incorrectly enable the launcher fast path.
    let tmp_path = install_root.join(".synapse-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_synapse_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Synapse desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_synapse_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::synapse_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $SynapseHome which Stage-Repository clones
    // the repo INTO at $SynapseHome\synapse-agent. Mirrors synapse_constants.
    let synapse_home = args
        .synapse_home
        .clone()
        .unwrap_or_else(|| crate::paths::synapse_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&synapse_home).join("synapse-agent");
    ...
    // Copy ourselves to SYNAPSE_HOME/synapse-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_synapse_home() {
        tracing::warn!(?err, "failed to copy installer into SYNAPSE_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    synapse_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, synapse_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn resolve_synapse_desktop_app_finds_built_bundle() {
        ...
        let resolved = resolve_synapse_desktop_app(&root)
    ...
    #[test]
    fn resolve_synapse_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_synapse_desktop_app(&root).is_none(),
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".synapse-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".synapse-bootstrap-complete");
        let tmp_path = root.join(".synapse-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    #[test]
    fn synapse_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".synapse-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".synapse-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".synapse-bootstrap-complete.tmp").exists(),
