...
export const $synapseHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.synapse/logs/bootstrap-installer.log')
    $synapseHome.set('~/.synapse')
  ...
  try {
    const [logPath, synapseHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_synapse_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $synapseHome.set(synapseHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Synapse" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Synapse-Setup.exe --update);
...
export async function launchSynapseDesktop(): Promise<void> {
  ...
  await invoke('launch_synapse_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Synapse repository' },
