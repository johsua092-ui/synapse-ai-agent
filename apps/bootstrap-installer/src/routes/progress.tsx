...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Synapse' : 'Setting up Synapse Agent'

  const description = isUpdate
    ? 'Synapse is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Synapse installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="synapse-fade-in flex h-full flex-col">
