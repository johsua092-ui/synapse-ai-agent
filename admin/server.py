"""
Synapse Agent — Railway admin server.
...
Responsibilities:
  ...
  - Reverse proxy at / and /* → native Synapse dashboard (synapse_cli/web_server, on 127.0.0.1:9119)
  - Managed subprocesses: `synapse gateway` (agent) and `synapse dashboard` (native UI)
  - Cookie-based session auth at /login (HMAC-signed, 7-day expiry, httponly)

Auth model: Basic Auth was dropped in favor of cookies because the Synapse React
...
Once configured, / proxies to the Synapse dashboard. A small "← Setup" widget is
...
ENV_FILE = Path(SYNAPSE_HOME) / ".env"

# The Synapse release this image pins. The Dockerfile promotes its `ARG
# SYNAPSE_REF` to an ENV so we can read it here; a Railway service variable of
...
SYNAPSE_VERSION = os.environ.get("SYNAPSE_REF", "").strip()


def _resolve_pairing_dir() -> Path:
    """Locate the pairing store the same way synapse' get_synapse_dir() does.

    synapse resolves ``PAIRING_DIR = get_synapse_dir("platforms/pairing", "pairing")``:
    it honours the legacy ``$SYNAPSE_HOME/pairing/`` ONLY when that dir has
    content, otherwise it uses the consolidated ``platforms/pairing/``. The rule
    changed in **v2026.7.1** — before it (v2026.6.19 and earlier) get_synapse_dir
    ...
    consolidated path. Re-verify this against get_synapse_dir on the next bump.
    """
    legacy = Path(SYNAPSE_HOME) / "pairing"
    try:
        if legacy.is_dir() and any(legacy.iterdir()):
            return legacy
    except OSError:
        # Can't inspect (e.g. permissions) — assume occupied rather than risk
        # orphaning legacy data, matching synapse' _legacy_path_has_content.
        return legacy
    return Path(SYNAPSE_HOME) / "platforms" / "pairing"


def pairing_dir() -> Path:
    """Resolve the pairing store on every call — never cache it at import.

    `/setup/api/backup/restore` shells out to `synapse import`, which can create
    a *populated* legacy ``pairing/`` mid-process. That flips the dir synapse
...
def _consolidate_pairing_dirs() -> None:
    ...
    synapse >= v2026.7.20 added ``_migrate_split_pairing_dirs()``
    ...
    while this panel shows them removed. synapse' own ``revoke()`` has the same
    ...
    Safety: the active dir always wins on key conflict (matching synapse'
    ...
    legacy = Path(SYNAPSE_HOME) / "pairing"
    consolidated = Path(SYNAPSE_HOME) / "platforms" / "pairing"
    try:
    ...
    for src in sorted(alternate.glob("*.json")):
        ...
        if not stale:
            ...
            # is inert: synapse' own merge skips it too.
            continue
...
# ── Global emergency stop / pause (synapse >= v2026.8.13) ─────────────────────
ESTOP_FILE = Path(SYNAPSE_HOME) / "ESTOP"


def estop_state() -> dict | None:
    """Pause details, or None when the bot is accepting work.

    v2026.8.13 added `synapse pause` and the in-chat `/pause`, which write a
    sentinel at ``$SYNAPSE_HOME/ESTOP`` (agent/estop.py). While it exists synapse
    refuses every NEW gateway turn, cron dispatch and kanban dispatch with
    "⏸️ Synapse is paused" — but the process stays alive, so `/health` keeps
...
# Native Synapse dashboard — runs on loopback, fronted by our reverse proxy.
SYNAPSE_DASHBOARD_HOST = "127.0.0.1"
SYNAPSE_DASHBOARD_PORT = int(os.environ.get("SYNAPSE_DASHBOARD_PORT", "9119"))
SYNAPSE_DASHBOARD_URL = f"http://{SYNAPSE_DASHBOARD_HOST}:{SYNAPSE_DASHBOARD_PORT}"

# Header synapse' own SPA uses to present its per-process session token
# (synapse_cli/web_server.py's _SESSION_HEADER_NAME) — see
# set_active_model_via_synapse()/_get_synapse_session_token() for why our own
# server-to-server calls to the dashboard need it even on our loopback bind.
_SESSION_TOKEN_HEADER = "X-Synapse-Session-Token"
...
# synapse's own /api/env/reveal and OAuth endpoints, and keep `cookie` since
# some synapse endpoints read it. Aggressive stripping was masking requests in
...
ENV_VARS = [
    ...
    # global (api.minimax.io) and China (api.minimaxi.com). Synapse ships both as
    ...
    # Added in v2026.4.23+ (synapse v0.11.0+). All plain API-key auth — synapse
    ...
    # are set up via the dashboard's Keys tab or SYNAPSE_AUTH_JSON_BOOTSTRAP.
    # New in synapse v2026.8.13. Plain API key (ac_…); ACTUAL_BASE_URL is
    ...
    # Custom OpenAI-compatible endpoint — one slot; more via Synapse dashboard.
...
# Maps our own provider-key env var to synapse' OWN canonical provider id
# (synapse_cli/auth.py PROVIDER_REGISTRY, verified against v2026.7.1). Used by
# set_active_model_via_synapse() to pin an explicit model.provider via synapse'
...
# synapse_cli/auth.py on a Synapse version bump (same audit as the WS allowlist).
...
SYNAPSE_PROVIDER_IDS = {
    "OPENROUTER_API_KEY":    "openrouter",
    "DEEPSEEK_API_KEY":      "deepseek",
    "DASHSCOPE_API_KEY":     "alibaba",       # "Qwen Cloud" in synapse' own UI
    ...
    # These three are NOT in synapse' own PROVIDER_REGISTRY — verified against
    # BOTH synapse_cli/auth.py (resolve_provider(), used by the CLI/"auto"
    # env-var auto-detect loop) AND synapse_cli/runtime_provider.py
    ...
    # The only way any of them work is the same mechanism synapse' OWN
    ...
    # (synapse_cli/runtime_provider.py's bare-"custom" trust path reads
    ...
    # set_active_model_via_synapse(). Re-verify FIREWORKS_API_KEY/NOVITA_API_KEY
    # base URLs against those providers' own docs (not synapse') if they ever
...
# SYNAPSE_PROVIDER_IDS with value "custom" is automatically covered by both
# api_config_put()'s pin call and write_config_yaml()'s fallback below —
# no other code needs to change.
SYNAPSE_CUSTOM_STYLE_KEYS = {k for k, v in SYNAPSE_PROVIDER_IDS.items() if v == "custom"}
...
def write_config_yaml(data: dict[str, str], *, reset_model: bool = False) -> None:
    ...
    Previously this overwrote ``$SYNAPSE_HOME/config.yaml`` with a hardcoded template
    body on every boot, silently erasing user-managed top-level keys. The most
    common casualty is ``mcp_servers`` — Synapse reads downstream MCP servers
    *only* from this file (see ``synapse_cli/mcp_config.py:_get_mcp_servers``), so
    the wipe broke ``synapse mcp add/test/list`` state across every container
    ...
    """
    import yaml  # synapse-agent already pulls pyyaml; deferred import keeps cold start light

    model = data.get("LLM_MODEL", "")
    config_path = Path(SYNAPSE_HOME) / "config.yaml"
    ...
    if reset_model:
        ...
        # the user configures). Everything else — synapse tuning defaults,
        ...
        # set_active_model_via_synapse() below (which delegates to synapse' own
        # POST /api/model/set) or by synapse' own dashboard — PRESERVE it here.
        ...
        # the very next restart. "auto" resolves by scanning synapse' own
        ...
        # DIFFERENT provider — the exact bug that made synapse route a
        ...
        if not current_provider:
            named_key = next(
                (k for k in PROVIDER_KEYS if k not in SYNAPSE_CUSTOM_STYLE_KEYS and data.get(k)),
                None,
            )
            custom_style_key = next((k for k in SYNAPSE_CUSTOM_STYLE_KEYS if data.get(k)), None)
            if named_key:
                ...
                # NOT in synapse' own PROVIDER_REGISTRY (see SYNAPSE_PROVIDER_IDS'
                ...
                # set_active_model_via_synapse() pin in api_config_put(): this
        ...
        # over provider" upstream (synapse_cli/config.py), so a leftover — e.g.
        # a former `base_url: https://openrouter.ai/api/v1` from the synapse
        ...
        # Strip them here, mirroring synapse' own clear_model_endpoint_credentials()
        # on a switch-away-from-custom. Skipped only for "custom"/"local" —
        # synapse' own convention for a user-supplied (or fixed-URL aggregator)
        # endpoint that legitimately needs its own base_url/api_key set
        # directly on model.* (see the "custom_style_key" branch above —
        # synapse' runtime resolver reads model.base_url/api_key directly, NOT
    ...
    # someone who deliberately picks Browser Use in synapse' own settings (or
    ...
    # preference, so a value chosen in synapse' own settings survives.
    ...
    if custom_base_url:
        ...
        # Sanitise to a valid synapse provider name (lowercase alphanumeric + hyphens).
...
def build_synapse_env() -> dict[str, str]:
    """Merge OS env + SYNAPSE_HOME + .env file contents for a synapse subprocess.

    .env values take priority over Railway env vars. We build the env this way
    so synapse's own dotenv loading (which reads the same file) doesn't shadow
    our values. Shared by every synapse subprocess we spawn (gateway, dashboard)
    ...
    SYNAPSE_HOME/.env, not in our own os.environ.
    """
    env = {**os.environ, "SYNAPSE_HOME": SYNAPSE_HOME}
    env.update(read_env(ENV_FILE))
    # Retire synapse' own respawn-storm breaker (new in v2026.7.20,
    # synapse_cli/gateway.py `run_gateway`): once >5 starts land in a rolling
    # 120s window it BLOCKS with time.sleep() before the gateway boots, counted
    # in a persistent ledger ($SYNAPSE_HOME/gateway-starts.log) that this
    ...
    env.setdefault("SYNAPSE_RESTART_AFTER_TURN_TIMEOUT", "0")
    # Never hand a synapse subprocess SYNAPSE_PARENT_PID. v2026.8.13's new
    # _start_parent_death_watchdog() (synapse_cli/web_server.py) polls that PID
    # and calls os._exit(0) once it is gone — it is the Electron desktop's
    # orphan guard, and it is NOT gated on SYNAPSE_DESKTOP, so it arms itself on
    # any `synapse dashboard` whose environment carries the key. Killing the
    ...
    #
    # It is deliberately NOT the whole fix: synapse also loads $SYNAPSE_HOME/.env
    ...
    env.pop("SYNAPSE_PARENT_PID", None)
    return env


# Keys that must never survive in $SYNAPSE_HOME/.env, because synapse re-reads
...
# SYNAPSE_PARENT_PID=999999 in .env the dashboard os._exit(0)s seconds after
...
ENV_FILE_FORBIDDEN_KEYS = ("SYNAPSE_PARENT_PID",)


def _sanitize_env_file() -> None:
    """Drop keys from .env that would let synapse kill its own dashboard."""
    try:
...
# any redirect URL. The client_id matches synapse's own Grok CLI credential.
...
def _has_xai_oauth_tokens() -> bool:
    ...
    auth_path = Path(SYNAPSE_HOME) / "auth.json"
...
def _save_xai_auth_json(tokens: dict) -> None:
    """Write xAI OAuth tokens to auth.json in synapse's expected format."""
    auth_path = Path(SYNAPSE_HOME) / "auth.json"
    existing: dict = {}
    if auth_path.exists():
        try:
...
#      the Synapse React SPA's plain fetch() calls don't reliably include it,
#      causing every proxied API call to 401.
# Cookies are auto-included on every same-origin request (navigation + XHR)
# so both the setup UI and the proxied Synapse dashboard work with one login.
#
...
<title>Synapse Agent — Sign in</title>
...
<div class="card">
  <div class="brand">
    <div class="brand-logo">synapse<span>/admin</span></div>
...
# in-band `/restart` (inside a container synapse exits 75 expecting a supervisor
...
class Gateway:
    ...
    async def start(self, *, reset_budget: bool = True):
        ...
        self._stopping = False
        try:
            env = build_synapse_env()
            model = env.get("LLM_MODEL", "")
            provider_key = next((env.get(k, "") for k in PROVIDER_KEYS if env.get(k)), "")
            print(f"[gateway] model={model or '⚠ NOT SET'} | provider_key={'set' if provider_key else '⚠ NOT SET'}", flush=True)
            # Write config.yaml so synapse picks up the model (env vars alone aren't always enough)
            ...
            # process (e.g. synapse' own dashboard spawns its own detached
            # `synapse gateway restart` via its native /api/gateway/restart
            # action, entirely outside this class's tracking) makes every
            # subsequent plain `synapse gateway` invocation refuse to start
            ...
            # die. --replace is synapse' own blessed fix for exactly this
    ...
    async def stop(self):
        ...
        try:
            ...
            # outrun 10s, and SIGKILL mid-teardown skips synapse' atexit pid
            # cleanup — which is exactly the leftover _clear_stale_pidfile()
            # then has to mop up. Still well inside synapse' own drain+60s
    ...
    async def _supervise_respawn(self, dead_pid: int | None):
        ...
        if not is_config_complete():
            ...
            return
        # Clear a pid file left stale by a hard crash (SIGKILL/OOM skips synapse'
    ...
    def _clear_stale_pidfile(self, dead_pid: int | None) -> None:
        if dead_pid is None:
            return
        pid_file = Path(SYNAPSE_HOME) / "gateway.pid"
        try:
...
# ── Synapse dashboard subprocess ───────────────────────────────────────────────
class Dashboard:
    """Manages the `synapse dashboard` subprocess (native Synapse web UI).
    ...
    Spawned with the same merged env (OS env + SYNAPSE_HOME + .env contents)
    as the gateway — see build_synapse_env(). Without it, the dashboard process
    ...
    because synapse' own provider auto-resolution (synapse_cli/auth.py) reads
    ...
    async def start(self):
        ...
        try:
            self.proc = await asyncio.create_subprocess_exec(
                "synapse", "dashboard",
                "--host", SYNAPSE_DASHBOARD_HOST,
                "--port", str(SYNAPSE_DASHBOARD_PORT),
                "--no-open",
                # --skip-build: the Dockerfile pre-builds the React dashboard
                # into synapse_cli/web_dist/ at image time. This flag tells
                # synapse to trust that dist and skip its npm build check,
                # which would otherwise add ~30s to first startup (synapse >= v2026.5.16).
                "--skip-build",
                # NOTE: the embedded Chat tab (/api/pty + /api/ws + /api/events)
                # is unconditionally enabled as of synapse v2026.6.5 — the old
                ...
                # Dockerfile still pre-builds ui-tui/dist/ (via SYNAPSE_TUI_DIR)
                # so the PTY child spawns instantly on first chat connect.
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.STDOUT,
                env=build_synapse_env(),
            )
            print(f"[dashboard] spawned pid={self.proc.pid} → {SYNAPSE_DASHBOARD_URL}", flush=True)
...
_SYNAPSE_SESSION_TOKEN_RE = re.compile(r'__SYNAPSE_SESSION_TOKEN__\s*=\s*"([^"]*)"')


async def _get_synapse_session_token() -> str:
    """Scrape the dashboard's own ephemeral session token from its SPA shell.

    Synapse gates every non-public ``/api/*`` route behind a per-process
    random ``_SESSION_TOKEN`` — a legacy check in synapse_cli/web_server.py's
    ...
    the SPA itself does: on a loopback (ungated) bind, synapse injects it into
    every served HTML shell as ``window.__SYNAPSE_SESSION_TOKEN__="..."``
    (synapse_cli/web_server.py's ``_serve_index``). That HTML-serving catch-all
    ...
    synapse_cli/web_server.py on a Synapse version bump — if it's ever renamed
    or removed, this degrades to the pre-existing "no token" 401 handled
    below, not a crash.
    """
    client = get_http_client()
    resp = await client.get(f"{SYNAPSE_DASHBOARD_URL}/", timeout=httpx.Timeout(10.0))
    resp.raise_for_status()
    match = _SYNAPSE_SESSION_TOKEN_RE.search(resp.text)
    return match.group(1) if match else ""


async def set_active_model_via_synapse(
    provider_id: str, model: str, *, base_url: str = "", api_key: str = ""
) -> str | None:
    """Pin model.provider + model.default via synapse' own POST /api/model/set.

    Delegates to synapse_cli/web_server.py's _apply_main_model_assignment — the
    same code path its dashboard's "Switch Model" dialog and flat Config page
    use — instead of us hand-writing config.yaml's model block. Synapse always
    ...
    unconditional default) lets synapse resolve to the WRONG provider — the
    ...
    ``base_url``/``api_key`` fields (synapse_cli/web_server.py's
    ``ModelAssignment`` schema — "Only honored for custom/local providers on
    the main slot"). REQUIRED for provider_id="custom": synapse' actual
    runtime resolver (synapse_cli/runtime_provider.py, what the gateway/Chat
    ...
    display/bookkeeping only, for synapse' own Keys-tab picker). Passing them
    lets synapse write model.base_url/model.api_key itself; it also
    ...
    client = get_http_client()
    try:
        session_token = await _get_synapse_session_token()
    except httpx.HTTPError as e:
        return f"Could not fetch a Synapse session token to pin {provider_id} ({e}); using auto-resolution instead."
    headers = {_SESSION_TOKEN_HEADER: session_token} if session_token else {}

    try:
        resp = await client.post(
            f"{SYNAPSE_DASHBOARD_URL}/api/model/set",
            json={
                "scope": "main",
                ...
                "base_url": base_url,
                "api_key": api_key,
                # We have no UI to show synapse' own "this model looks
        ...
        return f"Could not reach the Synapse dashboard to pin {provider_id} ({e}); using auto-resolution instead."
    except httpx.RequestError as e:
        return f"Synapse model/set request failed ({e}); using auto-resolution instead."

    if resp.status_code != 200:
        return f"Synapse rejected the {provider_id} model/provider pin (HTTP {resp.status_code}); using auto-resolution instead."
    try:
        data = resp.json()
    except Exception:
        return None  # 200 with an unparseable body — nothing actionable to report
    if data.get("ok") is False:
        return data.get("confirm_message") or f"Synapse did not apply the {provider_id} model/provider pin; using auto-resolution instead."
    return None
...
async def api_config_put(request: Request):
    ...
    try:
        ...
        # toggling a messaging channel). See set_active_model_via_synapse().
        ...
        model_warning = None
        synapse_provider_id = SYNAPSE_PROVIDER_IDS.get(active_provider_key)
        model_value = merged.get("LLM_MODEL", "").strip()
        if synapse_provider_id and model_value:
            pin_base_url = ""
            pin_api_key = ""
            if synapse_provider_id == "custom":
                pin_base_url = (
            ...
            model_warning = await set_active_model_via_synapse(
                synapse_provider_id, model_value, base_url=pin_base_url, api_key=pin_api_key
            )
...
async def api_status(request: Request):
    ...
    return JSONResponse({"gateway": gw.status(), "providers": providers,
                         "channels": channels, "synapse_version": SYNAPSE_VERSION,
                         # None when running; a dict (possibly with null fields)
                         # when synapse' ESTOP sentinel is engaged — see
...
async def api_estop_resume(request: Request):
    ...
    Mirrors synapse' own `synapse resume` / `/pause off`, which is just
    ``disengage()`` unlinking ``$SYNAPSE_HOME/ESTOP`` (agent/estop.py). synapse
...
# ── Pairing ───────────────────────────────────────────────────────────────────
# Pending-request file format (synapse >= v0.15 / v2026.5.29.x, gateway/pairing.py):
...
# ── Backup & Restore ─────────────────────────────────────────────────────────
# Thin wrapper around synapse' OWN `synapse backup` / `synapse import` CLI
# (synapse_cli/backup.py, verified against v2026.7.1) rather than reimplementing
...
# ourselves. Re-verify both on a future Synapse version bump, same as every
# other upstream-CLI assumption this template makes.
BACKUP_DIR = Path(SYNAPSE_HOME) / "backups"   # synapse' own pre-update-backup convention;
                                              # this dir is itself in synapse' backup
                                              # exclusion list, so snapshots here never
                                              # bloat a future full backup.
PRE_RESTORE_KEEP = 3
BACKUP_SUBPROCESS_TIMEOUT = 600  # 10 min ceiling for both `synapse backup` and `synapse import`

# synapse >= v2026.8.13 serializes backups across processes: `run_backup` takes a
# flock on $SYNAPSE_HOME/.backup.lock with a 0.25s acquire timeout and, on a
# miss, raises SystemExit(2) after printing "another Synapse backup is already
# running". Our own asyncio `backup_lock` cannot prevent this — it only
# serializes OUR two callers, while synapse' snapshot path is reachable
...
# 2 on an unrecognised flag (verified — `synapse backup --nonsense` -> rc 2).
# Our argv is fixed, so that can only happen if a future synapse renames `-o`,
...
BACKUP_BUSY_MESSAGE = ("Another Synapse backup or snapshot is running right now "
                       "(they cannot run at the same time). Try again in a moment.")


def _is_backup_busy(rc: int, output: str) -> bool:
    """True when `synapse backup` bailed because it lost the cross-process lock."""
...
async def _run_synapse_cli(*args: str, timeout: float = BACKUP_SUBPROCESS_TIMEOUT) -> tuple[int, str]:
    """Run a `synapse <args>` subcommand, capturing combined stdout+stderr.

    Shares build_synapse_env() with Gateway/Dashboard so the CLI sees provider
    ...
    try:
        ...
        return 127, f"Could not launch synapse {' '.join(args)}: {e}"
    try:
        raw, _ = await asyncio.wait_for(proc.communicate(), timeout=timeout)
    except asyncio.TimeoutError:
        proc.kill()
        await proc.wait()
        return 124, f"synapse {' '.join(args)} timed out after {timeout}s"
    return proc.returncode, raw.decode(errors="replace")


async def _synapse_version() -> str:
    """Best-effort `synapse --version`, used only for the restore-time compat hint."""
    try:
        rc, out = await _run_synapse_cli("--version", timeout=15)
...
def _sweep_stale_backup_tmpdirs() -> None:
    """Clean up /tmp/synapse-backup-* left behind by a client aborting a download
    ...
    """
    for stale in Path(tempfile.gettempdir()).glob("synapse-backup-*"):
        shutil.rmtree(stale, ignore_errors=True)

    # v2026.8.13 made `synapse backup -o` atomic: it builds the archive at
    ...
    # backup_lock, but synapse' dashboard has its own detached `synapse backup`
...
# Mirrors synapse' own _EXCLUDED_DIRS (synapse_cli/backup.py, v2026.8.13) so
# _live_db_names() can never demand a database synapse deliberately skips. That
...
_BACKUP_EXCLUDED_DIRS = {
    "synapse-agent", "__pycache__", ".git", "node_modules", "backups",
...
def _live_db_names() -> set[str]:
    ...
    synapse' `_safe_copy_db` fails CLOSED — it drops a database it cannot
    snapshot from the archive while `synapse backup` still exits 0. A check
    naming only state.db therefore certifies an archive as sound while other
    databases are silently absent from it.

    Names, not paths: synapse writes some of these nested (cron/…), and
...
def _incomplete_backup_reason(zip_path: Path) -> str | None:
    ...
    `rc == 0` stopped proving that in synapse v2026.7.20: `_safe_copy_db`
    (synapse_cli/backup.py) lost its raw-copy fallback and now fails CLOSED, so a
    SQLite snapshot that can't be taken is DROPPED from the archive while
    `synapse backup` still exits 0 — the only trace is the word "incomplete" in
...
async def api_backup_download(request: Request) -> Response:
    ...
    async with backup_lock:
        tmp_dir = tempfile.mkdtemp(prefix="synapse-backup-")
        zip_path = Path(tmp_dir) / "backup.zip"
        rc, output = await _run_synapse_cli("backup", "-o", str(zip_path))
        ...
        try:
            version = await _synapse_version()
            with zipfile.ZipFile(zip_path, "a") as zf:
                zf.writestr("template_manifest.json", json.dumps({
                    "synapse_version": version,
                    "created_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                    "template": "synapse-agent-railway-template",
                }))
        except Exception:
            pass

        filename = f"synapse-backup-{int(time.time())}.zip"
        headers = {}
...
async def api_backup_restore(request: Request) -> Response:
    ...
    async with backup_lock:
        tmp_fd, tmp_name = tempfile.mkstemp(suffix=".zip", prefix="synapse-restore-")
        ...
        try:
            ...
            with zipfile.ZipFile(upload_path) as zf:
                ...
                if not names & {"config.yaml", ".env", "state.db"}:
                    return JSONResponse(
                        {"error": "This doesn't look like a synapse backup (no config.yaml/.env/state.db found)"},
                        status_code=400,
                    )
                if "template_manifest.json" in names:
                    try:
                        manifest = json.loads(zf.read("template_manifest.json"))
                        backup_version = manifest.get("synapse_version", "")
                        current_version = await _synapse_version()
                        if backup_version and current_version != "unknown" and backup_version != current_version:
                            warning = (
                                f"Backup was created with synapse {backup_version}, this deployment runs "
            ...
            rc, output = await _run_synapse_cli("backup", "-o", str(snap_path))
            ...
            await dash.stop()
            try:
                rc, output = await _run_synapse_cli("import", str(upload_path), "--force")
            finally:
                # An imported archive can carry the legacy `pairing/` layout,
                # which leaves two populated pairing dirs. Collapse them before
                # anything reads the store again — synapse' own merge would
                ...
                # A restored .env is the only realistic way SYNAPSE_PARENT_PID
...
# ── Reverse proxy → Synapse dashboard ──────────────────────────────────────────
_WIDGET_LINK_STYLE = (
...
BACK_TO_SETUP_WIDGET = (
    '<div id="synapse-back-widget" style="position:fixed;bottom:14px;right:14px;'
...
# (synapse_cli/web_routers/tools.py) npm/pip-installs into /opt/synapse-agent and
...
# $SYNAPSE_HOME/mcp-installs on the volume, so it does survive a redeploy.
...
# synapse_cli/web_routers/tools.py). Flipping a toolset ON now spawns
# `synapse tools post-setup <key>` in the background whenever that toolset's
# provider has a post_setup hook with an UNSATISFIED install-state predicate.
# Today `_POST_SETUP_INSTALLED` (synapse_cli/tools_config.py) holds exactly one
...
IMMUTABLE_INSTALL_WARNING_JS = (
    '<script>(function(){'
    'var f=window.fetch;if(!f||window.__synapseImmutableWarn)return;'
    'window.__synapseImmutableWarn=1;'
...
<body><div class="card">
<h1>⚠ Synapse dashboard unavailable</h1>
<p>The native Synapse dashboard is not responding on port %d.<br>
...
</body></html>""" % SYNAPSE_DASHBOARD_PORT


async def _proxy_to_dashboard(request: Request) -> Response:
    """Forward an authenticated request to the Synapse dashboard subprocess.

    Assumes edge auth (basic auth middleware) has already validated the caller.
    HTTP-only: the native Synapse dashboard does not use WebSockets.
    """
    client = get_http_client()
    target = f"{SYNAPSE_DASHBOARD_URL}{request.url.path}"
    if request.url.query:
    ...
    # Surface non-2xx responses from synapse into Railway logs so we can
...
async def route_proxy(request: Request) -> Response:
    """Catch-all: forward any unmatched path to the Synapse dashboard."""
    if err := guard(request): return err
    ...
    if kind == "warned":
        ...
        # background `synapse tools post-setup`, so keep it even though the
        # install itself most likely lands on the volume.
        print(f"[proxy] toolset enable may trigger an install-on-enable: "
              f"{request.method} {request.url.path} (synapse >= v2026.8.13)", flush=True)
    return await _proxy_to_dashboard(request)
...
async def lifespan(app):
    ...
    # Strip .env keys that would make synapse shut its own dashboard down before
...
# The synapse dashboard exposes several WebSocket endpoints when started with
...
#   /api/console              text frames   — Synapse Console modal (System tab →
...
#                             up in railway logs).
#   /api/plugins/<name>/...   plugin-contributed sockets. Mounted by synapse
...
#   * Upstream: synapse's own ?token=<_SESSION_TOKEN> query param. The SPA
...
# Mirrors synapse' own ws_max_size (v2026.8.3, web_server.py:362) so this proxy
# is never the narrow end. Our hops default lower — 1 MiB inbound from synapse,
...
SYNAPSE_WS_MAX_BYTES = 384 * 1024 * 1024
...
async def ws_proxy(websocket: WebSocket) -> None:
    """Reverse-proxy a single WebSocket from browser → synapse dashboard.

    Order matters: connect upstream BEFORE accepting the client. If synapse
    ...
    #    contains the synapse session token + channel id).
    path = websocket.url.path
    qs = websocket.url.query
    upstream_url = f"ws://{SYNAPSE_DASHBOARD_HOST}:{SYNAPSE_DASHBOARD_PORT}{path}"
    if qs:
    ...
    try:
        upstream = await websockets.connect(
            upstream_url,
            open_timeout=5,
            # No keepalive on the loopback hop. synapse v2026.7.20 disabled its
            ...
            # 20s/20s default would re-create the bug synapse just fixed: a long
            # tool call drops Chat/sidecar mid-turn. Liveness is unaffected — a
            # dead synapse closes the socket for real (ConnectionClosed in the
            # pumps), and the browser-facing hop keeps uvicorn's own ping.
            ping_interval=None,
            ping_timeout=None,
            # synapse -> us leg: the narrower hop, 1 MiB by default.
            max_size=SYNAPSE_WS_MAX_BYTES,
            # Don't forward client cookies/headers — synapse WS auth is
        ...
        # Synapse dashboard down, restarting, or rejected the upgrade
...
routes = [
    ...
    # Reverse-proxy synapse's dashboard WebSockets (Chat tab + sidecar).
    ...
    # Synapse Console modal, new in v2026.7.20 (synapse_cli/web_server.py's
    ...
    # Plugin-contributed sockets, mounted by synapse under /api/plugins/<name>/
    # (e.g. kanban's /api/plugins/kanban/events). Prefix-matched so new plugin
    # WS endpoints in future synapse releases proxy without re-touching this list.
    ...
    # Catch-all: everything else proxies to the Synapse dashboard subprocess.
...
if __name__ == "__main__":
    ...
    config = uvicorn.Config(app, host="0.0.0.0", port=port, log_level="info", loop="asyncio",
                            ws_max_size=SYNAPSE_WS_MAX_BYTES)
