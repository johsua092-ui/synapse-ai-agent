...
Synapse middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Synapse'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Synapse calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Synapse retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Synapse guardrails,
...
- `telemetry_schema_version`: currently `synapse.observer.v1`
- `middleware_schema_version`: currently `synapse.middleware.v1`
...
Synapse stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Synapse runs
them as a nested chain in registration order. Middleware failures are fail-open:
Synapse logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Synapse applies middleware in this order:
...
For each tool call, Synapse applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Synapse pre-execution path against the effective arguments:
...
For isolated local testing, use one `SYNAPSE_HOME` for plugin enablement and the
agent run:

```bash
export SYNAPSE_HOME=/tmp/synapse-middleware-test
mkdir -p "$SYNAPSE_HOME"
synapse plugins enable <plugin-name>
synapse chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run synapse plugins enable <plugin-name>
uv run synapse chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["synapse_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/synapse-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Synapse expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Synapse treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Synapse preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Synapse does not convert downstream
