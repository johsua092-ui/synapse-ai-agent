...
## 2026-07-13: Scope plugin manager state by Synapse home/profile (keyed cache)

Status: Accepted

Context:
Synapse supports multiple profiles via different Synapse home directories.
Homes are switched two ways in a running process: the `SYNAPSE_HOME`
environment variable (single-profile CLI/gateway processes), and the
context-local `set_synapse_home_override()` (`synapse_constants.py`), which
...
`get_synapse_home() / "plugins"`, and context-engine plugins (e.g.
`synapse-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_synapse_home_override()` was invisible to a
   naive "did `SYNAPSE_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `synapse_plugins.<slug>` by
   ...
   `synapse_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  Synapse home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
  `get_plugin_manager()` resolves the current home via `get_synapse_home()`
  (which itself already consults `get_synapse_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `SYNAPSE_HOME` or `set_synapse_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_synapse_home_override()`) rather than only the env-var path, and
