...
Synapse supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.synapse/config.yaml`:
...
Or set the env var: `SYNAPSE_KANBAN_DISPATCH_IN_GATEWAY=false`
