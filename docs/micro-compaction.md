...
has to be thrown away or summarized. Synapse has always done this in one batch:
...
Synapse folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Synapse's normal session flush is
...
With `micro_compact` unset or `false` Synapse behaves exactly as it always has:
...
Defaults to `$SYNAPSE_HOME/logs/agent.log`. It reports passes, outcome mix, net
