...
The canonical transcript store is SQLite via `SessionDB` (from `synapse_state`). The
...
Counts consecutive restarts via a JSON file (`{SYNAPSE_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `synapse update`, `synapse gateway restart`,
or `/restart`.
