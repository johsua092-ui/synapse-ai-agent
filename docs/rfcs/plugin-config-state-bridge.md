...
or change arbitrary Synapse configuration.
...
small host fact requested by the RFC. Settings writes use Synapse'
...
`SYNAPSE_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<SYNAPSE_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Synapse home on every operation.
...
The implementation is covered with real temporary-Synapse-home tests for:
...
- [Issue #64227](https://github.com/johsua092-ui/synapse-ai-agent/issues/64227)
- [RFC PR #58542](https://github.com/johsua092-ui/synapse-ai-agent/pull/58542) by Topher Ross
