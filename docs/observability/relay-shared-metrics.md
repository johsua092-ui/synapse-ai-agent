...
Synapse includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Synapse and does not require a Synapse observability plugin. Synapse remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Synapse execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `synapse-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Synapse `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `SYNAPSE_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `SYNAPSE_NEMO_RELAY_ATOF_*` and `SYNAPSE_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Synapse does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Synapse requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Synapse installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Synapse process so configured interceptors can
...
Collection remains off unless Synapse policy enables it:
...
opt-in. Set `SYNAPSE_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Synapse does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Synapse
...
retain the same `session_id` and add `synapse.session.segment` plus
`synapse.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Synapse
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Synapse process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Synapse logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Synapse process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Synapse
process and share the same policy boundary.

Run profiles in separate Synapse processes when they require different trust
...
Synapse core owns one Relay host and one isolated Relay session scope per Synapse
session. Core lifecycle producers use
`synapse_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Synapse plugin registration. Shared-metrics marks must
...
```text
Synapse turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Synapse shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Synapse sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Synapse used for the logical call, such as
...
New calls use `synapse.model_route.count`. The previous
`synapse.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `synapse.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Synapse
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Synapse processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `synapse.task_run`, parented to
the owning Synapse session. The start counter contains only bounded execution
...
provider attempts for the same Synapse API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Synapse tool-call
...
cancellations. Active task ownership follows the task ID if Synapse rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`synapse.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Synapse
...
tool or plugin names. Synapse does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `synapse.tool_approval` marks and
...
Successful skill mutations emit `synapse.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `synapse.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Synapse derives reuse and patch-generation
...
```text
$SYNAPSE_HOME/telemetry/shared_metrics/metrics.sqlite3
$SYNAPSE_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Synapse version,
...
schema field name, its current scope is one `SYNAPSE_HOME`, so it is more
...
Deleting `$SYNAPSE_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `SYNAPSE_HOME`. To reset it, stop Synapse
processes and remove `$SYNAPSE_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Synapse CLI turn against the deterministic local model server:
