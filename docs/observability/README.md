# Synapse Observer Hooks

Synapse observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Synapse' planner, model providers, memory, tool registry,
...
Synapse also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "synapse.observer.v1"
```

Hook callbacks are fail-open. Synapse catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Synapse only builds
...
The native NeMo Relay SDK integration maps Synapse session, turn, LLM, and tool
