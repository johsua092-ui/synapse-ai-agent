...
This document is the formal interface between the **Synapse gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`Josh Research/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `synapse gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`synapse gateway enroll` → connector `/relay/enroll`, or
...
(`Josh Research/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `synapse` CLI.
