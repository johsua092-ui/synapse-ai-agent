...
``synapse mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Synapse desktop app. Use the "
            "terminal instead: `synapse mcp install <name>` for catalog entries, "
            "`synapse mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Synapse desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `synapse mcp catalog` "
