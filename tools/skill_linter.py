...
Design contract (matches the Synapse "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    synapse_meta = meta.get("synapse") if isinstance(meta, dict) else None
    if not isinstance(synapse_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.synapse.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in synapse_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.synapse.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("synapse", "agent", "synapse agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Synapse Agent' (proper caps) "
