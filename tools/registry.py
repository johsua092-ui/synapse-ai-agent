"""Central registry for all synapse-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (synapse_constants itself is stdlib-only, so no cycle).
        from synapse_constants import get_synapse_home

        return Path(get_synapse_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``synapse tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Synapse-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("SYNAPSE_SESSION_ID", ""),
            get_session_env("SYNAPSE_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("SYNAPSE_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved SYNAPSE_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("synapse_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``synapse_plugins.{name}``), not the exact module string.
                # A handler defined in ``synapse_plugins.pkg.handlers`` is
                # still owned by the ``synapse_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``synapse tools enable foo``)
