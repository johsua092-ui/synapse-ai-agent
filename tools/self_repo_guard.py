...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Synapse's live source checkout "
        ...
        "checkout, stop Synapse, run the command externally, then restart "
        "Synapse."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(synapse_home).expanduser() if synapse_home else Path.home() / ".synapse"
