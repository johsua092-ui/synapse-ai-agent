...
Synapse Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own SYNAPSE root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in synapse_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "synapse" if profile == "default" else profile
    if (
...
def _synapse_cli() -> str:
    """Resolve the synapse CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``synapse``
    entrypoint. A bare ``"synapse"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("synapse.exe" if sys.platform == "win32" else "synapse")
    if sibling.is_file():
        return str(sibling)
...
# turns concurrently: deliveries spawn separate ``synapse`` subprocesses, so
