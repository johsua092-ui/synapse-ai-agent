...
One ``CDPSupervisor`` runs per Synapse ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Synapse logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "synapse-dialog-bridge.invalid"
...
(() => {
  if (window.__synapseDialogBridgeInstalled) return;
  window.__synapseDialogBridgeInstalled = true;
  const ENDPOINT = "http://synapse-dialog-bridge.invalid/";
