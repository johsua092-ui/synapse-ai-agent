...
it is automatically downloaded from GitHub releases to $SYNAPSE_HOME/bin/tirith.
...
def _get_synapse_home() -> str:
    """Return the Synapse home directory, respecting SYNAPSE_HOME env var."""
    return str(get_synapse_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_synapse_home(), ".tirith-install-failed")
...
def _synapse_bin_dir() -> str:
    """Return $SYNAPSE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_synapse_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SYNAPSE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_synapse_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SYNAPSE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SYNAPSE_HOME/bin/tirith
    ...
    synapse_bin = os.path.join(_synapse_bin_dir(), "tirith")
    if os.path.isfile(synapse_bin) and os.access(synapse_bin, os.X_OK):
        _resolved_path = synapse_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return synapse_bin
