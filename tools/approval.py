...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("SYNAPSE_YOLO_MODE", ""))
...
# Synapse session id (observability identity, distinct from the gateway
...
# os.environ["SYNAPSE_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export SYNAPSE_INTERACTIVE.
_synapse_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "synapse_interactive",
    default=None,
)


def set_synapse_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["SYNAPSE_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``SYNAPSE_INTERACTIVE`` env var for legacy callers.
    """
    return _synapse_interactive_ctx.set("1" if interactive else "")


def reset_synapse_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_synapse_interactive_context`."""
    _synapse_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``SYNAPSE_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _synapse_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("SYNAPSE_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Synapse session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SYNAPSE_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("SYNAPSE_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("SYNAPSE_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("SYNAPSE_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("SYNAPSE_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``synapse chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``SYNAPSE_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("SYNAPSE_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("SYNAPSE_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SYNAPSE_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SYNAPSE_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SYNAPSE_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("SYNAPSE_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``SYNAPSE_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $SYNAPSE_HOME, or by the resolved absolute
# active profile home path such as /home/synapse/.synapse/config.yaml. The
# resolved-absolute form is folded into the ~/.synapse/ patterns at detection
...
# go stale when SYNAPSE_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SYNAPSE_ENV_PATH = (
    r'(?:~\/\.synapse/|'
    r'(?:\$home|\$\{home\})/\.synapse/|'
    r'(?:\$synapse_home|\$\{synapse_home\})/)'
    r'\.env\b'
)
# ~/.synapse/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _SYNAPSE_ENV_PATH; matches the SYNAPSE_HOME override form as
# well as ~/.synapse/.
_SYNAPSE_CONFIG_PATH = (
    r'(?:~\/\.synapse/|'
    r'(?:\$home|\$\{home\})/\.synapse/|'
    r'(?:\$synapse_home|\$\{synapse_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_SYNAPSE_ENV_PATH}|'
    rf'{_SYNAPSE_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_synapse_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Synapse: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Synapse
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]synapse[^\n]*\.env\b', "access to Synapse secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `synapse` and `gateway` (e.g. `synapse -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bsynapse\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart synapse gateway (kills running agents)"),
    (r'\bsynapse\s+update\b', "synapse update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `synapse gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `synapse ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart synapse-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart synapse-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(synapse|gateway|cli\.py)\b', "kill synapse/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill synapse` but not
    # `kill -9 $(pgrep -f synapse)` because the substitution is opaque
    ...
    # the `synapse gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.synapse.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "synapse"/"ai.synapse" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.synapse.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "synapse" appear
    ...
    # an unrelated "synapse" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:synapse|ai\.synapse)\b)', "stop/restart synapse launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Synapse file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.synapse/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Synapse-managed security file (~/.synapse/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_SYNAPSE_CONFIG_PATH}|{_SYNAPSE_ENV_PATH})', "in-place edit of Synapse config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_SYNAPSE_CONFIG_PATH}|{_SYNAPSE_ENV_PATH})', "in-place edit of Synapse config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_SYNAPSE_CONFIG_PATH}|{_SYNAPSE_ENV_PATH})', "in-place edit of Synapse config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.synapse/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / SYNAPSE_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Synapse home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\synapse), so folding the user home
    # first would eat the prefix the Synapse-home fold needs.
    command = _rewrite_resolved_synapse_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / SYNAPSE_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.synapse``); the matched
...
def _rewrite_resolved_synapse_home(command: str) -> str:
    """Rewrite the resolved absolute Synapse home prefix to ``~/.synapse/``.

    Resolves the active ``SYNAPSE_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.synapse/`` so the static ``_SYNAPSE_CONFIG_PATH`` / ``_SYNAPSE_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/synapse/.synapse/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$SYNAPSE_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from synapse_constants import get_synapse_home
        home = get_synapse_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.synapse")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``synapse send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Synapse ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"synapse-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart synapse gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.synapse.gateway`` auto-approved
    ...
    anchored on a synapse-gateway identifier — reusing its prose
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Synapse YOLO.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["SYNAPSE_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "SYNAPSE_SPINNER_PAUSE" in os.environ:
            del os.environ["SYNAPSE_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Synapse approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``SYNAPSE_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            SYNAPSE_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export SYNAPSE_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set SYNAPSE_INTERACTIVE or "
                "SYNAPSE_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set SYNAPSE_INTERACTIVE or "
            "SYNAPSE_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("SYNAPSE_EXEC_ASK"):
        ...
        # can see (SYNAPSE_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``SYNAPSE_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from synapse_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from synapse_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("SYNAPSE_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # SYNAPSE_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from synapse_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (SYNAPSE_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # SYNAPSE_EXEC_ASK (and sometimes a session platform marker) can leak
