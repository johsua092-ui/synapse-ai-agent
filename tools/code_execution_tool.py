...
Lets the LLM write a Python script that calls Synapse tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `synapse_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `synapse_tools.py` with file-based RPC stubs
...
# available on every platform Synapse itself runs on.
...
# match a safe prefix, the operational SYNAPSE_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Synapse
# code can lose the DB-layer Kanban mutation guard while still inheriting
# SYNAPSE_HOME.
#
# NB: the broad "SYNAPSE_" prefix was deliberately removed (#27303) — it leaked
# SYNAPSE_*-named config that lacks a secret substring (e.g. SYNAPSE_BASE_URL,
# SYNAPSE_KANBAN_DB, SYNAPSE_*_WEBHOOK).  The child only needs the few
# location/profile vars in _SYNAPSE_CHILD_ALLOWED below; SYNAPSE_RPC_SOCKET /
# SYNAPSE_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational SYNAPSE_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set synapse_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_SYNAPSE_CHILD_ALLOWED = frozenset({
    "SYNAPSE_HOME",
    "SYNAPSE_PROFILE",
    "SYNAPSE_CONFIG",
    "SYNAPSE_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational SYNAPSE_* vars (_SYNAPSE_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret SYNAPSE_* vars dropped by the tightened allowlist (#27303). The
    # broad "SYNAPSE_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like SYNAPSE_KANBAN_DB / SYNAPSE_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _SYNAPSE_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        ...
        if k.startswith("SYNAPSE_"):
            ...
            # allowlist — a deliberately-dropped SYNAPSE_* var.
            _dropped_synapse.append(k)
    if _dropped_synapse:
        logger.debug(
            "execute_code: dropped %d non-allowlisted SYNAPSE_* var(s) from the "
            ...
            len(_dropped_synapse),
            ", ".join(sorted(_dropped_synapse)),
        )
...
# ---------------------------------------------------------------------------
# synapse_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Synapse-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Synapse-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    synapse_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'synapse_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_synapse_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the synapse_tools.py stub module.
...
"""Auto-generated Synapse tools RPC stubs."""
...
def _connect():
    ...
    SYNAPSE_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SYNAPSE_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("SYNAPSE_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Synapse tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SYNAPSE_RPC_DIR") or os.path.join(tempfile.gettempdir(), "synapse_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated synapse_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/synapse_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_synapse_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/synapse_tools.py", tools_src)
        ...
        env_prefix = (
            f"SYNAPSE_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"SYNAPSE_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SYNAPSE_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.synapse.gateway`, but the
    ...
    # --- Set up temp directory with synapse_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="synapse_sandbox_")
    ...
    # selector from SYNAPSE_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"synapse_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated synapse_tools module.
        ...
        tools_src = generate_synapse_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "synapse_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  SYNAPSE_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["SYNAPSE_RPC_SOCKET"] = rpc_endpoint
        child_env["SYNAPSE_RPC_TOKEN"] = rpc_token
        ...
        # SYNAPSE_TIMEZONE is an internal Synapse setting and must not leak
        # into child processes.
        _tz_name = os.getenv("SYNAPSE_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("SYNAPSE_TIMEZONE", None)
        ...
        # ``synapse_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # Synapse's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Synapse's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip Synapse-owned entries that
        # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
        # so it passes the scrub).  They are redundant for same-Synapse-
        ...
        _strip_synapse_owned_pythonpath(child_env)
        _synapse_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_synapse_python_environment(_child_python):
            _pp_parts.append(_synapse_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import synapse_constants suddenly
            # fails" reports are diagnosable without log spam.
            _external_env_logged.add(_child_python)
            logger.info(
                "execute_code: child interpreter %s is outside the Synapse "
                "environment; synapse root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.synapse/.env')).
...
def _load_config() -> dict:
    ...
    ``synapse --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (synapse-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the Synapse environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the synapse root from every
...
def _uses_synapse_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Synapse's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``synapse tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and synapse-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.synapse/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Synapse tools programmatically. "
        ...
        f"Available via `from synapse_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from synapse_tools import {import_str}` "
