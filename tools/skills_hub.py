#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Synapse Skills Hub.
...
Used by synapse_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _synapse_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            synapse_meta = metadata.get("synapse", {})
            if isinstance(synapse_meta, dict):
                tags = synapse_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            synapse_meta = metadata.get("synapse", {})
            if isinstance(synapse_meta, dict):
                raw_tags = synapse_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Josh Research) but not activated
    ...
    OFFICIAL_REPO = "johsua092-ui/synapse-ai-agent"
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``synapse update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                synapse_meta = meta_block.get("synapse", {})
                if isinstance(synapse_meta, dict):
                    tags = synapse_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``synapse_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Synapse centralized index source
# ---------------------------------------------------------------------------

SYNAPSE_INDEX_URL = "https:///docs/api/skills-index.json"
SYNAPSE_INDEX_TTL = 6 * 3600  # 6 hours


def _synapse_index_cache_file() -> Path:
    return _index_cache_dir() / "synapse-index.json"


def _load_synapse_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SYNAPSE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    synapse_index_cache_file = _synapse_index_cache_file()
    if synapse_index_cache_file.exists():
        try:
            age = time.time() - synapse_index_cache_file.stat().st_mtime
            if age < SYNAPSE_INDEX_TTL:
                return json.loads(synapse_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Synapse index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Synapse index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Synapse index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        synapse_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        synapse_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class SynapseIndexSource(SkillSource):
    """Skill source backed by the centralized Synapse Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_synapse_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "synapse-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "synapse-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SynapseIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "synapse-index"
