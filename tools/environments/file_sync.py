...
_FORCE_SYNC_ENV = "SYNAPSE_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.synapse") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.synapse to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``SYNAPSE_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, synapse_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.synapse/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(synapse_home=synapse_home)

    def _sync_back_transaction(self, synapse_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="synapse-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.synapse/skills/a.md`` →
        ``~/.synapse/skills/a.md``, a new remote file at
        ``/root/.synapse/skills/b.md`` maps to ``~/.synapse/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
