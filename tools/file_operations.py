...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__SYNAPSE_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__SYNAPSE_FENCE_" in line or "\x1b]" in line
...
NOT_REGULAR_SENTINEL = "__synapse_not_regular__"
...
class ShellFileOperations(FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        paths — and Synapse disables MSYS argument conversion for its bash
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.synapse-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".synapse-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.synapse-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.synapse-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('SYNAPSE_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
        ...
        marker = stdout.find("SYNAPSE_UTF16:OK")
        if result.exit_code != 0 or marker < 0:
            return None
        payload = stdout[marker + len("SYNAPSE_UTF16:OK"):].strip()
        try:
    ...
    def write_file(self, path: str, content: str,
        ...
        # clean the temp up on any failure so we never leak a ``.synapse-tmp``
    ...
    def _check_lint(self, path: str, content: Optional[str] = None) -> LintResult:
        ...
        # globals (`Window.synapseDesktop` → TS2339) that are defined by the
    ...
    def _search_with_rg(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # nothing converts back — Synapse sets MSYS_NO_PATHCONV for its bash).
