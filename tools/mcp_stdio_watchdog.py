...
<url>``) is spawned as a direct child of the Synapse process. Synapse's own
...
Synapse process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Synapse startup's opt-in
