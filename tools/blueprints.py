...
`synapse skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.synapse.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.synapse.blueprint`` is a mapping containing
    ...
    synapse = meta.get("synapse") if isinstance(meta, dict) else None
    blueprint = synapse.get("blueprint") if isinstance(synapse, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.synapse.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.synapse.blueprint`` block) they can hand
    to ``synapse skills publish`` to share. ``body`` is the plain-language
