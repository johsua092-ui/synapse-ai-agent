...
def auth_json_path():
    """Return the Synapse auth store path, respecting SYNAPSE_HOME overrides."""
    return get_synapse_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`synapse tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Synapse release, and
