...
_PRIVATE_BROWSER_SENTINEL = "_SYNAPSE_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# synapse: pin this named session to its own tab (once per daemon process)
def _synapse_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "synapse-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_synapse_ensure_own_tab()
del _synapse_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Synapse's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Synapse's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_synapse_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `synapse tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Synapse' own bin dir ($SYNAPSE_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from synapse_constants import get_synapse_home

        return str(Path(get_synapse_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Synapse' own ``$SYNAPSE_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Synapse-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Synapse' managed uv (bootstrapped on demand via
    ``synapse_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$SYNAPSE_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Synapse-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through synapse tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_synapse_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `synapse tools` → Browser Automation."
        )
