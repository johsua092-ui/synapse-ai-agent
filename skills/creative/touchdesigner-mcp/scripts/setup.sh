...
SYNAPSE_CFG="${SYNAPSE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Synapse config has twozero_td MCP entry ──
if [[ ! -f "$SYNAPSE_CFG" ]]; then
    echo -e " ${FAIL} Synapse config not found at ${SYNAPSE_CFG}"
    manual_steps+=("Create ${SYNAPSE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SYNAPSE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Synapse config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Synapse config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$SYNAPSE_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SYNAPSE_CFG} manually"); }
    manual_steps+=("Restart Synapse session to pick up config change")
fi
