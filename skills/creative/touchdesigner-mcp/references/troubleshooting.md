...
`$SYNAPSE_HOME/config.yaml` (defaults to `~/.synapse/config.yaml` when `SYNAPSE_HOME` is unset)
...
Restart the Synapse session for changes to take effect. The MCP connection is
