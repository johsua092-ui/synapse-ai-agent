#!/usr/bin/env bash
# GitHub environment detection helper for Synapse Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _synapse_env="${SYNAPSE_HOME:-$HOME/.synapse}/.env"; [ -f "$_synapse_env" ] && grep -q "^GITHUB_TOKEN=" "$_synapse_env" 2>/dev/null; then
