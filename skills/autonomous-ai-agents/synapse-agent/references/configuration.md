...
Edit with `synapse config edit` or `synapse config set section.key value`.
Full reference: https:///docs/user-guide/configuration
...
`synapse config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `synapse tools` (interactive) or `synapse tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SYNAPSE_CORE_TOOLS` is the default bundle most platforms inherit).
