...
## Layer 1 — Is this thing a Synapse plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Synapse process**. Its LLM calls go through Synapse's already-configured provider. | Already uses Portal if user's Synapse is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to ``. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Synapse. That LLM call is already authenticated
via Synapse's provider config. The plugin's own server auth (e.g.
...
Portal at `https:///v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from ` → API Keys`, or
...
What Synapse Agent has that *feels* like OAuth — `synapse login --provider nous`
opening a browser, user signs in, token lands in `~/.synapse/auth.json` — is a
**Synapse-specific browser flow**. Under the hood it produces a credential
Synapse uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Synapse's existing Portal credential out of `~/.synapse/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `/v1` with that
   bearer attached.
...
rides on the credential Synapse already holds.

Where this could live in Synapse:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Synapse itself.
...
1. First decide: Synapse plugin (runs inside Synapse) or separate app? If plugin,
   it already uses Portal via Synapse's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https:///v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Synapse agent itself") is fully supported. Synapse treats agent
...
pitch in the Tool Gateway announcement is specifically about Synapse Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Synapse is configured to use Portal as its provider. It is
