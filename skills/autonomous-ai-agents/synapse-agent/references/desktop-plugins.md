...
Write plugins for the Synapse desktop app: statusbar items, layout panes,
...
system (`~/.synapse/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SYNAPSE_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SYNAPSE_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Synapse desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SYNAPSE_HOME/desktop-plugins/` (usually
  `~/.synapse/desktop-plugins/`).

## How to Run

1. Create `$SYNAPSE_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.synapse/...` by default, or `~/.synapse/profiles/<profile>/...` under a
...
The ONLY import surface is `@synapse/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Synapse
  ...
  (`synapse://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.synapse/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@synapse/plugin-sdk`, `react`, and
