...
Config: `~/.synapse/config.yaml` (settings), `~/.synapse/.env` (API keys) — both under `$SYNAPSE_HOME` when it is set.
...
`_SYNAPSE_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_synapse_home()` for paths,
never hardcode `~/.synapse`. For custom/local-only tools, write a plugin in
`~/.synapse/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `synapse_cli/commands.py`
...
- Tests auto-redirect `SYNAPSE_HOME` to temp dirs — never touch real `~/.synapse/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/synapse_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_synapse_home()` from `synapse_constants` for all paths (profile-safe)
