...
Set via `synapse model` (picker) or `synapse setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https:///docs/integrations/providers
...
| nous | OAuth device code | `synapse auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `synapse auth add openai-codex` |
| qwen-oauth | OAuth | `synapse auth add qwen-oauth` |
| minimax-oauth | OAuth | `synapse auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`synapse auth`).
Fallback chain when the primary fails: `synapse fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`synapse_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   synapse config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
