# Themes / Skins — Author a Synapse Color Theme

Author a Synapse **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`synapse_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.synapse/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Synapse itself.
...
- Write access to the Synapse home dir — `~/.synapse` by default, or `$SYNAPSE_HOME`
  / the active profile's dir. Skins live in `<synapse-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `synapse config set`).
...
3. `write_file` it to `<synapse-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `synapse config set display.skin default` (or they can `/skin default`).
...
```
synapse skin set <key> <hex>      # e.g. synapse skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `synapse skin set background "#08201f"` changes
only the background; `synapse skin use <name>` / `synapse skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.synapse`** when a profile is active — resolve the real home
  from `$SYNAPSE_HOME` first, falling back to `~/.synapse`.
...
- **Never hand-edit `config.yaml` to activate.** Use `synapse config set
...
- **You apply it, not the user.** `synapse config set display.skin <name>` is
...
- `read_file` the written `<synapse-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `synapse config get display.skin` and confirm it reports `<name>`.
