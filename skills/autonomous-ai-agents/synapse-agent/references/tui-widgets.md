...
Author widget apps for the Synapse TUI (`synapse --tui`): glanceable ambient
...
- The TUI must be in use (`synapse --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.synapse/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.synapse/logs/tui_gateway_crash.log` if a widget never appears.
