#!/usr/bin/env python3
"""Google Workspace API CLI for Synapse Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Synapse-facing JSON contract and falls back to the Python client
...
from _synapse_home import get_synapse_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SYNAPSE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Synapse Agent")
