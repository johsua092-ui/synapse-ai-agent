...
Ledger path resolution (first wins):
  --ledger PATH
  $SYNAPSE_CITATION_LEDGER
  $SYNAPSE_HOME/cache/citations/ledger.json
"""
...
from _synapse_home import get_synapse_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("SYNAPSE_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_synapse_home() / "cache" / "citations" / "ledger.json"
