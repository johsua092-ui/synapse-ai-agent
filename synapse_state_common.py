...
Extracted verbatim from synapse_state.py so the SessionDB mixin modules
(synapse_state_search / synapse_state_schema / synapse_state_portability) can
reference them without importing synapse_state (which would be a cycle).
synapse_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``synapse sessions optimize-storage``. A legacy DB sits at
...
# `synapse sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `synapse db optimize`. This is
...
#
# Several independent Synapse processes routinely share one state.db (gateway
# service, the Desktop app's `synapse serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `synapse_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not synapse_state)
# because the search/schema mixins cannot import synapse_state (cycle).
#
