...
:::tip 快速开始
Tool Gateway 包含在付费 Nous Portal 订阅中。**[管理订阅 →](https:///manage-subscription)**
:::

**Tool Gateway** 让已付费的 Nous Portal 用户通过同一份订阅，直接使用网页搜索、文生图、语音合成（TTS）与浏览器自动化，而**不必**再分别注册 Firecrawl、FAL、OpenAI、Browser Use 等服务的 API Key。
...
Tool Gateway 仅对 **[付费](https:///manage-subscription)** Nous Portal 订阅开放；免费档不可用——请 [升级订阅](https:///manage-subscription) 后解锁。
...
运行 `synapse model` 并选择 Nous Portal 作为提供商时，Synapse 会主动询问是否启用 Tool Gateway：
...
网关使用 Nous Portal 凭据认证（在 `synapse model` 完成后写入 `~/.synapse/auth.json`）。
...
- **选择为具体厂商**（如 `fal`、`firecrawl`）→ 直连；若对应 Key 缺失则报错并提示运行 `synapse tools`，**不会**静默回退到网关
- **从未配置过的类别** → 按可用凭据自动检测（行为不变）；但一旦存在选择，仅往 `.env` 加 Key 不会改变路由

（旧版的 `use_gateway` 布尔键已废弃：不再写入，读取时 `use_gateway: true` 等同于 `nous`。请改用 `synapse tools` 选择提供商。）
...
在 `synapse tools` 中选择非网关提供商时，选择键会被改写为该厂商名（旧的 `use_gateway` 键若存在会被一并移除），避免配置自相矛盾。
...
若使用自建或自定义网关，可在 `~/.synapse/.env` 中用环境变量覆盖端点：

```bash
TOOL_GATEWAY_DOMAIN=     # 网关路由基础域名
...
不需要。类别选择为 **Nous Subscription**（`nous`）时，运行时会忽略该类别的直连 Key；Key 仍保留在 `.env`。之后在 `synapse tools` 里改回直连提供商，Key 即恢复生效。
...
经网关路由的工具会停止工作，直到你 [续订](https:///manage-subscription) 或通过 `synapse tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 Nous 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `synapse setup terminal` 或在 `config.yaml` 中配置。
