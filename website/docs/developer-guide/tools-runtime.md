...
Synapse tools are self-registering functions grouped into toolsets and executed through a central registry/dispatch system.
...
2. **Plugin tools** — `synapse_cli.plugins.discover_plugins()` loads user/project/pip plugins that may register additional tools.
...
Toolsets are named bundles of tools. Synapse resolves them through:

- explicit enabled/disabled toolset lists
- platform presets (`synapse-cli`, `synapse-telegram`, etc.)
- dynamic MCP toolsets
- curated special-purpose sets like `synapse-acp`
