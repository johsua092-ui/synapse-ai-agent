...
The ACP adapter wraps Synapse' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
synapse acp / synapse-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.synapse/.env
  -> configure stderr logging
  -> construct SynapseACPAgent
...
- `allow_once` -> Synapse `once`
- `allow_always` -> Synapse `always`
- reject options -> Synapse `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Synapse tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["synapse-acp"])
...
Instead it reuses Synapse' runtime resolver:

- `acp_adapter/auth.py`
- `synapse_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Synapse provider/credentials. It also always advertises a terminal setup auth method (`synapse-setup`, args `--setup`) so first-run ACP clients can open Synapse' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.synapse/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `synapse-acp` toolset definition
- `synapse_cli/main.py` — `synapse acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `synapse-acp` script
