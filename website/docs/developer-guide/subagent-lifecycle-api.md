...
Plugins can launch and supervise fresh Synapse child sessions without importing
...
active agent turn fails closed with `No active Synapse parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Synapse can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Synapse's existing unsafe-tool block remains enforced.
