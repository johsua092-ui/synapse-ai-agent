...
| Variable | Meaning |
|----------|---------|
| `SYNAPSE_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `SYNAPSE_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `SYNAPSE_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Synapse itself — they're private to these helpers. The variables Synapse *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `synapse --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_synapse_root)" || { echo "htui: not in a Synapse checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$SYNAPSE_MAIN_CHECKOUT/.venv/bin/python" -m synapse_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `SYNAPSE_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_synapse_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `SYNAPSE_TUI_DIR` are mutually exclusive
`SYNAPSE_TUI_DIR` points Synapse at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `synapse --tui --dev` exits with an error. Run `unset SYNAPSE_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_synapse_root)" || { echo "hgui: not in a Synapse checkout" >&2; return 1; }
  deps="${SYNAPSE_GUI_DEPS_CHECKOUT:-$SYNAPSE_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _synapse_link_deps "$desktop" "$deps/apps/desktop"
    _synapse_link_deps "$root" "$deps"
  else
  ...
  trap '_synapse_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    SYNAPSE_DESKTOP_SYNAPSE_ROOT="$root" \
    SYNAPSE_DESKTOP_PYTHON="$SYNAPSE_MAIN_CHECKOUT/.venv/bin/python" \
    SYNAPSE_DESKTOP_IGNORE_EXISTING=1 \
    SYNAPSE_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `SYNAPSE_DESKTOP_SYNAPSE_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `synapse`. |
| `SYNAPSE_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `SYNAPSE_DESKTOP_IGNORE_EXISTING` | Ignores any `synapse` on `PATH` so it can't shadow the worktree. |
| `SYNAPSE_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Synapse checkout.
_synapse_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/synapse_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_synapse_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'synapse_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
