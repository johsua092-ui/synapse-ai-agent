...
title: "Desktop Plugin SDK (@synapse/plugin-sdk)"
description: "Extend the native Synapse Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Synapse Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `SynapsePlugin`.
It imports one module — `@synapse/plugin-sdk` — and gets everything: the app's
...
`$SYNAPSE_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Synapse. This page is the **native
desktop app** (`synapse desktop`) SDK — the `@synapse/plugin-sdk` module and
`$SYNAPSE_HOME/desktop-plugins/`. The **web dashboard** (`synapse dashboard`) has
its own, unrelated plugin system on `window.__SYNAPSE_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Synapse Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SYNAPSE_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$SYNAPSE_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `SynapsePlugin` contract, appear in **Settings → Plugins**,
...
Create `$SYNAPSE_HOME/desktop-plugins/hello/plugin.js` (that's `~/.synapse/...`
by default, or `~/.synapse/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.synapse/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@synapse/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Synapse' }),
      jsx('div', {
...
The only importable specifiers are `@synapse/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `SynapsePlugin`:
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `synapse.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@synapse/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@synapse/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@synapse/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@synapse/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@synapse/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@synapse/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@synapse/plugin-sdk'
...
`window.synapse.send('get-price eth')` (or a declarative
`<button data-synapse-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@synapse/plugin-sdk'
...
for requests; `targetProfile` is the backend Synapse profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Synapse
...
ctx.os.notify({
  ...
  // Body click → focus Synapse + navigate. Same vocabulary as OS deep links:
  activate: 'synapse://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'synapse://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `synapse://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `synapse://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@synapse/plugin-sdk'
...
```javascript
import { queryClient } from '@synapse/plugin-sdk'
...
a package into `~/.synapse/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.synapse/plugins` is not reachable as a filesystem —
...
the `synapse://` scheme — a plain anchor on your website or README:
...
`synapse-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Synapse plugin and declare it in a
`manifest.json`:
...
synapse-agent codebase directly (`synapse_state`, `synapse_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.synapse/`)
...
(`synapse.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `SynapsePlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@synapse/plugin-sdk` alias.
2. It's still lint-fenced to `@synapse/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@synapse/plugin-sdk`, `react`,
...
| Plugin contract | `SynapsePlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `SynapseOpenTarget`, `Contribution` |
...
### Agents: the `synapse-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`synapse-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SYNAPSE_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `synapse logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@synapse/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.synapse/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.synapse/logs/errors.log` for
