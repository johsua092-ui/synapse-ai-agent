...
sidebar_position: 1
title: "Architecture"
description: "Synapse Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Synapse Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ synapse_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SynapseCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── synapse_state.py           # SQLite session/state database with FTS5
├── synapse_constants.py       # SYNAPSE_HOME, profile-aware paths
...
│
├── synapse_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `synapse` subcommands (large file)
...
│   ├── skills_config.py      # synapse skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # synapse tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # synapse gateway start/stop
│
...
```text
User input → SynapseCLI.process_input()
...
Three discovery sources: `~/.synapse/plugins/` (user), `.synapse/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `synapse plugins` or `config.yaml`.
...
Exposes Synapse as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`synapse -p <name>`) gets its own SYNAPSE_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
