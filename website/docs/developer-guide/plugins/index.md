...
title: "Build a Synapse Plugin"
description: "Step-by-step guide to building a complete Synapse plugin with tools, hooks, data files, and skills"
---

# Build a Synapse Plugin

This guide walks through building a complete Synapse plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Synapse has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `SYNAPSE_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.synapse/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `synapse skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Synapse can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Synapse already owns. It does not replace native `plugin.yaml` plus
...
```bash
synapse plugins install owner/repository --no-enable
synapse plugins list
synapse plugins enable <plugin-name>
```
...
Synapse validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Synapse.
...
Portable `streamable-http` entries are routed through Synapse' existing native
...
same full-trust posture as other installed Synapse plugins.
...
records it as Published. Synapse keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Synapse does not expose a
...
continue to work after a normal Synapse upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Synapse inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Synapse releases can therefore load a plugin whose manifest contains
...
Synapse enforces this contract with frozen external-plugin fixtures discovered
from an isolated `SYNAPSE_HOME`. Those tests load and invoke the plugin through
...
```bash
mkdir -p ~/.synapse/plugins/calculator
cd ~/.synapse/plugins/calculator
```

### Validate with Plugin Doctor

`synapse plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Synapse itself. It reports invalid hook names, callbacks that do
...
```bash
synapse plugins doctor . --ci
```

Doctor uses a temporary `SYNAPSE_HOME`, restores plugin registration state after
...
This tells Synapse: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `synapse_cli/plugin_capabilities.py` for the
...
`synapse_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`synapse_agent.plugins` entry point:

```toml
[project.entry-points."synapse_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."synapse_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Synapse reads these from installed metadata without importing your code, so
`synapse plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Synapse understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Synapse validates them, and `synapse plugins install` / `synapse plugins doctor` surface missing ones with a `pip install` hint, but Synapse **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Synapse' shared venv is a conflict and supply-chain
...
4. **Accept `**kwargs`:** Synapse may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `synapse my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Synapse continues fine
...
Use plugin-relative config keys for user-visible behavior. Synapse resolves them
...
`<SYNAPSE_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `SYNAPSE_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
SYNAPSE_PLUGINS_DEBUG=1 synapse plugins list
```
...
The same logs are always written to `~/.synapse/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
synapse logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `synapse plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.synapse/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `synapse plugins update` / `remove` git-pull or delete it — your
...
# <synapse home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.synapse/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.synapse/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `synapse plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Synapse into installing arbitrary packages — only specs Synapse itself ships are eligible |
...
auditable in `~/.synapse/logs/agent.log`. Plugins load after built-in
...
does not ship with Synapse core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `synapse plugins list`
...
loader, the plugin is disabled and Synapse continues. The gate exists
...
You normally never edit this key by hand. `synapse plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Synapse injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: synapse-agent"}
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $SYNAPSE_HOME/hook_outputs
```
...
- **The system prompt is Synapse's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `synapse_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Synapse sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`synapse.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Synapse still loads on an older one.

### Register CLI commands

Plugins can add their own `synapse <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for synapse my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: synapse my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for synapse my-plugin."""
...
After registration, users can run `synapse my-plugin status`, `synapse my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `synapse name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `synapse chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `SYNAPSE_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `synapse <subcommand>` (e.g. `synapse kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "synapse kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Synapse from a hook.
...
Synapse has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.synapse/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `synapse gateway status` without SDK instantiation.
...
Synapse also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Synapse without any Python plugin. Declare them in `~/.synapse/config.yaml`:
...
Synapse connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.synapse/hooks/<name>/`:

```yaml
# ~/.synapse/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.synapse/hooks/long-task-alert/handler.py
...
synapse skills search my-workflow --source myorg/skills-repo
synapse skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install synapse-plugin-calculator
# Plugin auto-discovered on next synapse startup
```
...
```python
# Wrong — will break if Synapse passes extra context
def handler(args):
    ...
