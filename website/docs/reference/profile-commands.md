...
This page covers all commands related to [Synapse profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `synapse profile` without a subcommand shows help.
...
```bash
synapse profile use <name>
```

Sets `<name>` as the active profile. All subsequent `synapse` commands (without `-p`) will use this profile.
...
synapse profile use default
```

## `synapse profile create`

```bash
synapse profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `synapse profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `synapse update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.synapse`), use `synapse skills opt-out` / `synapse skills opt-in`. |
...
synapse profile create work2 --clone-from work

# Clone everything from a specific profile
synapse profile create work2-backup --clone-from work --clone-all
```

## `synapse profile describe`

```bash
synapse profile describe [<name>] [options]
```
...
synapse profile describe researcher

# Set it explicitly
synapse profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
synapse profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
synapse profile describe --all --auto
```

## `synapse profile delete`

```bash
synapse profile delete <name> [options]
```
...
```bash
synapse profile delete mybot
synapse profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.synapse`) cannot be deleted — use `synapse uninstall` to remove everything.
:::

## `synapse profile show`

```bash
synapse profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Synapse home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ synapse profile show work
Profile: work
Path:    ~/.synapse/profiles/work
...
## `synapse profile alias`

```bash
synapse profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Synapse installation.
...
```bash
synapse profile alias work
# Creates/updates ~/.local/bin/work

synapse profile alias work --name mywork
# Creates ~/.local/bin/mywork

synapse profile alias work --remove
# Removes the wrapper script
```

## `synapse profile rename`

```bash
synapse profile rename <old-name> <new-name>
```
...
```bash
synapse profile rename mybot assistant
# ~/.synapse/profiles/mybot → ~/.synapse/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `synapse profile export`

```bash
synapse profile export <name> [options]
```
...
```bash
synapse profile export work
# Creates work.tar.gz in the current directory

synapse profile export work -o ./work-2026-03-29.tar.gz
```
...
## `synapse profile import`

```bash
synapse profile import <archive> [options]
```
...
```bash
synapse profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

synapse profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `synapse -p telemetry`). |
...
synapse profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
synapse profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
synapse profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
synapse profile install ./telemetry/
```

### `synapse profile update`

```bash
synapse profile update <name> [--force-config] [--yes]
```
...
### `synapse profile info`

```bash
synapse profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Synapse version, author, env var requirements, the source URL/path, and
...
`synapse profile list` also shows the distribution name and version in a
`Distribution` column, and `synapse profile show <name>` / `delete <name>`
...
synapse profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
synapse profile install https://github.com/your-org/internal-assistant.git
```
...
`synapse_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Synapse version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Synapse can clone from.
3. Tell recipients to run `synapse profile install <your-repo-url>`.
...
## `synapse -p` / `synapse --profile`

```bash
synapse -p <name> <command> [options]
synapse --profile <name> <command> [options]
```

Global flag to run any Synapse command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
synapse -p work chat -q "Check the server status"
synapse --profile dev gateway start
synapse -p personal skills list
synapse -p work config edit
```

## `synapse completion`

```bash
synapse completion <shell>
```
...
- `synapse profile <TAB>` — subcommands (list, use, create, etc.)
- `synapse profile use <TAB>` — profile names
- `synapse -p <TAB>` — profile names
