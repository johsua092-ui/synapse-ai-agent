---
sidebar_position: 8
title: "Use Voice Mode with Synapse"
description: "A practical guide to setting up and using Synapse voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Synapse
...
- you want Synapse sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Synapse.
...
## Step 1: make sure normal Synapse works first
...
Synapse supports both local and cloud speech stacks.
...
### If you use `synapse setup`

If you choose NeuTTS in the setup wizard, Synapse checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Synapse transcribes and responds
...
- asking Synapse to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Synapse loop.

## Tuning CLI behavior

### Silence threshold

If Synapse starts/stops too aggressively, tune:
...
Synapse stays a normal chat bot, but can speak replies.
...
- you want Synapse to function like a portable research or ops assistant
...
Synapse joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Synapse detects speech boundaries
- transcripts are posted in the associated text channel
- Synapse responds in text and audio
...
1. get text Synapse working
2. run `synapse setup tts` to enable voice support
