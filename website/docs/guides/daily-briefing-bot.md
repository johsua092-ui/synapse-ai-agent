...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `synapse setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Synapse spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.synapse/cron/output/` and you can read them anytime.
:::
...
Synapse will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Synapse Agent
```
...
Before creating cron jobs, ensure Synapse has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Synapse what you want:
...
Synapse will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Synapse to delegate each topic to a sub-agent:
...
Synapse will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
