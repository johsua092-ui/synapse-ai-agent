...
Open [/manage-subscription](https:///manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: nous` in `~/.synapse/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Synapse is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
synapse setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
synapse auth add nous --type oauth
# Then re-run `synapse setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Synapse Agent release notes" and summarize the top 3 hits.
```

You should see Synapse call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`synapse setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
synapse config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Synapse-4 for agent work

Synapse-4-70B and Synapse-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them for conversation/research work through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Synapse Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https:///info) carries this warning too — it's the official Nous guidance, not just a Synapse-side opinion.
...
These rows appear in `synapse tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Synapse runs the Portal login inline (without changing your inference provider or your other tools).

Verify your mix with:

```bash
synapse portal tools
```
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Synapse will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Synapse profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.synapse/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `synapse portal info` shows "not logged in" after `synapse setup --portal`
...
```bash
synapse config set model.provider nous
```
...
Re-verify with `synapse portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Synapse doesn't replay it endlessly. Just log in again:

```bash
synapse auth add nous
```
...
`synapse portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Synapse profiles where you're using the wrong one (check `synapse profile list`)

### Want to revoke and start clean

```bash
synapse auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Synapse tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Synapse configurations
