...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Synapse Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Synapse instantly when PRs are opened or updated.
:::
...
- **Synapse Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  synapse gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.synapse/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Synapse can access GitHub. Start a chat:
...
```
Run: gh pr list --repo johsua092-ui/synapse-ai-agent --state open --limit 3
```
...
Still in the chat, ask Synapse to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo johsua092-ui/synapse-ai-agent
```
...
A skill gives Synapse consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.synapse/skills/code-review
```

Create `~/.synapse/skills/code-review/SKILL.md`:
...
Verify it loaded — start `synapse` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Synapse your team's standards:
...
2. Teach Synapse your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
synapse gateway status    # Is the gateway running?
synapse cron list         # Is the job enabled?
```
