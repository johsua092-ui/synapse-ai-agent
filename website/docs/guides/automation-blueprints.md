...
Copy-paste blueprints for common automation patterns. Each blueprint uses Synapse's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`synapse webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `synapse webhook subscribe`) |
...
```bash
synapse cron create "0 2 * * *" \
  "You are a project manager triaging the johsua092-ui/synapse-ai-agent GitHub repo.

1. Run: gh issue list --repo johsua092-ui/synapse-ai-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
synapse webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
synapse cron create "0 9 * * 1" \
  "Scan the johsua092-ui/synapse-ai-agent repo for documentation drift.

1. Run: gh pr list --repo johsua092-ui/synapse-ai-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (synapse_cli/commands.py, synapse_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (synapse_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
synapse cron create "0 6 * * *" \
  "Run a dependency security audit on the synapse-agent project.

1. cd ~/.synapse/synapse-agent && source .venv/bin/activate
...
```bash
synapse webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
synapse webhook subscribe alert-triage \
...
```python title="~/.synapse/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Synapse-Monitor/1.0"})
...
synapse cron create "every 30m" \
  ...
  --script ~/.synapse/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
synapse cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Synapse Agent development." \
...
```bash
synapse webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
synapse webhook subscribe stripe-payments \
...
```bash
synapse cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the synapse-agent codebase.
