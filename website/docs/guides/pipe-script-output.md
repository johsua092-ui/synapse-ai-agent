...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `synapse send`."
---

# Pipe Script Output to Messaging Platforms

`synapse send` is a small, scriptable CLI that pushes a message to any
messaging platform Synapse is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | synapse send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `synapse
...
synapse send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
synapse send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
synapse send --list

# Filter by platform
synapse send --list telegram
```
...
Any platform Synapse ships adapters for works as a target:
...
`synapse send` resolves the message body in this order:

1. **Positional argument** — `synapse send --to telegram "hi"`
2. **`--file PATH`** — `synapse send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | synapse send --to telegram`

When stdin is a TTY (no pipe), Synapse does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  synapse send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `synapse send` reuses your Synapse config, the same script works on
any host where Synapse is installed — no need to export bot tokens into
...
`synapse send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  synapse send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | synapse send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.synapse/bin/synapse send \
...
```bash
./train.py --epochs 200 && \
  synapse send --to telegram "training done" || \
  synapse send --to telegram "training failed (exit $?)"
```
...
synapse send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(synapse send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `synapse send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `synapse send` calls
the platform's REST endpoint directly using credentials from
`~/.synapse/.env` and `~/.synapse/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `synapse gateway start` and retry.
...
# Machine-readable
synapse send --list --json
```

The listing is built from `~/.synapse/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`synapse gateway
...
| Approach | Multi-platform | Reuses Synapse creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `synapse send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`synapse send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `synapse send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `synapse send` shares with cron delivery.
