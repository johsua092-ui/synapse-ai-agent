...
```bash
synapse teams-pipeline validate
```
...
```bash
synapse teams-pipeline token-health
synapse teams-pipeline token-health --force-refresh
```
...
```bash
synapse teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
synapse teams-pipeline maintain-subscriptions
synapse teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Synapse cron (recommended if you already run the Synapse gateway)

Synapse ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.synapse/scripts/`. First create the script:

```bash
mkdir -p ~/.synapse/scripts
cat > ~/.synapse/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec synapse teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.synapse/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
synapse cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/synapse-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Synapse Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/synapse teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/synapse-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Synapse Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now synapse-teams-pipeline-maintain.timer
systemctl list-timers synapse-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/synapse teams-pipeline maintain-subscriptions >> /var/log/synapse/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.synapse/.env` at the top of a wrapper script that crontab calls.
...
```bash
synapse teams-pipeline subscriptions   # should show expirationDateTime advanced
synapse teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
synapse teams-pipeline list
synapse teams-pipeline list --status failed
synapse teams-pipeline show <job-id>
```

### Replay a stored job

```bash
synapse teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
synapse teams-pipeline fetch --meeting-id <meeting-id>
synapse teams-pipeline fetch --join-web-url "<join-url>"
synapse teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `synapse teams-pipeline maintain-subscriptions --dry-run`
- inspect `synapse teams-pipeline list --status failed`
...
- run `synapse teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `synapse teams-pipeline run`
...
- [ ] `synapse teams-pipeline validate` returns an OK snapshot
- [ ] `synapse teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Synapse cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
