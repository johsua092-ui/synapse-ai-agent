---
title: "Honcho — Configure and troubleshoot Honcho memory for Synapse"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Synapse"
---
...
Configure and troubleshoot Honcho memory for Synapse.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `synapse skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Synapse

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Synapse profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/synapse#running-honcho-locally-with-synapse

### Verify

```bash
synapse honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Synapse profile's AI peer
...
Honcho models conversations as interactions between **peers**. Synapse creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Synapse instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Synapse run |
| `global` | Single session across all directories |

Manual override: `synapse honcho map my-project-name`
...
Each Synapse profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block synapse.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `synapse.coder` host block in `honcho.json`
...
```bash
synapse honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Synapse when Honcho memory is active.
...
Config file: `$SYNAPSE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `synapse honcho setup`. Ensure `memory.provider: honcho` is in `~/.synapse/config.yaml`.

### Memory not persisting across sessions
Check `synapse honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `synapse profile create <name> --clone`. For existing profiles: `synapse honcho sync`.
...
| Command | Description |
|---------|-------------|
| `synapse honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `synapse honcho status` | Show resolved config, connection test, peer info for active profile |
| `synapse honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `synapse honcho disable` | Disable Honcho for the active profile |
| `synapse honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `synapse honcho peers` | Show peer identities across all profiles |
| `synapse honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `synapse honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `synapse honcho sessions` | List known directory-to-session-name mappings |
| `synapse honcho map <name>` | Map current working directory to a Honcho session name |
| `synapse honcho identity` | Seed AI peer identity or show both peer representations |
| `synapse honcho sync` | Create host blocks for all Synapse profiles that don't have one yet |
| `synapse honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Synapse + Honcho |
| `synapse memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `synapse memory status` | Show active memory provider and config |
| `synapse memory off` | Disable external memory provider |
