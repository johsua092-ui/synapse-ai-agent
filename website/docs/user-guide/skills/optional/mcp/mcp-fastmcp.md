...
| | |
|---|---|
| Source | Optional — install with `synapse skills install official/mcp/fastmcp` |
...
| Related skills | [`synapse-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-synapse-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Synapse or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Synapse. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.synapse/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.synapse/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Synapse integration, either:

- configure the server in `~/.synapse/config.yaml` using the `native-mcp` skill, or
...
### Synapse cannot see the deployed server

The server-building part may be correct while the Synapse config is not. Load the `native-mcp` skill and configure the server in `~/.synapse/config.yaml`, then restart Synapse.
