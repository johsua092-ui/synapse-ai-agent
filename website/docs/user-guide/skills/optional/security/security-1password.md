...
| | |
|---|---|
| Source | Optional — install with `synapse skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Synapse Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Synapse terminal calls (desktop app flow only)
...
### Service Account (recommended for Synapse)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SYNAPSE_HOME:-~/.synapse}/.env` (the skill will prompt for this on first load).
...
## Synapse Execution Pattern (desktop app flow)

Synapse terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/synapse-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/synapse-op.sock"
