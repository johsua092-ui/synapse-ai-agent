...
| | |
|---|---|
| Source | Optional — install with `synapse skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Synapse MCP catalog. The
...
Two halves, in this order: the editor side must be up before Synapse connects.
...
### One-time, Synapse side

    synapse mcp install unreal-engine
...
`~/.synapse/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Synapse — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Synapse connects
...
2. Start the Synapse session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Synapse
they appear as:

| Synapse tool | Purpose |
|---|---|
...
then happens at `synapse mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Synapse runs batched calls
...
- **Start order matters.** Editor + server up first, then the Synapse
...
- **Timeouts on long operations.** Synapse' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.synapse/config.yaml` for
