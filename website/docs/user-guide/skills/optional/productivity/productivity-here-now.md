...
| | |
|---|---|
| Source | Optional — install with `synapse skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SYNAPSE_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SYNAPSE_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${SYNAPSE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client synapse
```
...
```bash
PUBLISH="${SYNAPSE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client synapse
```
...
```bash
DRIVE="${SYNAPSE_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `synapse`)    |
