...
| | |
|---|---|
| Source | Optional — install with `synapse skills install official/productivity/shopify` |
...
4. Save to `${SYNAPSE_HOME:-~/.synapse}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Synapse","productType":"Apparel","tags":["test"]}}'
```
