...
| | |
|---|---|
| Source | Optional — install with `synapse skills install official/productivity/siyuan` |
...
3. Store it in `${SYNAPSE_HOME:-~/.synapse}/.env`:
   ```
...
```yaml
# In ~/.synapse/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
