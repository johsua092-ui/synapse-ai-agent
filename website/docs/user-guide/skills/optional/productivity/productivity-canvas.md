...
| | |
|---|---|
| Source | Optional — install with `synapse skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Synapse Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${SYNAPSE_HOME:-~/.synapse}/.env`:
...
```bash
CANVAS="python $SYNAPSE_HOME/skills/productivity/canvas/scripts/canvas_api.py"
