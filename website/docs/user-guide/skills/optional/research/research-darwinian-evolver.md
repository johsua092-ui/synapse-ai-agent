...
| | |
|---|---|
| Source | Optional — install with `synapse skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Synapse Agent |
| License | MIT |
...
import upstream classes into Synapse itself.
...
```bash
mkdir -p ~/.synapse/cache/darwinian-evolver && cd ~/.synapse/cache/darwinian-evolver
...
```bash
cd ~/.synapse/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.synapse/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.synapse/skills/research/darwinian-evolver
DE_DIR=~/.synapse/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Synapse core.
   Custom driver scripts under `~/.synapse/skills/...` are user-side and fine.
