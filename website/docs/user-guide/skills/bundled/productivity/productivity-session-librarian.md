...
| List/filter by metadata (age, source, cost, tokens, workspace) | `synapse sessions list` / `stats` via terminal |
| Rename | `synapse sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `synapse sessions archive <filters>` |
| Delete (destructive) | `synapse sessions delete` / `synapse sessions prune <filters>` |
| Export before deleting anything valuable | `synapse sessions export --session-id <id> --format md` |
...
`synapse sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `synapse sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `synapse sessions list`
