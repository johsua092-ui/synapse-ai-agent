...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`synapse-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-synapse-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Synapse terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Synapse itself, `model.provider: openai-codex` uses Synapse-managed Codex
OAuth from `~/.synapse/auth.json` after `synapse auth add openai-codex`. For the
...
## Synapse Gateway Caveat

When invoking the Codex CLI from a Synapse gateway/service context (for example,
