...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _synapse_env="${SYNAPSE_HOME:-$HOME/.synapse}/.env"; [ -f "$_synapse_env" ] && grep -q "^GITHUB_TOKEN=" "$_synapse_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_synapse_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${SYNAPSE_HOME:-$HOME/.synapse}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Synapse Agent\",
    \"comments\": [
...
```bash
source "${SYNAPSE_HOME:-$HOME/.synapse}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Synapse Agent. Code looks clean — good test coverage, no security concerns."
