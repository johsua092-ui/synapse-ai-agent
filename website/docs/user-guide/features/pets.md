...
Synapse can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SYNAPSE_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Synapse renders the sprite at full fidelity when your
...
synapse pets list
synapse pets list cat

# Install a pet and make it active in one step
synapse pets install boba --select
...
## `synapse pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `synapse pets list [query] [--limit N]` |
| List installed pets | `synapse pets list --installed` |
| Install a pet | `synapse pets install <slug> [--select] [--force]` |
| Set the active pet | `synapse pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `synapse pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `synapse pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `synapse pets off` |
| Remove an installed pet | `synapse pets remove <slug>` |
| Diagnose setup | `synapse pets doctor` |

`synapse pets show` flags:
...
Beyond installing pre-made pets from the gallery, Synapse can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `synapse pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `synapse tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SYNAPSE_PET_IMAGE_PROVIDER` env var (e.g. `SYNAPSE_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Synapse is
...
Run `synapse pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Synapse uses its own
  profile-scoped `<SYNAPSE_HOME>/pets/` instead — install through `synapse pets`.

## See also

- The [`synapse-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-synapse-agent.md)
