...
When Synapse Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.synapse/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.synapse/config.yaml` (switchable per session
via `/personality`).
...
If you use Synapse' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.synapse/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Synapse Agent's deliverable
