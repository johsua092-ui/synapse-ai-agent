...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Synapse's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Synapse instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
synapse memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `synapse` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Synapse syncs them back at session init.
...
The `synapse honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `synapse memory setup honcho` (or run `synapse memory setup` and pick it from the list); the `synapse honcho` subcommand then appears on the next invocation.

```bash
synapse memory setup honcho    # Configure Honcho directly (works before activation)
synapse honcho status          # Connection status, config, and key settings
synapse honcho setup           # Redirects to `synapse memory setup` (post-activation alias)
synapse honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
synapse honcho peer            # Show or update peer names + dialectic reasoning level
synapse honcho mode            # Show or set recall mode (hybrid/context/tools)
synapse honcho tokens          # Show or set token budget for context and dialectic
synapse honcho identity        # Seed or show the AI peer's Honcho identity
synapse honcho sync            # Sync Honcho config to all existing profiles
synapse honcho peers           # Show peer identities across all profiles
synapse honcho sessions        # List known Honcho session mappings
synapse honcho map             # Map current directory to a Honcho session name
synapse honcho enable          # Enable Honcho for the active profile
synapse honcho disable         # Disable Honcho for the active profile
synapse honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `synapse honcho`

If you previously used the standalone `synapse honcho setup`:
...
No re-login or re-setup needed. Run `synapse memory setup` and select "honcho" — the wizard detects your existing config.
