...
Synapse Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Synapse integration. It
...
**Fresh installs already have the driver.** The Synapse installer
...
- **`synapse tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
synapse computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `synapse computer-use
status` to verify the install.

Already have cua-driver? Synapse reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `synapse update`, and the first
`computer_use` call of a session, Synapse checks the local version and
...
selected with `SYNAPSE_CUA_DRIVER_CMD` stays
under your control, so Synapse reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Synapse autodetection is a
planned cua-driver follow-up, so currently point Synapse at that directory or
...
interface. The built-in toolset provides Synapse actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `synapse computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Synapse host identity. |
...
```
synapse -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.synapse/config.yaml`.

## Permission modes and logged-in browser profiles

Synapse maps its existing approval UX onto cua-driver's immutable runtime
...
| Synapse session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Synapse approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Synapse risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Synapse then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.synapse/cua-manifest.yaml
```
...
for the format). Synapse launches a private runtime with
...
resetting or closing the Synapse session, cancellation cleanup, or process exit
closes that transport session. Synapse also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Synapse
...
embedded service under the Synapse host identity.
...
## `synapse computer-use doctor` — your first triage stop

`synapse computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ synapse computer-use doctor
...
Synapse run declares a public cua-driver **session name** (something like
`synapse-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
Synapse keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Synapse-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Synapse
autodetection is a planned cua-driver follow-up, so currently point Synapse at
...
These are **platform deep dives, not duplicates of the Synapse skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Synapse
...
image capture also saves a bounded, shareable copy under Synapse' image cache
...
Synapse applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.synapse/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Synapse applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Synapse agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Synapse agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Synapse warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Synapse from inside the RDP / console
...
```
SYNAPSE_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
SYNAPSE_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Synapse disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Synapse sets
...
When it's on, `synapse computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Synapse at a binary you built from source instead
of the published release. Synapse resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`SYNAPSE_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Synapse testing (see notes).
...
### Option B — point Synapse straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`SYNAPSE_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
SYNAPSE_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
SYNAPSE_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Synapse is using your build

- `synapse computer-use status` prints the resolved binary path and
  version.
- `synapse computer-use doctor` confirms the binary is reachable and
...
- **Synapse spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Synapse YOLO, Synapse instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Synapse
  change needed either way.
- **Local builds skip the version check.** Synapse warns when the
...
**First action when anything's off: run `synapse computer-use doctor`.**
...
Run `synapse computer-use install` to fetch the cua-driver binary, or
run `synapse tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `synapse computer-use doctor` will
...
- **Synapse-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Synapse `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Synapse autodetection is a
  planned follow-up; currently point Synapse at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Synapse among them)
