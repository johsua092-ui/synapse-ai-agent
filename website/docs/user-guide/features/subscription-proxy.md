...
chat completions — use your Synapse-managed provider subscription as their
...
| Use case | "Use Synapse as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Synapse stores
the refresh token in `~/.synapse/auth.json` — the same place all Synapse
...
Model:      Synapse-4-70B    # or Synapse-4.3-36B, Synapse-4-405B
```
...
interface in `synapse_cli/proxy/adapters/`.
...
If you see `not logged in`, run `synapse portal`. If you see
...
`synapse_cli/proxy/adapters/<provider>.py` and registering it in
