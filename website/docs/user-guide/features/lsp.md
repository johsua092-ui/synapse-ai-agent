...
Synapse runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Synapse
...
1. Synapse captures a baseline of current diagnostics for the file.
...
…). Synapse auto-detects the binary on PATH or in
`<SYNAPSE_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Synapse at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<SYNAPSE_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`synapse lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Synapse installs both packages together when you
run `synapse lsp install typescript` or auto-install fires on first
use.

## CLI

```
synapse lsp status          # service state + per-server install status
synapse lsp list            # registry, optionally --installed-only
synapse lsp install <id>    # eagerly install one server
synapse lsp install-all     # try every server with a known recipe
synapse lsp restart         # tear down running clients
synapse lsp which <id>      # print resolved binary path
```

`synapse lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SYNAPSE_HOME>/lsp/bin
...
When `install_strategy: auto`, Synapse installs binaries into
`<SYNAPSE_HOME>/lsp/bin/`. NPM packages land in
`<SYNAPSE_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Synapse-owned and is
...
**`synapse lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SYNAPSE_HOME>/lsp/bin/`. Run
`synapse lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `synapse lsp status`**
...
When `synapse lsp status` shows a `Backend warnings` section, install
...
`~/.synapse/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.synapse/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `synapse lsp restart` to clear the set;
