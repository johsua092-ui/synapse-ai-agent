...
Top-level model calls run in the background automatically. Synapse returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
One exception: when the parent has a resolved workspace directory, every subagent's system prompt embeds that workspace's **project context files** (`.synapse.md` > AGENTS.md chain > CLAUDE.md > `.cursorrules` — the same discovery, priority, and size caps as the main agent's system prompt; SOUL.md is excluded). Subagents working in a repo operate under the repo's own conventions without having to rediscover them.
...
When a top-level agent provides a `tasks` array, Synapse returns one background handle, runs the subagents in parallel, and posts one consolidated result after every child finishes. An orchestrator subagent waits for its batch in the current turn so it can synthesize the results.
...
When a background delegation finishes, Synapse stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If Synapse restarts after completion but before delivery, the pending
...
Synapse cannot prove whether its external side effects happened. Pending and
...
```
<synapse_home>/cache/delegation/live/<delegation_id>/task-<n>.log
```
...
```bash
tail -f ~/.synapse/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
Top-level model-facing `delegate_task` calls run in the background automatically where the session supports later delivery. Synapse returns a handle immediately, and the result re-enters the conversation after the child or batch finishes. Orchestrator subagents wait for their workers in the current turn because they must synthesize those results before returning. Stateless request/response endpoints fall back to synchronous execution when they cannot deliver a detached result later.
...
- A Synapse process restart does **not** resume a running child. Its attempt becomes `unknown` because Synapse cannot prove which side effects happened.
...
- Each child starts its terminal in `<repo>/.worktrees/subagent-<id>` on its
  own branch `synapse-subagent/subagent-<id>`, and its goal message tells it to
