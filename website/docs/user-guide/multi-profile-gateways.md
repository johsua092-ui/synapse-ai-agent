...
If you only run one Synapse agent, you don't need this page — see
...
[Connecting Desktop to Many Synapse Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Synapse agents that should all
...
(`ai.synapse.gateway-<name>.plist`) or systemd user service
(`synapse-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
synapse profile create coder
synapse profile create personal-bot
synapse profile create research
...
```bash
synapse config set gateway.multiplex_profiles true
synapse gateway restart
```

Equivalently, in the default profile's `~/.synapse/config.yaml`:
...
You do **not** run `synapse gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `synapse gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.synapse/profiles/coder/.env`; the default listener key is rejected.
...
home). `synapse status` reports the multiplexer and the profiles it serves;
`synapse status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/synapse-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: synapse-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    synapse gateway "$action"
  else
    synapse -p "$profile" gateway "$action"
  fi
}
...
```bash
synapse-gateways start      # start every configured profile
synapse-gateways stop       # stop every configured profile
synapse-gateways restart    # restart all
synapse-gateways status     # status across all
synapse-gateways list       # delegates to `synapse gateway list`
```

:::tip
The `default` profile is targeted with `synapse gateway <action>` (no `-p`),
not `synapse -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `synapse -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.synapse.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/synapse-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.synapse.gateway.plist` /
`synapse-gateway.service`.
...
```bash
# Default profile
tail -f ~/.synapse/logs/gateway.log
tail -f ~/.synapse/logs/gateway.error.log

# Named profile
tail -f ~/.synapse/profiles/<name>/logs/gateway.log
tail -f ~/.synapse/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.synapse/logs/gateway.log ~/.synapse/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
synapse logs -f                  # follow default profile
synapse -p coder logs -f         # follow one profile
synapse logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
synapse profile list             # profiles + model + gateway state
synapse-gateways status          # full status across every profile
launchctl list | grep synapse    # macOS — PIDs and labels
systemctl --user list-units 'synapse-gateway-*'   # Linux — units
```
...
```
~/.synapse/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.synapse/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
synapse config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.synapse/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=synapse --why="gateways running" \
  sleep infinity &
...
`synapse-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.synapse/.env ~/.synapse/profiles/*/.env
```

## Updating the code

`synapse update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `synapse gateway start` after a previous `synapse gateway stop`. The
...
```bash
ps -ef | grep "synapse_cli.*-p <profile>"
cat ~/.synapse/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.synapse.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.synapse.gateway-<profile>.plist

# Linux
systemctl --user restart synapse-gateway-<profile>.service
```

### Health check

```bash
synapse doctor                  # default profile
synapse -p <profile> doctor     # one profile
```
