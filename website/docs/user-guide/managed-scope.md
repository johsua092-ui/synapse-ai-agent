...
`~/.synapse/config.yaml`, `~/.synapse/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/synapse`:

```text
/etc/synapse/
├── config.yaml     # managed config layer (wins over ~/.synapse/config.yaml)
└── .env            # managed env layer (wins over ~/.synapse/.env + shell)
```
...
The location can be relocated with the `SYNAPSE_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SYNAPSE_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Synapse.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SYNAPSE_MANAGED_DIR=/opt/org/synapse-policy
```

:::warning
A user who can set `SYNAPSE_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `synapse doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/synapse/config.yaml` (managed) | `/etc/synapse/.env` (managed) |
| 2 | `~/.synapse/config.yaml` (user) | `~/.synapse/.env` (user) |
...
```bash
synapse config        # shows a header naming the managed source + the pinned keys
synapse doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Synapse refuses and names the source:

```bash
$ synapse config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/synapse/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `synapse config set` / setup will not write
...
sudo tee /etc/synapse/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/synapse/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/synapse
sudo chmod 0644 /etc/synapse/config.yaml /etc/synapse/.env
```

Changes take effect on the next Synapse start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`synapse doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Synapse as `root`), managed scope is advisory.
