...
title: "Git Worktrees"
description: "Run multiple Synapse agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Synapse Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Synapse so each session has a clean, isolated working directory.

## Why Use Worktrees with Synapse?

Synapse treats the **current working directory** as the project root:

- CLI: the directory where you run `synapse` or `synapse chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.synapse/config.yaml`
...
Synapse creates `.worktrees/my-experiment/` inside the repo (branch
`synapse/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `synapse-<id>`
...
git worktree add ../repo-feature feature/synapse-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/synapse-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Synapse there:

```bash
cd ../repo-feature

# Start Synapse in the worktree
synapse
```
...
git worktree add ../repo-experiment-a feature/synapse-a
git worktree add ../repo-experiment-b feature/synapse-b
```
...
Each Synapse process:

- Works on its own branch (`feature/synapse-a` vs `feature/synapse-b`).
...
- Synapse checkpoint data under `~/.synapse/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Synapse experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/synapse-checkpoints-docs`, `feature/synapse-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Synapse from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `synapse -w` (Automatic Worktree Mode)

Synapse has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `synapse/synapse-<hash>`).
...
```bash
synapse -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `synapse -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Synapse session its own clean checkout.
