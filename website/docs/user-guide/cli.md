...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Synapse Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Synapse Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `synapse setup --portal` — and you're ready to `synapse chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Synapse also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `synapse --tui` — see the [TUI](tui.md) guide.
:::
...
synapse chat --query-file prompt.txt
synapse chat --query-file - < prompt.txt

# With a specific model
synapse chat --model "anthropic/claude-sonnet-4"

# With a specific provider
synapse chat --provider nous        # Use Nous Portal
synapse chat --provider openrouter  # Force OpenRouter

# With specific toolsets
synapse chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
synapse -s synapse-agent-dev,github-auth
synapse chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
synapse --continue             # Resume the most recent CLI session (-c)
synapse --resume <session_id>  # Resume a specific session by ID (-r)
synapse --resume latest        # Resume the most recent session (same as -c)
synapse --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
synapse chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
synapse -w                         # Interactive mode in worktree
synapse -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`synapse -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
synapse worktree list              # audit: age, size, verdict, reason per tree
synapse worktree prune             # remove safe trees + delete merged branches
synapse worktree prune --dry-run   # show the plan without changing anything
synapse worktree prune --trees-only     # leave local branches alone
synapse worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running synapse session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.synapse/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
The `synapse plugins` commands manage native Synapse plugins and portable Agent
...
synapse plugins disable <plugin-name>
synapse plugins update <plugin-name>
synapse plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Synapse currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Synapse CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Synapse CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `SYNAPSE_YOLO_MODE` is on (either `synapse --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Synapse to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.synapse/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Synapse supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Synapse replies |
...
```bash
synapse -s synapse-agent-dev,github-auth
synapse chat -s github-pr-workflow -s github-auth
```

Synapse loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.synapse/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.synapse/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Synapse..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Synapse also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Synapse recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Synapse — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Synapse is working, Synapse prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Synapse to the background — just like any terminal process. The shell prints a confirmation:

```
Synapse Agent has been suspended. Run `fg` to bring Synapse Agent back.
```
...
Resume this session with:
  synapse --resume 20260225_143052_a1b2c3
...
```bash
synapse --continue                          # Resume the most recent CLI session
synapse -c                                  # Short form
synapse -c "my project"                     # Resume a named session (latest in lineage)
synapse --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
synapse --resume "refactoring auth"         # Resume by title
synapse --resume latest                     # Resume the most recent session (same as -c)
synapse --resume latest --in ./my-project   # Latest session for ./my-project's workspace
synapse -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `synapse sessions rename <id> <title>` from the command line. Use `synapse sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Synapse's SQLite state database under `~/.synapse/state.db`. The database keeps:
...
Synapse immediately confirms the task and gives you back the prompt:
