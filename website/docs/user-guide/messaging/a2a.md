...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Synapse A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Synapse over HTTP.

It interoperates with any A2A-compliant peer — another Synapse, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Synapse ↔ Synapse across machines** — let your desktop agent hand tasks to a Synapse on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Synapse so other frameworks' agents can send it tasks.
...
```bash
synapse gateway setup      # pick A2A
```
...
```bash
synapse tools enable a2a --platform cli        # CLI/TUI sessions
synapse tools enable a2a --platform telegram   # or any messaging platform
synapse tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, Synapse serves:
...
- **Audit log** — every exchange appends to `~/.synapse/a2a_audit.jsonl`.
