...
Connect Synapse to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Synapse supports two WeCom integration modes:
...
> Run `synapse gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Synapse decrypts the message, queues it for the agent
...
(Use `synapse gateway start` only after `synapse gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Synapse and the token the
...
in `~/.synapse/.env` values around `=` will also break signature checks. After
fixing, restart `synapse gateway run`.
...
Check `synapse gateway run` logs for the bound host/port. If the adapter bound to
