...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Synapse Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Synapse setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Synapse Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Synapse Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Synapse for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Synapse working on my machine | `synapse setup` | Run a real chat and verify it responds |
| I already know my provider | `synapse model` | Save the config, then start chatting |
| I want a bot or always-on setup | `synapse gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `synapse model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `synapse model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Synapse cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Synapse Agent
### With the Synapse Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, download the Synapse Desktop installer from our website and run it.

### Without Synapse Desktop:
For a command-line only install without Synapse Desktop, run:
...
The single most important setup step. Use `synapse model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `synapse setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `synapse update`. Re-enable anything later with `synapse tools`, seed skills with `synapse skills opt-in --sync`, or tune settings with `synapse setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `synapse model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `synapse model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `synapse model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `synapse_cli/models.py` may change between releases) | `synapse model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `synapse model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `synapse model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `synapse model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `synapse model` (requires `copilot` CLI + `copilot login`) |
...
Synapse Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `synapse model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Synapse separates secrets from normal config:

- **Secrets and tokens** → `~/.synapse/.env`
- **Non-secret settings** → `~/.synapse/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
synapse config set model anthropic/claude-opus-4.6
synapse config set terminal.backend docker
synapse config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
synapse            # classic CLI
synapse --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Synapse ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `synapse` vs `synapse --tui`.
:::
...
```bash
synapse --continue    # Resume the most recent session
synapse -c            # Short form
```
...
```bash
synapse gateway setup    # Interactive platform configuration
```
...
- `synapse tools` — tune tool access per platform
- `synapse skills` — browse and install reusable workflows
...
```bash
synapse config set terminal.backend docker    # Docker isolation
synapse config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `synapse egress setup && synapse egress start`; `synapse setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Synapse install directory (the curl installer placed it at
# ~/.synapse/synapse-agent on Linux/macOS or %LOCALAPPDATA%\synapse\synapse-agent on Windows):
cd ~/.synapse/synapse-agent
...
Skills are on-demand instruction documents that teach Synapse how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Synapse ships with a catalog of bundled skills already installed in `~/.synapse/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
synapse skills browse                      # list everything available
synapse skills search kubernetes           # find skills by keyword
synapse skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `synapse skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Synapse ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.synapse/synapse-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Synapse opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `synapse model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `synapse gateway setup` and check `synapse gateway status` |
| `synapse --continue` can't find old session | Switched profiles or session never saved | Check `synapse sessions list` and confirm you're in the right profile |
...
| `synapse doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `synapse model`
3. `synapse setup`
4. `synapse sessions list`
5. `synapse --continue`
6. `synapse gateway status`
...
| `synapse tools` | Configure which tools are enabled per platform |
...
| `synapse gateway` | Start the messaging gateway |
| `synapse --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-synapse-to-another-machine)** — `synapse backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
