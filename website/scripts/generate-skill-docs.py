...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/johsua092-ui/synapse-ai-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    synapse_meta = (fm.get("metadata") or {}).get("synapse") or {}
    tags = synapse_meta.get("tags") or []
    related = synapse_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `synapse skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Synapse Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Synapse ships with a large built-in skill library copied into `~/.synapse/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Synapse also syncs bundled skills on `synapse update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.synapse/skills/` tree, it is still shipped with Synapse; restore it with `synapse skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with synapse-agent — install via synapse skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with synapse-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "synapse skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "synapse skills install official/blockchain/solana",
        "synapse skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "synapse skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
