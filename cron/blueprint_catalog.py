...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``synapse://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``synapse://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"synapse://blueprint/{quote(blueprint.key)}{qs}"
