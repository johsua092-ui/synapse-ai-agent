...
Write path is the CLI (``synapse cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_synapse_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`synapse cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `synapse cron notepad {job_id} delete "
