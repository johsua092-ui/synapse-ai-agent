"""Centralized logging setup for Synapse Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.synapse/logs/`` (profile-aware via ``get_synapse_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``synapse_cli.web_server``,
    ``synapse_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Synapse (TUI, gateway, ``hy_memory`` server, MCP
...
from synapse_constants import get_config_path, get_synapse_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_synapse_session_injector", False):
    ...
    _session_record_factory._synapse_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``synapse logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``synapse logs --component gateway``.
    "gateway": ("gateway", "synapse_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Synapse logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    synapse_home
        Override for the Synapse home directory.  Falls back to
        ``get_synapse_home()`` (profile-aware).
    log_level
    ...
    home = synapse_home or get_synapse_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_synapse_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._synapse_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the synapse group. However, both ``_open()`` (initial creation)
...
# the module header): when several Synapse processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._synapse_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="synapse-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_synapse_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse synapse_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when synapse_cli.config isn't importable (bare
        # synapse_logging consumers).
        try:
            from synapse_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
