"""Anthropic Messages API adapter for Synapse Agent.

Translates between Synapse's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in synapse_cli/runtime_provider.py.
    """
...
# Synapse effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_nous_portal_endpoint(base_url: str | None) -> bool:
    ...
    ``https:///v1/messages``.  Portal-specific
    ...
    Lookalikes such as ``.attacker.test`` are
    rejected (hostname match, not substring).
    """
    if base_url_host_matches(base_url or "", ""):
        return True
    try:
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to synapse's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to synapse's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + SynapseAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://")
        headers.setdefault("X-Title", "Synapse Agent")
        headers.setdefault("User-Agent", f"SynapseAgent/{_SYNAPSE_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Synapse loads that into the process env from
    # ``~/.synapse/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Synapse notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Synapse historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``synapse models``) never mutates ``~/.synapse/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Synapse)
      ...
      5. Anthropic credential_pool OAuth entry (~/.synapse/auth.json)
    ...
    # 1. Synapse-managed OAuth/setup token env var
    ...
    # 5. Synapse credential_pool OAuth entry.
...
# ── Synapse-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.synapse/.anthropic_oauth.json (our own file).
...
def _get_synapse_oauth_file() -> Path:
    return get_synapse_home() / ".anthropic_oauth.json"
...
def run_synapse_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Synapse-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Synapse with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from synapse_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_synapse_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Synapse-managed OAuth credentials from ~/.synapse/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Synapse OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See synapse-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See synapse-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See synapse-agent#13848 (Kimi) and
    synapse-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Synapse Agent", "Claude Code")
                text = text.replace("Synapse agent", "Claude Code")
                text = text.replace("synapse-agent", "claude-code")
                text = text.replace("Josh Research", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Synapse-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Synapse surfaces in its CLI. We
