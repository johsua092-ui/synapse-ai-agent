...
Storage
-------
Bundles live in ``~/.synapse/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SYNAPSE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``synapse bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SYNAPSE_HOME.

    Honors ``SYNAPSE_BUNDLES_DIR`` for tests; falls back to
    ``<SYNAPSE_HOME>/skill-bundles``.
    """
    override = os.environ.get("SYNAPSE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_synapse_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `synapse bundles` CLI subcommand.
# ---------------------------------------------------------------------------
