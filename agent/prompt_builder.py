...
_SYNAPSE_MD_NAMES = (".synapse.md", "SYNAPSE.md")


def _find_synapse_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.synapse.md`` or ``SYNAPSE.md``.
    ...
    # could pick up a .synapse.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _SYNAPSE_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Synapse Agent, an intelligent AI assistant created by Josh Research. "
...
SYNAPSE_AGENT_HELP_GUIDANCE = (
    "You run on Synapse Agent (by Josh Research). When the user needs help with "
    "Synapse itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https:///docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `synapse-agent` skill with skill_view(name='synapse-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.synapse/kanban.db`. Your task id is in "
    "`$SYNAPSE_KANBAN_TASK`; your workspace is `$SYNAPSE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SYNAPSE_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $SYNAPSE_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${SYNAPSE_KANBAN_BRANCH:-wt/$SYNAPSE_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$SYNAPSE_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`synapse profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `synapse kanban <verb>` for board operations. Use "
...
#
# The synapse-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to synapse-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Synapse YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Synapse launches that runtime.\n\n"
        ...
        "run `synapse computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Synapse "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Synapse floating over another application, so the
    user is rarely asking about Synapse — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Synapse "
        ...
        "rather than anything inside Synapse. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Synapse terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Synapse desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.synapse.send(\"prompt\") — or a "
        "data-synapse-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the Synapse WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Synapse itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Synapse runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Synapse itself is running. The host OS, home, and cwd "
                f"of the Synapse process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Synapse "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Synapse
    ...
    extra = (os.getenv("SYNAPSE_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_synapse_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("SYNAPSE_PLATFORM") or os.environ.get("SYNAPSE_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.synapse/skills/`` directory.  External dirs
    ...
    # SYNAPSE_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_synapse_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.synapse/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_synapse_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Synapse Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `synapse-agent` skill "
            "first. It has the actual commands (e.g. `synapse config set …`, `synapse tools`, "
            "`synapse setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: synapse setup, synapse setup tools, synapse setup terminal, synapse status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from SYNAPSE_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the SYNAPSE_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from synapse_cli.config import ensure_synapse_home
        ensure_synapse_home()
    except Exception as e:
        logger.debug("Could not ensure SYNAPSE_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_synapse_home()
...
def _load_synapse_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.synapse.md / SYNAPSE.md — walk to git root."""
    synapse_md_path = _find_synapse_md(cwd_path)
    if not synapse_md_path:
        return ""
    try:
        content = synapse_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = synapse_md_path.name
        try:
            rel = str(synapse_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".synapse.md", context_length=context_length,
            read_path=str(synapse_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", synapse_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .synapse.md / SYNAPSE.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SYNAPSE_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Synapse install/source
    ...
    # explicitly configured cwd is honored verbatim — the Synapse tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Synapse in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Synapse install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_synapse_md(cwd_path, context_length)
    ...
    # SOUL.md from SYNAPSE_HOME only — skip when already loaded as identity
    if not skip_soul:
