...
``auth.json``.  They intentionally have no dependency on ``synapse_cli.auth`` so
...
# Sources Synapse owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Synapse-owned OAuth/device-code state)
