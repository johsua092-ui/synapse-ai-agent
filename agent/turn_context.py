...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``synapse logs``.
    ...
    if (
        ...
        # Synapse only initiates compaction in "synapse" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Synapse preflight compression for codex app-server "
                "(mode=%s); Synapse will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Synapse (via _emit_status), so it always
