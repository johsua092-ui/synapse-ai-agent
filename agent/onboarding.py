...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Synapse starts and finds ``~/.openclaw/``.

    Points users at ``synapse claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``synapse claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Synapse, run "
        "`synapse claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `synapse claw cleanup` (renames it to ~/.openclaw.pre-migration — "
