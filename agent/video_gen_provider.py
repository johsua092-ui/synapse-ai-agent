...
as ``kind: backend``) or ``~/.synapse/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``synapse tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SYNAPSE_HOME/cache/videos/``, creating parents as needed."""
    from synapse_constants import get_synapse_home

    path = get_synapse_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SYNAPSE_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$SYNAPSE_HOME/cache/videos/``.
