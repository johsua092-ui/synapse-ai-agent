...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SYNAPSE_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Synapse runtime / deployment
    "SYNAPSE_HOME", "SYNAPSE_PROFILE", "SYNAPSE_GATEWAY_LOCK_DIR",
    "SYNAPSE_MAX_ITERATIONS", "SYNAPSE_MAX_TOKENS", "SYNAPSE_API_TIMEOUT",
    "SYNAPSE_REDACT_SECRETS", "SYNAPSE_NOUS_TIMEOUT_SECONDS",
    "_SYNAPSE_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SYNAPSE_KANBAN_DB", "SYNAPSE_KANBAN_WORKSPACES_ROOT", "SYNAPSE_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "SYNAPSE_KANBAN_",
    "SYNAPSE_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Synapse writes
    ...
    ``synapse_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Synapse parser: save_env_value
    ...
    from synapse_cli.config import _parse_env_value
...
def build_profile_secret_scope(synapse_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(synapse_home)
    secrets = load_env_file(home / ".env")

    try:
        from synapse_cli.env_loader import get_secret_source_values
