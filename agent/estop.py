...
``synapse pause`` writes a sentinel file at ``$SYNAPSE_HOME/ESTOP``;
``synapse resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Synapse is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.synapse/ESTOP``.
...
def _synapse_home() -> Path:
    """Resolve the active SYNAPSE_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active SYNAPSE_HOME."""
    return _synapse_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on SYNAPSE_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Synapse is paused ({reason}). New work is on hold; "
            "run `synapse resume` to pick things back up."
        )
    return (
        "⏸️ Synapse is paused. New work is on hold; "
        "run `synapse resume` to pick things back up."
    )
