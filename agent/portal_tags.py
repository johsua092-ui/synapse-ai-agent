...
Every Synapse request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Synapse Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=synapse-agent",
        "client=synapse-client-v<__version__>",
    ]

The version is sourced live from ``synapse_cli.__version__`` so it auto-aligns
...
``synapse_cli.__version__`` is the canonical source of truth.
"""
...
def _synapse_version() -> str:
    """Return the current Synapse release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``synapse_cli`` cannot be imported (should
...
def synapse_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=synapse-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=synapse-client-v{_synapse_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Synapse session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Synapse conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.synapse/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Synapse
    ...
    """
    tags = ["product=synapse-agent", synapse_client_tag()]
