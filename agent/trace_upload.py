"""Upload a Synapse session transcript to Hugging Face as an agent trace.

Synapse stores sessions in its own SQLite store (``synapse_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``synapse trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Synapse' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "synapse-traces"
_SYNAPSE_VERSION = "synapse-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Synapse OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Synapse' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Synapse conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.synapse/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `synapse trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/synapse-traces`` dataset. Returns a
