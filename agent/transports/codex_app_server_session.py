...
Owns one Codex thread per Synapse session. Drives `turn/start`, consumes
...
# Synapse' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SYNAPSE_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Synapse/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Synapse session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SYNAPSE_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SYNAPSE_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="synapse",
            client_title="Synapse Agent",
            client_version=_get_synapse_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Synapse' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Synapse' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own synapse-tools callback we
            # auto-accept — the user already approved Synapse' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Synapse
        approval-mode/timeout logic. The Synapse-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Synapse approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Synapse approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Synapse returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Synapse approval mode/timeout resolution
    ...
    # "prompt expired"; the Synapse-side messaging already distinguishes them.
    return "decline"
...
def _get_synapse_version() -> str:
    """Best-effort Synapse version string for codex's userAgent line."""
    try:
