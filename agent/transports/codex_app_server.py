...
projection into Synapse' display, approval bridging, transcript projection into
...
"codex_app_server"`. Synapse' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Synapse secret — gateway
        ...
        spawn_env = synapse_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SYNAPSE_KANBAN_TASK"):
            kanban_db = spawn_env.get("SYNAPSE_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "SYNAPSE_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SYNAPSE_HOME", os.path.expanduser("~/.synapse")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "synapse",
        client_title: str = "Synapse Agent",
