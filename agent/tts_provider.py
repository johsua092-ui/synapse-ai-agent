...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Synapse with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.synapse/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Synapse
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``synapse tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``synapse setup`` for
