...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_SYNAPSE_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Synapse expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile SYNAPSE_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped SYNAPSE_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``synapse_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_synapse_home()`` themselves.
        """
        if "synapse_home" not in kwargs:
            from synapse_constants import get_synapse_home
            kwargs["synapse_home"] = str(get_synapse_home())
        for provider in self._providers:
            try:
