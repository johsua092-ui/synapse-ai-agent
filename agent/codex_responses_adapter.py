...
def _clamp_responses_call_id(call_id: str) -> str:
    ...
    component the built-in ``synapse-tools`` server already overflows 64 chars,
...
# output items.  Synapse injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Synapse-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Synapse threading encrypted reasoning back across
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: Synapse'
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Synapse-side metadata key and not part
...
def _normalize_codex_response(
    ...
    *,
    ...
    if tool_calls:
        ...
        # See https://github.com/johsua092-ui/synapse-ai-agent/issues/64434
