...
Plugins built on Synapse Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Synapse
...
:class:`~synapse_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Synapse' main config
...
handles every provider, fallback chain, and per-task override Synapse
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`synapse_cli.plugins.PluginContext`
