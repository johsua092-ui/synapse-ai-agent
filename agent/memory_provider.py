...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `synapse backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - synapse_home (str): The active SYNAPSE_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.synapse``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "synapse").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'synapse memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], synapse_home: str) -> None:
        ...
        Called by 'synapse memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``synapse_home`` is the active SYNAPSE_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SYNAPSE_HOME.

        ``synapse backup`` only walks SYNAPSE_HOME, so any provider state kept
        ...
        of the archive, and ``synapse import`` restores them to their original
