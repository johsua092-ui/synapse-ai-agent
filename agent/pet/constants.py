...
row taxonomy is inferred from the atlas shape so Synapse can render both legacy
...
# rows (1536x1872), while older Synapse/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Synapse' activity state names. They are not always identical to the
...
# Legacy Synapse/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Synapse activity names -> accepted row-name aliases in descending
