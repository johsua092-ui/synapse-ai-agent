...
cache problem must never block Synapse startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Synapse home used for cache paths.

    ``home_path`` is whatever ``load_synapse_dotenv()`` already resolved;
    falling back to ``$SYNAPSE_HOME`` / ``~/.synapse`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from synapse_constants import get_synapse_home

        home_path = get_synapse_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<synapse_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SYNAPSE_HOME is ever made traversable.
            try:
