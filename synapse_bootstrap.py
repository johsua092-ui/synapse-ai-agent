"""Windows UTF-8 bootstrap for Synapse entry points.
...
should be imported at the very top of every Synapse entry point
(``synapse``, ``synapse-agent``, ``synapse-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``synapse_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``synapse_bootstrap``, never ``synapse_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Synapse modules.

    Synapse ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Synapse source root
    ...
    """
    root = src_root or os.environ.get("SYNAPSE_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SYNAPSE_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SYNAPSE_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import synapse_bootstrap``
# (or ``from synapse_bootstrap import apply_windows_utf8_bootstrap``) at
