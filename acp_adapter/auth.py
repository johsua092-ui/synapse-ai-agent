"""ACP auth helpers — detect and advertise Synapse authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "synapse-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Synapse runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Synapse can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Synapse.
    ...
    not have Synapse provider credentials configured yet, so Synapse always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Synapse using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Synapse provider",
            description=(
                "Open Synapse' interactive model/provider setup in a terminal. "
                "Use this when Synapse has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
