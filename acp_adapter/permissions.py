"""ACP permission bridging for Synapse dangerous-command approvals."""
...
# Maps ACP permission option ids to Synapse approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True,
    ...
    """Return ACP options that match Synapse approval semantics."""
    ...
    # Smart DENY override — the editor must not offer a scope Synapse
    ...
    if not once_only:
        options.append(PermissionOption(
            ...
            # hint while keeping Synapse semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_synapse(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Synapse approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_SYNAPSE.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Synapse-compatible approval callback that bridges to ACP.
    ...
    def _callback(
        ...
        **_: object,
        ...
        return _map_outcome_to_synapse(
            response.outcome,
