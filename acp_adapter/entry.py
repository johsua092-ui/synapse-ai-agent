"""CLI entry point for the synapse-agent ACP adapter.

Loads environment variables from ``~/.synapse/.env``, configures logging
...
try:
    ...
    # shadowing Synapse's own modules — ``synapse acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from SYNAPSE_HOME (default ``~/.synapse``)."""
    from synapse_cli.env_loader import load_synapse_dotenv

    synapse_home = get_synapse_home()
    loaded = load_synapse_dotenv(synapse_home=synapse_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", synapse_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="synapse-acp",
        description="Run Synapse Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Synapse version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Synapse provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.synapse/node/ "
...
def _print_version() -> None:
    ...
    print(synapse_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import SynapseACPAgent  # noqa: F401

    print("Synapse ACP check OK")


def _run_setup() -> None:
    from synapse_cli.main import main as synapse_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "synapse", "model"]
        synapse_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting synapse-agent ACP adapter")
    ...
    import acp
    from .server import SynapseACPAgent
    ...
    if os.environ.get("SYNAPSE_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
