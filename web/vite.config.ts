...
const BACKEND = process.env.SYNAPSE_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `synapse dashboard` server injects a one-shot
 * session token into `index.html` (see `synapse_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__SYNAPSE_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function synapseDevToken(): Plugin {
  const TOKEN_RE = /window\.__SYNAPSE_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__SYNAPSE_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "synapse:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[synapse] Could not find session token in ${BACKEND} — ` +
              `is \`synapse dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__SYNAPSE_SESSION_TOKEN__="${match[1]}";` +
              `window.__SYNAPSE_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[synapse] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`synapse dashboard\` or set SYNAPSE_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@synapse/shared": path.resolve(import.meta.dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `synapse dashboard` must serve these; Vite has no
