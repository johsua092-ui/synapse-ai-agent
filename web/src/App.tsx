          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="synapse-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="synapse-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSynapse = status?.can_update_synapse === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'synapse update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "synapse update";
    return (
      t.status.updateSynapseConfirmMessage ??
      `This will run 'synapse update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSynapseConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSynapse) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSynapse,
      runningLabel: t.status.updatingSynapse,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Synapse gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSynapseConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSynapseConfirmTitle ?? `${t.status.updateSynapse}?`}
    />
    </>
  );
}
