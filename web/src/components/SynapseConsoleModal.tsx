...
interface SynapseConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function SynapseConsoleModal({ open, onClose }: SynapseConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("synapse> ");
  const inputPromptRef = useRef("synapse> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "synapse> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Synapse Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="synapse-console-title"
    >
      <div
        className={cn(
          themedBody,
