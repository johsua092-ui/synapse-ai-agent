...
import { api, SYNAPSE_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "synapse:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${SYNAPSE_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${SYNAPSE_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?synapse_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-synapse-plugin", manifest.name);
