import { buildSynapseWebSocketUrl } from "@synapse/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/synapse/). The Python backend
// injects ``window.__SYNAPSE_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SYNAPSE_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SYNAPSE_BASE_PATH = readBasePath();
const BASE = SYNAPSE_BASE_PATH;
...
declare global {
  interface Window {
    __SYNAPSE_SESSION_TOKEN__?: string;
    __SYNAPSE_BASE_PATH__?: string;
    ...
    __SYNAPSE_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Synapse-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__SYNAPSE_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``synapse update``, ``synapse gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SYNAPSE_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SYNAPSE_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__SYNAPSE_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``synapse_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SYNAPSE_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Synapse-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``synapse_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SYNAPSE_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildSynapseWebSocketUrl({
...
export const api = {
  ...
  updateSynapse: () =>
    fetchJSON<ActionResponse>("/api/synapse/update", { method: "POST" }),
  checkSynapseUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/synapse/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** synapse-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``synapse update``. */
  can_update_synapse?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the SYNAPSE_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the SYNAPSE_HOME volume — no staleness dimension, so no
 * sampled_at. */
