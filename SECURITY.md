# Synapse Agent Security Policy

This document describes Synapse Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/johsua092-ui/synapse-ai-agent/security/advisories/new)
or **security@**. Do not open public issues for
security vulnerabilities. **Synapse Agent does not operate a bug
bounty program.**
...
- Environment details (`synapse --version`, commit SHA, OS, Python
  version).
...
Synapse Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Synapse Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Synapse Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Synapse Agent's documentation
...
Synapse Agent supports two OS-level isolation postures. They address
...
Synapse Agent supports this in two ways:

- **Synapse Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Synapse Agent's in-process heuristics
...
Synapse Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Synapse Agent
itself. Bugs in Synapse Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Synapse Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Synapse Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Synapse Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  synapse-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Synapse Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Synapse Agent. Bugs in Synapse
...
- Synapse Agent includes supply-chain guards for MCP server
