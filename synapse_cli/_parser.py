"""
Top-level argparse construction for the synapse CLI.
...
# argparse runs (it sets ``SYNAPSE_HOME`` and strips itself from ``sys.argv``),
...
# tests/synapse_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``synapse --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``synapse_cli.relaunch`` should carry over when
...
Examples:
    ...
    synapse chat -q "Hello"        Single query mode
    synapse --tui                  Launch the modern TUI (or set display.interface: tui)
    synapse --cli                  Force the classic REPL (overrides display.interface: tui)
    synapse -c                     Resume the most recent session
    synapse -c "my project"        Resume a session by name (latest in lineage)
    synapse --resume <session_id>  Resume a specific session by ID
    synapse --resume latest        Resume the most recent session (same as -c)
    synapse --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    synapse setup                  Run setup wizard
    synapse logout                 Clear stored authentication
    synapse auth add <provider>    Add a pooled credential
    synapse auth list              List pooled credentials
    synapse auth remove <p> <t>    Remove pooled credential by index, id, or label
    synapse auth reset <provider>  Clear exhaustion status for a provider
    synapse model                  Select default model
    synapse fallback [list]        Show fallback provider chain
    synapse fallback add           Add a fallback provider (same picker as `synapse model`)
    synapse fallback remove        Remove a fallback provider from the chain
    synapse config                 View configuration
    synapse config edit            Edit config in $EDITOR
    synapse config set model gpt-4 Set a config value
    synapse gateway                Run messaging gateway
    synapse -s synapse-agent-dev,github-auth
    synapse -w                     Start in isolated git worktree
    synapse gateway install        Install gateway background service
    synapse sessions list          List past sessions
    synapse sessions browse        Interactive session picker
    synapse sessions rename ID T   Rename/title a session
    synapse logs                   View agent.log (last 50 lines)
    synapse logs -f                Follow agent.log in real time
    synapse logs errors            View errors.log
    synapse logs --since 1h        Lines from the last hour
    synapse debug share             Upload debug report for support
    synapse console                Open the safe Synapse command console
    synapse update                 Update to latest version
    synapse dashboard              Start web UI dashboard (port 9119)
    synapse dashboard --stop       Stop running dashboard processes
    synapse dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="synapse",
        description="Synapse Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `synapse chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via SYNAPSE_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `synapse setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to SYNAPSE_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.synapse/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Synapse Agent",
    )
    ...
    # top-level parser: when the user writes `synapse -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/synapse_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also SYNAPSE_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.synapse/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Synapse itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
