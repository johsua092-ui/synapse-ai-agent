...
Used by ``synapse profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``synapse_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Synapse Agent kanban board.
...
Rules:
  ...
  - Never write "Synapse Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_synapse_home())
        else:
