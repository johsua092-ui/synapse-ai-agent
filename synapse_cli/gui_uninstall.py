"""
Synapse Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``synapse desktop`` / ``synapse gui``)
     Built inside the agent checkout under ``$SYNAPSE_HOME/synapse-agent/``:
       ...
       - ``$SYNAPSE_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Synapse"), separate from ``$SYNAPSE_HOME``:
  - macOS:   ``~/Library/Application Support/Synapse``
  - Windows: ``%APPDATA%\\Synapse``
  - Linux:   ``$XDG_CONFIG_HOME/Synapse`` (default ``~/.config/Synapse``)
...
``synapse uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(synapse_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return synapse_home / "synapse-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Synapse"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Synapse"
...
def source_built_gui_artifacts(synapse_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``synapse desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``synapse-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(synapse_home)
    ...
    return [
        ...
        synapse_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Synapse" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Synapse).
            local_base / "Programs" / "Synapse",
            # Older / alternate layout some builds used.
            local_base / "synapse-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Synapse")
    else:
        ...
        # ``uninstall_gui``.
        from synapse_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `synapse desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Synapse.desktop",
        ]
    return paths


def agent_is_installed(synapse_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SYNAPSE_HOME.
    ...
    if (agent_root / "synapse_cli").is_dir():
        return True
...
def gui_is_installed(synapse_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(synapse_home):
        if p.exists():
            return True
...
def gui_install_summary(synapse_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = synapse_home if synapse_home is not None else get_synapse_home()
...
def uninstall_gui(synapse_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``synapse-agent/synapse_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SYNAPSE_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Synapse disappears from the launcher.
        try:
            from synapse_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove synapse' or "
            "'sudo dnf remove synapse'). AppImage builds are a single file you "
