...
Pure logic: walks a Synapse config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `synapse doctor` and a future `synapse migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Synapse config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Synapse config and return retirement issues.
...
def apply_migration(
    ...
    from synapse_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `synapse migrate xai` reports nothing to migrate
