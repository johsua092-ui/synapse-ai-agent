"""Install and remove the Linux desktop entry (``synapse.desktop``).

``synapse desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "synapse.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``synapse.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``synapse desktop``.

    Prefer the real ``synapse`` executable (argv[0] or PATH). When Synapse
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from synapse_cli.relaunch import resolve_synapse_bin
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `synapse` script with
            ...
            # sys.executable is the interpreter actually running Synapse (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "synapse_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Synapse' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Synapse\n"
        "GenericName=Synapse Desktop\n"
        "Comment=Launch Synapse Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Synapse desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "synapse"
