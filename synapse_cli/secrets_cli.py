"""CLI handlers for ``synapse secrets bitwarden ...``.
...
# that process is ``synapse update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``synapse_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``synapse_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import synapse_cli.secrets_cli``
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Synapse process."
    )
    console.print(
        "  Status:  [cyan]synapse secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]synapse secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]synapse secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]synapse secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Synapse will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `synapse secrets bitwarden setup` to pick a "
    ...
    console.print(
        ...
        "Takes effect on the next Synapse invocation."
    )
    if not bw_cfg.get("enabled"):
        console.print(
            "[yellow]Note: the Bitwarden integration is currently disabled — "
            "run `synapse secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`synapse secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]synapse[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]synapse secrets bitwarden setup[/cyan] and pick EU or "
