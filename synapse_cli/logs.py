"""``synapse logs`` — view and filter Synapse log files.
...
Usage examples::
    ...
    synapse logs                    # last 50 lines of agent.log
    synapse logs -f                 # follow agent.log in real time
    synapse logs errors             # last 50 lines of errors.log
    synapse logs gateway -n 100    # last 100 lines of gateway.log
    synapse logs gui -f            # follow gui.log (dashboard/pty/ws)
    synapse logs desktop -f        # follow desktop.log (Electron app boot/backend)
    synapse logs --level WARNING    # only WARNING+ lines
    synapse logs --session abc123   # filter by session ID substring
    synapse logs --component tools  # only tool-related lines
    synapse logs --since 1h         # lines from the last hour
    synapse logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_synapse_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Synapse runs — try 'synapse chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_synapse_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_synapse_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_synapse_home()}/logs/")
        return

    print(f"Log files in {display_synapse_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'synapse chat' to generate logs)")
