...
Usage:
    synapse pairing list              # Show all pending + approved users
    synapse pairing approve <platform> <request-id|code>  # Approve a pairing request
    synapse pairing revoke <platform> <user_id> # Revoke user access
    synapse pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle synapse pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: synapse pairing {list|approve|revoke|clear-pending}")
        print("Run 'synapse pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: synapse pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.synapse/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'synapse pairing list' to see pending requests.\n")
