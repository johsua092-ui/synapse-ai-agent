"""synapse hooks — inspect and manage shell-script hooks.

Usage::

    synapse hooks list
    synapse hooks test <event> [--for-tool X] [--payload-file F]
    synapse hooks revoke <command>
    synapse hooks doctor

Consent records live under ``~/.synapse/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.synapse/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``synapse hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: synapse hooks {list|test|revoke|doctor}")
        print("Run 'synapse hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.synapse/config.yaml.")
        print("See `synapse hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.synapse/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `synapse hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `synapse hooks test` and `synapse hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Synapse wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `synapse hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `synapse hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `synapse hooks doctor`.")
