"""
Profile management for multiple isolated Synapse instances.

Each profile is a fully independent SYNAPSE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.synapse/profiles/<name>/`` by default.

The "default" profile is ``~/.synapse`` itself — backward compatible,
zero migration needed.

Usage::

    synapse profile create coder          # fresh profile + bundled skills
    synapse profile create coder --clone  # also copy config, .env, SOUL.md, skills
    synapse profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    synapse -p coder chat                 # or via flag
    synapse profile use coder             # set as sticky default
    synapse profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See synapse_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.synapse``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   synapse-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `synapse backup` archives
...
# Marker file written by `synapse profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `synapse
...
# `synapse skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Synapse infrastructure directories that only the default profile
         (``~/.synapse``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_synapse_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.synapse) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "synapse-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when SYNAPSE_HOME equals the
...
# into the export. The set below identifies the *known Synapse profile
# artifacts* at the root of SYNAPSE_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``synapse_constants``.
...
_RESERVED_NAMES = frozenset({
    "synapse", "default", "test", "tmp", "root", "sudo",
})

# Synapse subcommands that cannot be used as profile names/aliases
_SYNAPSE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the synapse root, NOT to the current SYNAPSE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SYNAPSE_HOME points outside
    ``~/.synapse``, profiles live under ``SYNAPSE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_synapse_home() / "profiles"


def _get_default_synapse_home() -> Path:
    """Return the default (pre-profile) SYNAPSE_HOME path.

    In standard deployments this is ``~/.synapse``.
    In Docker/custom deployments where SYNAPSE_HOME is outside ``~/.synapse``
    (e.g. ``/opt/data``), returns SYNAPSE_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_synapse_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Synapse installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SYNAPSE_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_synapse_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Synapse home
    (:func:`synapse_constants.get_synapse_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, synapse subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SYNAPSE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a synapse subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            synapse_exe = shutil.which("synapse") or "synapse"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(synapse_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Synapse. If we
    ...
    try:
        ...
        # not be migrated. The next `synapse doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``synapse -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``synapse profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``synapse -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``synapse profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SYNAPSE_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Synapse config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_synapse_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, synapse_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SYNAPSE_HOME. When
    ...
    The returned ``synapse_home`` is the path to pass to
    ``set_synapse_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_synapse_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``synapse update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.synapse)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.synapse/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `synapse -p <name> auth add`) had no file until first
    ...
    if not soul_path.exists():
        try:
            from synapse_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `synapse update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`synapse profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Synapse (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `synapse profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `synapse -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SYNAPSE_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``synapse profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SYNAPSE_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_synapse_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Synapse *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Synapse backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SYNAPSE_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `synapse -p <canon> profile
    ...
    synapse_markers = ("synapse_cli.main", "synapse-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('synapse')` resolution) spawns the
    ...
    # interpreter, not "synapse", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _SYNAPSE_CONSOLE_SCRIPT_NAMES = frozenset({"synapse", "synapse-agent", "synapse-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Synapse process: either an entrypoint marker in argv, a
            # resolved executable named `synapse`, or a python interpreter
            # directly exec'ing a `synapse`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_synapse = (
                any(marker in joined for marker in synapse_markers)
                or exe_name == "synapse"
                or exe_name.startswith("synapse")
            )
            if not is_synapse and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: synapse, synapse-agent,
                # synapse-acp) rather than a bare `startswith("synapse")` --
                ...
                # "synapse-notes.py" or "synapse-unrelated-tool" as the shim,
                ...
                is_synapse = script_stem in _SYNAPSE_CONSOLE_SCRIPT_NAMES
            if not is_synapse:
                continue
            ...
            # ...or by SYNAPSE_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SYNAPSE_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.synapse).\n"
            "To remove everything, use: synapse uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_synapse_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``SYNAPSE_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from synapse_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SYNAPSE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SYNAPSE_HOME")
    try:
        os.environ["SYNAPSE_HOME"] = str(profile_dir)
        from synapse_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        elif "SYNAPSE_HOME" in os.environ:
            del os.environ["SYNAPSE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.synapse/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: synapse profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SYNAPSE_HOME.

    Returns ``"default"`` if SYNAPSE_HOME is not set or points to ``~/.synapse``.
    Returns the profile name if SYNAPSE_HOME points into ``~/.synapse/profiles/<name>``.
    Returns ``"custom"`` if SYNAPSE_HOME is set to an unrecognized path.
    """
    from synapse_constants import get_synapse_home
    synapse_home = get_synapse_home()
    resolved = synapse_home.resolve()

    default_resolved = _get_default_synapse_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      SYNAPSE_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Synapse file the
      user may have lying alongside SYNAPSE_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``synapse sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``SYNAPSE_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.synapse itself — its parent is ~/ and its
        # directory name is ".synapse", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: synapse profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.synapse itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.synapse). "
            "Specify a different name: synapse profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="synapse_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"synapse_{old_name}"
    legacy_old_host = f"synapse.{old_name}"
    new_host = f"synapse_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_synapse_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("synapse_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SYNAPSE_HOME path string.

    Called early in the CLI entry point, before any synapse modules
    are imported, to set the SYNAPSE_HOME environment variable.

    When SYNAPSE_HOME is already set, the configured spelling IS the
