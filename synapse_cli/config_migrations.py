...
``synapse_cli.config.migrate_config``. Each step is a function
...
``synapse_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``synapse_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``synapse_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``synapse_cli.config`` (e.g.
``patch("synapse_cli.config.read_raw_config", ...)``) keep working, because
...
#: SYNAPSE_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_synapse_home()}/config.yaml and run `synapse setup` to "
...
def _cfg():
    """Return the live ``synapse_cli.config`` module (lazy, cycle-free)."""
    from synapse_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_synapse_home = _c.get_synapse_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$SYNAPSE_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `synapse plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `synapse curator status` has no
    ...
    #   3. Creates `~/.synapse/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_synapse_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_synapse_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `synapse config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `synapse config set`"
                )
...
# behavior set it to true explicitly via `synapse config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from synapse_cli.relay_plugin_cutover import legacy_relay_plugin_keys
