"""Windows ConPTY bridge for the `synapse dashboard` chat tab.

Drop-in counterpart to ``synapse_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``synapse_cli.web_server`` consumes: ``spawn``, ``read``,
