...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with synapse-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.synapse/plugins/<name>/``
3. **Project plugins** – ``./.synapse/plugins/<name>/`` (opt-in via
   ``SYNAPSE_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``synapse_agent.plugins``
   entry-point group.
...
from synapse_cli.config import cfg_get, load_config_readonly
from synapse_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from synapse_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from synapse_cli.plugin_capabilities import (
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``SYNAPSE_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("SYNAPSE_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``SYNAPSE_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.synapse/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("SYNAPSE_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When SYNAPSE_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Synapse loggers.
    """
    ...
    logger.debug(
        "SYNAPSE_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Synapse' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by synapse_cli.kanban_db when a task
    ...
    # separate `synapse -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `synapse kanban dispatch`),
    ...
    # ``synapse kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: synapse_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINTS_GROUP = "synapse_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "synapse_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``synapse_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- synapse-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- synapse-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- synapse-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``synapse:<event>``.
SYNAPSE_EVENT_NAMESPACE = "synapse"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Synapse home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "synapse", "depends",
}

# Highest manifest schema version this Synapse understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Synapse "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `synapse plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.synapse/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``synapse plugins list``. For a flat plugin at
    ...
    # ``synapse_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Synapse never
    ...
    # (``synapse plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Synapse paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_synapse_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        from synapse_cli import config as config_mod
        ...
        # threads. Other Synapse config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from synapse_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Synapse profile name (e.g. ``"default"``).

        Derived from ``SYNAPSE_HOME`` via
        :func:`synapse_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.synapse/profiles/<name>``, or ``"custom"`` when
        ``SYNAPSE_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Synapse core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``synapse honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``synapse <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`synapse_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from synapse_cli.dashboard_auth import DashboardAuthProvider
        from synapse_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_synapse_dotenv()`` startup — after
        ``~/.synapse/.env`` loads, before Synapse reads credentials — when
        ...
        NOTE ON TIMING: ``load_synapse_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_synapse_dotenv``) so the first process sees them (#64177).
    ...
    def register_slack_action_handler(
        ...
        Synapse' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``synapse model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: synapse_cli.main imports plugins indirectly
        from synapse_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``synapse:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``synapse:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, SYNAPSE_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{SYNAPSE_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``synapse:<event>``
    ...
    def register_skill(
        ...
        ``~/.synapse/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or synapse_home_key()
        ...
        # by resolved synapse home). The ledger is therefore keyed per manager
        # — i.e. per (synapse_home, plugin_id) — and every release/restore
        ...
        # contributed, so `synapse plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("SYNAPSE_SAFE_MODE"):
                logger.info("SYNAPSE_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_synapse_dotenv() already ran at import time. Re-pull so the
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from synapse_cli.env_loader import load_synapse_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Synapse plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Synapse core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Synapse core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Synapse Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with synapse and must
            ...
            # all ~20 of them added several seconds to every `synapse`
            # invocation — including plain `synapse chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Synapse ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `synapse plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from synapse_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_synapse_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_synapse_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.synapse/plugins/)
        user_dir = get_synapse_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.synapse/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("SYNAPSE_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".synapse" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set SYNAPSE_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("SYNAPSE_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from synapse_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from synapse_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Synapse process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``synapse_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``synapse plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`synapse plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``synapse tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `synapse plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Synapse validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Synapse does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `synapse plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from synapse_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_synapse_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``synapse_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``synapse_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different Synapse home (profile switch reusing a slug
        # like "synapse-lcm") or from an earlier force=True reload in the
...
# Keyed cache: resolved Synapse home -> PluginManager. Synapse supports
# multiple profiles via different SYNAPSE_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_synapse_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# synapse_constants.set_synapse_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_synapse_home() / "plugins"`` and some
    plugins (notably context engines such as synapse-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Synapse home (env var *or* the context-local
    ``set_synapse_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Synapse home instead of
    being one process-wide singleton.
    """
    try:
        return get_synapse_home().expanduser().resolve()
    except Exception:
        return get_synapse_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``synapse_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``synapse_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Synapse profile/home.
    ...
    profile switch — via ``SYNAPSE_HOME`` or the context-local
    ``set_synapse_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=synapse_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_synapse_home() / "cache" / "plugin_toolset_keys.json"
...
def get_plugin_toolset_keys_nowait() -> "set[str]":
    ...
    if t is not None and t.is_alive():
        ...
        # until a daemon restart, so union the live (non-blocking) keys in.
        live = {ts_key for ts_key, _, _ in get_plugin_toolsets()}
        merged = cached | live
        if live - cached:
            logger.debug(
                "plugin toolset keys: union adds freshly-discovered toolset(s) %s "
                "beyond last launch's cache %s",
                sorted(live - cached), sorted(cached),
            )
        return merged
    discover_plugins()
    return {ts_key for ts_key, _, _ in get_plugin_toolsets()}
...
def _get_pre_tool_call_directive_details(
    ...
    from synapse_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="synapse-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``synapse:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``synapse tools`` TUI so plugin-provided toolsets appear
