...
One read-only pass that answers, BEFORE any mutation: what Synapse runtimes
...
`detect_install_method`) — so `synapse update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `synapse update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `synapse serve`; it respawns
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Synapse runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "synapse update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set) -> str:
    ...
    if pid in service_pids:
        try:
            from synapse_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"synapse -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        from synapse_cli.gateway import _get_service_pids
    ...
    try:
        from synapse_cli.gateway import find_profile_gateway_processes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Synapse services: none detected — code swap only.")
        return
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.profile in relaunched or r.profile in external:
                ...
                r.profile in unit or (r.profile == "default" and "synapse-gateway" in unit)
                for unit in failed_set
            ):
                outcome = "failed"
            elif any(
                r.profile in svc or (r.profile == "default" and "synapse-gateway" in svc)
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    print("      synapse gateway restart                # active profile")
    print("      synapse -p <profile> gateway restart   # named profile")
    return True
