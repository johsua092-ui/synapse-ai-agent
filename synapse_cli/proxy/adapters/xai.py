...
# xAI's public API is OpenAI-compatible for the endpoints Synapse commonly
# uses. The Responses endpoint is included because Synapse' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Synapse-managed OAuth credentials."""

    auth_hint = "synapse auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`synapse auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`synapse auth reset xai-oauth` or re-authenticate with "
                    "`synapse auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `synapse auth add xai-oauth --type oauth`."
            )
