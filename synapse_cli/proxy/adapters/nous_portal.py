...
Reads the user's Nous OAuth state from ``~/.synapse/auth.json`` through the
...
# Endpoints  actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `synapse auth add nous` first."
                )
    ...
    # to synapse_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
