"""CLI handlers for the ``synapse proxy`` subcommand."""
...
from synapse_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "synapse proxy requires aiohttp. Run `synapse setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"synapse auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Synapse proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Synapse proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: synapse proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``synapse proxy <subcommand>``."""
    ...
    print(
        "synapse proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  synapse proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
