...
manual BotFather token copy-paste. Synapse talks only to the Nous onboarding
...
DEFAULT_MANAGER_BOT = "SynapseSetupBot"

DEFAULT_BOT_NAME = "Synapse Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``synapse_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``synapse_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"synapse_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Synapse Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Synapse Telegram onboarding service.")
