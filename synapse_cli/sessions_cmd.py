"""``synapse sessions`` command — extracted from ``synapse_cli/main.py``.
...
Helpers that stay in ``synapse_cli.main`` (``get_synapse_home``,
...
``synapse_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `synapse sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`synapse sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from synapse_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    synapse sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from synapse_cli.session_recovery import (
    ...
    if action == "import":
        from synapse_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from synapse_state import workspace_key as _ws_key
        ...
        from synapse_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from synapse_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_synapse_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from synapse_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_synapse_home() / "session-exports"
        ...
        # `synapse sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `synapse sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `synapse sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `synapse sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: synapse sessions pin <session_id>"
            )
            return
        ...
        # Launch synapse --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`synapse sessions optimize` later to reclaim freed space.)")
