"""
synapse fallback — manage the fallback provider chain.
...
Subcommands:
  synapse fallback [list]   Show the current fallback chain (default when no subcommand)
  synapse fallback add      Pick provider + model via the same picker as `synapse model`,
                           then append the selection to the chain
  synapse fallback remove   Pick an entry to delete from the chain
  synapse fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.synapse/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from synapse_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `synapse model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  synapse fallback add")
        print()
        return
    ...
    print("  Docs: https:///docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `synapse model`, then append the selection to the chain."""
    from synapse_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `synapse model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `synapse fallback list` to view, or `synapse fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``synapse fallback [subcommand]``."""
