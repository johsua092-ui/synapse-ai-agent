"""
Interactive setup wizard for Synapse Agent.
...
from synapse_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
from synapse_cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from synapse_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https:///docs"
...
# display_synapse_home imported lazily at call sites (stale-module safety during synapse update)
...
from synapse_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from synapse_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Synapse Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Synapse using environment variables or config commands:")
    print_info("  synapse config set model.provider custom")
    print_info("  synapse config set model.base_url http://localhost:8080/v1")
    print_info("  synapse config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'synapse setup' in an interactive terminal to use the full wizard.")
    print()
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "synapse_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``SYNAPSE_NONINTERACTIVE=1`` (see ``synapse_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("SYNAPSE_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``SYNAPSE_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from synapse_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'synapse setup')")


def _print_setup_summary(config: dict, synapse_home):
    ...
    try:
        from synapse_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Synapse cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    synapse model            (pick any provider/model)")
        print_info("    synapse setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'synapse setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from synapse_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `synapse tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from synapse_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'synapse setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'synapse setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'synapse tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'synapse setup terminal'"))
    ...
    # Spotify (OAuth via synapse auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'synapse setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {synapse_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('synapse setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('synapse setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('synapse setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('synapse setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('synapse setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('synapse config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('synapse config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('synapse config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('synapse', Colors.GREEN)}              Start chatting")
    print(f"   {color('synapse gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('synapse doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `synapse tools` and `synapse setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``synapse model``)
    ...
    provider added to ``synapse model`` is automatically available here.
    ...
    # Delegate to the shared synapse model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: synapse model")
    ...
    # on demand via `synapse auth add`, `synapse setup` vision, and
    # `synapse setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from synapse_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``synapse model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from synapse_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.synapse/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Synapse auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via synapse setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'synapse setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Synapse runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `synapse setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `synapse setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `synapse egress setup` then `synapse egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `synapse egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'synapse-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $SYNAPSE_HOME/bin is never on PATH, so a bare
            # which() misses the uv Synapse installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'synapse-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into SYNAPSE_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("SYNAPSE_MAX_ITERATIONS")
    ...
    print_info("  Run `synapse setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # SYNAPSE_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.synapse/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from synapse_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_synapse_home(Path(get_synapse_home()))
    except Exception:
        pass
...
def _profile_name_from_synapse_home(synapse_home) -> str | None:
    """Return the active profile name when SYNAPSE_HOME is a profile dir."""
    if synapse_home.parent.name == "profiles":
        return synapse_home.name
    return None
...
def _setup_bluebubbles():
    ...
    print_info("Connects Synapse to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from synapse_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https:///docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https:///docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  synapse config edit")
    print_info("   Open config in your editor:  synapse config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from synapse_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Synapse from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'synapse setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     synapse config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `synapse import` or `synapse setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `synapse setup tools` and `synapse tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Synapse profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``synapse_cli.auth`` — lists every supported
    ...
    """
    try:
        from synapse_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from synapse_cli.config
    ...
    if section_key == "model":
        ...
        from synapse_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_synapse.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_synapse migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_synapse", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Synapse.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Synapse to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Synapse at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Synapse at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Synapse at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Synapse at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Synapse equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Synapse config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Synapse.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Synapse's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(synapse_home: Path) -> bool:
    ...
    print_info("Synapse can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: synapse claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=synapse_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: synapse claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Synapse configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Synapse config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Synapse (use synapse claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``synapse setup --portal`` and ``synapse portal``. This is the
    ...
    Synapse session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``synapse setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``synapse model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``synapse portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Synapse Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `synapse portal` == quick setup's Nous step.
    try:
        from synapse_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `synapse portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `synapse portal`: %s", exc)
        print()
    ...
    print_info("  Run `synapse portal info` to inspect routing.")
    print_info("  Run `synapse` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from synapse_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``synapse model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      synapse setup           — full or quick (auto-detected)
      synapse setup model     — just model/provider
      synapse setup tts       — just text-to-speech
      synapse setup terminal  — just terminal backend
      synapse setup gateway   — just messaging platforms
      synapse setup tools     — just tool configuration
      synapse setup telemetry — just local shared metrics
      synapse setup agent     — just agent settings
    """
    from synapse_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Synapse Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Synapse Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Synapse Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, synapse_home))]
            )
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Synapse configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'synapse setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(synapse_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Synapse?",
            [
                ...
                [
                    (
                        ...
                        lambda: _run_first_time_quick_setup(
                            config, synapse_home, is_existing
                        ),
                    )
                ]
            )
    ...
    print_info(f"Data folder:  {synapse_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'synapse config edit'")
    ...
    # Tune later with `synapse setup agent`.
    if not is_existing:
    ...
    def _gateway_step() -> None:
        ...
        from synapse_cli.gateway import ensure_gateway_service
    ...
    _print_setup_summary(config, synapse_home)


def _run_first_time_quick_setup(config: dict, synapse_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``synapse setup <section>``
    or switch providers with ``synapse model``.
    """
    from synapse_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'synapse setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `synapse import` from another machine).
        from synapse_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    synapse setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  synapse setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from synapse_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``synapse setup agent`` /
    ``synapse config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, synapse_home, is_existing: bool):
    ...
    _blank_slate_walkthrough(config, synapse_home)


def _blank_slate_walkthrough(config: dict, synapse_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`synapse update` runs from re-injecting them. Opt back in any")
            print_info("time with `synapse skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `synapse tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `synapse plugins list` / `synapse plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `synapse plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `synapse mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `synapse mcp add`.")
    ...
    print_info("  Enable more tools:   synapse tools")
...
def _run_quick_setup(config: dict, synapse_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'synapse setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Synapse to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'synapse setup gateway'.")
