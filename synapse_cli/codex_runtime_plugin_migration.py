"""Migrate Synapse' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Synapse' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Synapse mcp_servers.<n>.command/args/env  → codex stdio transport
  Synapse mcp_servers.<n>.url/headers       → codex streamable_http transport
  Synapse mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Synapse mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Synapse-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Synapse' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Synapse config.")
...
# Synapse keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Synapse' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, synapse_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Synapse MCP server config to the codex inline-table dict
    ...
    if not isinstance(synapse_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(synapse_cfg.get("command"))
    has_url = bool(synapse_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (SYNAPSE_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Synapse)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Synapse' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Synapse' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Synapse
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="synapse-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a SYNAPSE_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed synapse-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.synapse`` symlink that happens to live under ``/private/var/folders``
...
def _build_synapse_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Synapse' own
    ...
    so a synapse installed under /opt/, /usr/local/, or a venv all work.
    SYNAPSE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # SYNAPSE_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_synapse_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever SYNAPSE_HOME its launcher sets
    ...
    # — that would override the launcher's SYNAPSE_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("SYNAPSE_HOME", tmp_path) would
    ...
    synapse_home = os.environ.get("SYNAPSE_HOME") or ""
    if synapse_home and _looks_like_test_tempdir(synapse_home):
        synapse_home = ""
    if synapse_home:
        env["SYNAPSE_HOME"] = synapse_home
    # PYTHONPATH passes through so a worktree-launched synapse finds the
    ...
    env["SYNAPSE_QUIET"] = "1"
    env["SYNAPSE_REDACT_SECRETS"] = env.get("SYNAPSE_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_synapse_tools: bool = True,
) -> MigrationReport:
    """Translate Synapse mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        synapse_config: full ~/.synapse/config.yaml dict
        ...
        expose_synapse_tools: when True (default), register Synapse' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Synapse for tools
    ...
    synapse_servers = (synapse_config or {}).get("mcp_servers") or {}
    if not isinstance(synapse_servers, dict):
        report.errors.append(
            "mcp_servers in Synapse config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in synapse_servers.items():
    ...
    # Inject Synapse' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Synapse for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/synapse_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_synapse_tools:
        translated["synapse-tools"] = _build_synapse_tools_mcp_entry()
        if "synapse-tools" not in report.migrated:
            report.migrated.append("synapse-tools")
