"""Persistent session goals — the Ralph loop for Synapse.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Synapse feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.SynapseCLI`` or the gateway
...
# creation, and the first synapse_cli.config import (journal-mode
...
def _bootstrap_session_db(home: str, done: threading.Event) -> None:
    ...
    try:
        ...
        # SYNAPSE_HOME). It then caches the wrong profile's DB under this
        # profile's key.
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current SYNAPSE_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``synapse_home`` path so profile switches still pick up the right DB.
        ...
        if state.consecutive_transport_failures >= DEFAULT_MAX_CONSECUTIVE_TRANSPORT_FAILURES:
            ...
            return {
                ...
                "message": (
                    ...
                    "Check the goal_judge provider/key in ~/.synapse/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: deepseek\n"
...
def run_kanban_goal_loop(
    *,
    ...
    (``synapse -p <profile> chat -q "work kanban task <id>"``). The worker's
