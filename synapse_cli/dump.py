"""
Dump command for synapse CLI.

Outputs a compact, plain-text summary of the user's Synapse setup
...
from synapse_cli.config import get_synapse_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.synapse/.env.
    ...
    exports. ``synapse debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.synapse_build_sha`` by the Dockerfile's
    ``SYNAPSE_GIT_SHA`` build-arg (see ``synapse_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``synapse dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from synapse_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(synapse_home: Path) -> int:
...
def _cron_summary(synapse_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = synapse_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"synapse_home:      {display_synapse_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.synapse/.env: a managed
        ...
        # A credential added via `synapse auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `synapse auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["synapse-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(synapse_home)}")
    lines.append(f"  skills:             {_count_skills(synapse_home)}")
