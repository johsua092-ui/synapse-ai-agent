"""
Synapse Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.synapse/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Synapse PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing synapse-agent or synapse PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Synapse Agent" comment and following line
                if '# Synapse Agent' in line or '# synapse-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('synapse' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing synapse
                if 'synapse' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Synapse-owned file, and
...
def remove_wrapper_script():
    """Remove the synapse wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "synapse",
        Path.home() / ".local" / "bin" / "synapse-acp",
        Path.home() / ".local" / "bin" / "synapse-agent",
        Path("/usr/local/bin/synapse"),
        Path("/usr/local/bin/synapse-acp"),
        Path("/usr/local/bin/synapse-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains synapse_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'synapse_cli' in content or 'synapse-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(synapse_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``synapse`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Synapse home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (synapse_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``synapse gateway run`` processes
    ...
    try:
        from synapse_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from synapse_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SYNAPSE_HOME`` and ``SYNAPSE_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\synapse\git\cmd``, ``%LOCALAPPDATA%\synapse\git\bin``,
#      ``%LOCALAPPDATA%\synapse\git\usr\bin``, ``%LOCALAPPDATA%\synapse\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\synapse\git\`` and Node to
#      ``%LOCALAPPDATA%\synapse\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``synapse dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\synapse\gateway-service\`` and sometimes
...
def _synapse_path_markers(synapse_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Synapse-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the synapse launchers and the managed uv) — only wanted when
    ...
    """
    root = str(synapse_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare synapse-agent install dir.
    markers = [root + "\\synapse-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if SYNAPSE_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "synapse".  We don't want to catch
    # unrelated entries like "csynapse-foo" or "ephermeral", so we look for
    # backslash-synapse as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(synapse_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Synapse-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<synapse_home>\\bin`` (the managed binary
    dir holding the synapse launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _synapse_path_markers(synapse_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_synapse_env_vars_windows() -> list[str]:
    """Delete SYNAPSE_HOME and SYNAPSE_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SYNAPSE_HOME", "SYNAPSE_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``synapse`` launchers install.ps1 staged in the managed
    binary dir (the default Synapse root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``synapse`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_synapse_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from synapse_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from synapse_constants import get_default_synapse_root

        bin_dir = get_default_synapse_root() / "bin"
    except Exception as e:
...
def _is_default_synapse_home(synapse_home: Path) -> bool:
    """Return True when ``synapse_home`` points at the default (non-profile) root."""
    try:
        from synapse_constants import get_default_synapse_root
        return synapse_home.resolve() == get_default_synapse_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SYNAPSE_HOME directory.

    We shell out to ``synapse -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SYNAPSE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m synapse_cli.main` so we don't depend on a `synapse`
    #    wrapper that may be half-removed mid-uninstall.
    synapse_invocation = [_sys.executable, "-m", "synapse_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                synapse_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's SYNAPSE_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``synapse uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SYNAPSE_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from synapse_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(synapse_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Synapse Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Synapse Chat GUI installation was found.")
        print(f"  Checked: {synapse_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Synapse agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(synapse_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Synapse agent at {synapse_home / 'synapse-agent'}")
        print(f"  • Your config, sessions, and secrets under {synapse_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Synapse agent is still installed. Run 'synapse' to use the CLI,")
    print("or 'synapse uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.synapse/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.synapse/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie SYNAPSE_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_synapse_home(synapse_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.synapse data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Synapse Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {synapse_home / 'config.yaml'}")
    print(f"  Secrets: {synapse_home / '.env'}")
    print(f"  Data:    {synapse_home / 'cron/'}, {synapse_home / 'sessions/'}, {synapse_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SYNAPSE_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Synapse data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Synapse code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, synapse_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Synapse PATH entries from shell configs / Windows User PATH")
    print("  • Synapse wrapper scripts and Synapse-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Synapse config/data: {synapse_home}")
        if _is_default_synapse_home(synapse_home):
        ...
        print(f"  • Keep Synapse config/data: {synapse_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``synapse`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SYNAPSE_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in synapse_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\synapse\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (synapse\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_synapse_home(synapse_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(synapse_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Synapse-owned PATH entries in User environment")

        log_info("Removing SYNAPSE_HOME / SYNAPSE_GIT_BASH_PATH User env vars...")
        removed_env = remove_synapse_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Synapse-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing synapse command...")
    ...
    #     would dangle — `synapse` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows synapse launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows synapse launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Synapse home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Synapse-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(synapse_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Synapse-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(synapse_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SYNAPSE_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from synapse_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(synapse_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.synapse/, just remove the synapse-agent subdir
            if synapse_home in project_root.parents or project_root.parent == synapse_home:
    ...
    #     under SYNAPSE_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(synapse_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(synapse_home)
    ...
    # 5. Optionally remove ~/.synapse/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SYNAPSE_HOME dirs live under
        ...
        try:
            if synapse_home.exists():
                ...
                log_success(f"Removed {synapse_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {synapse_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {synapse_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {synapse_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https:///install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https:///install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Synapse Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m synapse_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import synapse_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``synapse_constants`` + ``synapse_cli.colors``
    (and lazily ``synapse_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m synapse_cli.uninstall")
    parser.add_argument(
        "--mode",
