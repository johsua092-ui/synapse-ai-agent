...
1. **Update receipt** — a machine-readable JSON record of what one
   ``synapse update`` run discovered, did, skipped (and why), written to
   ``<SYNAPSE_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``synapse_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``synapse update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``synapse update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_synapse_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``synapse update`` has many early-termination paths (Windows
