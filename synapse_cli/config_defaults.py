"""Default configuration data for Synapse Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from synapse_cli/config.py. Must not import from synapse_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["synapse-cli"],
    # SQLite journal mode used by every Synapse database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Synapse server processes.
    ...
    "session": {
        # Per-terminal `synapse -c`: each CLI session drops a breadcrumb file
        # under $SYNAPSE_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by synapse_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `synapse chat --run-budget N`.
        ...
        # an interactive `synapse gateway restart` must never block for hours
        ...
        # the Synapse-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Synapse
        ...
        # identity slot (SOUL.md). Empty by default. The SYNAPSE_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Synapse adds a coding operating brief
        ...
        # `synapse status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``SYNAPSE_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # bot_relay from a short-lived `synapse -p <bot> chat -Q` recipient
        ...
        #             SYNAPSE_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SYNAPSE_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Synapse auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Synapse sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Synapse
        ...
        # bundled Synapse image, which drops to the `synapse` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `synapse tools` picker's
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Synapse sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Synapse should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``synapse chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.synapse/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: synapse sweeps the checkpoint base at startup
        ...
        # ``synapse checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .synapse.md, or .cursorrules before Synapse applies
    ...
    # Single-query (``synapse -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Synapse truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Synapse' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); synapse =
                                      # Synapse' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Synapse' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Synapse does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``synapse kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``synapse kanban decompose`` and the kanban
        ...
        # ``synapse profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `synapse model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `synapse` (and `synapse chat`) launches by default:
        ...
        # REPL and `--tui` (or SYNAPSE_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `synapse --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `synapse -c` muscle memory.  Default off so existing
        # users aren't surprised.  SYNAPSE_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `synapse --tui` drops a one-time hint
        ...
        # model — see synapse_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to SYNAPSE_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/synapse`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `synapse pets`. Disabled until
        ...
        "pet": {
            ...
            # get_synapse_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see synapse_cli/web_server.py — an event-loop stall
        ...
        # SYNAPSE_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (SYNAPSE_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``SYNAPSE_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SYNAPSE_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``SYNAPSE_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SYNAPSE_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``SYNAPSE_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/synapse``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Synapse appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.synapse/cache/piper-voices/
    ...
    # "Hey Synapse" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey synapse",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_synapse" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.synapse/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.synapse/cache/delegation/
    ...
    # response. If not, Synapse feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Synapse auto-pauses the goal and
    ...
    "moa": {
        ...
        # <synapse_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.synapse/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.synapse/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``synapse skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``synapse skills trust`` / ``synapse skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${SYNAPSE_SKILL_DIR} and ${SYNAPSE_SESSION_ID} in SKILL.md
        ...
        # (`synapse skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.synapse/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.synapse/.curator_backups/blobs/. Enables `synapse curator ledger`
        # and single-mutation `synapse curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `synapse curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `synapse curator run --consolidate` overrides
        ...
        # normally restored on every `synapse update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `synapse curator purge` deletes
        ...
        # skipped), snapshot ~/.synapse/skills/ into
        # ~/.synapse/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `synapse curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for synapse-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Synapse Agent*" header.
    ...
    # dangerous command. -q runs export SYNAPSE_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; SYNAPSE_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Synapse' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.synapse/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SYNAPSE_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .synapse config) always require human
        ...
        # longer trigger the startup banner. Add via `synapse doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``synapse_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Synapse to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SYNAPSE_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via SYNAPSE_CRON_MAX_PARALLEL env var.
        ...
        # SYNAPSE_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # SYNAPSE_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via SYNAPSE_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `synapse -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `synapse` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `synapse kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     synapse-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Synapse tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.synapse/logs/.
    ...
    # update model picker lists without shipping a synapse-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https:///docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `synapse model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Synapse provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal SYNAPSE_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.synapse/sessions/sessions.json entirely.
        ...
        # the SYNAPSE_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.synapse.gateway` that defenses 1-2 don't
        ...
        # `synapse update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``SYNAPSE_GATEWAY_MAX_STARTS`` / ``SYNAPSE_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.synapse/.env,
        ...
        # behavior: files must live under the Synapse cache, under
        ...
        # user. Bridged to SYNAPSE_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Synapse
        # cache (~/.synapse/cache/{documents,images,audio,video,screenshots})
        ...
        # to SYNAPSE_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SYNAPSE_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SYNAPSE_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.synapse/state.db.
    ...
    "sessions": {
        ...
        # the default of ``synapse sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.synapse/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `synapse sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `synapse update` prints a one-line notice with
        ...
        # ~/.synapse/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to SYNAPSE_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # SYNAPSE_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `synapse sessions export`. Checked
    ...
    # ``synapse doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `synapse doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``synapse update`` behaviour.
    "updates": {
        ...
        #     DBs) into <SYNAPSE_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``synapse backup``-style zip
        #     of SYNAPSE_HOME into <SYNAPSE_HOME>/backups/, restorable with
        #     ``synapse import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `synapse update` does with uncommitted local changes to the
        ...
        # When `synapse update` finds the source checkout parked on a feature
        ...
        #                         merge. `synapse update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<SYNAPSE_HOME>/lsp/bin/`` on first use.
    ...
    # `synapse tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.synapse/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.synapse/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.synapse/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Synapse may use this encrypted
            ...
            # ~/.synapse/bin/ on first use.  When False you must install
            ...
            # `synapse secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.synapse/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Synapse disables it for our users unless they
        # explicitly opt in here. When false (default), Synapse sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Synapse also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Synapse computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Synapse passes the
    ...
    #
    # Configure with `synapse egress setup`.  Disabled by default — the rest of
    # Synapse works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.synapse/bin/ on
    ...
    # Synapse Desktop (Electron app) launch options. These only affect
    # `synapse desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SYNAPSE_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the SYNAPSE_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Synapse mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'synapse model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g.  -> ",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Synapse auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Synapse' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Synapse, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @synapse:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SYNAPSE_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #synapse)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: synapse-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'synapse-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Synapse API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Synapse API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Synapse API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SYNAPSE_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to SYNAPSE_TOOL_PROGRESS_MODE for backward
    ...
    # SYNAPSE_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "SYNAPSE_PREFILL_MESSAGES_FILE": {
    ...
    "SYNAPSE_EPHEMERAL_SYSTEM_PROMPT": {
