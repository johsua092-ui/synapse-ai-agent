"""``synapse debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Synapse Agent. Use 'synapse debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    synapse debug share              Upload debug report (asks for confirmation)
    synapse debug share --yes        Skip confirmation (for scripts/CI)
    synapse debug share --lines 500  Include more log lines
    synapse debug share --expire 30  Keep paste for 30 days
    synapse debug share --local      Print report locally (no upload)
    synapse debug share --no-redact  Disable upload-time secret redaction
    synapse debug share --nous       Upload to Nous-internal storage (private)
    synapse debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'synapse debug share'",
    )
