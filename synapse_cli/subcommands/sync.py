"""``synapse sync`` subcommand parser — Skill Sync.

Cloned from ``synapse_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    synapse sync status                 show gate/opt-in/head state
    synapse sync pull                   pull and materialize opted-in skills
    synapse sync push                   push opted-in skills
    synapse sync now                    reconcile: pull then push
    synapse sync enable <skill>         opt a skill into sync
    synapse sync disable <skill>        opt a skill out of sync
    synapse sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    synapse sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  synapse sync status            what is synced, and from where\n"
            "  synapse sync enable my-skill   include a skill in your sync\n"
            "  synapse sync now               pull, then push\n"
            "  synapse sync propose my-skill  share a skill with your team\n"
        ),
