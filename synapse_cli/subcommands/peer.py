"""``synapse peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Synapse gateway (any machine: homelab, Spark, Synapse
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    synapse peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    synapse peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``synapse -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.synapse/.env`` as
  ``SYNAPSE_PEER_<NAME>_KEY``.
...
def _peer_key_env(name: str) -> str:
    return f"SYNAPSE_PEER_{name.upper().replace('-', '_')}_KEY"
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's synapse-agent, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}."
            ) from exc
        raise
    # Real api_server wraps the row: {"object": "synapse.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (synapse peer dm <peer>[/<agent>] ...)")
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.synapse/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  synapse peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.synapse/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: synapse peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: synapse peer list", file=sys.stderr)
            return 1
        key = _peer_secret(peer_name)
        if not key:
            print(
                f"No API key for peer '{peer_name}'. Set it: synapse peer add {peer_name} "
                f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.synapse/.env)",
                file=sys.stderr,
            )
            return 1
    ...
    print("Unknown peer action. See: synapse peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Synapse gateways)",
        description=(
            "Register other Synapse gateways as peers and message their agents. "
            "'synapse peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'synapse -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.synapse/.env."
        ),
        epilog=(
            "Examples:\n"
            "  synapse peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  synapse peer list\n"
            '  synapse peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  synapse peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  synapse peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.synapse/.env)")
