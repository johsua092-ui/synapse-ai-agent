...
This module is imported by ``synapse_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no synapse_cli.config,
...
``synapse_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(synapse_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect SYNAPSE_HOME?"""
    active_profile = os.path.join(synapse_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    synapse_home = os.environ.get("SYNAPSE_HOME", "").strip()
    if synapse_home:
        return synapse_home
...
def container_mode_may_be_active() -> bool:
    ...
    synapse_home = os.environ.get("SYNAPSE_HOME", "").strip()
    if synapse_home:
        if os.path.exists(os.path.join(synapse_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(synapse_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(synapse_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".synapse")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect SYNAPSE_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``synapse --version`` output (also used by /version).
    ...
    ``synapse version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from synapse_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from synapse_cli import __release_date__, __version__

        print(f"Synapse Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from synapse_cli.config import detect_install_method
    ...
    try:
        from synapse_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``synapse --version`` before the heavy import wall.
    ...
    container. Termux keeps the SYNAPSE_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("SYNAPSE_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
