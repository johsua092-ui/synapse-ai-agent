"""Cross-process mutual exclusion for in-flight Synapse updates.

Three different surfaces can start an update of the same install tree:

* ``synapse update`` from a terminal,
* the dashboard's Update button (``POST /api/synapse/update`` →
  ``_spawn_synapse_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``synapse-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``synapse update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <SYNAPSE_HOME>/.synapse-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``synapse update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Synapse is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``synapse-setup`` binary under ``~/.synapse`` is only refreshed by a full
  installer run (``copy_self_to_synapse_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Synapse is still running") on every
  GUI update forever, with no Synapse process actually running.
"""
...
MARKER_NAME = ".synapse-update-in-progress"

# Set by an orchestrating updater (the Tauri `synapse-setup --update` flow) to
# its own pid before spawning `synapse update` as a child stage. The parent
...
HANDOFF_PID_ENV = "SYNAPSE_UPDATE_HANDOFF_PID"
...
# to show "Synapse is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Synapse home (never the context-local profile override):
    the Rust updater resolves ``$SYNAPSE_HOME`` or the platform default, and the
    ...
    return get_process_synapse_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``synapse update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``synapse-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Synapse update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `synapse update` as a stage): we run under ITS claim
