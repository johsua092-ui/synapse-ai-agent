"""
Single source of truth for provider identity in Synapse Agent.
...
2. **Synapse overlays** — transport type, auth patterns, aggregator flags,
...
# -- Synapse overlay ----------------------------------------------------------
# Synapse-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SynapseOverlay:
    """Synapse-specific provider metadata layered on top of models.dev."""
...
SYNAPSE_OVERLAYS: Dict[str, SynapseOverlay] = {
    "moa": SynapseOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": SynapseOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nous": SynapseOverlay(
        ...
        base_url_override="https:///v1",
    ),
    "openai-codex": SynapseOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": SynapseOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": SynapseOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": SynapseOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SYNAPSE_QWEN_BASE_URL",
    ),
    "lmstudio": SynapseOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": SynapseOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SynapseOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": SynapseOverlay(
    ...
    "kimi-for-coding": SynapseOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": SynapseOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": SynapseOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": SynapseOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": SynapseOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": SynapseOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": SynapseOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": SynapseOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": SynapseOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": SynapseOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": SynapseOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": SynapseOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": SynapseOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": SynapseOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": SynapseOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": SynapseOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": SynapseOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": SynapseOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": SynapseOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": SynapseOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "synapse", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Synapse overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Synapse overlay
    ...
    overlay = SYNAPSE_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + synapse extra
    ...
    if overlay is not None:
        # Synapse-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and SYNAPSE_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Synapse overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in SYNAPSE_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Synapse provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from synapse_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
