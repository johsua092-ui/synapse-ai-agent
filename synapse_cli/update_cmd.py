"""Synapse update pipeline — extracted from ``synapse_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``synapse_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``synapse_cli.main`` reference — so existing call sites and test monkeypatches
that target ``synapse_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``synapse_cli.main``.
...
Imports are one-way: ``synapse_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``synapse_cli.main`` reference.

    Lets callers keep patching ``synapse_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``synapse_cli.main`` -> ``synapse_cli.update_cmd`` stays one-way
    at import time.
    """
...
#: ``_purge_stale_synapse_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "synapse_cli.synapse_logging",
    }
)


def _purge_stale_synapse_modules() -> None:
    """Evict every cached Synapse module after the checkout changed in-place.

    ``synapse update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from synapse_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``synapse_cli.gateway`` does
    ``from synapse_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Synapse
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Synapse module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Synapse modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``synapse update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``synapse update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``synapse_cli.config_defaults``,
    ``synapse_cli.config``, and ``synapse_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``synapse_cli._subprocess_compat`` and
    ``synapse_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "synapse_cli.config_defaults",
        "synapse_cli.config",
        "synapse_cli.config_migrations",
        "synapse_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from synapse_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from synapse_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``synapse update`` historically
    ...
    caller): scope config reads/writes via the context-local SYNAPSE_HOME
    ...
    try:
        ...
        from synapse_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_synapse_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_synapse_home_override(entry)
            try:
...
# Critical files that Synapse must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "synapse_cli/main.py",
    "synapse_cli/config.py",
    "synapse_cli/__init__.py",
    "synapse_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "synapse_constants.py",
)
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``synapse.exe`` has
    ...
    Skipping is safe because Synapse pins its editable finder to a *static*
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``synapse`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="synapse-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``synapse update`` can be driven by a
    ...
    """
    from synapse_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from synapse_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('synapse_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``synapse update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `synapse update`.
    ...
    print("  Preview now:  synapse curator run --dry-run")
    print("  Pause it:     synapse curator pause")
    print(
        "  Docs:         https:///docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `synapse update`.
    ...
    try:
        ...
        return
    db_path = get_synapse_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `synapse sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    synapse sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  synapse sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``synapse update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``synapse update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: synapse curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from synapse_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    synapse dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``synapse_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.synapse-update-staging"
    backup = f"{dst}.synapse-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `synapse update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``synapse_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``synapse update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && synapse update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("SYNAPSE_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== synapse-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Synapse launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["synapse.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Synapse")
    print("  Desktop, a gateway, or another synapse REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `synapse` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``synapse.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.synapse-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".synapse-update-staging", ".synapse-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `synapse update`.")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Synapse Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `synapse update "
            f"--branch {branch}`, or update against main with `synapse update`."
        )
        _m().sys.exit(1)
    ...
    tmp_dir = tempfile.mkdtemp(prefix="synapse-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"synapse-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for synapse-agent itself
        ...
        # GitHub ZIPs extract to synapse-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"synapse-agent-{branch}")
        ...
        print(
            "  Re-run `synapse update` to retry; if the agent won't start, "
    ...
    from synapse_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from synapse_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `synapse update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: synapse skills list-modified  "
    ...
    try:
        from synapse_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from synapse_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_synapse_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_synapse_home())
    ...
    try:
        from synapse_cli.update_receipt import finalize_update_receipt
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "synapse-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``synapse update --keep-stash`` (the desktop updater's mode): the
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Synapse behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes synapse completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Synapse couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Synapse couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Synapse behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Synapse couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Synapse couldn't drop "
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_synapse_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_synapse_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Synapse repository.")
        print("  This means you may miss updates from johsua092-ui/synapse-ai-agent.")
        print()
        try:
            response = (
...
def _invalidate_update_cache():
    ...
    ``synapse update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from synapse_constants import get_default_synapse_root

    default_home = get_default_synapse_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "synapse.exe"
    lines = ["✗ Another synapse.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Synapse Desktop, exit any open `synapse` REPLs, and")
    lines.append("  stop the gateway (`synapse gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `synapse update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``synapse tools``."""
    try:
        from synapse_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Synapse Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``synapse tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Synapse Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Synapse Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``synapse update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `synapse update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `synapse update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Synapse' editable-install extras or
    ...
    try:
        from synapse_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from synapse_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SYNAPSE_HOME/bin/uv``, but on Termux the official
    ...
    """
    from synapse_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `synapse update` is exactly the step
...
def _npm_lockfile_changed(synapse_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `synapse
    ...
    try:
        ...
        cache_file = synapse_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(synapse_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    synapse update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `synapse update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from synapse_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `synapse update`.")
            failed = []
            if any(
    ...
    # Synapse profile using this checkout. Keep one per-checkout cache under the
    # shared Synapse root rather than rerunning npm once per named profile.
    shared_synapse_root = get_default_synapse_root()
    ...
    # `synapse update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_synapse_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `synapse update`.")
        return list(labels)
    ...
    nixos_env = with_synapse_node_path(_m()._nixos_build_env())
    ...
    # `synapse update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_synapse_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.synapse/logs/update.log`` only, never the terminal.

    During ``synapse update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``synapse update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``synapse update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from synapse_cli.config import format_docker_update_message
    ...
    from synapse_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from synapse_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``synapse update`` without requiring a reinstall.  The
    ...
    ``synapse`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/synapse.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v synapse'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/synapse, code at /usr/local/lib/synapse-agent).
    fhs_link = Path("/usr/local/bin/synapse")
    ...
    path_comment = (
        "# Synapse Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``synapse-acp`` launcher next to the ``synapse`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``synapse update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``synapse-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Synapse as not installed even when it is.

    The shim simply delegates to the sibling ``synapse`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``synapse`` / ``synapse-acp``
    launchers into the managed binary dir ``$SynapseHome\bin`` and puts THAT
    ...
    ``synapse_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``synapse-acp`` is already present next to the
    ``synapse`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        synapse_cmd = bin_dir / "synapse"
        acp_cmd = bin_dir / "synapse-acp"
        try:
            if not (synapse_cmd.is_file() or synapse_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of SYNAPSE_HOME under
      ``backups/`` (restorable via ``synapse import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from synapse_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from synapse_constants import
        # get_synapse_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from synapse_cli.config import get_synapse_home as _get_home
        ...
        try:
            from synapse_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from synapse_cli.update_receipt import record_step
    ...
    try:
        from synapse_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from synapse_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_synapse_home so the user sees ~/.synapse/...
    try:
        from synapse_constants import get_synapse_home, display_synapse_home

        home = get_synapse_home()
        try:
            display_path = f"{display_synapse_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  synapse import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``synapse update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``synapse update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run synapse from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.synapse-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".synapse-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The synapse.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m synapse_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``synapse
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m synapse_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "synapse_cli.main" in cmdline_low:
        ...
        # the `-m synapse_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``synapse_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``synapse.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``synapse`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `synapse` launch will complete it")
...
def _synapse_holder_subcommand(cmdline: str) -> str | None:
    """The actual Synapse SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``synapse_cli.main`` / ``synapse(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("synapse_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("synapse", "synapse.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``synapse dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Synapse processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Synapse backend (if the Desktop app is open, close it)",
        "dashboard": "  ← synapse dashboard (stop it: synapse dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _synapse_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the Synapse desktop app / other Synapse terminals, then re-run:"
    )
    lines.append("    synapse update")
    lines.append("  (or use `synapse update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``synapse update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from synapse_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning synapse-setup, and the update-in-progress marker
    ...
    the update with "Synapse is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Synapse backend (``synapse_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.synapse-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "synapse_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``synapse_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Synapse ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``synapse.exe``) is open, NOTHING legitimate
    ...
    - Only Synapse backends (``synapse_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``synapse.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.synapse-runtime`` interpreter child alive and holding the install open
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``synapse serve`` / ``synapse dashboard`` argv.
    ...
    """
    if "synapse_cli.main" not in (cmdline or "").lower():
        return False
    return _synapse_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _desktop_owns_gateway_lifecycle() -> bool:
    ...
    """
    try:
        from synapse_cli.process_identity import ledger_entries, spawner_is_dead
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    synapse.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from synapse_cli.gateway import (
    ...
    print("→ Stopping Windows gateway process(es) before updating Synapse...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m synapse_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: synapse gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``synapse gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from synapse_cli import gateway_windows
        from synapse_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``synapse-gateway*.service``/``synapse-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("synapse-serve")`` alone would also accept the
        # unrelated ``synapse-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "synapse-gateway.service"
            or unit.startswith("synapse-gateway-")
            or unit == "synapse-serve.service"
            or unit.startswith("synapse-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``synapse-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``synapse-serve*`` units (#83438) don't, so sending them
    ...
    (``synapse-gateway-helper`` is fine — profile units are
    ``synapse-gateway-<profile>`` — but ``synapse-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "synapse-gateway" or svc_name.startswith("synapse-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from synapse_cli.gateway import is_macos
    ...
    print("    synapse gateway status")
    if any(not name.startswith("ai.synapse.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.synapse.") for name in ordered):
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.synapse.gateway*`` LaunchAgent must reload it — restarting only the
            ...
            try:
                ...
                if wait_for_launchd_gateway_supervision(label=current_label):
                    ...
                    print(
                        ...
                        "supervising it.\n"
                        "    Check logs, then: synapse gateway restart"
                    )
    except (FileNotFoundError, subprocess.TimeoutExpired):
        pass
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``synapse_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``synapse_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``synapse update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``synapse-setup.exe``) executes
    ``$SYNAPSE_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_synapse_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``synapse update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``synapse update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `synapse desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "synapse_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Synapse-managed Node on PATH: when
    # `synapse update` runs inside the desktop updater chain (Desktop →
    # synapse-setup → synapse update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from synapse_constants import with_synapse_node_path

    build_env = with_synapse_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `synapse desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from synapse_cli.update_receipt import begin_update_receipt
    ...
    # running Synapse runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another synapse.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_synapse_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `synapse serve` backend) will keep .pyd
    ...
    # the synapse.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Synapse backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Synapse backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .synapse-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Synapse is still
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Synapse backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_synapse_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Synapse backend process(es) "
    ...
    # process by design (a CLI `synapse update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by synapse.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Synapse windows/gateways and re-run: synapse update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Synapse gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in synapse_cli/config.py bricked
        # every user who ran ``synapse update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``synapse
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``synapse update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``synapse update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``synapse doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && synapse update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_synapse_home added to synapse_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``synapse`` launch finishes the install
        ...
        # symbols in synapse_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `synapse` again "
        ...
        if not import_ok:
            ...
            print("    Run `synapse update` again — if it persists, reinstall:")
            print("    https://")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_synapse_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.synapse/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit SYNAPSE_HOME so
        # it is not affected by sync_skills()'s module-level SYNAPSE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SYNAPSE_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from synapse_cli.profiles import backfill_profile_envs
        ...
        # ``synapse update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``synapse_cli.config`` and
        # ``synapse_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from synapse_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'synapse config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'synapse config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: synapse config migrate")
            else:
                print()
                print("Skipped. Run 'synapse config migrate' later to configure.")
        else:
        ...
        # context-local SYNAPSE_HOME override (never os.environ — other
        ...
        # it and warn loudly.
        try:
            from synapse_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            from synapse_cli.backup import restore_cron_jobs_all_profiles
        ...
        # check `synapse curator status`. Self-stamps after printing so it
        ...
        # Self-heal the synapse-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Synapse on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("synapse-acp launcher self-heal failed: %s", e)

        # Migrate the Windows synapse launchers to the managed binary dir
        # (the default Synapse root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (synapse-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `synapse`
        ...
        # the managed clone under the default Synapse root).
        try:
            from synapse_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``synapse update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from synapse_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `synapse update`
                # must stay fast; `synapse computer-use install --upgrade`
        ...
        # When running as ``synapse update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Synapse modules FIRST: the import below pulls
        ...
        _m()._purge_stale_synapse_modules()
        try:
            from synapse_cli.gateway import (
                is_macos,
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... synapse-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... synapse-gateway*)
            ...
            try:
                from synapse_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all synapse-gateway* units (default + profiles) plus
            # synapse-serve* units (the Desktop app's backend, #83438).
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: synapse gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # synapse-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `synapse update` restart it automatically, allow\n"
                        ...
                        # path in `synapse gateway restart`
            ...
            # Restart EVERY ai.synapse.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: synapse gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: synapse -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_synapse_home() / ".update_exit_code"
                    try:
            ...
            try:
                from synapse_cli.update_receipt import record_gateway_restart
        ...
        # Warn if legacy Synapse gateway unit files are still installed.
        # When both synapse.service (from a pre-rename install) and the
        # current synapse-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `synapse update` surfaces the issue until the user migrates.
        try:
            from synapse_cli.gateway import (
                has_legacy_synapse_units,
                _find_legacy_synapse_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_synapse_units():
                print()
                print("⚠ Legacy Synapse gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_synapse_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (synapse.service) fight the current")
                print("  synapse-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    synapse gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes synapse-serve*,
        ...
        print("  synapse model              # Select provider and model")
        ...
        try:
            from synapse_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import synapse_cli.update_receipt as _ur
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from synapse_cli.main import main; main()" update --yes'
                    )
            try:
                from synapse_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``synapse_cli.gateway`` no longer imports,
