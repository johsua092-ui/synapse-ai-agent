"""Dashboard process-hygiene helpers — extracted from ``synapse_cli/main.py``.
...
``_detect_concurrent_synapse_instances``) are lifted verbatim. References to
helpers that STAY in ``synapse_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``synapse_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``synapse_cli.main`` resolve unchanged.
"""
...
def _scan_dashboard_processes(
    *,
    ...
    ``synapse dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``synapse update`` replaces files on disk, the running
    ...
    ``synapse-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Synapse Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``synapse serve`` as
    ...
    environment variable ``SYNAPSE_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`synapse serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "synapse.*dashboard"`) keeps us consistent
            # with `synapse_cli.gateway._scan_gateway_pids` and avoids the
...
def _synapse_home_for_pid(pid: int) -> str | None:
    """Best-effort ``SYNAPSE_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("SYNAPSE_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"SYNAPSE_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Synapse Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``synapse update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], synapse_home: str | None = None
) -> str:
    """Stable owner key: ``SYNAPSE_HOME`` when known, else ``--profile`` / ``-p``.

    ``SYNAPSE_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.synapse`` roots) keep a resolved ``home:`` key so
    ...
    if synapse_home:
        try:
            home_path = Path(synapse_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(synapse_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(synapse_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``synapse update``.

    Each candidate is ``(pid, argv, synapse_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``SYNAPSE_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``SYNAPSE_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``synapse update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, synapse_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, synapse_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``synapse dashboard`` / ``synapse serve`` processes.

    Called at the end of ``synapse update`` (default ``reason``) and also
    from ``synapse dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``synapse update`` path), a detected
    ``synapse-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``synapse-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``synapse update``'s systemd
    fleet-restart loop, which restarts ``synapse-serve*`` units before this
    ...
    # When the Synapse Desktop Electron app spawns this dashboard as a
    # backend child, it sets SYNAPSE_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("SYNAPSE_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `synapse update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot SYNAPSE_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _synapse_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. synapse update's
    ...
    #    remote backend (synapse serve) under Restart=on-failure never comes
...
def _detect_concurrent_synapse_instances(
    ...
    The Synapse Desktop Electron app spawns ``synapse.EXE`` as a backend child,
    so during ``synapse update`` the user-invoked process and the desktop's
    ...
    shims (``synapse.exe`` / ``synapse-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``synapse update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``synapse.exe``) is a separate process from the Python
    ...
    for shim in _m()._synapse_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated synapse.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``synapse update``
    ...
    #      synapse.exe sitting *under* a non-Synapse parent (e.g. a Synapse
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        synapse serve --host 127.0.0.1 --port 0
        synapse serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "synapse" not in cmd and "synapse_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (SYNAPSE_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("SYNAPSE_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``synapse serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``synapse serve`` on a Mac
...
def _synapse_home_dir() -> Path:
    """Resolved Synapse home (SYNAPSE_HOME override or ~/.synapse)."""
    override = os.environ.get("SYNAPSE_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "synapsePath", "synapseHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated SYNAPSE_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{synapse_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _synapse_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local synapse serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``synapse serve`` backends with no parent.
    ...
    running under SYNAPSE_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never SYNAPSE_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports SYNAPSE_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # SYNAPSE_BACKEND_READY, so a sibling spawned seconds ago is not yet
