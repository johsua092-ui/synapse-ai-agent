"""MCP picker — interactive `synapse mcp picker` (also the default `synapse mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``synapse mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `synapse plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from synapse_cli.cli_output import prompt_yes_no
from synapse_cli.curses_ui import curses_single_select
from synapse_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Synapse session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from synapse_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `synapse mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: synapse mcp install <name>    Picker: synapse mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Synapse is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Synapse — run `synapse update` "
...
def show_catalog() -> None:
    """`synapse mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`synapse mcp picker` (and default `synapse mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`synapse mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `synapse mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
