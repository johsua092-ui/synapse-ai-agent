...
on Japanese locales, etc.).  Synapse's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``synapse_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Synapse is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SYNAPSE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SYNAPSE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Synapse-managed Git install directories so
    ...
    # already-running shells don't see the change — which means synapse
    ...
    # install without Synapse-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Synapse's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Synapse's
    ``synapse config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Synapse (User env var in Windows Settings, or
    export in a PowerShell profile) and Synapse picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Synapse-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\synapse\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``synapse`` from right after install) retain their old PATH.

    Any subprocess Synapse spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Synapse-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "synapse", "git", "cmd"),
        os.path.join(local_appdata, "synapse", "git", "bin"),
        os.path.join(local_appdata, "synapse", "git", "usr", "bin"),
        # Synapse venv Scripts directory — host of the synapse.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes synapse, but harmless to include.
        os.path.join(local_appdata, "synapse", "synapse-agent", "venv", "Scripts"),
