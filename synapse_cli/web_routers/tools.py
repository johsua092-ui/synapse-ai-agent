...
late-binding seam in :mod:`synapse_cli.web_deps` (``late`` for callables,
...
from synapse_cli.web_deps import late, LateState
...
_spawn_synapse_action = late("_spawn_synapse_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from synapse_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `synapse tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `synapse computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``synapse tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `synapse tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``synapse tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.synapse/.env`` via ``save_env_value`` —
    the same store ``synapse tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``synapse tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``synapse -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same SYNAPSE_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``synapse computer-use permissions grant`` as a background action.
