...
this. Capabilities govern the *host API surfaces* Synapse hands out (which
...
until the user re-consents (``synapse plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer Synapse may declare ids this build doesn't know; they can
