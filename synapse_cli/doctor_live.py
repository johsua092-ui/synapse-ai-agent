"""``synapse doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from synapse_cli.doctor import SYNAPSE_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (SYNAPSE_HOME / "node" / "bin",
                          SYNAPSE_HOME / "node",
                          SYNAPSE_HOME / "node_modules" / ".bin"):
    ...
    # synapse_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``synapse mcp test``.
    """
