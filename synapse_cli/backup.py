"""
Backup and import commands for synapse CLI.

`synapse backup` creates a zip archive of the entire ~/.synapse/ directory
(excluding the synapse-agent repo and transient files).

`synapse import` restores from a backup zip, overlaying onto the current
SYNAPSE_HOME root.
"""
...
from synapse_constants import get_default_synapse_root, get_synapse_home, display_synapse_home
from utils import (
    _preserve_file_mode,
...
from synapse_cli.sizefmt import format_bytes as _format_size
...
# Where ``synapse backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``synapse-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/synapse-agent/``
...
# SYNAPSE_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "synapse-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profiles",
    # Python dependency trees (plugin / MCP-server venvs under SYNAPSE_HOME) —
...
# File names that ``synapse import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE SYNAPSE_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Synapse backup slot."""
...
@contextmanager
def _backup_operation_lock(synapse_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = synapse_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Synapse backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside SYNAPSE_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to synapse root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``synapse-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/synapse-agent/`` — must be preserved.
        if part == "synapse-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SYNAPSE_HOME.
...
# to the user as a hung `synapse update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # synapse_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Synapse home directory."""
    synapse_root = get_default_synapse_root()

    if not synapse_root.is_dir():
        print(f"Error: Synapse home directory not found at {synapse_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(synapse_root):
            _run_backup_locked(args, synapse_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, synapse_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"synapse-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"synapse-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_synapse_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(synapse_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(synapse_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``synapse-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "synapse-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(synapse_root)
    ...
    # outside SYNAPSE_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_synapse_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: synapse import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Synapse backup.
    ...
    # Look for telltale files that a synapse home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Synapse backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.synapse/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a synapse dir name
        if prefix in {".synapse", "synapse"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``synapse import``, which is the
    ...
    so without the chown a ``sudo synapse import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``synapse import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Synapse' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_synapse_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (synapse_root / "config.yaml").exists()
        has_env = (synapse_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Synapse configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under SYNAPSE_HOME.
            ...
            target = synapse_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(synapse_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_synapse_home()}."
            )
        ...
        profiles_dir = synapse_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # synapse_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: synapse profile list  (after installing synapse)")

        # Guidance
        print()
        if not (synapse_root / "synapse-agent").is_dir():
            print("Note: The synapse-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: synapse update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  synapse -p {pname} gateway install")
        ...
        try:
            from synapse_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Synapse configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and synapse backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SYNAPSE_HOME).
...
# — `synapse update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $SYNAPSE_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside SYNAPSE_HOME and the entry is silently
...
def _quick_snapshot_root(synapse_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: synapse snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(synapse_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside SYNAPSE_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``synapse update``.
    ...
    Args:
        ...
        synapse_home: Override for the Synapse home directory (tests).
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, synapse_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(synapse_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for synapse backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_synapse_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, synapse_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(synapse_root):
            return _write_full_zip_backup_locked(out_path, synapse_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, synapse_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``synapse_root`` to ``out_path``.
...
def _pre_update_backup_dir(synapse_home: Optional[Path] = None) -> Path:
    home = synapse_home or get_synapse_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SYNAPSE_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SYNAPSE_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``synapse update``) should continue even if the backup fails.
    """
    synapse_root = synapse_home or get_default_synapse_root()
    if not synapse_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(synapse_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, synapse_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `synapse claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SYNAPSE_HOME under ``backups/`` before a
    ``synapse claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``synapse import <archive>``.  Writes to
    ``<SYNAPSE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `synapse import` and the
