"""synapse import-agent — import Claude Code / Codex CLI setups into Synapse.

Usage:
    synapse import-agent                       # auto-detect ~/.claude or ~/.codex
    synapse import-agent claude-code           # import from ~/.claude
    synapse import-agent codex                 # import from ~/.codex
    synapse import-agent claude-code --dry-run # preview only, no changes
    synapse import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``synapse claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_synapse.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in SYNAPSE_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → SYNAPSE_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in SYNAPSE_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in SYNAPSE_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → SYNAPSE_HOME/skills/codex-imports/<name>/
...
deliberately via ``synapse setup`` or config.yaml.
"""
...
# Same entry delimiter as the Synapse memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`synapse config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_synapse.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Synapse command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Synapse glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Synapse equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        synapse_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → SYNAPSE_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``synapse import-agent`` (invoked from synapse_cli.main)."""
    from synapse_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: synapse import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: synapse import-agent claude-code   or   synapse import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Synapse — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: synapse import-agent "
    ...
    print_info(f"Target:      {synapse_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'synapse setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: synapse import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'synapse setup' "
               "to configure providers, or add them to ~/.synapse/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from synapse_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'synapse setup' or ~/.synapse/.env.")
        print()
