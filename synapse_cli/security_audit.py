"""On-demand supply-chain audit for Synapse Agent installs.

Scans three surfaces a Synapse user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Synapse venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.synapse/plugins``
...
def _discover_plugins(synapse_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.synapse/plugins``.
...
def _discover_components(
    *,
    ...
    home = synapse_home or Path(get_synapse_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `synapse security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, synapse_home=home
    )
