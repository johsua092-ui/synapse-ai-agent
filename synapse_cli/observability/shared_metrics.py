"""Durable aggregation and local export for Synapse shared metrics."""
...
from synapse_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "synapse.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_synapse_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "synapse_version": row["synapse_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Synapse processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "synapse_version": period_row["synapse_version"],
