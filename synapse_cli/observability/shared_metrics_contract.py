"""Bounded product contract for the first Synapse shared-metrics slice."""
...
SCHEMA_KEY = "synapse.metrics.schema_version"
SCHEMA_VERSION = "synapse.metrics.event.v2"
MODEL_CALL_SCOPE = "synapse.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "synapse.task_run"
TOOL_CALL_SCOPE = "synapse.tool_call"
CLIENT_ACTIVE_MARK = "synapse.client.active"
TOOL_APPROVAL_MARK = "synapse.tool_approval"
SKILL_LIFECYCLE_MARK = "synapse.skill.lifecycle"
SKILL_LOAD_MARK = "synapse.skill.load"
SUBSCRIBER_NAME = "synapse.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "synapse.client.active"
LEGACY_MODEL_CALL_METRIC = "synapse.model_call.count"
MODEL_ROUTE_METRIC = "synapse.model_route.count"
TASK_STARTED_METRIC = "synapse.task_run.started"
TASK_FINISHED_METRIC = "synapse.task_run.finished"
TOOL_CALL_METRIC = "synapse.tool_call.count"
TOOL_APPROVAL_METRIC = "synapse.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "synapse.skill.lifecycle.count"
SKILL_LOAD_METRIC = "synapse.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Synapse installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(synapse_version or "").strip()
    ...
    return {
        ...
        "synapse_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("synapse_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Synapse logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("synapse.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from synapse_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Synapse terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Synapse registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "synapse-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Synapse tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Synapse."""
