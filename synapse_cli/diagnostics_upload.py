"""Client for uploading ``synapse debug share`` bundles to Nous-internal S3.

This is the opt-in (``--nous``) destination for ``synapse debug share``.
...
NAS_BASE = os.environ.get(
    "SYNAPSE_DIAGNOSTICS_BASE_URL", "https://"
)
...
_USER_AGENT = "synapse-agent/debug-share"
