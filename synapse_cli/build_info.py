"""
Baked-in build metadata for Synapse Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``synapse_cli/dump.py`` and ``synapse_cli/banner.py``).  That doesn't work inside
...
To make ``synapse dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$SYNAPSE_GIT_SHA``
arg into ``<project_root>/.synapse_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``SYNAPSE_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".synapse_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    rev-parse`` for source installs, the baked ``.synapse_build_sha`` for
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.synapse_build_sha`` if present.  The file is
    written by the Dockerfile's ``SYNAPSE_GIT_SHA`` build-arg and contains
