...
def _is_synapse_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Synapse ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Synapse gateway shapes
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``synapse update``
    ...
    if not _is_launchd_supervised(environ):
        return argv
    if not _is_synapse_gateway_run_argv(argv):
        return argv
