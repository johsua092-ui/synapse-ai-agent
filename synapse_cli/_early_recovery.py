"""Dependency-light venv recovery that runs BEFORE synapse_cli.main's imports.

The ``synapse`` console entry point is ``synapse_cli.main:main``.  Importing
``synapse_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``synapse_cli.env_loader``, ``yaml`` via ``synapse_cli.config``, ...).  In
...
a corrupted venv.  ``synapse_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``synapse_cli.main`` to become
...
#
# ``synapse update`` renames the live ``synapse*.exe`` shims aside
# (``synapse.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``synapse`` on PATH, and the command that would repair it IS ``synapse
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY synapse
    invocation, and ``synapse acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Synapse already vendors
    (``~/.synapse/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".synapse" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``synapse acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``synapse_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``synapse_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``synapse update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `synapse update`: CLI and Desktop retries preserve
...
# every `synapse` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``synapse update`` (or the self-lock preflight, #83569) left the
    ...
    ``synapse_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``synapse acp`` included).
    ...
    """
    try:
        from synapse_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; synapse will keep working from "
