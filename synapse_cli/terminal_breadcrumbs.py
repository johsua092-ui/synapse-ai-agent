"""Per-terminal session breadcrumbs for ``synapse -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$SYNAPSE_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``synapse -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_synapse_home() / "terminal-sessions"
