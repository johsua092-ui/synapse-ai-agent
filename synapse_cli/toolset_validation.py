...
the tool registry or launching Synapse (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``synapse-cli`` to the non-existent ``synapse``. ``resolve_toolset('synapse')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``synapse-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``synapse`` instead of ``synapse-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `synapse tools` to reconfigure."
        )
    return warnings
