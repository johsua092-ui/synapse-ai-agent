"""`synapse checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.synapse/checkpoints/``.  Actions:

    synapse checkpoints               # same as `status`
    synapse checkpoints status        # total size, project count, breakdown
    synapse checkpoints list          # per-project checkpoint counts + workdir
    synapse checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    synapse checkpoints clear [-f]    # nuke the entire base (asks first)
    synapse checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    synapse checkpoints
    synapse checkpoints prune --retention-days 3 --max-size-mb 200
    synapse checkpoints clear -f
...
from synapse_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: synapse checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``synapse checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `synapse checkpoints` → status
