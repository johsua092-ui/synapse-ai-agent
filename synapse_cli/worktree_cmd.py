"""``synapse worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``synapse_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    synapse worktree list             # audit: verdict + reason per tree
    synapse worktree prune            # reap safe trees + merged branches
    synapse worktree prune --dry-run  # show the plan, change nothing
    synapse worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `synapse worktree prune`."
        )
