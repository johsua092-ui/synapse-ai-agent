"""synapse webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.synapse/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _synapse_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: synapse gateway run
"""
...
def webhook_command(args):
    """Entry point for 'synapse webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: synapse webhook {subscribe|list|remove|test}")
        print("Run 'synapse webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (synapse gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: synapse webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from synapse webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (synapse gateway run)")
