...
Profile-aware location: ``$SYNAPSE_HOME/logs/dashboard-auth.log``.
...
from ``synapse_constants`` or other synapse_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SYNAPSE_HOME/logs/dashboard-auth.log``.

    Uses ``synapse_constants.get_synapse_home()`` (a leaf module — no import
    ...
    return get_synapse_home() / "logs" / "dashboard-auth.log"
