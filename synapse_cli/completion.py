"""Shell completion script generation for synapse CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_synapse_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Synapse Agent bash completion
# Add to ~/.bashrc:
#   eval "$(synapse completion bash)"

_synapse_profiles() {{
    local profiles_dir="$HOME/.synapse/profiles"
...
_synapse_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_synapse_profiles)" -- "$cur"))
        return
    fi
...
complete -F _synapse_completion synapse
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef synapse
# Synapse Agent zsh completion
# Add to ~/.zshrc:
...
_synapse_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.synapse/profiles" ]]; then
        profiles+=($HOME/.synapse/profiles/*(N/:t))
    fi
...
_synapse() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_synapse_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'synapse command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Synapse Agent fish completion",
        "# Add to your config:",
        "#   synapse completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __synapse_profiles",
        "    echo default",
        "    if test -d $HOME/.synapse/profiles",
        "        for d in $HOME/.synapse/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c synapse -f -s p -l profile"
        " -d 'Profile name' -xa '(__synapse_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__synapse_profiles)' -d 'Profile name'"
                )
