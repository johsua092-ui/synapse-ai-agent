...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.synapse/.env`` into the gateway process via
    ``synapse_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from synapse_cli.config import reload_env
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m synapse_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "synapse_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m synapse_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=synapse_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from synapse_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind SYNAPSE_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_synapse_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("SYNAPSE_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("SYNAPSE_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_synapse_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off SYNAPSE_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from synapse_constants import set_synapse_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from synapse_constants import reset_synapse_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``synapse journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from synapse_cli.skills_hub import do_install
        ...
        if action == "browse":
            from synapse_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from synapse_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `synapse mcp` offers, so
    ...
    try:
        ...
        from synapse_cli import mcp_catalog
...
# (synapse_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes SYNAPSE_HOME via set_synapse_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses synapse_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from synapse_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from synapse_cli.config import load_config, save_config, save_env_value
        from synapse_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from synapse_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.synapseDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``synapse mcp login`` uses
    ...
    try:
        ...
        synapse_home = str(get_synapse_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(synapse_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``synapse plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.synapse/plugins/`` (non-interactive).
    ...
    plugins live under each profile's SYNAPSE_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Synapse plugin.
