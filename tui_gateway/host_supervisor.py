...
def _default_registry_path() -> Path:
    return get_synapse_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_synapse_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_synapse_home = expected_synapse_home if expected_synapse_home is not None else str(get_synapse_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["SYNAPSE_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("synapse_home") or "")
        if got_home and got_home != self.expected_synapse_home:
            raise RuntimeError(f"compute host SYNAPSE_HOME mismatch: {got_home} != {self.expected_synapse_home}")
