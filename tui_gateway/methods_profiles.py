...
handlers delegate to the same `synapse_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Synapse profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(profile_path):
        ...
        describe the same session (synapse-agent#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(profile_path):
        ...
        sessions never surface in conversation lists (synapse-agent#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's synapse-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``synapse setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (synapse_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local SYNAPSE_HOME override — the same
        ...
        try:
            ...
            token = set_synapse_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from synapse_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from synapse_cli.config import load_config_readonly, read_user_config_raw
            from synapse_cli.web_routers.profiles import _write_profile_model
            from synapse_constants import (
                reset_synapse_home_override,
                set_synapse_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the SYNAPSE_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from synapse_cli.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `synapse tools`
            ...
            # composites (synapse-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from synapse_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `synapse tools` / Settings is where
            ...
            description = ""
            try:
                from synapse_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from synapse_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from synapse_cli.skills_config import save_disabled_skills
