...
# from shadowing Synapse's own top-level modules.  ``synapse_bootstrap`` lives at
...
import synapse_bootstrap

synapse_bootstrap.harden_import_path()
...
# shared owner in synapse_cli.mcp_startup — the wait/in-flight/join helpers
...
# synapse_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `SYNAPSE_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("SYNAPSE_TUI_SIDECAR_URL")
...
# tested; override via ``SYNAPSE_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("SYNAPSE_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``SYNAPSE_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``synapse --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``synapse_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from synapse_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → synapse_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's SYNAPSE_HOME first), so a launch profile with no
    ...
    try:
        from synapse_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``synapse --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``synapse dashboard`` spawn theirs via
    ``synapse_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``synapse_cli.mcp_startup`` thread used by the
    ...
    try:
        from synapse_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from synapse_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's SYNAPSE_HOME override — the shared owner in
    ``synapse_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Synapse activates mid-conversation.
    ...
    try:
        from synapse_cli.model_switch import prewarm_picker_cache_async
