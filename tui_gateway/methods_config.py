    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero Synapse sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from synapse_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_synapse_home), "display": display_synapse_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _synapse_home / "config.yaml"
        try:
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Synapse provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    collection + force-redaction pipeline as ``synapse debug share --nous``
