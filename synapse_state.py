#!/usr/bin/env python3
"""
SQLite State Store for Synapse Agent.
...
# transitive dependency via synapse_state_common). run_agent keeps its own
# predating copy — synapse_state cannot import run_agent (circular) — guarded
...
from synapse_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from synapse_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from synapse_state_portability import SessionPortabilityMixin
from synapse_state_schema import SessionSchemaMixin
from synapse_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``synapse -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_synapse_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.synapse`` even when a test fixture later
    redirects ``SYNAPSE_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_synapse_home()`` fresh so a runtime
       ``SYNAPSE_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_synapse_home() / "state.db"
...
# ~/.synapse/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects SYNAPSE_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without SYNAPSE_HOME, a stale worktree
# without the re-pin, or a developer shell that exports SYNAPSE_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "SYNAPSE_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.synapse``).  The test conftest injects the pre-sandbox production
#: root here so custom-``SYNAPSE_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Synapse root for the guard.

    Deliberately avoids ``Path.home()`` / ``synapse_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``synapse_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "synapse"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "synapse"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".synapse"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``SYNAPSE_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Synapse test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``SYNAPSE_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "SYNAPSE_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``SYNAPSE_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``synapse`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Synapse home *root*.
    ...
    under ``~/.synapse/synapse-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``SYNAPSE_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``SYNAPSE_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Synapse root {root}). "
                "Tests must run against a temporary SYNAPSE_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "SYNAPSE_HOME. If this test genuinely needs the live "
...
# synapse_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``synapse sessions optimize`` (FTS merge + VACUUM)
    ...
    ``synapse_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``synapse_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``synapse update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Synapse-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Synapse"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Synapse-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `synapse doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see synapse_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with synapse_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``synapse doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Synapse host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``synapse serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Synapse's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``synapse_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship synapse_state without synapse_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``synapse_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Synapse home tree (``get_synapse_home()``) — the safe repair scope:
      Synapse owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``synapse_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_synapse_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Synapse needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (synapse_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_synapse_state.py:600-632)
        ...
        probe_session_id = f"_synapse_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Synapse deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (synapse gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.synapse/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``synapse sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("SYNAPSE_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_synapse_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("SYNAPSE_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship synapse_state without synapse_cli). A
    ...
    try:
        ...
        logger.debug(
            "synapse_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch synapse_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``synapse_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `synapse snapshot list` / "
                "`synapse snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (synapse doctor, dashboards) ──────────
...
_SYNAPSE_CMDLINE_MARKERS = ("synapse_cli.main", "synapse_cli/main", "synapse serve",
                           "synapse-agent", "synapse gateway", "synapse chat")


def _looks_like_synapse(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Synapse process?
    ...
    We only flag processes that look like Synapse, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _SYNAPSE_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple synapse processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Synapse
    ...
    # `synapse update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `synapse snapshot list` / "
                    f"`synapse snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`synapse db optimize`),
            ...
            # ``synapse_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `synapse update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `synapse sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Synapse process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Synapse process —
                        # only flag uninspectable holders that look like
                        # another Synapse instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_synapse(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from synapse_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `synapse sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`synapse sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``synapse sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``synapse --resume`` into a fresh
    ...
    # of truth: synapse_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: synapse_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``synapse -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``synapse sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``SYNAPSE_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Synapse, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Synapse session.

        A Synapse session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Synapse session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `synapse sessions vacuum` runs in a transient
