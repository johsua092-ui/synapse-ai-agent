...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with synapse-agent)
2. User plugins: ``$SYNAPSE_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``synapse_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SYNAPSE_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SYNAPSE_HOME/plugins/model-providers/`` if it exists."""
    try:
        from synapse_constants import get_synapse_home

        d = get_synapse_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SYNAPSE_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_synapse_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``synapse_agent.plugins``
    ...
    * **Provider targets only.** The ``synapse_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$SYNAPSE_HOME``) keep their
    ...
    try:
        from synapse_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SYNAPSE_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``synapse_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$SYNAPSE_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with synapse-agent.
    ...
    # 2. User plugins — under $SYNAPSE_HOME/plugins/model-providers/<name>/.
