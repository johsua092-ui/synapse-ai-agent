...
This plugin ships bundled with Synapse but is **opt-in** — it only loads when
...
synapse tools  # → Langfuse Observability
...
Set these in `~/.synapse/.env` (or via `synapse tools`):
...
synapse chat -q "hello"              # then check Langfuse for a "Synapse turn" trace
```

Generation observations include the Synapse system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `SYNAPSE_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
SYNAPSE_LANGFUSE_ENV=production       # environment tag
SYNAPSE_LANGFUSE_RELEASE=v1.0.0       # release tag
SYNAPSE_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
SYNAPSE_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
SYNAPSE_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
SYNAPSE_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`SYNAPSE_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
synapse plugins disable observability/langfuse
```
