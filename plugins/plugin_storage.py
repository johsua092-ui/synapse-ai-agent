...
``<synapse home>/plugins/<name>/``. That tree is the plugin *install* dir —
``synapse plugins remove`` deletes it and ``synapse plugins update`` git-pulls
...
``<synapse home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Synapse.
...
# Mirrors the plugin-name shape `synapse plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<synapse home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`synapse_constants.get_synapse_home` on every call. Don't cache the
    result across profile switches.
    """
    from synapse_constants import get_synapse_home

    root = get_synapse_home() / "plugin-data" / _validate_name(name)
