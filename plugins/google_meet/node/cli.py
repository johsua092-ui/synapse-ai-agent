"""`synapse meet node ...` subcommand tree.

Wired into the existing ``synapse meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``synapse meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="synapse-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``synapse meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             synapse meet node approve <name> ws://<host>:{args.port} {token}")
        try:
