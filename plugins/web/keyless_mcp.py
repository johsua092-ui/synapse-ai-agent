...
two endpoints so a fresh Synapse install with **zero web credentials** still
...
def provider_tier(name: str) -> str:
    ...
    ``synapse tools`` picker's Free/Paid rows). ``free`` forces the keyless
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "backend via `synapse tools` for reliable service."
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "via `synapse tools` for reliable service."
            ),
        }
