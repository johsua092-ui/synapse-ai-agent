...
``{title, url, description, position}`` rows every other Synapse web
provider produces.
...
prefers Synapse-managed xAI Grok OAuth (via ``synapse auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.synapse/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``synapse tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `synapse auth` to sign in with "
