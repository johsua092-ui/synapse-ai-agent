/**
 * Synapse Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.synapse/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SYNAPSE_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SYNAPSE_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "synapse-kanban-dot-triage",
    todo: "synapse-kanban-dot-todo",
    ready: "synapse-kanban-dot-ready",
    running: "synapse-kanban-dot-running",
    blocked: "synapse-kanban-dot-blocked",
    review: "synapse-kanban-dot-review",
    done: "synapse-kanban-dot-done",
    archived: "synapse-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-synapse-task";
  ...
  const DOCS_URL = "https:///docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https:///docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "synapse.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="synapse-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])synapse-kanban-md-code\1/i.test(attrs)) {
      return ' class="synapse-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "synapse-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "synapse-kanban-md",
  ...
  // standard `drop` event and our `synapse-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("synapse-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("synapse-kanban-column--drop");
          if (target) target.classList.add("synapse-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("synapse-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("synapse-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("synapse-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SYNAPSE_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "synapse-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "synapse-kanban-attention",
        "synapse-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "synapse-kanban-attention-bar" },
        h("span", { className: "synapse-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "synapse-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "synapse-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "synapse-kanban-attention-row",
                  "synapse-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "synapse-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "synapse-kanban-attention-row-id" }, task.id),
                h("span", { className: "synapse-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "synapse-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "synapse-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "synapse-kanban-diag-action-btn",
      action.suggested ? "synapse-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " synapse-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".synapse-kanban-drawer-comment-row input, .synapse-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "synapse-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("synapse-kanban-diag", sevClass) },
      h("div", { className: "synapse-kanban-diag-header" },
        h("span", { className: "synapse-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "synapse-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "synapse-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "synapse-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "synapse-kanban-diag-data-row" },
                  h("span", { className: "synapse-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "synapse-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "synapse-kanban-diag-data-row" },
                h("span", { className: "synapse-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "synapse-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "synapse-kanban-diag-reassign-row" },
            h("span", { className: "synapse-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "synapse-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "synapse-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "synapse-kanban-diag-msg",
              msg.ok ? "synapse-kanban-diag-msg--ok" : "synapse-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "synapse-kanban-section" },
      h("div", { className: "synapse-kanban-section-head-row" },
        h("span", { className: "synapse-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "synapse-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "synapse-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "synapse-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "synapse-kanban-docs-link",
      title: "Open Synapse Kanban docs in a new tab",
      "aria-label": "Synapse Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `synapse kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "synapse-kanban-msg-ok" : "synapse-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "synapse-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "synapse-kanban-boardswitcher" },
      h("div", { className: "synapse-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "synapse-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "synapse-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "synapse-kanban-dialog-title" },
        ...
        h("div", { className: "synapse-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Synapse profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "synapse-kanban-bulk" },
      h("span", { className: "synapse-kanban-bulk-count" },
      ...
      h("div", { className: "synapse-kanban-bulk-priority",
      ...
      h("div", { className: "synapse-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Synapse profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "synapse-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("synapse-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("synapse-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "synapse-kanban-trash",
        dragOver ? "synapse-kanban-trash--drop" : "",
        props.draggingTaskId ? "synapse-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "synapse-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "synapse-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".synapse-kanban-card")) return true;
      if (target.closest && target.closest(".synapse-kanban-column-add")) return true;
      if (target.closest && target.closest(".synapse-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("synapse-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("synapse-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".synapse-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "synapse-kanban-columns",
        isScrollable ? "synapse-kanban-columns--scrollable" : "",
        isPanning ? "synapse-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("synapse-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("synapse-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "synapse-kanban-column",
        dragOver ? "synapse-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "synapse-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "synapse-kanban-col-check",
        ...
        h("span", { className: cn("synapse-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "synapse-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "synapse-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "synapse-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "synapse-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "synapse-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "synapse-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "synapse-kanban-lane" },
                  h("div", { className: "synapse-kanban-lane-head" },
                    h("span", { className: "synapse-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "synapse-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "synapse-kanban-card--stale-red";
    if (age >= tier.amber) return "synapse-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".synapse-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "synapse-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "synapse-kanban-card",
        props.selected ? "synapse-kanban-card--selected" : "",
        props.failed ? "synapse-kanban-card--failed" : "",
        props.draggingSource ? "synapse-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "synapse-kanban-card-content" },
          h("div", { className: "synapse-kanban-card-row" },
            h("label", {
              className: "synapse-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "synapse-kanban-card-check",
            ...
            h("span", { className: "synapse-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or synapse kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "synapse-kanban-warning-badge",
                    "synapse-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "synapse-kanban-needs-assignee",
          ...
          h("div", { className: "synapse-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "synapse-kanban-card-row synapse-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "synapse-kanban-assignee",
                            title: `Assigned to Synapse profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "synapse-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "synapse-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "synapse-kanban-ago",
  ...
  // centered modal reusing the synapse-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "synapse-kanban-dialog synapse-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Synapse profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Synapse profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "synapse-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "synapse-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "synapse-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "synapse-kanban-drawer-close",
        ...
        data ? h("div", { className: "synapse-kanban-drawer-comment-foot" },
          h("div", {
            className: "synapse-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "synapse-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "synapse-kanban-section" },
      h("div", { className: "synapse-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "synapse-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "synapse-kanban-drawer-body" },
      h("div", { className: "synapse-kanban-drawer-title" },
        h("span", { className: cn("synapse-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "synapse-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "synapse-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "synapse-kanban-section" },
            h("div", { className: "synapse-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "synapse-kanban-section" },
            h("div", { className: "synapse-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "synapse-kanban-done-no-result synapse-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "synapse-kanban-section" },
            h("div", { className: "synapse-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "synapse-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "synapse-kanban-section" },
        h("div", { className: "synapse-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "synapse-kanban-comment" },
            h("div", { className: "synapse-kanban-comment-head" },
              h("span", { className: "synapse-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "synapse-kanban-diag-action-btn",
      ...
      h("div", { className: "synapse-kanban-section" },
        h("div", { className: "synapse-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "synapse-kanban-comment" },
                h("div", { className: "synapse-kanban-comment-head" },
                  h("span", { className: "synapse-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "synapse-kanban-comment-ago" },
      ...
      h("div", { className: "synapse-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "synapse-kanban-event",
              isDiag ? "synapse-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "synapse-kanban-event-header" },
                  h("span", { className: "synapse-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "synapse-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "synapse-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "synapse-kanban-event-header-plain" },
                  h("span", { className: "synapse-kanban-event-kind" }, e.kind),
                  h("span", { className: "synapse-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "synapse-kanban-event-phantom-row" },
                  h("span", { className: "synapse-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "synapse-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "synapse-kanban-event-payload" },
    ...
    return h("div", { className: "synapse-kanban-section" },
      h("div", { className: "synapse-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "synapse-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `synapse-kanban-run--${r.outcome || r.status || "ended"}`
          : "synapse-kanban-run--active";
        return h("div", { key: r.id, className: cn("synapse-kanban-run", outcomeClass) },
          h("div", { className: "synapse-kanban-run-head" },
            h("span", { className: "synapse-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "synapse-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "synapse-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "synapse-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "synapse-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "synapse-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "synapse-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "synapse-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "synapse-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "synapse-kanban-pre synapse-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "synapse-kanban-meta-row" },
      h("span", { className: "synapse-kanban-meta-label" }, props.label),
      h("span", { className: "synapse-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "synapse-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "synapse-kanban-meta-row" },
        h("span", { className: "synapse-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "synapse-kanban-meta-value synapse-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "synapse-kanban-meta-row" },
        h("span", { className: "synapse-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "synapse-kanban-meta-value synapse-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "synapse-kanban-meta-row" },
        h("span", { className: "synapse-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "synapse-kanban-meta-value synapse-kanban-editable",
    ...
    return h("div", { className: "synapse-kanban-meta-row" },
      h("span", { className: "synapse-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "synapse-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "synapse-kanban-section" },
      h("div", { className: "synapse-kanban-section-head-row" },
        h("span", { className: "synapse-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "synapse-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "synapse-kanban-section" },
      h("div", { className: "synapse-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "synapse-kanban-deps-row" },
        h("span", { className: "synapse-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "synapse-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "synapse-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "synapse-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "synapse-kanban-dep-chip-x",
      ...
      h("div", { className: "synapse-kanban-deps-row" },
        h("span", { className: "synapse-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "synapse-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "synapse-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "synapse-kanban-msg-ok"
          : "synapse-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "synapse-kanban-section" },
      ...
      h("div", { className: "synapse-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "synapse-kanban-home-sub synapse-kanban-home-sub--on"
              : "synapse-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SYNAPSE_PLUGINS__ && typeof window.__SYNAPSE_PLUGINS__.register === "function") {
    window.__SYNAPSE_PLUGINS__.register("kanban", KanbanPage);
  }
})();
