"""Meta Model API (Muse Spark) provider plugin for Synapse Agent.
...
Bundled from https://github.com/albertodepaola/synapse-meta-provider. Synapse'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to synapse' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Synapse' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in synapse_cli/providers.py and the retention hint in
