...
Resolution order for config file:
  1. $SYNAPSE_HOME/honcho.json  (instance-local, enables isolated Synapse instances)
...
from synapse_cli.profiles import _get_default_synapse_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Synapse profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Synapse profile.

    Resolution order:
      1. SYNAPSE_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``synapse_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"synapse"`` (default profile)
    """
    explicit = os.environ.get("SYNAPSE_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named Synapse
    # profile share that same host.  Keep named profiles isolated; only the
    # default Synapse profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $SYNAPSE_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.synapse/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_synapse_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_synapse_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.synapse
    ...
    # resolve_config_path()/get_synapse_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $SYNAPSE_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Synapse profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.synapse block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the Synapse-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Synapse session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Synapse session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_synapse_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_synapse_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'synapse honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.synapse_home) if config.synapse_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/synapse_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'synapse honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `synapse honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                synapse_cfg = load_config()
                honcho_cfg = synapse_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
