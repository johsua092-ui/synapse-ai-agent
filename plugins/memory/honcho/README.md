...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/synapse>
...
```bash
synapse memory setup honcho   # configure Honcho directly (works on a fresh install)
synapse memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
synapse config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.synapse/.env
```

> `synapse honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `synapse memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `synapse model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SYNAPSE_HOME/honcho.json` | Profile-local (isolated Synapse instances) |
| 2 | `~/.synapse/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Synapse profile: `synapse` (default) or `synapse_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `synapse honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Synapse session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`synapse-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`synapse`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-synapse-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening synapse in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Synapse session ID (timestamp + hex). Every `synapse` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Synapse profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`synapse`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Synapse profile: `synapse` (default) or `synapse_<profile>` (e.g. `synapse -p coder` -> host key `synapse_coder`). Older `synapse.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `SYNAPSE_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `synapse-agent`) |
...
|---------|-------------|
| `synapse memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `synapse honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `synapse memory setup`) |
| `synapse honcho status` | Show resolved config for active profile |
| `synapse honcho enable` / `disable` | Toggle Honcho for active profile |
| `synapse honcho mode <mode>` | Change recall or observation mode |
| `synapse honcho peer --user <name>` | Update user peer name |
| `synapse honcho peer --ai <name>` | Update AI peer name |
| `synapse honcho tokens --context <N>` | Set context token budget |
| `synapse honcho tokens --dialectic <N>` | Set dialectic max chars |
| `synapse honcho map <name>` | Map current directory to a session name |
| `synapse honcho sync` | Create host blocks for all Synapse profiles |
