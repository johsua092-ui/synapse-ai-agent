...
``synapse://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Synapse OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "synapse-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``synapse-desktop`` / ``synapse-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Synapse.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local SYNAPSE_HOME override can't reach.
