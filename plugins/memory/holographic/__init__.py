"""synapse-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $SYNAPSE_HOME/config.yaml (profile-scoped):
  plugins:
    synapse-memory-store:
      db_path: $SYNAPSE_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "synapse-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, synapse_home):
        """Write config to config.yaml under plugins.synapse-memory-store."""
        ...
        try:
            ...
            existing["plugins"]["synapse-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_synapse_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _synapse_home = str(get_synapse_home())
        _default_db = _synapse_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $SYNAPSE_HOME in user-supplied paths so config values like
        # "$SYNAPSE_HOME/memory_store.db" or "~/.synapse/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$SYNAPSE_HOME", _synapse_home)
            db_path = db_path.replace("${SYNAPSE_HOME}", _synapse_home)
