...
```bash
synapse memory setup    # select "mem0"
```
...
Behavioral settings live in `$SYNAPSE_HOME/mem0.json` (set them via `synapse memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.synapse/.env`.
...
| `user_id` | `synapse-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Synapse session and call `mem0_search` — it connects to your server.
...
```bash
synapse memory setup mem0 --mode oss \
...
```bash
synapse memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SYNAPSE_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.synapse/mem0_qdrant"}}
  }
}
```
...
```bash
synapse memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$SYNAPSE_HOME/mem0.json`).
