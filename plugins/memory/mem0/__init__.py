...
Configuration
-------------
Secret (lives in $SYNAPSE_HOME/.env or the environment):
...
Behavioral settings (live in $SYNAPSE_HOME/mem0.json, set via `synapse memory
setup`):
  ...
  agent_id           — Agent identifier (default: synapse)
...
def _load_config() -> dict:
    """Load config from env vars, with $SYNAPSE_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "synapse"),
        "oss": {},
    }
    ...
    config_path = get_synapse_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, synapse_home):
        """Write config to $SYNAPSE_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, synapse_home: str, config: dict) -> None:
        ...
        post_setup(synapse_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $SYNAPSE_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "synapse")
