...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # synapse_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(synapse_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(synapse_home) / "mem0.json"
    existing = {}
...
def _setup_platform(synapse_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "synapse-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "synapse"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.synapse/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.synapse/.env (or unset it) or Synapse will keep "
    ...
    provider.save_config(provider_config, synapse_home)

    if env_writes:
        _write_env(Path(synapse_home) / ".env", env_writes)
...
def _setup_selfhosted(synapse_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "synapse-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "synapse")
...
def _setup_oss(synapse_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(synapse_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "synapse-user")
    ...
    if env_writes:
        _write_env(Path(synapse_home) / ".env", env_writes)
    _save_mem0_json(synapse_home, {"mode": "oss", "user_id": user_id, "agent_id": "synapse", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, synapse_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # synapse_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "synapse-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "synapse"
...
def _setup_oss_interactive(synapse_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], synapse_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], synapse_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'synapse-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "synapse-user")

    agent_id = input("  Agent ID [synapse]: ").strip()
    agent_id = agent_id or "synapse"
    ...
    if env_writes:
        _write_env(Path(synapse_home) / ".env", env_writes)
    _save_mem0_json(synapse_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(synapse_home: str, config: dict) -> None:
    """Entry point called by synapse memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(synapse_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(synapse_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(synapse_home, config, flags)
        return
