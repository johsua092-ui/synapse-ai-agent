...
- Hosted: API key from [app.supermemory.ai/integrations?connect=synapse](http://app.supermemory.ai/integrations?connect=synapse)
...
Before running `synapse memory setup`, add the local endpoint to
...
Then run `synapse memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SYNAPSE_HOME/supermemory.json`
...
| `container_tag` | `synapse` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `synapse-{identity}` → `synapse-coder`). |
...
`https://api.supermemory.ai`. Synapse resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: synapse`, and document writes stamp
`metadata.sm_source: synapse`. This is a **functional routing key, not telemetry**:
it groups Synapse-written memories into a dedicated "Synapse" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Synapse profile:

```json
{
  "container_tag": "synapse-{identity}"
}
```

For a profile named `coder`, this resolves to `synapse-coder`. The default profile resolves to `synapse-default`. Without `{identity}`, all profiles share the same container.
