...
# ``model_tools`` import time, so ``synapse status``, ``synapse chat``, etc.
...
# ``synapse_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Synapse is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a synapse MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "synapse-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SYNAPSE_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from synapse_constants import get_synapse_home as _get_synapse_home
            _synapse_home = _get_synapse_home()
        except (ModuleNotFoundError, ImportError):
            _synapse_home = _Path.home() / ".synapse"
        self._thread_count_store = _ThreadCountStore(
            _synapse_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Synapse is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SYNAPSE_HOME", str(_Path.home() / ".synapse"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SYNAPSE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a synapse MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Synapse is thinking…' marker message.
        ...
        "Synapse is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Synapse is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Synapse is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Synapse is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``synapse setup``.

    The setup wizard at ``synapse_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.synapse/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. synapse-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.synapse/.env")
    print_info("Restart the gateway: synapse gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``synapse cron`` running as a
    separate process from ``synapse gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Synapse plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `synapse setup` to install Google Chat support.",
        ...
        # the "Synapse is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Synapse is thinking…' marker message "
