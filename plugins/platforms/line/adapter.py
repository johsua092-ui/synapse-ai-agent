"""
LINE Messaging API platform adapter for Synapse Agent.
...
to Synapse Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SYNAPSE_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            synapse_home = Path(get_synapse_home()).resolve()
        except Exception:
            synapse_home = Path.home().joinpath(".synapse").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``synapse status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``synapse status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``synapse setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.synapse/.env`` via ``synapse_cli.config``.
    """
    print()
    ...
    try:
        from synapse_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("synapse_cli.config not available; set LINE_* vars manually in ~/.synapse/.env")
        return
