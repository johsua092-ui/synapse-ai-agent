...
This plugin connects Synapse Agent to iMessage (and other Spectrum
...
TypeScript-only, Synapse runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Synapse channel,
...
Run `synapse photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.synapse/.env` (the same place every other
...
Management metadata lives in `~/.synapse/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `synapse photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Synapse
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Synapse coalesces
...
- **Native polls are supported.** Synapse posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `synapse send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<synapse-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `synapse photon status`, a DM and a group roundtrip,
