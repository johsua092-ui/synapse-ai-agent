// Synapse Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Synapse): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Synapse -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Synapse-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `synapse photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Synapse can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "synapse-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Synapse'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Synapse allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Synapse, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-synapse-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Synapse supplied them so a known-good
