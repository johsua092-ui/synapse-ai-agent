...
Small Node helper that bridges Synapse Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Synapse is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Synapse-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Synapse can receive inbound messages
...
The Synapse plugin's `synapse photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Synapse is Python, so replies go through this sidecar
