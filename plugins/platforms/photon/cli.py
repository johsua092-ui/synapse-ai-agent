"""
``synapse photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Synapse
...
# Writable sidecar runtime dir (mirrors to SYNAPSE_HOME on immutable
...
# wiring imports this module for `synapse --help`).
...
# when called later (gateway start, synapse status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `synapse photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Synapse Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``synapse photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Synapse Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.synapse/.env,
    ...
    try:
        from synapse_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  synapse gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `synapse photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`synapse photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.synapse/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `synapse photon telemetry on` / `synapse photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.synapse/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  synapse gateway restart")
    return 0
...
#
# `synapse gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``synapse photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `synapse gateway setup` wizard."""
