"""ntfy platform adapter (Synapse plugin).
...
a Synapse dependency.

This adapter ships as a Synapse platform plugin under
``plugins/platforms/ntfy/``. The Synapse plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "synapse-in"              # subscribe topic (incoming)
          publish_topic: "synapse-out"     # optional — defaults to topic
...
_ECHO_TAG = "synapse-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``synapse cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Synapse dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `synapse gateway status` without
