"""
Email platform adapter for the Synapse gateway.

Allows users to interact with Synapse by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "synapse-agent" "version" "{_synapse_version}" '
            '"vendor" "Josh Research" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `synapse config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `synapse gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Synapse Agent")
        ...
        msg_id = f"<synapse-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# synapse_cli/gateway.py, and the _send_email dispatch in
