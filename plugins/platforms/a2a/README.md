# A2A — Agent-to-Agent protocol for Synapse
...
peer (another Synapse, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
synapse gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Synapse serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.synapse/a2a_audit.jsonl`.
- Conversations persist to `~/.synapse/a2a_conversations/` — they survive context
