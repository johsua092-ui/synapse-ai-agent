"""SimpleX Chat platform adapter (Synapse plugin).
...
This adapter ships as a Synapse platform plugin under
``plugins/platforms/simplex/``. The Synapse plugin loader scans the
...
Optional environment variables:
    ...
    SYNAPSE_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``synapse setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Synapse — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SYNAPSE_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``synapse setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.synapse/.env`` via
    ``synapse_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "synapse_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.synapse/.env"
        )
        return
