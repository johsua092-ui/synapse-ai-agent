"""disk_cleanup — ephemeral file cleanup for Synapse Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SYNAPSE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SYNAPSE_HOME and /tmp/synapse-*
Never touches: ~/.synapse/logs/ or any system directory.
"""
...
try:
    ...
    def get_synapse_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SYNAPSE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".synapse").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SYNAPSE_HOME/logs/``."""
    return get_synapse_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SYNAPSE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SYNAPSE_HOME or ``/tmp/synapse-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    synapse_home = get_synapse_home()
    try:
        path.resolve().relative_to(synapse_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/synapse-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("synapse-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "synapse-agent", "backups", "profiles", ".worktrees",
...
# Paths under $SYNAPSE_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SYNAPSE_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        synapse_home = get_synapse_home()
        for parent in ("cron", "cronjobs"):
            base = synapse_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SYNAPSE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SYNAPSE_HOME, but never recurse into known
    # durable state trees.  Some installs place the Synapse checkout, venv,
    # and desktop build under SYNAPSE_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    synapse_home = get_synapse_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in synapse_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    synapse_home = get_synapse_home()
    try:
        rel = path.resolve().relative_to(synapse_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "synapse-agent",
        ...
        # Path isn't under SYNAPSE_HOME (e.g. /tmp/synapse-*) — fall through.
        pass
