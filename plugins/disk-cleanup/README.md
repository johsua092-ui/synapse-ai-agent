...
Auto-tracks and cleans up ephemeral files created during Synapse Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SYNAPSE_HOME` and `/tmp/synapse-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SYNAPSE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SYNAPSE_HOME` or `/tmp/synapse-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SYNAPSE_HOME/disk-cleanup/` is itself excluded
- `$SYNAPSE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
