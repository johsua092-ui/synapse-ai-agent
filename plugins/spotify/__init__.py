...
``_check_spotify_available()`` — when the user has not run ``synapse auth
spotify``, the tools remain registered (so they appear in ``synapse
...
The Spotify auth flow (``synapse auth spotify``), CLI plumbing, and docs
