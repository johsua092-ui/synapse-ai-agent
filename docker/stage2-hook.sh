...
#
# Wired into the image as /etc/cont-init.d/01-synapse-setup by the
...
SYNAPSE_HOME="${SYNAPSE_HOME:-/opt/data}"
INSTALL_DIR="/opt/synapse"

# Drop to synapse via s6-setuidgid, but skip it when already non-root.
as_synapse() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid synapse "$@"; }
...
# starts non-root. The baked install tree under /opt/synapse is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $SYNAPSE_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass SYNAPSE_UID/SYNAPSE_GID — or the PUID/PGID aliases — which the
...
# `id -u synapse` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# synapse UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u synapse)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-synapse UID).
...
and the baked /opt/synapse install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e SYNAPSE_UID=\$(id -u) -e SYNAPSE_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the synapse user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SYNAPSE_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid synapse mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SYNAPSE_HOME paths whose parents only
# root can create (e.g. `SYNAPSE_HOME=/home/synapse/.synapse` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SYNAPSE_UID/SYNAPSE_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SYNAPSE_UID/
# SYNAPSE_GID still win when both are set.  See #15290, salvages #25872.
SYNAPSE_UID="${SYNAPSE_UID:-${PUID:-}}"
SYNAPSE_GID="${SYNAPSE_GID:-${PGID:-}}"

if [ -n "${SYNAPSE_UID:-}" ] && validate_uid_gid "$SYNAPSE_UID" && [ "$SYNAPSE_UID" != "$(id -u synapse)" ]; then
    echo "[stage2] Changing synapse UID to $SYNAPSE_UID"
    usermod -u "$SYNAPSE_UID" synapse
fi
if [ -n "${SYNAPSE_GID:-}" ] && validate_uid_gid "$SYNAPSE_GID" && [ "$SYNAPSE_GID" != "$(id -g synapse)" ]; then
    echo "[stage2] Changing synapse GID to $SYNAPSE_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SYNAPSE_GID" synapse 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised synapse user (UID 10000)
...
# synapse process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes synapse. Idempotent across container
...
#   - synapse is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G synapse 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] synapse already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" synapse 2>/dev/null; then
        echo "[stage2] Added synapse to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group synapse failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SYNAPSE_UID is remapped or the top-level $SYNAPSE_HOME isn't owned by
# the runtime synapse UID, restore ownership to synapse — but ONLY for the
# directories synapse actually writes to. The full $SYNAPSE_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of synapse-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_synapse_uid=$(id -u synapse)
...
chown_synapse_tree() {
    ...
    chown -R synapse:synapse "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_synapse_owner() {
    target="$1"
    find "$target" \( ! -user synapse -o ! -group synapse \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$SYNAPSE_HOME" 2>/dev/null)" != "$actual_synapse_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SYNAPSE_HOME (targeted) to synapse ($actual_synapse_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SYNAPSE_HOME: chown the directory itself (not its contents)
    # so synapse can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SYNAPSE_HOME"; then
        :
    else
        chown synapse:synapse "$SYNAPSE_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SYNAPSE_HOME failed (rootless container?) — continuing"
    fi
    # Synapse-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by synapse (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$SYNAPSE_HOME/$sub" ] && tree_has_non_synapse_owner "$SYNAPSE_HOME/$sub"; then
            chown_synapse_tree "$SYNAPSE_HOME/$sub"
        fi
    done
fi
...
# synapse user. Hosted/container instances keep mutable state under
# $SYNAPSE_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# SYNAPSE_DISABLE_LAZY_INSTALLS=1. Keeping /opt/synapse root-owned and
...
# $SYNAPSE_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SYNAPSE_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to synapse
...
# Always reset ownership of $SYNAPSE_HOME/profiles to synapse on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> synapse …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as synapse and walks
...
if [ -d "$SYNAPSE_HOME/profiles" ] && tree_has_non_synapse_owner "$SYNAPSE_HOME/profiles"; then
    chown_synapse_tree "$SYNAPSE_HOME/profiles"
fi

# Always reset ownership of $SYNAPSE_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged synapse runtime even
...
# gate as profiles/).
if [ -d "$SYNAPSE_HOME/cron" ] && tree_has_non_synapse_owner "$SYNAPSE_HOME/cron"; then
    chown_synapse_tree "$SYNAPSE_HOME/cron"
fi

# Always ensure logs/gateways is synapse-owned (#45258). Formerly healed by
...
# top-level $SYNAPSE_HOME is mis-owned, so a warm volume with synapse-owned
# SYNAPSE_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid synapse mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as synapse.
if [ -d "$SYNAPSE_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$SYNAPSE_HOME/logs/gateways"; then
        :
    else
        chown synapse:synapse "$SYNAPSE_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# synapse pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged synapse gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $SYNAPSE_HOME is
...
# and cron/.
if [ -d "$SYNAPSE_HOME/platforms/pairing" ] && tree_has_non_synapse_owner "$SYNAPSE_HOME/platforms/pairing"; then
    chown_synapse_tree "$SYNAPSE_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$SYNAPSE_HOME/pairing" ] && tree_has_non_synapse_owner "$SYNAPSE_HOME/pairing"; then
    chown_synapse_tree "$SYNAPSE_HOME/pairing"
fi

# Reset ownership of synapse-owned top-level state files on every boot.
# The targeted data-volume chown above only covers synapse-owned
# *subdirectories*; loose state files living directly under $SYNAPSE_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> synapse …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged synapse runtime then hits
...
# sweep so host-owned files in a bind-mounted $SYNAPSE_HOME are never
...
# entries of synapse_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$SYNAPSE_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SYNAPSE_HOME/$f"; then
            :
        else
            chown synapse:synapse "$SYNAPSE_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the synapse runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SYNAPSE_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SYNAPSE_HOME/config.yaml"; then
        :
    else
        chown synapse:synapse "$SYNAPSE_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SYNAPSE_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as synapse user ---
# Run as synapse via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SYNAPSE_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_synapse mkdir -p \
    "$SYNAPSE_HOME/backups" \
    "$SYNAPSE_HOME/cron" \
    "$SYNAPSE_HOME/sessions" \
    "$SYNAPSE_HOME/logs" \
    "$SYNAPSE_HOME/logs/gateways" \
    "$SYNAPSE_HOME/hooks" \
    "$SYNAPSE_HOME/memories" \
    "$SYNAPSE_HOME/skills" \
    "$SYNAPSE_HOME/skins" \
    "$SYNAPSE_HOME/plans" \
    "$SYNAPSE_HOME/workspace" \
    "$SYNAPSE_HOME/home" \
    "$SYNAPSE_HOME/pairing" \
    "$SYNAPSE_HOME/platforms/pairing" \
    "$SYNAPSE_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/synapse/.install_method (see Dockerfile), NOT written here into
# $SYNAPSE_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $SYNAPSE_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.synapse:/opt/data) and sometimes shared with a
...
# 'synapse update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $SYNAPSE_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/synapse stamp, so deleting the data-dir copy is safe).
if [ -f "$SYNAPSE_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$SYNAPSE_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$SYNAPSE_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SYNAPSE_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$SYNAPSE_HOME/$dest"; then
            :
        else
            as_synapse cp "$INSTALL_DIR/$src" "$SYNAPSE_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/synapse/.env.example being present
...
# not generate one. Synapse loads $SYNAPSE_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$SYNAPSE_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$SYNAPSE_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $SYNAPSE_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$SYNAPSE_HOME/.env" ] && ! refuse_symlinked_path "clean" "$SYNAPSE_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$SYNAPSE_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$SYNAPSE_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$SYNAPSE_HOME/.env"; then
        :
    else
        if [ ! -f "$SYNAPSE_HOME/.env" ]; then
            ...
            (umask 077 && as_synapse touch "$SYNAPSE_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$SYNAPSE_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$SYNAPSE_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$SYNAPSE_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $SYNAPSE_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $SYNAPSE_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$SYNAPSE_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SYNAPSE_HOME/.env"; then
        :
    else
        chown synapse:synapse "$SYNAPSE_HOME/.env" 2>/dev/null || true
...
# as the unprivileged `synapse` user — without this it gets EACCES on every
...
# synapse group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp synapse /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted synapse group access to the Fly Machines API socket"
    else
...
# $SYNAPSE_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `synapse update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SYNAPSE_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SYNAPSE_HOME/config.yaml" ]; then
    s6-setuidgid synapse "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SYNAPSE_HOME/auth.json" ] && [ -n "${SYNAPSE_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SYNAPSE_HOME/auth.json"; then
        :
    else
        printf '%s' "$SYNAPSE_AUTH_JSON_BOOTSTRAP" > "$SYNAPSE_HOME/auth.json"
        chown synapse:synapse "$SYNAPSE_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SYNAPSE_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via SYNAPSE_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$SYNAPSE_HOME/auth.json" ] && [ -n "${SYNAPSE_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SYNAPSE_HOME/auth.json"; then
        :
    else
        s6-setuidgid synapse "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$SYNAPSE_HOME/auth.json" \
...
# can set SYNAPSE_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SYNAPSE_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SYNAPSE_HOME/gateway_state.json" ] && \
        [ "${SYNAPSE_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SYNAPSE_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SYNAPSE_HOME/gateway_state.json"
        chown synapse:synapse "$SYNAPSE_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SYNAPSE_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_synapse "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/synapse/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Synapse spawns for the
...
# main-wrapper.sh propagates it into the supervised ``synapse`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-synapse, dashboard, etc.).
