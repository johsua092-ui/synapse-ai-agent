...
# Synapse WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SYNAPSE_TINI_SHIM_TARGET overrides the `/init` path (and
# SYNAPSE_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SYNAPSE_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SYNAPSE_TINI_SHIM_WRAPPER:-/opt/synapse/docker/main-wrapper.sh}"
