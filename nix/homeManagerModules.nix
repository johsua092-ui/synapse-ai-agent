# nix/homeManagerModules.nix — the Home Manager module for synapse-agent
#
# This module is the user-level equivalent of nixosModules.default. Synapse is
...
#
# `services.synapse-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.synapse-agent.enable and
#                                        home.sessionVariables
#   added     programs.synapse-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.synapse")  -> synapseHome, set directly
#
# To use the module:
#   imports = [ synapse-agent.homeManagerModules.default ];
#   programs.synapse-agent = {
#     enable = true;          # the synapse CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.synapse-agent;
      ...
      # not reach it, and the application would open ~/.synapse while the
      # services use synapseHome. Thus the launcher carries the value itself.
      #
      # SYNAPSE_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        SYNAPSE_HOME = cfg.synapseHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) SYNAPSE_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        SYNAPSE_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("SYNAPSE_DESKTOP_REMOTE_URL is set
      # but SYNAPSE_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          SYNAPSE_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export SYNAPSE_DESKTOP_REMOTE_TOKEN
        else
          echo "synapse-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "synapse-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.synapse-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Synapse needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.synapse-agent` stays the authority for the state and the
      # configuration. This module reads synapseHome and the backend address
      # from it, and never the reverse.
      options.programs.synapse-agent = {
        enable = lib.mkEnableOption ''
          the Synapse Agent command line application.

          This adds `synapse` to home.packages, and exports SYNAPSE_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.synapse-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.synapse-agent.package";
          description = ''
            The synapse-agent package to install.

            The default follows `services.synapse-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Synapse Desktop application (Electron).

            This adds `synapse-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Synapse
            runtime that `package` gives, and reads the SYNAPSE_HOME of
            `services.synapse-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Synapse runtime with
            the usual distribution. This module gives it the Nix package
            instead, with SYNAPSE_DESKTOP_SYNAPSE. It installs no second copy
            of Synapse, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.synapseDesktop;
            defaultText = lib.literalExpression "config.programs.synapse-agent.package.synapseDesktop";
            description = ''
              The synapse-desktop package to use.

              The default follows `package`, and thus also
              `services.synapse-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Synapse runtime, and this module cannot make
      ...
      options.services.synapse-agent =
        ...
        // {
          synapseHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.synapse";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.synapse"'';
            description = ''
              The value of SYNAPSE_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.synapse` to it.
              This module sets SYNAPSE_HOME directly. Thus an existing
              ~/.synapse continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.synapse-work";
          };

          # `installPackage` moved to `programs.synapse-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `synapse` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.synapse-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.SYNAPSE_HOME = cfg.synapseHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # SYNAPSE_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put SYNAPSE_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `synapse` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.synapse-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.synapseAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) synapseHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.synapse-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.synapse-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.synapse-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "synapse-agent";
              };
            })

            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.synapse-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "synapse-backend";
              };
            })
          ]
        ))
