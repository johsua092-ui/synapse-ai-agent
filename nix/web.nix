# nix/web.nix — Synapse Web Dashboard (Vite/React) frontend build
{ synapseNpmLib, ... }:
synapseNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @synapse/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../synapse_cli/web_dist for the
