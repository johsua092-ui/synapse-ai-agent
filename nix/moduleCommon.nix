...
#
# `services.synapse-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  synapseHome, programs.synapse-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "synapse-config-attrs";
    description = "Synapse YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $SYNAPSE_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Synapse Agent";
      ...
      package = mkOption {
        ...
        description = "The synapse-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Synapse configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `synapse config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $SYNAPSE_HOME/.env. Synapse reads that file at each start, with
          load_synapse_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."synapse/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Synapse refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Synapse reads SOUL.md and memories/ from SYNAPSE_HOME, so put those
          files in `synapseHomeFiles`.
      ...
      synapseHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into SYNAPSE_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Synapse reads SOUL.md and the memory files from SYNAPSE_HOME and not
          from the working directory. Declare those files here, or Synapse
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the synapse plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Synapse discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          synapse_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as synapse (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in synapse-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `synapse gateway`.";
      };
      ...
      # ── The backend: `synapse serve` or `synapse dashboard` ──────────────
      # `synapse serve` and `synapse dashboard` are the same entry point,
      # synapse_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Synapse Desktop
      ...
      # controls an external gateway, with `synapse gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Synapse
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            SYNAPSE_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Synapse Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."synapse/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `synapse` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.synapse-agent.installPackage was removed. Synapse now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.synapse-agent.enable = ${lib.boolToString (value != false)};  # the synapse CLI, and SYNAPSE_HOME for your shells
        programs.synapse-agent.desktop.enable = true;  # the desktop application

      `services.synapse-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "synapse-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "synapse-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "synapse-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "synapse-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "synapse-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # SYNAPSE_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.synapseHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Synapse make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${synapseHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Synapse
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${synapseHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${synapseHome}/config.yaml
            ${run}chmod ${modes.config} ${synapseHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "synapse-managed" managedSystem} ${synapseHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${synapseHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${synapseHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${synapseHome}/auth.json"
        else
          ''
            if [ ! -e ${synapseHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${synapseHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree synapseHome cfg.synapseHomeFiles}
      ...
      ${run}find ${synapseHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "synapse-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${synapseHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/synapse"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps synapse as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "synapse-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "synapse-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "synapse-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          SYNAPSE_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export SYNAPSE_DASHBOARD_SESSION_TOKEN

          if [ -z "$SYNAPSE_DASHBOARD_SESSION_TOKEN" ]; then
            echo "synapse-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "synapse-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "synapse-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "synapse-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "synapse-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "synapse-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "synapse-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Synapse Agent web dashboard and desktop backend"
    else
      "Synapse Agent backend for Synapse Desktop";

  # The environment that each Synapse process needs, from either module.
  #
  # managedSystem gives the value of SYNAPSE_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Synapse an identity and a memory, use
          ${optionPath}.synapseHomeFiles instead. Those files go to
          SYNAPSE_HOME. Synapse reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of SYNAPSE_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
