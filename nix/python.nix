...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Synapse derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            synapse-agent = prev.synapse-agent.overrideAttrs (_old: {
              SYNAPSE_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $SYNAPSE_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$SYNAPSE_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        synapse-agent = prev.synapse-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "synapse-agent-env" {
    synapse-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "synapse-agent-editable-env" {
    synapse-agent = dependency-groups;
  };
}
