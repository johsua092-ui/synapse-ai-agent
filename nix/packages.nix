# nix/packages.nix — Synapse Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./synapse-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.synapseTui;
        web = full.synapseWeb;
        desktop = full.synapseDesktop;

        update-npm-lockfile = full.synapseNpmLib.updateNpmLockfile;
      };
    };
}
