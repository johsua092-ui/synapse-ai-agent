# ============================================================================
# Synapse Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$SynapseHome = $(if ($env:SYNAPSE_HOME) { $env:SYNAPSE_HOME } else { "$env:LOCALAPPDATA\synapse" }),
    [string]$InstallDir = $(if ($env:SYNAPSE_HOME) { "$env:SYNAPSE_HOME\synapse-agent" } else { "$env:LOCALAPPDATA\synapse\synapse-agent" }),
    ...
    # builds apps/desktop into a launchable Synapse.exe.
    #
    # Why opt-in:
    #   * Synapse-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Synapse.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Synapse.exe
    ...
    #     `synapse desktop` already builds on demand.
...
# SYNAPSE_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[synapse] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'SynapseInstall.LongPath').Type) {
            Add-Type -Namespace 'SynapseInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [SynapseInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -SynapseHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('SynapseHome')) {
    $SynapseHome = ConvertTo-LongPath $SynapseHome
} else {
    $SynapseHome = ConvertTo-LongPath $(
        if ($env:SYNAPSE_HOME) { $env:SYNAPSE_HOME } else { "$env:LOCALAPPDATA\synapse" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:SYNAPSE_HOME) { "$env:SYNAPSE_HOME\synapse-agent" } else { "$env:LOCALAPPDATA\synapse\synapse-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: SynapseHome=$SynapseHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * Synapse Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Josh Research.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `synapse update`'s npx
    ...
    $prefixDir = Join-Path $SynapseHome "node"
...
function Install-Uv {
    # Synapse owns its own uv at $SynapseHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (synapse_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `synapse update` stay in sync.
    $managedUv = Join-Path $SynapseHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $SynapseHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $SynapseHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $SynapseHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $SynapseHome "bin"
        ...
        # (synapse_cli/managed_uv.py looks only at $SynapseHome\bin\uv.exe).
...
# Put the Synapse-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone synapse-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Synapse-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Synapse-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("synapse-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and SYNAPSE_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Synapse-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Synapse setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Synapse uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\synapse\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Synapse needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Synapse copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\synapse\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SYNAPSE_GIT_BASH_PATH`` (User scope) so Synapse can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Synapse-managed PortableGit install instead..."
    }

    # Download PortableGit into $SynapseHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $SynapseHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Synapse features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Synapse needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SYNAPSE_GIT_BASH_PATH`` (User env scope) so Synapse can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $SynapseHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $SynapseHome\git\usr\bin\bash.exe
    $candidates += "$SynapseHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$SynapseHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SYNAPSE_GIT_BASH_PATH", $candidate, "User")
            $env:SYNAPSE_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SYNAPSE_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Synapse may not find Git Bash."
    Write-Info "If needed, set SYNAPSE_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Synapse requires Node >=26)"
    }

    # Prefer a Synapse-managed Node from a previous run over a too-old system one.
    $managedNode = "$SynapseHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$SynapseHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$SynapseHome\node"
        Write-Success "Node.js $version found (Synapse-managed)"
        ...
        Update-ManagedNpm "$SynapseHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Synapse-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $SynapseHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $SynapseHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\synapse-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$SynapseHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$SynapseHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$SynapseHome\node") {
                    try {
                        Rename-Item "$SynapseHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Synapse-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$SynapseHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $SynapseHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\synapse-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `synapse
                    # update` path (#4735).
                    ...
                    $stashName = "synapse-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. synapse-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``synapse update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Synapse behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running synapse processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\synapse-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `synapse update` (see the notes at the shared clone-path
    ...
    # next `synapse update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Synapse-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running synapse process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running synapse processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Synapse_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (synapse.exe) plus its child tree.
            & taskkill /F /T /IM synapse.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM synapse.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m synapse_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not synapse.exe.
            ...
            # Synapse children re-exec through .synapse-runtime, so killing only
            ...
            # user (see synapse_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Synapse " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `synapse update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Synapse (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `synapse.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'synapse-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install synapse-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and synapse.exe broken with
    ...
    # here, not 30 seconds later when the user runs `synapse`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Synapse processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Synapse processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Synapse and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register synapse.exe in dist-info/RECORD but fail to
        ...
        # `synapse` command while synapse-agent.exe / synapse-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m synapse_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `synapse dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\synapse_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `synapse dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `synapse dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: synapse_cli/web_server.py"
        }
    }
...
function Install-SynapseCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the synapse launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring synapse.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "synapse.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the synapse command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # synapse_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("synapse", "synapse-acp")) {
    ...
    $requiredExe = Join-Path $Destination "synapse.exe"
    $requiredCmd = Join-Path $Destination "synapse.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the synapse command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up synapse command..."
    
    if ($NoVenv) {
        $synapseBin = "$InstallDir"
    } else {
        # $SynapseHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `synapse update`'s autostash
        ...
        # installer staged under synapse-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-SynapseCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $synapseBin = "$SynapseHome\bin"
        Install-SynapseCommandLaunchers -Root $InstallDir -Destination $synapseBin | Out-Null
    }
    ...
    #   synapse-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The synapse-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept synapse via $synapseBin)"
        }
    }
    
    if ($currentPath -notlike "*$synapseBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$synapseBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $synapseBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SYNAPSE_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\synapse instead
    # of the Unix default ~/.synapse
    $currentSynapseHome = [Environment]::GetEnvironmentVariable("SYNAPSE_HOME", "User")
    if (-not $currentSynapseHome -or $currentSynapseHome -ne $SynapseHome) {
        [Environment]::SetEnvironmentVariable("SYNAPSE_HOME", $SynapseHome, "User")
        Write-Success "Set SYNAPSE_HOME=$SynapseHome"
    }
    $env:SYNAPSE_HOME = $SynapseHome
    
    # Update current session
    $env:Path = "$synapseBin;$env:Path"
    
    Write-Success "synapse command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.synapse-bootstrap-complete which tells the Synapse
    ...
    # Synapse-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `synapse update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".synapse-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SYNAPSE_HOME directory structure ($SynapseHome, default %LOCALAPPDATA%\synapse)
    New-Item -ItemType Directory -Force -Path "$SynapseHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SynapseHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SynapseHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SynapseHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SynapseHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SynapseHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SynapseHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SynapseHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SynapseHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$SynapseHome\.env"
    ...
    # Create config.yaml
    $configPath = "$SynapseHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Synapse's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$SynapseHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in synapse_cli/default_soul.py. The runtime
        ...
        $soulContent = @"
You are Synapse Agent, an intelligent AI assistant created by Josh Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
"@
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    ...
    Write-Success "Configuration directory ready: $SynapseHome"
    
    # Seed bundled skills into $SynapseHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $SynapseHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $SynapseHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SynapseHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $SynapseHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Synapse-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'synapse setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\synapse-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\synapse-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\synapse-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `synapse tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $SynapseHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Synapse' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Synapse' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'synapse tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `synapse
...
# paths still lazy-install via install_cua_driver() (synapse_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `synapse computer-use install` runs. Bounded
        ...
        # Synapse' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'synapse tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: synapse computer-use install"
            }
        } else {
...
# the final `electron` -> `Synapse` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Synapse.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_synapse_desktop command
    # resolves apps/desktop/release/win-unpacked/Synapse.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Synapse-managed
    ...
    # The --dir mode produces an unpacked Synapse.exe in
    ...
    # are NOT trying to work around). The Synapse icon + product name are
    # stamped onto Synapse.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\synapse-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Synapse.exe",
        "$desktopDir\release\win-arm64-unpacked\Synapse.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Synapse.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Synapse icon + identity are stamped onto Synapse.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, synapse-agent#38216).
    ...
    #    Synapse.exe. We deliberately do NOT point them at `synapse desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Synapse.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Synapse.lnk')
        )
        ...
        # with the Synapse icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.synapse/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'synapse setup'."
        return
    }
    ...
    # Run synapse setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m synapse_cli.main setup
    } else {
        python -m synapse_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $synapseCmd = "$InstallDir\venv\Scripts\synapse.exe"
    if (-not (Test-Path $synapseCmd)) {
        $synapseCmd = "synapse"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SynapseHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'synapse whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: synapse gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SynapseHome\logs\gateway.log"
            Start-Process -FilePath $synapseCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SynapseHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: synapse gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: synapse gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SynapseHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SynapseHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SynapseHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SynapseHome\synapse-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   synapse              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   synapse setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   synapse config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   synapse config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   synapse gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   synapse update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Synapse repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Synapse-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Synapse to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https:///install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
