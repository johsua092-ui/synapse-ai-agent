...
if [ -n "${SYNAPSE_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SYNAPSE_INSTALL_DIR"
...
#   code at /usr/local/lib/synapse-agent, command at /usr/local/bin/synapse,
#   data still at /root/.synapse (SYNAPSE_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SYNAPSE_HOME/.no-bundled-skills so future"
            echo "                   'synapse update' runs never inject bundled skills either"
            ...
            echo "  --synapse-home PATH  Data directory (default: ~/.synapse, or \$SYNAPSE_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Synapse installs the code under"
            echo "  /usr/local/lib/synapse-agent and links the command into"
            echo "  /usr/local/bin/synapse (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SYNAPSE_HOME"
            echo "  (default /root/.synapse).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SYNAPSE_HOME/synapse-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Synapse Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Josh Research.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `synapse update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Synapse-Setup) passes it so
...
# Decide where the repo checkout + venv live, and where the `synapse` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SYNAPSE_HOME/synapse-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SYNAPSE_HOME/synapse-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/synapse-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SYNAPSE_HOME/synapse-agent — then preserve it)
#
# Always no-op when the user set --dir or $SYNAPSE_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SYNAPSE_HOME.
    if is_termux; then
        INSTALL_DIR="$SYNAPSE_HOME/synapse-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SYNAPSE_HOME/synapse-agent/.git" ]; then
            INSTALL_DIR="$SYNAPSE_HOME/synapse-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/synapse-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/synapse-agent"
        ...
        # /usr/local/bin/synapse wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/synapse"
        log_info "  Data:    $SYNAPSE_HOME (unchanged)"
...
# Point a Synapse-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SYNAPSE_HOME/node/bin — which is NOT on
...
# Synapse's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Synapse-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SYNAPSE_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SYNAPSE_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$SYNAPSE_HOME/node/etc/npmrc"
}

get_synapse_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/synapse" ]; then
        echo "$link_dir/synapse"
    else
        echo "synapse"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https:///install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Synapse owns its own uv at $SYNAPSE_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (synapse_cli/managed_uv.py) looks in the same
    # place, so install.sh and `synapse update` stay in sync.
    local _managed_uv="$SYNAPSE_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SYNAPSE_HOME/bin ..."
    mkdir -p "$SYNAPSE_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/synapse-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/synapse-uv-installer.$$.sh")"
    ...
    # directly into $SYNAPSE_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SYNAPSE_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Synapse-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Synapse-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Synapse-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Synapse-managed Node from a previous run over a too-old system one.
    if [ -x "$SYNAPSE_HOME/node/bin/node" ] && [ -x "$SYNAPSE_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$SYNAPSE_HOME/node/bin/node" --version)"; then
        export PATH="$SYNAPSE_HOME/node/bin:$PATH"
        log_success "Node.js $("$SYNAPSE_HOME/node/bin/node" --version) found (Synapse-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Synapse-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Synapse requires Node >=26) — installing Synapse-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.synapse/node/..."
    ...
    # Place into ~/.synapse/node/ and symlink binaries into the same bin dir
    # the synapse command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SYNAPSE_HOME/node"
    mkdir -p "$SYNAPSE_HOME"
    mv "$extracted_dir" "$SYNAPSE_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$SYNAPSE_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SYNAPSE_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SYNAPSE_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$SYNAPSE_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.synapse/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Synapse Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `synapse update` path
                # (#4735).
                ...
                local stash_name
                stash_name="synapse-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``synapse update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Synapse behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up synapse command..."

    if [ "$USE_VENV" = true ]; then
        SYNAPSE_BIN="$INSTALL_DIR/venv/bin/python"
        SYNAPSE_ENTRYPOINT="$INSTALL_DIR/synapse"
    else
        SYNAPSE_BIN="$(which synapse 2>/dev/null || echo "")"
        if [ -z "$SYNAPSE_BIN" ]; then
            log_warn "synapse not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$SYNAPSE_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$SYNAPSE_ENTRYPOINT" ]; }; then
        log_warn "Synapse launcher prerequisites not found"
    ...
    # Create a user-facing shim for the synapse command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SYNAPSE_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SYNAPSE_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/synapse"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/synapse" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SYNAPSE_BIN" "$SYNAPSE_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/synapse" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/synapse"
    log_success "Installed synapse launcher → $command_link_display_dir/synapse"

    # Also expose `synapse-agent`. The `synapse-agent` console script declared in
    ...
    rm -f "$command_link_dir/synapse-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/synapse-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SYNAPSE_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/synapse-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SYNAPSE_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/synapse-agent"
    log_success "Installed synapse-agent launcher → $command_link_display_dir/synapse-agent"

    # Also expose `synapse-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `synapse-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Synapse as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/synapse-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/synapse-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SYNAPSE_BIN" "$SYNAPSE_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/synapse-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SYNAPSE_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/synapse-acp"
    log_success "Installed synapse-acp launcher → $command_link_display_dir/synapse-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "synapse command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v synapse' \
                >/dev/null 2>&1; then
        ...
        log_info "synapse not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Synapse Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Synapse Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Synapse Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so synapse works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.synapse directory structure (config at top level, code in subdir)
    mkdir -p "$SYNAPSE_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.synapse/.env (top level, easy to find)
    if [ ! -f "$SYNAPSE_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SYNAPSE_HOME/.env"
            log_success "Created ~/.synapse/.env from template"
        else
            touch "$SYNAPSE_HOME/.env"
            log_success "Created ~/.synapse/.env"
        fi
    else
        log_info "~/.synapse/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SYNAPSE_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.synapse/config.yaml (top level, easy to find)
    if [ ! -f "$SYNAPSE_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SYNAPSE_HOME/config.yaml"
            log_success "Created ~/.synapse/config.yaml from template"
        fi
    else
        log_info "~/.synapse/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in synapse_cli/default_soul.py — the
    ...
    if [ ! -f "$SYNAPSE_HOME/SOUL.md" ]; then
        cat > "$SYNAPSE_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.synapse/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.synapse/"

    # Seed bundled skills into ~/.synapse/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `synapse update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'synapse update'." \
            > "$SYNAPSE_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SYNAPSE_HOME/.no-bundled-skills"
        log_info "  Future 'synapse update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.synapse/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.synapse/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SYNAPSE_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SYNAPSE_HOME/skills/" 2>/dev/null || true
...
strip_snap_browser_override() {
    ...
    local env_file="$SYNAPSE_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Synapse Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Synapse will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Synapse Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `synapse update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `synapse tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$SYNAPSE_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Synapse' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$SYNAPSE_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'synapse tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `synapse tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `synapse computer-use install` on their own).
    ...
    # Same upstream installer `synapse computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Synapse install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Synapse'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'synapse tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: synapse computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'synapse setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m synapse_cli.main setup < /dev/tty
    else
        python -m synapse_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SYNAPSE_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Synapse to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SYNAPSE_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'synapse whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SYNAPSE_CMD="$(get_synapse_command_path)"
                $SYNAPSE_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'synapse whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'synapse gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SYNAPSE_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SYNAPSE_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: synapse gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: synapse gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SYNAPSE_CMD gateway > "$SYNAPSE_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.synapse/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: synapse gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: synapse gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.synapse-bootstrap-complete, which tells the Synapse
    ...
    local marker_path="$INSTALL_DIR/.synapse-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SYNAPSE_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SYNAPSE_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SYNAPSE_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}synapse${NC}              Start chatting"
    echo -e "   ${GREEN}synapse setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}synapse config${NC}       View/edit configuration"
    echo -e "   ${GREEN}synapse config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}synapse gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}synapse update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'synapse' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'synapse' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'synapse' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SYNAPSE_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SYNAPSE_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SYNAPSE_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SYNAPSE_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Synapse` rename
...
# _purge_electron_build_cache() used by `synapse desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Synapse-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `synapse desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Synapse" ]; then
            app="$desktop_dir/release/linux-unpacked/Synapse"
        elif [ -x "$desktop_dir/release/linux-unpacked/synapse" ]; then
            app="$desktop_dir/release/linux-unpacked/synapse"
        fi
    else
        local cand
    ...
    # `synapse desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading SYNAPSE_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if SYNAPSE_HOME="$SYNAPSE_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from synapse_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `synapse update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Synapse-managed Node
            # provisioned during prerequisites/node-deps (at $SYNAPSE_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SYNAPSE_HOME.
    # $SYNAPSE_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'synapse update' on this host install.
