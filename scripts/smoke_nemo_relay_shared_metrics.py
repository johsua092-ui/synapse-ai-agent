"""Run a real Synapse CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_synapse_executable(synapse_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "synapse",
        Path(".venv") / "Scripts" / "synapse.exe",
    ):
        candidate = synapse_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("synapse")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Synapse executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Synapse source checkout containing .venv/bin/synapse",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated SYNAPSE_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["synapse.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['synapse.client.active']}"
        )
    [model] = by_name["synapse.model_route.count"]
    expected_model = {
        "name": "synapse.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['synapse.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["synapse.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['synapse.task_run.started']}"
        )
    [terminal] = by_name["synapse.task_run.finished"]
    ...
    [tool] = by_name["synapse.tool_call.count"]
    ...
    lifecycle = by_name["synapse.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["synapse.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Synapse development environment requires jsonschema"
        ) from exc
    ...
    if metrics["synapse.client.active"] != [
        {
            "name": "synapse.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['synapse.client.active']}"
        )
    [model] = metrics["synapse.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['synapse.model_route.count']}"
        )
    [terminal] = metrics["synapse.task_run.finished"]
    ...
    [tool] = metrics["synapse.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["synapse.skill.lifecycle.count"]
    ...
    loads = metrics["synapse.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    synapse_repo = args.synapse_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    synapse = _resolve_synapse_executable(synapse_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="synapse-relay-shared-metrics-"))
    home = root / "synapse-home"
    ...
    (root / "synapse.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "synapse.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Synapse exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Synapse model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Synapse did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Synapse did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from synapse_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "synapse.shared_metrics.v2.schema.json",
    )

    print("Synapse -> NeMo Relay shared-metrics smoke test passed")
