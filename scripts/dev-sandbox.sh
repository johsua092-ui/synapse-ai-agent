...
unprivileged `synapse` user, giving the layout most people have —
$SYNAPSE_HOME/synapse-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  SYNAPSE_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .synapse-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `synapse setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${SYNAPSE_DEV_SANDBOX_UPSTREAM:-https://github.com/johsua092-ui/synapse-ai-agent.git}"
...
GIT_ROOT="${SYNAPSE_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${SYNAPSE_DEV_SANDBOX_DIR:-.synapse-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t synapse-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t synapse-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  ...
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http//install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http//install.sh"
  fi
  set -- bash -c '
    set +e
    ...
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      ...
      if git --git-dir=/work/repos/synapse-agent.git update-ref refs/heads/main "$next_main"; then
...
if [ "$RUN_AS_USER" = true ]; then
  SANDBOX_UID=1000
  SANDBOX_GID=1000
  SANDBOX_USER=synapse
  SANDBOX_HOME=/home/synapse
else
  SANDBOX_UID=0
  SANDBOX_GID=0
  SANDBOX_USER=root
...
# A user-level install writes the `synapse` launcher to ~/.local/bin and the
# checkout to $SYNAPSE_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/synapse-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q synapse-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t synapse-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Synapse sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/synapse-sandbox/next"
...
if [ ! -f "$SANDBOX_ROOT/root/certs/ca.pem" ]; then
  if ! ca_error="$(OPENSSL_CONF="$SANDBOX_ROOT/root/certs/openssl.cnf" \
    ...
    -subj '/CN=Synapse dev sandbox CA' \
