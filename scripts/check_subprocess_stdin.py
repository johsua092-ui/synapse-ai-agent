...
When Synapse runs in TUI mode, the gateway child process communicates with
...
# ``get_synapse_home() / "plugins"`` (user) and ``./.synapse/plugins/`` (project,
# gated behind ``SYNAPSE_ENABLE_PROJECT_PLUGINS``) — see
# ``synapse_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``synapse_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import synapse_constants (this script
    ...
    # plugins in get_synapse_home()/plugins/ and project plugins in
    # ./.synapse/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_synapse_home() / "plugins"]
    if os.environ.get("SYNAPSE_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".synapse" / "plugins")
