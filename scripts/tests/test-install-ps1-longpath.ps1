...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[synapse] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[synapse`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[synapse`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        SynapseHome = $(if ($paths) { $paths.synapse_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\synapse\synapse-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-synapse'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-synapse') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
