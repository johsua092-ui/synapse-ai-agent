#!/usr/bin/env python3
"""Live test harness for Synapse Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any synapse imports.
ORIGINAL_HOME = os.environ.get("SYNAPSE_HOME")
ORIGINAL_AUTH = Path.home() / ".synapse" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.synapse/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.synapse/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="synapse_ts_live_"))
    synapse_home = home_dir / ".synapse"
    synapse_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, synapse_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".synapse" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, synapse_home / ".env")
        ...
        load_synapse_dotenv(synapse_home=str(Path.home() / ".synapse"))
    ...
    (synapse_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return synapse_home
...
def reset_module_state():
    """Drop cached modules so the new SYNAPSE_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "synapse_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original SYNAPSE_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SYNAPSE_HOME"] = ORIGINAL_HOME
    else:
