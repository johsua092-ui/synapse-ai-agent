...
# tree generates llms.txt — the index every LLM (Synapse included, via the
# synapse-agent skill) reads to learn what Synapse can do — and
...
_MCP_CATALOG_FILES = {"synapse_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Synapse-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``synapse serve`` backend, the Docker image)
