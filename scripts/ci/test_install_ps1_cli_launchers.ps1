# Behavioral test for install.ps1's synapse launcher staging (PR #92092,
...
#
# The test lifts the real Install-SynapseCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $SynapseHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-SynapseCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-SynapseCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'synapse-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'synapse-agent'
    ...
    $missingThrew = $false
    try {
        Install-SynapseCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing synapse.exe fails the launcher stage'
    ...
    $synapseV1 = [byte[]](77, 90, 1)
    $synapseV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'synapse.exe'), $synapseV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-SynapseCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $synapseV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'synapse.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'synapse-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'synapse.exe'), $synapseV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'synapse-acp.exe'), $acp)
    Install-SynapseCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $synapseV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'synapse.exe'))) `
        'installer refreshes an existing Synapse launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'synapse-acp.exe'))) `
    ...
    Install-SynapseCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'synapse.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'synapse.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'synapse.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'synapse.exe')) -and $cmdBody.Contains('%*')) `
