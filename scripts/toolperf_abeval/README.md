...
batch (tracker: [#77056](https://github.com/johsua092-ui/synapse-ai-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Synapse home, same model, same tasks, same reps.
...
1. Create a dedicated Synapse home with credentials for the models under test:
   ...
   integration at it; no Synapse observability plugin needs to be enabled.
