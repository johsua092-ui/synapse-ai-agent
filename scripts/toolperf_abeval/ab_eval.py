...
measured in the production session DB) through `synapse chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    SYNAPSE_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Synapse home with credentials for the
...
(tracker: johsua092-ui/synapse-ai-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "SYNAPSE_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "synapse_cli.main", "chat", "--query", q,
