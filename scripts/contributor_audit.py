...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Synapse\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (synapse-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "synapse@",
    "synapse-audit@example.com",
    "nousbot@",
    "synapse@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "johsua092-ui/synapse-ai-agent",
                "--state", "merged",
