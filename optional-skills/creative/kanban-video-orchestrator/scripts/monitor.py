#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `synapse kanban list` and
...
def synapse_available() -> bool:
    return shutil.which("synapse") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed synapse CLI."""
    try:
        out = subprocess.run(
            ["synapse", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `synapse kanban list`
    out = subprocess.run(
        ["synapse", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["synapse", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not synapse_available():
        print("ERROR: 'synapse' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
