...
Maps each role archetype to the Synapse skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public synapse-agent
...
## Synapse skills relevant to video production

### Visual / rendering skills (`synapse-agent/skills/creative/`)
...
### Design / pre-production skills (`synapse-agent/skills/creative/`)
...
### Audio / media skills (`synapse-agent/skills/creative/` + `skills/media/`)
...
These are **not** Synapse skills but external CLIs / APIs that profiles invoke.
...
## Built-in Synapse tools for media review

These are native Synapse tools — not invoked via terminal but through their own
...
key is present in `${SYNAPSE_HOME:-~/.synapse}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Synapse profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SYNAPSE_HOME:-~/.synapse}/.env` → environment variable.
...
When a new Synapse-public video skill ships:
