...
the actual `setup.sh` that creates the project workspace, configures Synapse
...
> [Josh Research/kanban-video-pipeline](https://github.com/johsua092-ui/kanban-video-pipeline).
...
2. **Create profiles** — `synapse profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.synapse/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `synapse kanban create` assigned to the director
...
```bash
synapse profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.synapse/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.synapse/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Synapse
...
Each profile gets a `SOUL.md` at `~/.synapse/profiles/<name>/SOUL.md` that
...
```bash
synapse kanban create "Direct production of <video title>" \
    --assignee director \
...
the Synapse `.env` (`${SYNAPSE_HOME:-$HOME/.synapse}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _synapse_env="${SYNAPSE_HOME:-$HOME/.synapse}/.env"
    if grep -q "^${var}=" "$_synapse_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_synapse_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_synapse_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY synapse ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY synapse OPENROUTER_API_KEY || exit 1
# ...
```
