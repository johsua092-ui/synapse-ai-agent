...
synapse kanban watch --tenant <project-slug>

# Snapshot of the board
synapse kanban list --tenant <project-slug>
synapse kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
synapse kanban stats --tenant <project-slug>

# Visual dashboard (browser)
synapse dashboard

# Inspect a specific task (includes comments + events)
synapse kanban show <task-id>

# Follow a single task's event stream
synapse kanban tail <task-id>
```

Verify available subcommands with `synapse kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `synapse kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `synapse kanban block <id>` then `synapse kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `synapse kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `synapse kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `synapse profile list` (profile exists?), `synapse status` (gateway/dispatcher up?), and verify tenant. |
...
synapse kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
synapse kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(synapse kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
synapse kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
synapse kanban block <task-id>
synapse kanban archive <task-id>

# Diagnose what happened
synapse kanban show <task-id>      # task body, comments, recent events
synapse kanban tail <task-id>      # follow the live event stream
synapse kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `synapse kanban create` for the director
...
while true; do
    clear
    synapse kanban list --tenant <slug>
    echo "---"
    synapse kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `synapse kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `synapse profile list` and start
