...
### 1. Start order: editor first, Synapse second

Synapse probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Synapse session.
...
restart the editor, restart the Synapse session.
...
server fails to bind (Output Log) or Synapse' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.synapse/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Synapse

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Synapse' connection
lives in `~/.synapse/config.yaml` via `synapse mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Synapse — just don't
mistake it for the Synapse setup step.
...
explicitly warns against overlapping calls. Synapse executes same-turn tool
...
### 9. Timeouts: Synapse gives up before Unreal does

Synapse' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Synapse runs elsewhere (SSH backend,
