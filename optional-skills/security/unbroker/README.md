...
The design is **Synapse-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
synapse skills install official/security/unbroker
```

Then start a new Synapse session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Synapse session:
...
- **PII handling.** Dossiers live under the Synapse home directory (`0600`, optionally
