...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SYNAPSE_HOME", os.path.expanduser("~/.synapse")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SYNAPSE_HOME", Path.home() / ".synapse")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Synapse config paths
# ═══════════════════════════════════════════════════════════════════

SYNAPSE_HOME = Path(os.getenv("SYNAPSE_HOME", Path.home() / ".synapse"))
CONFIG_PATH = SYNAPSE_HOME / "config.yaml"
PREFILL_PATH = SYNAPSE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Synapse models — already uncensored, just needs clean prompt
    "synapse": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "synapse" in model_lower or "nous" in model_lower:
        return "synapse"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Synapse config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.synapse/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Synapse for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Synapse for changes to take effect.")
