...
## 5. GODMODE FAST — Synapse 4 405B (Zero Refusal)

**Model:** `joshresearch/synapse-4-405b`
**Strategy:** Synapse models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Synapse Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.synapse/config.yaml`:
