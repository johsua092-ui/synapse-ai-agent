...
Usage:
  ...
  <server_name> matches the files in $SYNAPSE_HOME/mcp-tokens/<server>.json etc.
...
def _synapse_home():
    # Prefer Synapse' own resolver (profile-safe); fall back to env then ~/.synapse.
    try:
        from synapse_constants import get_synapse_home
        return str(get_synapse_home())
    except Exception:
        return os.environ.get("SYNAPSE_HOME") or os.path.expanduser("~/.synapse")


def _tokens_dir():
    return os.path.join(_synapse_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "synapse-diag", "version": "1.0"}},
        },
        headers={
