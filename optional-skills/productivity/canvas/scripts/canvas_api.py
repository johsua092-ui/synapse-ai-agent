#!/usr/bin/env python3
"""Canvas LMS API CLI for Synapse Agent.
...
def _check_config():
    ...
    if missing:
        synapse_env = os.path.join(
            os.environ.get("SYNAPSE_HOME", os.path.expanduser("~/.synapse")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {synapse_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Synapse Agent"
    )
