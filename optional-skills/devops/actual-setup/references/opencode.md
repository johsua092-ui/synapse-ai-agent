...
is typically only in `~/.synapse/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Synapse,
...
`synapse-custom-providers` skill, pitfall 2). Synapse hits this because it forwards
