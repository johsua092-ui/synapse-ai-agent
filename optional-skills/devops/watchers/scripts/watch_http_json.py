...
Usage (via cron with --no-agent):
    ...
    synapse cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SYNAPSE_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Synapse-Watcher/1.0"})
