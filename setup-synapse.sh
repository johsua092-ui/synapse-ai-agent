...
# 5. Symlinks the 'synapse' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Synapse Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/synapse-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink synapse into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up synapse command..."

SYNAPSE_BIN="$SCRIPT_DIR/venv/bin/synapse"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SYNAPSE_BIN" "$COMMAND_LINK_DIR/synapse"
echo -e "${GREEN}✓${NC} Symlinked synapse → $COMMAND_LINK_DISPLAY_DIR/synapse"
...
# ============================================================================
# Seed bundled skills into ~/.synapse/skills/
# ============================================================================

SYNAPSE_SKILLS_DIR="${SYNAPSE_HOME:-$HOME/.synapse}/skills"
mkdir -p "$SYNAPSE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.synapse/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SYNAPSE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  synapse status        # Check configuration"
if is_termux; then
    echo "  synapse gateway       # Run gateway in foreground"
else
    echo "  synapse gateway install # Install gateway service (messaging + cron)"
fi
echo "  synapse cron list     # View scheduled jobs"
echo "  synapse doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m synapse_cli.main setup
fi
