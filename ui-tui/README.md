...
React + Ink terminal UI for Synapse. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SYNAPSE_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `synapse setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.synapse/.synapse_history` or under `SYNAPSE_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `synapse setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/synapse-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external synapse setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.synapse/tui-parent.log
    ...
    types/
      synapse-ink.d.ts            type declarations for @synapse/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SynapseCLI subprocess for slash commands
```
