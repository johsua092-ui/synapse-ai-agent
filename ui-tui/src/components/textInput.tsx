import type { InputEvent, Key } from '@synapse/ink'
import * as Ink from '@synapse/ink'
 ...
 * but that accepts `key.meta` on macOS — and synapse-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.SYNAPSE_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which synapse-ink decodes as meta+'d'; without this branch it
