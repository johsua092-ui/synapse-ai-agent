...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SYNAPSE_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Synapse on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SYNAPSE_TUI_TRUECOLOR=1.
