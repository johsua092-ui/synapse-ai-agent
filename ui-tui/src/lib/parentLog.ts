...
const logDir = join(process.env.SYNAPSE_HOME?.trim() || join(homedir(), '.synapse'), 'logs')
...
// a real ~/.synapse (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('synapse-tui: parent lifecycle log unavailable\n')
    }
  }
}
