 ...
 * `synapse-boot-background` / `synapse-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports SYNAPSE_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.synapse matches get_synapse_home()'s default.
const bootFilePath = () => join(process.env.SYNAPSE_HOME ?? join(homedir(), '.synapse'), 'tui-theme-boot.json')

// Never touch the user's real ~/.synapse from test runs (the TS suite has no
// SYNAPSE_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into SYNAPSE_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.SYNAPSE_TUI_THEME || env.SYNAPSE_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.SYNAPSE_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.SYNAPSE_TUI_BACKGROUND
  ) {
    env.SYNAPSE_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.SYNAPSE_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.SYNAPSE_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into SYNAPSE_TUI_THEME.
