...
const ENV_KEYS = ['SYNAPSE_AUTO_HEAPDUMP', 'SYNAPSE_HEAPDUMP_DIR', 'SYNAPSE_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'synapse-heapdump-test-'))
    process.env.SYNAPSE_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without SYNAPSE_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without SYNAPSE_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when SYNAPSE_AUTO_HEAPDUMP=1', async () => {
    process.env.SYNAPSE_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.SYNAPSE_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of SYNAPSE_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.SYNAPSE_HEAPDUMP_DIR
    savedMax = process.env.SYNAPSE_HEAPDUMP_MAX_BYTES
    delete process.env.SYNAPSE_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'synapse-heapdump-prune-'))
    process.env.SYNAPSE_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.SYNAPSE_HEAPDUMP_DIR
    } else {
      process.env.SYNAPSE_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.SYNAPSE_HEAPDUMP_MAX_BYTES
    } else {
      process.env.SYNAPSE_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.SYNAPSE_HEAPDUMP_MAX_BYTES = String(2 * 1024)
