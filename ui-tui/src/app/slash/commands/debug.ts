...
import { terminalBackgroundHex } from '@synapse/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SYNAPSE_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SYNAPSE_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SYNAPSE_TUI_BACKGROUND', process.env.SYNAPSE_TUI_BACKGROUND ?? '(unset)'],
            ['SYNAPSE_TUI_THEME', process.env.SYNAPSE_TUI_THEME ?? '(unset)'],
