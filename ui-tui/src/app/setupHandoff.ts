import type { RunExternalProcess } from '@synapse/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`synapse ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching synapse: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`synapse ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
