...
import { Box, Text } from '@synapse/ink'
...
/**
 * User widget apps — Synapse authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$SYNAPSE_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.synapse/plugins/`: files under SYNAPSE_HOME execute
...
const widgetsDir = () => join(process.env.SYNAPSE_HOME?.trim() || join(homedir(), '.synapse'), 'tui-widgets')
 ...
 *  every change, so a widget Synapse writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Synapse) ever writes must hot-load too — a 10s poll here read
