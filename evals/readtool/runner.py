"""Run the read-tool eval through the REAL Synapse AIAgent.

For each task: fresh temp SYNAPSE_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    synapse_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".synapse"
    ...
    try:
        ...
        shutil.rmtree(synapse_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.synapse/.env; set +a  — then relaunch."
        )
