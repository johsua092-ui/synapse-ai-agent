...
# node:26 image instead (Synapse pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/synapse is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/synapse/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when synapse
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# synapse process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('synapse')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main synapse process, the dashboard,
...
# 'Synapse WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via SYNAPSE_UID at runtime
RUN useradd -u 10000 -m -d /opt/data synapse
...
#
# ui-tui/packages/synapse-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/synapse-ink/ ui-tui/packages/synapse-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as synapse-ink above).
...
# lock on the @synapse/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Synapse when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Synapse production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/synapse/.venv at first
...
# gives the non-root synapse user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link synapse-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/synapse are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the synapse user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/synapse/bin && \
    cp /opt/synapse/docker/synapse-exec-shim.sh /opt/synapse/bin/synapse && \
    chmod 0755 /opt/synapse /opt/synapse/bin/synapse && \
    printf 'docker\n' > /opt/synapse/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $SYNAPSE_HOME. $SYNAPSE_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``synapse update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the synapse user via
# `s6-setuidgid synapse` in its run script. If SYNAPSE_UID is unset, services
# run as the default synapse user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `synapse dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the SYNAPSE_GIT_SHA build-arg to
# /opt/synapse/.synapse_build_sha at build time, and have
# synapse_cli/build_info.py read it at runtime.  Both `synapse dump` and
...
ARG SYNAPSE_GIT_SHA=
RUN if [ -n "${SYNAPSE_GIT_SHA}" ]; then \
        printf '%s\n' "${SYNAPSE_GIT_SHA}" > /opt/synapse/.synapse_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-synapse + dashboard.
...
# `exec synapse`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $SYNAPSE_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/synapse/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-synapse-setup && \
    chmod +x /etc/cont-init.d/01-synapse-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV SYNAPSE_WEB_DIST=/opt/synapse/synapse_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/synapse/ui-tui/dist/entry.js`) and skip the
...
ENV SYNAPSE_TUI_DIR=/opt/synapse/ui-tui
ENV SYNAPSE_HOME=/opt/data
ENV SYNAPSE_WRITE_SAFE_ROOT=/opt/data
ENV SYNAPSE_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/synapse (root-owned, read-only) so a runtime
...
# is seeded + chowned to the synapse user by docker/stage2-hook.sh and lives
...
ENV SYNAPSE_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> synapse ...` they default to root, and any file the
# command writes under $SYNAPSE_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/synapse/bin/synapse, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid synapse`
# when invoked as root. Non-root callers (supervised processes,
# `--user synapse`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/synapse/.venv/bin/synapse). See the shim source for
# the opt-out env var (SYNAPSE_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/synapse-exec-shim.sh /opt/synapse/bin/synapse
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/synapse/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> synapse ...` and any
# subprocess that doesn't activate the venv first still find synapse.
#
# /opt/synapse/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/synapse/bin:/opt/synapse/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. synapse
# subcommand vs. no-args), drops to the synapse user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/synapse/docker/entrypoint-dispatch.sh" ]
CMD [ ]
