...
_SYNAPSE_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (SYNAPSE_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `synapse tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``synapse tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SYNAPSE_KANBAN_TASK env "
    ...
    # Coding posture (base Synapse — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in synapse_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Synapse toolsets (CLI + messaging platforms)
    #
    ...
    # the `synapse send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "synapse-cli": {
        ...
        "tools": _SYNAPSE_CORE_TOOLS,
        "includes": []
    },

    "synapse-cron": {
        # Mirrors synapse-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `synapse tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as synapse-cli; gated by `synapse tools`",
        "tools": _SYNAPSE_CORE_TOOLS,
        "includes": []
    },
    ...
    "synapse-email": {
        "description": "Email bot toolset - interact with Synapse via email (IMAP/SMTP)",
        "tools": _SYNAPSE_CORE_TOOLS,
        "includes": []
    },
    ...
    "synapse-sms": {
        "description": "SMS bot toolset - interact with Synapse via SMS (Twilio)",
        "tools": _SYNAPSE_CORE_TOOLS,
        "includes": []
    },
    ...
    "synapse-webhook": {
        ...
        "tools": _SYNAPSE_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "synapse-gateway": {
        ...
        "tools": [],
        "includes": ["synapse-telegram", "synapse-discord", "synapse-whatsapp", "synapse-slack", "synapse-signal", "synapse-bluebubbles", "synapse-homeassistant", "synapse-email", "synapse-sms", "synapse-mattermost", "synapse-matrix", "synapse-dingtalk", "synapse-feishu", "synapse-wecom", "synapse-wecom-callback", "synapse-weixin", "synapse-qqbot", "synapse-webhook", "synapse-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``synapse-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SYNAPSE_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``synapse-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (synapse-<name>).
        # Gives them _SYNAPSE_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("synapse-"):
            platform_name = name[len("synapse-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SYNAPSE_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
